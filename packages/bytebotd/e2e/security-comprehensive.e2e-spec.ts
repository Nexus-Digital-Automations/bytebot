...
describe('Security E2E - Comprehensive Testing', () => {
      ...
      if (clientData.requests.length >= maxRequests) {
        return res.status(429).json({
          ...
          retryAfter: Math.ceil(
            (windowMs - (now - (clientData.requests[0] ?? now))) / 1000,
          ),
        });
      }
  ...
  describe('Authentication Flow Security', () => {
    it('should complete full authentication flow successfully', async () => {
      ...
      const refreshResponse = await request(app.getHttpAdapter().getInstance())
        .post('/auth/refresh')
        .send({ refreshToken })
        .expect(200);
  ...
  describe('Multi-Role Security Scenarios', () => {
    it('should enforce role-based access control across all roles', async () => {
      ...
      for (const roleTest of roleTests) {
        ...
        const taskResponse = await request(app.getHttpAdapter().getInstance())
          .post('/api/tasks')
          .set('Authorization', `Bearer ${token}`)
        ...
        const usersResponse = await request(app.getHttpAdapter().getInstance())
          .get('/api/admin/users')
          .set('Authorization', `Bearer ${token}`);
    ...
    it('should prevent horizontal privilege escalation', async () => {
      ...
      // Login as operator
      const operatorLogin = await request(app.getHttpAdapter().getInstance())
        .post('/auth/login')
        .send({
          email: 'operator@bytebot.ai',
          password: 'Operator123!@#',
  ...
  describe('Security Middleware Integration', () => {
    ...
    it('should enforce rate limiting', async () => {
      ...
      for (let i = 0; i < 105; i++) {
        requests.push(
          request(app.getHttpAdapter().getInstance())
            .get('/health')
    ...
    it('should handle mixed legitimate and attack traffic', async () => {
      ...
      const promises = [
        ...
        ...Array(15)
          .fill(null)
          .map(() =>
            request(app.getHttpAdapter().getInstance()).get(
              '/api/admin/metrics',
            ),
          ),
      ];
  ...
  describe('Security Monitoring and Alerting', () => {
    it('should track and log security events', async () => {
      ...
      const viewerLogin = await request(app.getHttpAdapter().getInstance())
        .post('/auth/login')
        .send({
          email: 'viewer@bytebot.ai',
          password: 'Viewer123!@#',
    ...
    it('should provide detailed audit information', async () => {
      ...
      // Access audit logs
      const auditResponse = await request(app.getHttpAdapter().getInstance())
  ...
  describe('Cross-Component Security Consistency', () => {
    it('should maintain consistent security across all endpoints', async () => {
      ...
      for (const endpoint of endpoints) {
        const requestAgent = request(app.getHttpAdapter().getInstance());
