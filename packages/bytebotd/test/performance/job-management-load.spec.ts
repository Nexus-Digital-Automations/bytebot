...
const PERF_ACTIONS = {
  ...
  mixed: [
    ...
    { action: 'click_mouse', coordinates: { x: 200, y: 200 } } as ComputerAction,
  ],
};
...
describe('Job Management Service - Performance Tests', () => {
  ...
  describe('High-Throughput Job Submission', () => {
    it('should achieve >1000 jobs/second submission rate', async () => {
      ...
      for (let i = 0; i < targetJobs; i++) {
        submissions.push(jobManagementService.createJob(PERF_ACTIONS.lightweight, {
      ...
      expect(actualRate).toBeGreaterThan(PERF_CONFIG.benchmarks.jobSubmissionRate);
    ...
    it('should maintain submission rate under concurrent load', async () => {
  ...
  describe('Job Execution Performance', () => {
    it('should achieve <500ms job execution latency', async () => {
      const testJobs = 100;
      const jobIds: string[] = [];

      // Submit jobs
      for (let i = 0; i < testJobs; i++) {
        const jobId = await jobManagementService.createJob(PERF_ACTIONS.lightweight);
        jobIds.push(jobId);
      ...
      expect(latencyStats.p95).toBeLessThan(PERF_CONFIG.benchmarks.responseTime95th);
    }, PERF_CONFIG.test.timeout);
    ...
    it('should handle mixed workload efficiently', async () => {
      ...
      for (let i = 0; i < lightweightJobs; i++) {
        allJobs.push(jobManagementService.createJob(PERF_ACTIONS.lightweight, {
          priority: JobPriority.HIGH,
        }));
      }

      for (let i = 0; i < mediumJobs; i++) {
        allJobs.push(jobManagementService.createJob(PERF_ACTIONS.medium, {
          priority: JobPriority.NORMAL,
        }));
      }

      for (let i = 0; i < heavyJobs; i++) {
        allJobs.push(jobManagementService.createJob(PERF_ACTIONS.heavy, {
  ...
  describe('Redis Performance and Scalability', () => {
    it('should achieve <15ms Redis operation latency', async () => {
      ...
      for (let i = 0; i < operationCount; i++) {
        ...
        const operation = redisClient.set(`perf:test:${i}`, JSON.stringify({
          id: i,
          timestamp: Date.now(),
      ...
      const p95Latency = operationTimes.sort((a, b) => a - b)[Math.floor(operationTimes.length * 0.95)];
      ...
      expect(p95Latency).toBeLessThan(PERF_CONFIG.benchmarks.redisOperationLatency);
  ...
  describe('Memory Usage and Efficiency', () => {
    it('should maintain memory efficiency under load', async () => {
      ...
      for (let i = 0; i < jobCount; i++) {
        const jobId = await jobManagementService.createJob(PERF_ACTIONS.medium);
        jobIds.push(jobId);
      }
      ...
      const memoryIncrease = memoryAfterCreation.heapUsed - initialMemory.heapUsed;
      ...
      const finalMemoryDelta = memoryAfterProcessing.heapUsed - memoryAfterCreation.heapUsed;
    ...
    it('should handle memory cleanup effectively', async () => {
      ...
      for (let batch = 0; batch < 5; batch++) {
        ...
        for (let i = 0; i < 200; i++) {
          batchJobs.push(jobManagementService.createJob(PERF_ACTIONS.lightweight));
        }
  ...
  describe('Worker Scaling Performance', () => {
    it('should demonstrate worker scaling efficiency', async () => {
      ...
      results.forEach(result => {
        console.log(`${result.jobCount} jobs: ${result.throughput.toFixed(0)} jobs/sec`);
      });
  ...
  describe('Priority Queue Fairness', () => {
    it('should maintain priority fairness under load', async () => {
      const jobsPerPriority = 100;
      const priorities = [JobPriority.URGENT, JobPriority.HIGH, JobPriority.NORMAL, JobPriority.LOW];
      ...
      for (const priority of priorities) {
        ...
        for (let i = 0; i < jobsPerPriority; i++) {
          await jobManagementService.createJob(PERF_ACTIONS.lightweight, { priority });
        }
      ...
      priorities.forEach(priority => {
        ...
        console.log(`${priority} priority submission time: ${submissionTime?.toFixed(0)}ms`);
      });
    }, PERF_CONFIG.test.timeout);
  });
});