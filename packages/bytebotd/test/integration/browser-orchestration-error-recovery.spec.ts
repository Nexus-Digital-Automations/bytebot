...
describe('Browser Orchestration Error Handling and Recovery', () => {
  ...
  const errorScenarios = {
    network: {
      timeouts: ['https://httpbin.org/delay/30', 'https://httpbin.org/delay/60'],
      ...
      sslErrors: ['https://self-signed.badssl.com/', 'https://expired.badssl.com/'],
    },
    browser: {
  ...
  describe('Network Error Handling', () => {
    it('should handle connection timeouts with exponential backoff retry', async () => {
      ...
      expect(recoveryMetrics.errorHandling.timeoutsDetected).toBeGreaterThan(0);
      expect(recoveryMetrics.errorHandling.retryAttempts).toBeGreaterThan(0);
      expect(recoveryMetrics.recovery.systemStabilityAfterErrors).toBeGreaterThan(80);
      expect(recoveryMetrics.recovery.recoveryTasksSuccessful).toBeGreaterThan(0);
    }, testConfig.testTimeout);

    it('should handle unreachable domains with fallback mechanisms', async () => {
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(unreachableTestDto, {
        expectPartialSuccess: true,
      ...
      expect(recoveryMetrics.errorHandling.unreachableDomainsDetected).toBeGreaterThan(0);
      expect(recoveryMetrics.errorHandling.fallbackActivations).toBeGreaterThan(0);
    ...
    it('should handle SSL certificate errors gracefully', async () => {
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(sslErrorTestDto, {
        expectPartialSuccess: true,
      ...
      expect(recoveryMetrics.errorHandling.sslErrorsDetected).toBeGreaterThan(0);
  ...
  describe('Browser Process Error Handling', () => {
    it('should recover from browser process crashes', async () => {
      const browserCrashTestDto: CreateOrchestrationDto = {
        ...
        tasks: [
          ...
          },
          {
            ...
            instructions: 'Execute memory-intensive operation to trigger crash',
            priority: TaskPriority.NORMAL,
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(browserCrashTestDto, {
        expectPartialSuccess: true,
      ...
      expect(recoveryMetrics.processFailures.crashesDetected).toBeGreaterThan(0);
      expect(recoveryMetrics.processFailures.automaticRestarts).toBeGreaterThan(0);
    ...
    it('should handle resource exhaustion scenarios', async () => {
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(resourceExhaustionDto, {
        expectPartialSuccess: true,
      ...
      expect(recoveryMetrics.resourceUsage.limitViolationsDetected).toBeGreaterThan(0);
  ...
  describe('Agent Coordination Error Handling', () => {
    it('should handle agent communication failures', async () => {
      ...
      expect(recoveryMetrics.communication.failuresDetected).toBeGreaterThan(0);
      expect(recoveryMetrics.communication.partitionsHandled).toBeGreaterThan(0);
    ...
    it('should handle agent load balancing failures', async () => {
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(loadBalancingFailureDto, {
        expectPartialSuccess: true,
  ...
  describe('Task Dependency Error Handling', () => {
    it('should handle dependency chain failures with alternative paths', async () => {
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(dependencyFailureDto, {
        expectPartialSuccess: true,
      ...
      expect(recoveryMetrics.dependencies.failuresDetected).toBeGreaterThan(0);
      expect(recoveryMetrics.dependencies.alternativePathsUsed).toBeGreaterThan(0);
    ...
    it('should detect and handle circular dependencies', async () => {
      const circularDependencyDto: CreateOrchestrationDto = {
        ...
        tasks: [
          ...
          },
          {
            ...
            instructions: 'Task C - depends on B (creates circular dependency)',
            priority: TaskPriority.HIGH,
            dependencies: ['task_b'],
            taskId: 'task_c',
          },
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(circularDependencyDto, {
        expectPartialSuccess: true,
      ...
      expect(recoveryMetrics.dependencies.circularDependenciesDetected).toBeGreaterThan(0);
      expect(recoveryMetrics.dependencies.circularDependenciesBroken).toBeGreaterThan(0);
      expect(recoveryMetrics.recovery.independentTasksCompleted).toBeGreaterThan(0);
      expect(recoveryMetrics.errorHandling.validationErrors).toBeGreaterThan(0);
    }, testConfig.testTimeout);
  });

  describe('System-Level Error Recovery', () => {
    it('should handle database connection failures with retry and recovery', async () => {
    ...
    it('should maintain orchestration state consistency during failures', async () => {
      ...
      const recoveryMetrics = await executeErrorRecoveryTest(stateConsistencyDto, {
        expectPartialSuccess: true,
      ...
      expect(recoveryMetrics.stateManagement.checkpointsCreated).toBeGreaterThan(0);
      expect(recoveryMetrics.stateManagement.rollbacksExecuted).toBeGreaterThan(0);
  ...
  async function executeErrorRecoveryTest(
    ...
    try {
      ...
      const maxMonitoringAttempts = Math.floor(testConfig.testTimeout / testConfig.healthCheckIntervalMs);

      while (status === OrchestrationStatus.RUNNING && monitoringAttempts < maxMonitoringAttempts) {
        await new Promise(resolve => setTimeout(resolve, testConfig.healthCheckIntervalMs));
        ...
        try {
          ...
          if (options.trackRetryAttempts) {
            metrics.errorHandling.retryAttempts = statusResponse.body.retryAttempts || 0;
          }

          if (options.trackProcessFailures) {
            metrics.processFailures.crashesDetected = statusResponse.body.processFailures?.crashes || 0;
            metrics.processFailures.automaticRestarts = statusResponse.body.processFailures?.restarts || 0;
          }

          if (options.trackResourceUsage) {
            metrics.resourceUsage.limitViolationsDetected = statusResponse.body.resourceViolations || 0;
          }
  ...
  function calculateSystemStability(orchestrationStatus: any, agentStatus: any): number {
    // Calculate system stability score based on various factors
    let stabilityScore = 100;

    // Deduct points for failures
    if (orchestrationStatus.progress?.failedTasks > 0) {
      stabilityScore -= (orchestrationStatus.progress.failedTasks / orchestrationStatus.progress.totalTasks) * 30;
    }

    // Deduct points for agent issues
    if (agentStatus.unhealthyAgents > 0) {
      stabilityScore -= (agentStatus.unhealthyAgents / agentStatus.totalAgents) * 20;
    }
