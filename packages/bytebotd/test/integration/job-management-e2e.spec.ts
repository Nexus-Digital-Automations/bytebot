...
describe('Job Management Service - Integration Tests', () => {
  ...
  describe('Complete Job Lifecycle Integration', () => {
    it('should complete full job lifecycle: create → queue → execute → complete', async () => {
      // Step 1: Create job
      const jobId = await jobManagementService.createJob(TEST_ACTIONS.screenshot, {
      ...
      while (attempts < maxAttempts) {
        ...
        if (finalStatus.status === JobStatus.COMPLETED || finalStatus.status === JobStatus.FAILED) {
          break;
        }
    ...
    it('should handle job execution with different action types', async () => {
      const testCases = [
    ...
    it('should handle job priority ordering correctly', async () => {
      ...
      const urgentJob = await jobManagementService.createJob(TEST_ACTIONS.screenshot, {
        priority: JobPriority.URGENT,
      });

      const lowJob = await jobManagementService.createJob(TEST_ACTIONS.moveMouse, {
        priority: JobPriority.LOW,
      });

      const highJob = await jobManagementService.createJob(TEST_ACTIONS.clickMouse, {
        priority: JobPriority.HIGH,
      });

      const normalJob = await jobManagementService.createJob(TEST_ACTIONS.writeFile, {
        priority: JobPriority.NORMAL,
      });

      // Verify priority indexes in Redis
      const urgentJobs = await redisClient.smembers('bytebot:jobs:priority:urgent');
      const highJobs = await redisClient.smembers('bytebot:jobs:priority:high');
      const normalJobs = await redisClient.smembers('bytebot:jobs:priority:normal');
  ...
  describe('Multi-Worker Coordination Integration', () => {
    it('should handle concurrent job processing by multiple workers', async () => {
      const jobCount = 20;
      const jobs: Promise<string>[] = [];

      // Create multiple jobs simultaneously
      for (let i = 0; i < jobCount; i++) {
        const action = i % 2 === 0 ? TEST_ACTIONS.screenshot : TEST_ACTIONS.moveMouse;
        jobs.push(jobManagementService.createJob(action, {
    ...
    it('should handle worker scaling under load', async () => {
      const batchCount = 5;
      const batchSize = 10;
      ...
      for (let batch = 0; batch < batchCount; batch++) {
        ...
        for (let i = 0; i < batchSize; i++) {
          batchJobs.push(jobManagementService.createJob(TEST_ACTIONS.screenshot, {
            priority: JobPriority.NORMAL,
          }));
        }
  ...
  describe('Error Recovery and Resilience Integration', () => {
    it('should handle Redis connection interruptions gracefully', async () => {
      // Create initial job
      const jobId = await jobManagementService.createJob(TEST_ACTIONS.screenshot);
    ...
    it('should handle job retry scenarios with backoff', async () => {
    ...
    it('should handle job timeout scenarios correctly', async () => {
      ...
      while (attempts < 50) {
        ...
        if (timeoutStatus.status === JobStatus.TIMEOUT ||
  ...
  describe('Job Cancellation Integration', () => {
    ...
    it('should handle cancellation of running jobs', async () => {
      ...
      const originalAction = computerUseService.action;
      computerUseService.action = jest.fn().mockImplementation(() =>
  ...
  describe('Resource Management Integration', () => {
    it('should perform cleanup of expired jobs', async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, TEST_CONFIG.job.maxAge + 1000));
    ...
    it('should handle memory optimization during processing', async () => {
  ...
  describe('Real-time Monitoring Integration', () => {
    it('should provide accurate queue statistics during processing', async () => {
      // Create jobs with different priorities
      const jobIds: string[] = [];
      const priorities = [JobPriority.LOW, JobPriority.NORMAL, JobPriority.HIGH, JobPriority.URGENT];

      for (let i = 0; i < 20; i++) {
        const priority = priorities[i % priorities.length];
        const jobId = await jobManagementService.createJob(TEST_ACTIONS.screenshot, { priority });
        jobIds.push(jobId);
      }
  ...
  describe('Data Consistency Integration', () => {
    it('should maintain data consistency across Redis operations', async () => {
      ...
      const statusMembers = await redisClient.smembers('bytebot:jobs:status:pending');
      expect(statusMembers).toContain(jobId);

      // Verify job in priority index
      const priorityMembers = await redisClient.smembers('bytebot:jobs:priority:high');
      ...
      const completedMembers = await redisClient.smembers('bytebot:jobs:status:completed');
      const pendingMembers = await redisClient.smembers('bytebot:jobs:status:pending');
    ...
    it('should handle concurrent job operations without race conditions', async () => {
      ...
      for (let i = 0; i < concurrentJobs; i++) {
        if (i % 3 === 0) {
          // Create job
          operations.push(jobManagementService.createJob(TEST_ACTIONS.screenshot));
        } else if (i % 3 === 1) {
          // Get queue stats
