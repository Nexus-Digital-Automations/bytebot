...
describe('Browser Orchestration Integration (E2E)', () => {
  ...
  const testConfig = {
    ...
    orchestratorPath: path.join(
      process.cwd(),
      '../../orchestrator/browser_orchestration',
    ),
    testTimeout: 60000,
  ...
  describe('Python Orchestrator Integration', () => {
    ...
    it('should handle Python orchestrator communication errors', async () => {
      ...
      expect(response.body.components.pythonOrchestrator.status).toBe(
        'disconnected',
      );
    ...
    it(
      'should synchronize orchestration state between systems',
      async () => {
  ...
  describe('Multi-Agent Coordination', () => {
    it(
      'should coordinate multiple browser agents for parallel execution',
      async () => {
        ...
        while (
          !finalStatus ||
          finalStatus.status === OrchestrationStatus.RUNNING
        ) {
          const statusResponse = await request(app.getHttpServer())
            .get(
              `/browser-orchestration/orchestrations/${orchestrationId}/status`,
        ...
        expect(agentStatusHistory.some((h) => h.status.agents.active > 1)).toBe(
          true,
        );
      },
      testConfig.testTimeout,
    ...
    it(
      'should handle agent failures and redistribute tasks',
      async () => {
  ...
  describe('Session Management Integration', () => {
    it(
      'should coordinate browser sessions across agents',
      async () => {
        ...
        while (
          !sessionMetrics ||
          sessionMetrics.status === OrchestrationStatus.RUNNING
        ) {
          const statusResponse = await request(app.getHttpServer())
            .get(
              `/browser-orchestration/orchestrations/${orchestrationId}/status`,
    ...
    it(
      'should reuse sessions efficiently across tasks',
      async () => {
        ...
        while (
          !finalMetrics ||
          finalMetrics.status === OrchestrationStatus.RUNNING
        ) {
          const statusResponse = await request(app.getHttpServer())
            .get(
              `/browser-orchestration/orchestrations/${orchestrationId}/status`,
        ...
        expect(finalMetrics.sessions.total).toBeLessThan(
          finalMetrics.progress.totalTasks,
        );
        expect(
          finalMetrics.resourceOptimization?.sessionsReused,
        ).toBeGreaterThan(0);
      },
      testConfig.testTimeout,
    );
  ...
  describe('Error Handling and Recovery', () => {
    it(
      'should handle Python orchestrator disconnection gracefully',
      async () => {
        ...
        expect(healthResponse.body.components.pythonOrchestrator.status).toBe(
          'disconnected',
        );
        ...
        expect(
          recoveryHealthResponse.body.components.pythonOrchestrator.status,
        ).toBe('connected');
      },
      testConfig.testTimeout,
    );
  ...
  describe('Performance and Scalability', () => {
    it(
      'should handle concurrent orchestrations efficiently',
      async () => {
        const concurrentOrchestrations = Array.from(
          { length: testConfig.maxConcurrentTests },
          (_, i) => ({
        ...
        expect(
          createResponses.every((r) => r.status === HttpStatus.CREATED),
        ).toBe(true);
        ...
        expect(executeResponses.every((r) => r.status === HttpStatus.OK)).toBe(
          true,
        );
        ...
        expect(
          finalStatuses.every(
            (s) =>
              s === OrchestrationStatus.COMPLETED ||
              s === OrchestrationStatus.FAILED,
          ),
        ).toBe(true);

        // Concurrent execution should be more efficient than sequential
        expect(executionTime).toBeLessThan(
          testConfig.maxConcurrentTests * 5000,
        );
      },
    ...
    it(
      'should scale agent pool dynamically under load',
      async () => {
  ...
  async function startPythonOrchestrator(): Promise<void> {
    return new Promise((resolve, reject) => {
      const orchestratorScript = path.join(
        testConfig.orchestratorPath,
        'integration_server.py',
      );

      pythonOrchestratorProcess = spawn(
        testConfig.pythonExecutable,
        [
          orchestratorScript,
          '--port',
          testConfig.orchestratorPort.toString(),
          '--test-mode',
        ],
        {
      ...
      pythonOrchestratorProcess.on('error', (error) => {
        reject(
          new Error(`Failed to start Python orchestrator: ${error.message}`),
        );
      });
  ...
  async function waitForOrchestratorReady(): Promise<void> {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (
          response.body.components?.pythonOrchestrator?.status === 'connected'
        ) {
          return;
        }
      } catch (_error) {
