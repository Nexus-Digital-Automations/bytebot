...
@Injectable()
export class ParlantStressTestingFrameworkService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(ParlantStressTestingFrameworkService.name);
   ...
   */
  async executeStressTest(config?: Partial<StressTestConfiguration>): Promise<StressTestResults> {
    if (this.stressTestActive) {
  ...
  private async executeRampUpPhase(config: StressTestConfiguration): Promise<void> {
    ...
    while (Date.now() - rampUpStartTime < rampUpDuration && this.stressTestActive) {
      const targetSessions = Math.min(
        config.maxConcurrentSessions,
        Math.floor(((Date.now() - rampUpStartTime) / 1000) * config.sessionRampUpRate)
      );

      // Create new sessions to reach target
      while (this.activeSessions.size < targetSessions && this.stressTestActive) {
  ...
  private async executeSustainedStressPhase(config: StressTestConfiguration): Promise<void> {
    ...
    while (Date.now() - sustainedStartTime < config.stressDuration && this.stressTestActive) {
  ...
  private async executeOverloadPhase(config: StressTestConfiguration): Promise<void> {
    ...
    while (Date.now() - overloadStartTime < overloadDuration && this.stressTestActive) {
      const currentMetrics = await this.captureCurrentMetrics();
      this.currentStressTest!.metricsHistory.push(currentMetrics);
  ...
  private async executeRecoveryPhase(config: StressTestConfiguration): Promise<void> {
    ...
    while (this.activeSessions.size > targetRecoverySessionCount) {
      ...
      const sessionsToRemove = sessionIds.slice(0, Math.min(50, sessionIds.length));
    ...
    while (Date.now() - recoveryStartTime < config.recoveryTestDuration && this.stressTestActive) {
      const currentMetrics = await this.captureCurrentMetrics();
      this.currentStressTest!.metricsHistory.push(currentMetrics);
   ...
   */
  private async executeConversationTurns(config: StressTestConfiguration): Promise<void> {
  ...
  private async executeSessionConversationTurn(
    ...
    try {
      ...
      for (let i = 0; i < config.validationRequestsPerTurn; i++) {
        ...
        const validationRequest = this.generateStressValidationRequest(session, i);

        // Simulate PARLANT validation (replace with actual service call)
        const response = await this.simulateParlantValidation(validationRequest);
      ...
      if (session.conversationTurns >= config.conversationTurnsPerSession ||
   ...
   */
  private async maintainTargetSessionCount(config: StressTestConfiguration): Promise<void> {
    ...
    while (this.activeSessions.size < config.maxConcurrentSessions && this.stressTestActive) {
      await this.createConversationalSession(config);
   ...
   */
  private async terminateConversationalSession(sessionId: string): Promise<void> {
  ...
  private async captureCurrentMetrics(): Promise<StressTestMetrics> {
    ...
    const recentRequestsCount = this.responseTimes.length > 100 ? 100 : this.responseTimes.length;
    ...
    const healthFactors = {
      ...
      memoryUsage: Math.max(0, 100 - (process.memoryUsage().heapUsed / (1024 * 1024 * 10))), // Deduct for high memory
    };
   ...
   */
  private async simulateParlantValidation(request: any): Promise<{ success: boolean; cacheHit: boolean }> {
  ...
  private async generateStressTestResults(): Promise<StressTestResults> {
    ...
    const peakMetrics = this.currentStressTest.metricsHistory.reduce((peak, current) =>
      current.activeSessions > peak.activeSessions ? current : peak
    );

    const finalMetrics = this.currentStressTest.metricsHistory[this.currentStressTest.metricsHistory.length - 1] ||
    ...
    const performanceAnalysis = {
      targetsMet: (
        finalMetrics.p95ResponseTime <= config.performanceTargets.maxP95ResponseTime &&
        finalMetrics.memoryUsageMB <= config.performanceTargets.maxMemoryUsageMB &&
        finalMetrics.cacheHitRate >= config.performanceTargets.minCacheHitRate &&
        finalMetrics.errorRate <= config.performanceTargets.maxErrorRate
      ),
      p95ResponseTime: finalMetrics.p95ResponseTime,
      maxMemoryUsage: Math.max(...this.currentStressTest.metricsHistory.map(m => m.memoryUsageMB)),
      finalCacheHitRate: finalMetrics.cacheHitRate,
      overallErrorRate: finalMetrics.errorRate,
      degradationEvents: this.currentStressTest.metricsHistory.filter(m => m.performanceGrade === 'D' || m.performanceGrade === 'F').length,
    ...
    if (peakMetrics.activeSessions < config.maxConcurrentSessions * 0.9) score -= 20;
  ...
  private monitorStressTestProgress(): void {
    if (this.stressTestActive && this.currentStressTest) {
      ...
      this.logger.log(`📊 Stress test progress: ${activeSessions} active sessions, ${Math.round(duration / 1000)}s elapsed`);
    }
  }
  ...
  getStressTestStatus(): {
    ...
    return {
      ...
      activeSessions: this.activeSessions.size,
      testDuration: this.currentStressTest ? Date.now() - this.currentStressTest.startTime : 0,
