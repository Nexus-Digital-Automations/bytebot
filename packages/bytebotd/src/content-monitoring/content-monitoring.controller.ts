...
export class ContentMonitoringController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates a new content monitor with specified configuration and starts monitoring if enabled',
  })
  @ApiBody({
    type: ContentMonitoringDto,
    description:
      'Monitor configuration including URL, detection method, notifications, and frequency',
  })
  @ApiResponse({
  ...
  async createMonitor(
    ...
    try {
      ...
      this.logger.log(
        `Monitor created successfully in ${Date.now() - startTime}ms`,
        { monitorId: config.id, status: result.status },
      );
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves detailed status, configuration, and statistics for a specific monitor',
  })
  @ApiParam({
    name: 'monitorId',
  ...
  async getMonitorStatus(
    ...
    try {
      const result =
        await this.contentMonitoringService.getMonitorStatus(monitorId);

      this.logger.log(
        `Monitor status retrieved in ${Date.now() - startTime}ms`,
        { monitorId, status: result.status },
      );
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves list of monitors with optional filtering by status and type, includes pagination support',
  })
  @ApiQuery({
    name: 'status',
    enum: MonitorStatus,
  ...
  async listMonitors(
    ...
    try {
      const result = await this.contentMonitoringService.listMonitors(
        status,
        type,
        page,
        pageSize,
      ...
      this.logger.log(
        `Listed ${result.monitors.length} monitors in ${Date.now() - startTime}ms`,
        {
  ...
  @ApiOperation({
    ...
    description:
      'Executes lifecycle operations on a monitor (start, stop, pause, resume, reset, update)',
  })
  @ApiParam({
    name: 'monitorId',
    description: 'Unique monitor identifier',
  ...
  @ApiBody({
    ...
    description:
      'Operation details including operation type and optional configuration for updates',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async performMonitorOperation(
    ...
    const startTime = Date.now();
    this.logger.log(
      `Performing operation: ${operation.operation} on monitor: ${monitorId}`,
    );
    try {
      const result =
        await this.contentMonitoringService.performMonitorOperation(
          monitorId,
          operation,
        );

      this.logger.log(
        `Operation ${operation.operation} completed in ${Date.now() - startTime}ms`,
        { monitorId, success: result.success, newStatus: result.newStatus },
      );
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        `Operation ${operation.operation} failed on monitor: ${monitorId}`,
        {
          error: errorMessage,
          duration: Date.now() - startTime,
        },
  ...
  @ApiOperation({
    ...
    description:
      'Executes operations on multiple monitors simultaneously with optional error handling',
  })
  @ApiBody({
    type: BulkMonitorOperationDto,
    description:
      'Bulk operation details including monitor IDs, operation type, and error handling preferences',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async performBulkOperation(
    ...
    try {
      const result =
        await this.contentMonitoringService.performBulkOperation(operation);

      this.logger.log(
        `Bulk operation ${operation.operation} completed in ${Date.now() - startTime}ms`,
        {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves historical change detection results for a monitor with optional date filtering and pagination',
  })
  @ApiParam({
    name: 'monitorId',
    description: 'Unique monitor identifier',
  ...
  async getChangeHistory(
    ...
    try {
      ...
      this.logger.log(
        `Retrieved ${result.changes.length} changes in ${Date.now() - startTime}ms`,
        { monitorId, totalChanges: result.totalChanges },
      );
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        `Failed to get change history for monitor: ${monitorId}`,
        {
          error: errorMessage,
          duration: Date.now() - startTime,
        },
  ...
  @ApiOperation({
    ...
    description:
      'Permanently deletes a monitor and all associated data including change history and statistics',
  })
  @ApiParam({
    name: 'monitorId',
    description: 'Unique monitor identifier',
  ...
  async deleteMonitor(@Param('monitorId') monitorId: string): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      this.logger.log(
        `Monitor deleted successfully in ${Date.now() - startTime}ms`,
        { monitorId },
      );
    } catch (error) {
      const errorMessage =
  ...
  @ApiOperation({
    ...
    description:
      'Manually triggers an immediate content check for a monitor, bypassing the scheduled interval',
  })
  @ApiParam({
    name: 'monitorId',
    description: 'Unique monitor identifier',
  ...
  async triggerCheck(@Param('monitorId') monitorId: string): Promise<any> {
    const startTime = Date.now();
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves overall system health metrics including active monitors, performance stats, and system status',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async getSystemHealth(): Promise<any> {
    ...
    try {
      ...
      this.logger.log(
        `System health retrieved in ${Date.now() - startTime}ms`,
        {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves comprehensive API documentation including usage examples, best practices, and integration guidelines',
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async getApiGuide(): Promise<any> {
    ...
    const documentation = {
      ...
      description:
        'Comprehensive API for web content monitoring, change detection, and automated notifications',
      endpoints: [
        {
          method: 'POST',
          ...
          example: {
            ...
            notifications: [{ method: 'email', target: 'admin@example.com' }],
          },
        },
        {
          method: 'GET',
        ...
        },
        {
          ...
          description:
            'Perform lifecycle operations (start, stop, pause, resume, reset, update)',
        },
        {
          method: 'DELETE',
          path: '/content-monitoring/monitors/{monitorId}',
      ...
      troubleshooting: {
        commonIssues: [
          ...
          },
          {
            ...
            solution:
              'Adjust sensitivity threshold or exclude dynamic elements',
          },
          {
            issue: 'Notifications not being delivered',
            solution:
              'Verify notification configuration and service availability',
          },
          {
            issue: 'High resource usage',
            solution:
              'Optimize monitoring intervals and reduce concurrent monitors',
          },
        ],
        supportContacts: {
    ...
    this.logger.log(
      `API documentation retrieved in ${Date.now() - startTime}ms`,
    );
