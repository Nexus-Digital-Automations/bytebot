...
export class ConversationalDatabaseService {
  ...
  private readonly operationRiskMap = new Map<
  ...
  private readonly approvalRequests = new Map<
    string,
    MultiPartyApprovalRequest
  >();
  ...
  constructor(
    ...
    this.logger.log(
      'PARLANT VALIDATION: All database operations now require conversational approval',
    );
  }
  ...
  async findById<T extends BaseEntity>(
    ...
    return this.executeWithAudit(operationContext, validation, () =>
  ...
  async create<T extends BaseEntity>(
    ...
    return this.executeWithAudit(operationContext, validation, async () => {
  ...
  async update<T extends BaseEntity>(
    ...
    if (currentEntity) {
      operationContext.metadata.previousValues =
        currentEntity as StrictRecord<unknown>;
    }
  ...
  async bulkDelete<T extends BaseEntity>(
    ...
    return this.executeWithAudit(operationContext, validation, async () => {
      ...
      if (operationContext.requiresBackup) {
        const recordsToDelete = await repository.findAll({
          filter,
        } as QueryOptions);
        this.createBackup(operationContext, {
  ...
  private createOperationContext(
    ...
    const riskLevel =
      this.operationRiskMap.get(operationType) ?? DatabaseRiskLevel.MEDIUM;
    ...
    return {
      ...
      requiresBackup: [
        ...
        'BULK_UPDATE',
        'BULK_DELETE',
      ].some((op) => operationType.includes(op)),
  ...
  private async validateDatabaseOperation(
    ...
    this.logger.debug(
      `[${context.operationId}] Validating database operation: ${context.operationType}`,
      {
    ...
    try {
      ...
      const validationResult: DatabaseValidationResult = {
        ...
        auditTrail: {
          ...
          reasoning:
            parlantResponse.reason ?? 'Conversational validation completed',
          evidence: { parlantResponse, operationContext: context },
        },
        performanceImpact: {
      ...
      this.logger.debug(
        `[${context.operationId}] Database operation validation completed`,
        {
          operationId: context.operationId,
      ...
      this.logger.error(
        `[${context.operationId}] Database validation failed: ${errorMessage}`,
        {
          operationId: context.operationId,
          operationType: context.operationType,
          error: errorMessage,
  ...
  private initializeRiskMappings(): void {
    // LOW risk operations
    this.operationRiskMap.set(
      DatabaseOperationType.FIND_BY_ID,
      DatabaseRiskLevel.LOW,
    );
    this.operationRiskMap.set(
      DatabaseOperationType.FIND_ALL,
      DatabaseRiskLevel.LOW,
    ...
    this.operationRiskMap.set(
      DatabaseOperationType.EXISTS,
      DatabaseRiskLevel.LOW,
    );
    this.operationRiskMap.set(
      DatabaseOperationType.SEARCH,
      DatabaseRiskLevel.LOW,
    );
    ...
    this.operationRiskMap.set(
      DatabaseOperationType.COMPLEX_QUERY,
      DatabaseRiskLevel.HIGH,
    );
  ...
  private async executeWithAudit<T>(
    ...
    this.logger.debug(
      `[${context.operationId}] Executing validated database operation: ${context.operationType}`,
    );
    try {
      ...
      this.logger.debug(
        `[${context.operationId}] Database operation completed successfully`,
        {
          operationId: context.operationId,
          operationType: context.operationType,
          duration,
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        `[${context.operationId}] Database operation failed: ${errorMessage}`,
        {
          operationId: context.operationId,
          operationType: context.operationType,
          conversationId: validation.conversationId,
   ...
   */
  private checkMultiPartyApproval(context: DatabaseOperationContext): {
    approved: boolean;
    reason?: string;
  } {
    ...
    return {
      approved: false,
      reason:
        'Multi-party approval required. Please request approval from system administrators.',
    };
  }
  ...
  private shouldCacheResult(
    ...
    if (
      context.riskLevel === DatabaseRiskLevel.MEDIUM &&
      result.approved &&
      ['FIND_', 'COUNT', 'SEARCH'].some((op) =>
        context.operationType.includes(op),
      )
    ) {
      return true;
    }
  ...
  private createFailsafeValidation(
    ...
    const approved =
      context.riskLevel === DatabaseRiskLevel.LOW &&
      ['FIND_', 'COUNT', 'SEARCH'].some((op) =>
        context.operationType.includes(op),
      );
  ...
  private getOperationDescription(context: DatabaseOperationContext): string {
    ...
    const recordInfo = context.affectedRecords
      ? ` affecting ${context.affectedRecords} records`
      : '';
  ...
  private updateMetrics(
    ...
    if (cacheHit) {
      this.metrics.cacheHitRate =
        (this.metrics.cacheHitRate * (this.metrics.totalOperations - 1) + 100) /
        this.metrics.totalOperations;
    } else {
      this.metrics.cacheHitRate =
    ...
    this.metrics.averageValidationTime =
      (this.metrics.averageValidationTime * (this.metrics.totalOperations - 1) +
        duration) /
      this.metrics.totalOperations;
  }
  ...
  cleanup(): void {
    ...
    this.validationCache.forEach((result, key) => {
      ...
      const maxAge =
        result.riskLevel === DatabaseRiskLevel.LOW ? 300000 : 60000;
