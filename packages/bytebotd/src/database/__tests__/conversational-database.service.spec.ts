...
describe('ConversationalDatabaseService', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should initialize with correct risk mappings', () => {
      // Test by trying operations with different risk levels
      expect(service).toBeDefined();
  ...
  describe('findById', () => {
    ...
    it('should return null when entity not found', async () => {
      ...
      // Act
      const result = await service.findById(mockRepository, 'non-existent-id');
  ...
  describe('Integration Tests', () => {
    it('should handle complex workflow with multiple operations', async () => {
      // This would test a complex scenario with create -> update -> delete
      // with proper validation, backup, and audit trail generation
      ...
      parlantService.validateOperation
        .mockResolvedValueOnce({ approved: true, conversationId: 'conv-create' })
        .mockResolvedValueOnce({ approved: true, conversationId: 'conv-update' })
        .mockResolvedValueOnce({ approved: true, conversationId: 'conv-delete' });

      mockRepository.create.mockResolvedValue(createdEntity);
      mockRepository.findById.mockResolvedValue(createdEntity);
      mockRepository.update.mockResolvedValue({ ...createdEntity, status: 'inactive' });
      mockRepository.delete.mockResolvedValue(true);
      ...
      const updated = await service.update(mockRepository, created.id, { status: 'inactive' }, {
        userId: 'admin-123',
        userRole: 'admin',
