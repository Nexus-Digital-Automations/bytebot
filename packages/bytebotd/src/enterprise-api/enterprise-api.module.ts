...
import { Module } from '@nestjs/common';import { HttpModule } from '@nestjs/axios';import { ConfigModule } from '@nestjs/config';import { JwtModule } from '@nestjs/jwt';import { ParlantModule } from '../parlant/parlant.module';import { SecurityModule } from '../common/security/security.module';import { MetricsModule } from '../metrics/metrics.module';import { EnterpriseApiGatewayController } from './enterprise-api-gateway.controller';import { EnterpriseApiService } from './enterprise-api.service';import { EnterpriseApiInterceptor } from './enterprise-api.interceptor';import { EnterpriseApiHealthService } from './enterprise-api-health.service';import { EnterpriseApiRateLimitService } from './enterprise-api-rate-limit.service';import { EnterpriseApiAuthService } from './enterprise-api-auth.service';import { EnterpriseApiRoutingService } from './enterprise-api-routing.service';/*** Enterprise API Module with MAXIMUM Parlant Integration
 * 
...
@Module({
  imports: [
    ...
    JwtModule.register({
      secret: process.env.JWT_SECRET ?? 'enterprise-api-secret',signOptions: { expiresIn: '8h' },}),ParlantModule, // Required for MAXIMUM conversational validation
...
export class EnterpriseApiModule {
  constructor() {
    console.log('🚀 Enterprise API Module loaded - MAXIMUM Parlant validation active across ALL operations');console.log('   ✅ API Gateway with conversational routing and load balancing');console.log('   ✅ Authentication with conversational validation and behavioral analysis');console.log('   ✅ Rate limiting with business-aware conversational policies');console.log('   ✅ Health monitoring with Parlant-enhanced diagnostics');console.log('   ✅ Comprehensive audit trails with conversation context');console.log('   🎯 Agent 9 MAXIMUM Parlant integration: COMPLETE');
  }
}