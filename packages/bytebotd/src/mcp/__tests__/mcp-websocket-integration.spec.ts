  ...
  async createClient(
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(
          new Error(`WebSocket connection timeout for client ${clientId}`),
        );
      }, 5000);
...
describe('MCP WebSocket Integration', () => {
  ...
  describe('WebSocket Connection Management', () => {
    it('should establish WebSocket connection successfully', async () => {
      ...
      console.log(
        `[${operationId}] Testing WebSocket connection establishment`,
      );

      const startTime = performance.now();
      const appUrl = await app.getUrl();

      const client = await webSocketManager.createClient(
        appUrl,
        'test-client-1',
      );
      ...
      console.log(
        `[${operationId}] WebSocket connected in ${connectionTime.toFixed(2)}ms`,
      );
    ...
    it('should handle WebSocket disconnection gracefully', async () => {
      ...
      expect(client.connected).toBe(true); // Test graceful disconnection
      ...
      console.log(
        `[${operationId}] WebSocket disconnection handled gracefully`,
      );
    });
    ...
    it('should handle connection errors and retry logic', async () => {
      ...
      console.log(
        `[${operationId}] Testing WebSocket connection error handling`,
      );
  ...
  describe('SSE Endpoint Communication', () => {
    ...
    it('should stream MCP events through SSE', async () => {
      ...
      const testEvents = [
        {
          event: 'tool_call',
          data: JSON.stringify({ tool: 'screenshot', params: {} }),
        },
        {
          event: 'tool_result',
          data: JSON.stringify({ success: true, result: 'completed' }),
        },
      ...
      console.log(
        `[${operationId}] SSE event streaming validated with ${chunks.length} events`,
      );
    });
  ...
  describe('Real-time Tool Execution via WebSocket', () => {
    it('should execute screenshot tool via WebSocket', async () => {
      ...
      console.log(
        `[${operationId}] Testing screenshot tool execution via WebSocket`,
      );
      ...
      expect(mockComputerUseService.screenshot).toHaveBeenCalledWith({
        display: 0,
      });

      console.log(
        `[${operationId}] Screenshot tool execution via WebSocket validated`,
      );
    });
    ...
    it('should execute mouse movement tool via WebSocket', async () => {
      ...
      console.log(
        `[${operationId}] Testing mouse movement tool execution via WebSocket`,
      );
      ...
      console.log(
        `[${operationId}] Mouse movement tool execution via WebSocket validated`,
      );
    });
    ...
    it('should handle tool execution errors via WebSocket', async () => {
      ...
      console.log(
        `[${operationId}] Testing tool execution error handling via WebSocket`,
      );
      ...
      const toolMessage = WebSocketTestData.createToolMessage(
        'invalid_tool',
        {},
      );
      ...
      console.log(
        `[${operationId}] Tool execution error handling via WebSocket validated`,
      );
    });
  });
  ...
  describe('Performance Monitoring', () => {
    ...
    it('should monitor message processing latency', async () => {
      ...
      console.log(
        `[${operationId}] Message processing latency: ${latency.toFixed(2)}ms`,
      );
    });
  });
});
