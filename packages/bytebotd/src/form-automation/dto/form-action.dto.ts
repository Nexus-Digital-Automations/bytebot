...
/**
 * Form field types supported by the automation system
 */
...
export class FormFieldDto {
  @ApiProperty({
    description: 'Field selector (CSS, XPath, or name)',
    example: '#email, input[name="email"], //input[@type="email"]'
  })
  @IsString()
  selector: string;
  ...
  @ApiPropertyOptional({
    description: 'Field label for identification',
  ...
  @ApiPropertyOptional({
    description: 'Field validation pattern (regex)',
...
export class FormAutomationConfigDto {
  @ApiPropertyOptional({
    description: 'Timeout for form operations in milliseconds',
    example: 5000,
    default: 10000
  })
  @IsOptional()
  ...
  @ApiPropertyOptional({
    description: 'Delay between field fills in milliseconds',
    example: 500,
    default: 100
  })
  @IsOptional()
  ...
  @ApiPropertyOptional({
    description: 'Whether to validate form before submission',
    example: true,
    default: true
  })
  @IsOptional()
  ...
  @ApiPropertyOptional({
    description: 'Whether to wait for form submission to complete',
    example: true,
    default: true
  })
  @IsOptional()
...
export class FormActionDto {
  @ApiProperty({
    description: 'Type of form action to perform',
    enum: FormActionType,
  ...
  @ApiPropertyOptional({
    description: 'Form automation configuration',
  ...
  @ApiPropertyOptional({
    ...
    example: { sessionId: 'abc123', userId: 'user456' }
  })
  @IsOptional()
  @IsObject()
...
export class FormDetectionDto extends FormActionDto {
  @ApiProperty({
    description: 'Action type for form detection',
    enum: [FormActionType.DETECT_FORM],
    example: FormActionType.DETECT_FORM
  })
  @IsEnum([FormActionType.DETECT_FORM])
  action: FormActionType.DETECT_FORM;

  @ApiPropertyOptional({
    description: 'Page URL to detect forms on',
  ...
  @ApiPropertyOptional({
    description: 'Whether to include field analysis',
    example: true,
    default: true
  })
  @IsOptional()
...
export class FormFillingDto extends FormActionDto {
  @ApiProperty({
    description: 'Action type for form filling',
    enum: [FormActionType.FILL_FORM],
    example: FormActionType.FILL_FORM
  })
  ...
  @ApiPropertyOptional({
    description: 'Whether to submit form after filling',
    example: false,
    default: false
  })
  @IsOptional()
...
export class FormSubmissionDto extends FormActionDto {
  @ApiProperty({
    description: 'Action type for form submission',
    enum: [FormActionType.SUBMIT_FORM],
    example: FormActionType.SUBMIT_FORM
  })
  ...
  @ApiPropertyOptional({
    ...
    example: 'button[type="submit"], input[type="submit"], #submitBtn'
  })
  @IsOptional()
  @IsString()
  ...
  @ApiPropertyOptional({
    description: 'Expected redirect URL after submission',
...
export class FormValidationDto extends FormActionDto {
  @ApiProperty({
    description: 'Action type for form validation',
    enum: [FormActionType.VALIDATE_FORM],
    example: FormActionType.VALIDATE_FORM
  })
...
export class FormAutoCompleteDto extends FormActionDto {
  @ApiProperty({
    description: 'Action type for auto-complete',
    enum: [FormActionType.AUTO_COMPLETE],
    example: FormActionType.AUTO_COMPLETE
  })
  @IsEnum([FormActionType.AUTO_COMPLETE])
  action: FormActionType.AUTO_COMPLETE;

  @ApiProperty({
    description: 'User profile data for auto-completion',
    example: {
      name: 'John Doe',
