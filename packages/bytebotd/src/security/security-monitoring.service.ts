...
export class SecurityMonitoringService {
  ...
  constructor(private readonly parlantService: ParlantIntegrationService) {
    ...
    this.logger.log(
      `[${operationId}] Initializing Security Monitoring Service with Parlant integration`,
      {
        ...
        conversationalValidationRequired:
          this.getMonitoringConfig().conversationalValidationRequired,
      },
    );
  ...
  async startSecurityMonitoring(
    ...
    try {
      ...
      const validationRequest: ParlantValidationRequest = {
        ...
        riskLevel: RiskLevel._HIGH, // Security monitoring is HIGH risk
        operationId: request.operationId,
      };
      ...
      const monitoringResult = await this.initializeMonitoringSession(
        request,
        validation.conversationId,
      );
    ...
    try {
      ...
      if (!validation.approved) {
        this.logger.warn(
          ...
          { operationId, eventId: event.id, reason: validation.reasoning },
        );
      ...
      const actionTaken = await this.executeSecurityEventProcessing(
        event,
        validation.conversationId,
      );
  ...
  async stopSecurityMonitoring(
    ...
    try {
      ...
      const validationRequest: ParlantValidationRequest = {
        ...
        context,
        riskLevel: RiskLevel._MODERATE, // Stopping monitoring is MEDIUM risk
        operationId,
      };
      ...
      const sessionResult = await this.finalizeMonitoringSession(
        monitoringId,
        validation.conversationId,
      );
  ...
  async getMonitoringStatistics(): Promise<{
    ...
    // Initialize counters
    Object.values(SecurityEventType).forEach(
      (type) => (eventsByType[type] = 0),
    );
    Object.values(SecurityEventSeverity).forEach(
    ...
    const performanceMetrics: SecurityMonitoringMetrics = {
      eventProcessingRate:
        this.totalEventsProcessed / Math.max(1, Math.floor(Date.now() / 60000)), // per minute
      averageResponseTime:
        this.totalEventsProcessed > 0
          ? this.totalResponseTime / this.totalEventsProcessed
          : 0,
      falsePositiveRate:
        this.totalEventsProcessed > 0
          ? this.falsePositives / this.totalEventsProcessed
          : 0,
  ...
  private async initializeMonitoringSession(
    ...
    const result: SecurityMonitoringResult = {
      ...
      conversationalAuditTrail: [
        `Monitoring started via conversation ${conversationId}`,
      ],
  ...
  private async startRealTimeMonitoring(
    ...
    this.logger.log(
      `Real-time monitoring started for session ${result.monitoringId}`,
    );
  }
  ...
  private async executeSecurityEventProcessing(
    ...
    switch (event.severity) {
      case SecurityEventSeverity.CRITICAL:
        actionTaken =
          'CRITICAL: Immediate escalation triggered, incident response initiated';
        break;
      case SecurityEventSeverity.HIGH:
        actionTaken =
          'HIGH: Security team notified, enhanced monitoring activated';
        break;
  ...
  private async triggerSecurityAlert(
    ...
    this.logger.warn(`SECURITY ALERT: ${event.eventType}`, {
      eventId: event.id,
      severity: event.severity,
      riskScore: event.riskScore,
      source: event.source,
  ...
  private async finalizeMonitoringSession(
    ...
    const mutableSession = session as SecurityMonitoringResult & {
      endTime: Date;
    };
    mutableSession.endTime = new Date();
    session.conversationalAuditTrail.push(
      `Monitoring stopped via conversation ${conversationId}`,
    );
  ...
  private cleanupOldEvents(): void {
    ...
    this.securityEvents.splice(
      0,
      this.securityEvents.findIndex((e) => e.timestamp.getTime() > cutoffTime),
    );
    const afterCount = this.securityEvents.length;

    if (beforeCount > afterCount) {
      this.logger.log(
        `Cleaned up ${beforeCount - afterCount} old security events`,
      );
    }
  }
  ...
  private getMonitoringConfig(): SecurityMonitoringConfig {
    return {
      ...
      realTimeAlertsEnabled: this.configService.get<boolean>(
        'REAL_TIME_ALERTS_ENABLED',
        true,
      ),
      eventRetentionDays: this.configService.get<number>(
      ...
      siemIntegrationEnabled: this.configService.get<boolean>(
        'SIEM_INTEGRATION_ENABLED',
        false,
      ),
      conversationalValidationRequired: this.configService.get<boolean>(
        'SECURITY_CONVERSATIONAL_VALIDATION',
        true,
      ),
    };
  }
