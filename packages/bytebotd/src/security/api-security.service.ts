...
export class ApiSecurityService {
  ...
  private readonly endpointProfiles = new Map<
    string,
    ApiEndpointSecurityProfile
  >();
  ...
  constructor(
    ...
    this.logger.log(
      `[${operationId}] Initializing API Security Service with Parlant integration`,
      {
        ...
        conversationalValidationRequired:
          this.getSecurityConfig().conversationalValidationRequired,
      },
    );
  ...
  async performApiSecurityScan(
    ...
    try {
      ...
      const validationRequest: ParlantValidationRequest = {
        ...
        riskLevel: this.getScanRiskLevel(
          request.scanType,
      ...
      const scanResult = await this.executeApiSecurityScan(
        request,
        validation.conversationId,
      );
  ...
  async detectApiThreat(
    ...
    try {
      ...
      if (
        ...
        const detectionResult = await this.executeThreatResponse(
          threatAnalysis,
          validation.conversationId,
        );
        ...
        this.logger.log(`[${operationId}] API threat detected and processed`, {
          operationId,
  ...
  async getApiSecurityStatistics(): Promise<{
    ...
    // Initialize counters
    Object.values(ApiSecurityRiskLevel).forEach(
      (level) => (endpointsByRiskLevel[level] = 0),
    );
    Object.values(ApiThreatType).forEach(
      (type) => (vulnerabilitiesByType[type] = 0),
    );
    ...
    this.scanHistory.forEach((scan) => {
      Object.entries(scan.complianceResults).forEach(
        ([standard, compliant]) => {
    ...
    return {
      ...
      threatBlockingRate:
        this.totalThreatsDetected > 0
          ? this.totalThreatsBlocked / this.totalThreatsDetected
          : 0,
  ...
  private async executeApiSecurityScan(
    ...
    for (const endpoint of request.targetEndpoints) {
      const endpointVulns = await this.scanEndpointForVulnerabilities(
        endpoint,
        request.scanType,
      );
  ...
  private async updateEndpointSecurityProfiles(
    scanResult: ApiSecurityScanResult,
  ): Promise<void> {
  ...
  private generateRecommendations(
    ...
    if (
      vulnerabilities.some(
        (v) => v.type === ApiThreatType.BROKEN_AUTHENTICATION,
      )
    ) {
    ...
    if (
      vulnerabilities.some(
        (v) => v.type === ApiThreatType.SECURITY_MISCONFIGURATION,
      )
    ) {
  ...
  private getScanRiskLevel(
    scanType: ApiSecurityScanType,
    config: ApiScanConfiguration,
  ): RiskLevel {
  ...
  private cleanupOldDetections(): void {
    ...
    this.threatDetections.splice(
      0,
      this.threatDetections.findIndex(
    ...
    if (beforeCount > this.threatDetections.length) {
      this.logger.log(
        `Cleaned up ${beforeCount - this.threatDetections.length} old threat detections`,
      );
    }
  }
  ...
  private updateScanMetrics(duration: number): void {
    ...
    this.averageScanTime =
      (this.averageScanTime * (this.totalScansPerformed - 1) + duration) /
      this.totalScansPerformed;
  }

  private getSecurityConfig(): ApiSecurityConfig {
    return {
      scanningEnabled: this.configService.get<boolean>(
        'API_SECURITY_SCANNING_ENABLED',
        true,
      ),
      threatDetectionEnabled: this.configService.get<boolean>(
        'API_THREAT_DETECTION_ENABLED',
        true,
      ),
      rateLimitingEnabled: this.configService.get<boolean>(
        'API_RATE_LIMITING_ENABLED',
        true,
      ),
      authenticationRequired: this.configService.get<boolean>(
      ...
      scanIntervalMinutes: this.configService.get<number>(
        'API_SCAN_INTERVAL_MINUTES',
        60,
      ),
    };
  }
