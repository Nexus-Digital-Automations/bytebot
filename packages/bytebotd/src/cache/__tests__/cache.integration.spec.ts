...
describe('Cache Redis Integration Tests', () => {
  ...
  describeWithRedis('Real Redis Integration', () => {
    beforeEach(async () => {
    ...
    describe('TTL and Expiration', () => {
      ...
      it('should handle different TTL values', async () => {
        ...
        await cacheService.set('short-ttl', 'expires quickly', { ttl: shortTtl });
        await cacheService.set('long-ttl', 'expires slowly', { ttl: longTtl });
        ...
        expect(shortResult).toBeNull(); // Should be expired
        expect(longResult).toBe('expires slowly'); // Should still exist
      });
      ...
      it('should use default TTL when not specified', async () => {
        ...
        // Should exist with default TTL (5 minutes)
        const retrieved = await cacheService.get(key);
        expect(retrieved).toBe(value);

        // Verify TTL was set in Redis
        const ttl = await redisClient.ttl('bytebot:default-ttl-key');
        expect(ttl).toBeGreaterThan(0);
        expect(ttl).toBeLessThanOrEqual(300); // 5 minutes or less
      });
    });
    ...
    describe('Bulk Operations', () => {
      ...
      it('should handle partial results in bulk operations', async () => {
        // Store only some keys
        await cacheService.set('exists-1', 'value 1');
        await cacheService.set('exists-3', 'value 3');

        // Try to get keys that don't all exist
        const keys = ['exists-1', 'missing-2', 'exists-3'];
        const results = await cacheService.mget(keys);

        expect(results.size).toBe(2);
        expect(results.get('exists-1')).toBe('value 1');
        expect(results.has('missing-2')).toBe(false);
        expect(results.get('exists-3')).toBe('value 3');
      });
    });
    ...
    describe('Cache Warming', () => {
      it('should warm cache with data provider', async () => {
        ...
        const dataProvider = jest.fn((key: string) => 
          Promise.resolve(`generated-data-for-${key}`)
        );
      ...
      it('should skip null values during cache warming', async () => {
        ...
        const validValue = await cacheService.get('warm-valid');
        const nullValue = await cacheService.get('warm-null');

        expect(validValue).toBe('data-for-warm-valid');
    ...
    describe('Namespacing', () => {
      ...
      it('should generate different Redis keys for different namespaces', async () => {
        ...
        expect(redisKeys).toContain('ns1:test-key');
        expect(redisKeys).toContain('ns2:test-key');
        expect(redisKeys.length).toBeGreaterThanOrEqual(2);
      });
    });
    ...
    describe('Error Resilience', () => {
      it('should handle Redis connection errors gracefully', async () => {
        ...
        await expect(cacheService.set('error-key', 'value')).resolves.toBeUndefined();
    ...
    describe('Performance and Concurrency', () => {
      it('should handle concurrent operations', async () => {
        ...
        for (let i = 0; i < keyCount; i++) {
          retrieveOperations.push(cacheService.get(`concurrent-key-${i}`));
        }
    ...
    describe('Statistics Tracking', () => {
      ...
      it('should maintain accurate statistics across multiple operations', async () => {
        ...
        await cacheService.get('nonexistent-1'); // miss
        await cacheService.get('nonexistent-2'); // miss
        await cacheService.get('stat-key-1'); // hit
    ...
    describe('Memory Usage and Cleanup', () => {
      it('should clean up expired keys automatically', async () => {
        ...
        exists = await redisClient.exists('bytebot:cleanup-test');
