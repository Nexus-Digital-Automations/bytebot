...
describe('CacheKeyGenerator', () => {
  ...
  describe('Object Key Generation', () => {
    it('should generate keys from simple objects', () => {
      ...
      expect(result.length).toBe('bytebot:'.length + 12); // MD5 hash is 12 chars
    });

    it('should generate consistent keys for same objects', () => {
      const key1 = { id: 123, type: 'user' };
      const key2 = { type: 'user', id: 123 }; // Different order, same content
  ...
  describe('Long Key Hashing', () => {
    it('should hash very long keys', () => {
      const longKey = 'a'.repeat(300); // Exceeds hash threshold
  ...
  describe('Metadata and Statistics', () => {
    ...
    it('should calculate average key length correctly', () => {
      generator.generate('short'); // 12 chars: bytebot:short
      generator.generate('a'.repeat(20)); // 28 chars: bytebot: + 20 chars

      const stats = generator.getStats();
      expect(stats.avgKeyLength).toBe((12 + 28) / 2);
    });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should use fallback key generation on errors', () => {
      ...
      const result = generator.generate('error-key', 'error-namespace');
      
      // Should fall back to simple key generation
