...
describe('CacheKeyGenerator', () => {
  ...
  describe('generate() - Basic Key Generation', () => {
    it('should generate simple string keys', () => {
      const key = 'simple-key';
      const result = generator.generate(key);
      expect(result).toBe('bytebot:simple-key');
      expect(result).toMatch(/^bytebot:/);
    });
    ...
    it('should add timestamp when requested', () => {
      ...
      const result = generator.generate(key, undefined, {
    ...
    it('should prioritize direct namespace parameter over options', () => {
      ...
      const result = generator.generate(key, 'direct', {
        namespace: 'options',
      });
  ...
  describe('Object Key Generation', () => {
    it('should generate keys from simple objects', () => {
    ...
    it('should generate different keys for different objects', () => {
      const key1 = { id: 123, type: 'user' };
      const key2 = { id: 456, type: 'user' };
      const result1 = generator.generate(key1);
      const result2 = generator.generate(key2);
  ...
  describe('Long Key Hashing', () => {
    it('should hash very long keys', () => {
      const longKey = 'a'.repeat(300); // Exceeds hash thresholdconst result = generator.generate(longKey);expect(result).toMatch(/^bytebot:hash_[a-f0-9]{16}$/);
      expect(result.length).toBeLessThan(50); // Much shorter than original
    });

    it('should not hash short keys', () => {
      const shortKey = 'short';
      const result = generator.generate(shortKey);
      expect(result).toBe('bytebot:short');
    ...
    it('should allow disabling hash for long keys', () => {
      ...
      const result = generator.generate(longKey, undefined, {
        hashLongKeys: false,
      });
  ...
  describe('API Key Generation', () => {
    ...
    it('should handle query parameters', () => {
      ...
      const result = generator.generateApiKey(
        'POST',
        '/api/search',
        queryParams,
      );
  ...
  describe('Key Validation', () => {
    ...
    it('should reject keys with whitespace', () => {
      ...
      jest
        .spyOn(
          generator as unknown as { sanitizeKey: jest.Mock },
          'sanitizeKey',
        )
        .mockReturnValue(keyWithTabs);
  ...
  describe('Metadata and Statistics', () => {
    ...
    it('should track hashed key metadata', () => {
      const longKey = 'a'.repeat(300);
      const result = generator.generate(longKey);
      const metadata = generator.getKeyMetadata(result);
      expect(metadata).toBeDefined();
    ...
    it('should update statistics on key generation', () => {
      ...
      generator.generate('test-key1');
      generator.generate('test-key2', 'custom');
      const updatedStats = generator.getStats();
      expect(updatedStats.totalGenerated).toBe(2);
      expect(updatedStats.namespaceUsage.get('bytebot')).toBe(1);
      expect(updatedStats.namespaceUsage.get('custom')).toBe(1);
      expect(updatedStats.avgKeyLength).toBeGreaterThan(0);
    });

    it('should track hashed keys in statistics', () => {
      const longKey = 'a'.repeat(300);
      generator.generate(longKey);
      const stats = generator.getStats();
      expect(stats.hashedKeys).toBe(1);
    });

    it('should calculate average key length correctly', () => {
      generator.generate('short'); // 12 chars: bytebot:shortgenerator.generate('a'.repeat(20)); // 28 chars: bytebot: + 20 charsconst stats = generator.getStats();expect(stats.avgKeyLength).toBe((12 + 28) / 2);
    });

    it('should clear statistics and metadata', () => {
      generator.generate('test-key');
      const beforeClear = generator.getStats();
      expect(beforeClear.totalGenerated).toBe(1);
      ...
      const metadata = generator.getKeyMetadata('bytebot:test-key');
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle null and undefined inputs', () => {
      ...
      const undefinedResult = generator.generate(
    ...
    it('should use fallback key generation on errors', () => {
      // Mock the generate method to throw an error during processingconst _originalNormalizeKey = (generator as unknown as { normalizeKey: jest.Mock }).normalizeKey;
      jest
        .spyOn(
          generator as unknown as { normalizeKey: jest.Mock },
          'normalizeKey',
        )
        .mockImplementation(() => {
          throw new Error('Normalization error');
        });
      const result = generator.generate('error-key', 'error-namespace'); // Should fall back to simple key generationexpect(result).toBe('error-namespace:error-key');// Restore original method(generator as unknown as { normalizeKey: jest.Mock }).normalizeKey.mockRestore();
    });

    it('should handle circular references in objects gracefully', () => {
      const circularObj: Record<string, unknown> = { prop: 'value' };
      circularObj.circular = circularObj; // Should not throw and should generate a hash
      expect(() => {
  ...
  describe('Performance and Scalability', () => {
    ...
    it('should handle many different namespaces', () => {
      for (let i = 0; i < 100; i++) {
        generator.generate('key', `namespace-${i}`);
      }
  ...
  describe('Complex Integration Scenarios', () => {
    it('should handle mixed key types consistently', () => {
      const stringKey = generator.generate('string-key');
      const arrayKey = generator.generate(['array', 'key']);
      const objectKey = generator.generate({ type: 'object', id: 1 });
      expect(stringKey).toMatch(/^bytebot:/);
      expect(arrayKey).toMatch(/^bytebot:/);
