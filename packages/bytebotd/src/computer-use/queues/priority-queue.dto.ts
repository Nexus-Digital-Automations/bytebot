...
export class QueueJobSubmissionDto {
  ...
  example: { action: 'screenshot', options: { format: 'png' } },})@IsObject()
  payload: unknown;
  ...
  example: 'user_123456789',})@IsOptional()
  @IsString()
  userId?: string;

  @ApiPropertyOptional({
    description: 'Session ID for job tracking',
  example: 'session_987654321',})@IsOptional()
  @IsString()
  sessionId?: string;

  @ApiPropertyOptional({
    description: 'Parent job ID for dependent jobs',
  example: 'job_parent_123',})@IsOptional()
  @IsString()
  parentJobId?: string;
  ...
  example: { department: 'operations', cost_center: 'automation' },})@IsOptional()
  @IsObject()
...
export class QueueJobResponseDto {
  ...
  example: '2023-12-19T10:30:45.789Z',})@IsString()
  submittedAt: string;

  @ApiPropertyOptional({
    description: 'Job processing start timestamp',
  example: '2023-12-19T10:30:50.123Z',})@IsOptional()
  @IsString()
  startedAt?: string;

  @ApiPropertyOptional({
    description: 'Job completion timestamp',
  example: '2023-12-19T10:31:15.456Z',})@IsOptional()
  @IsString()
  completedAt?: string;
  ...
  example: '2023-12-19T10:32:00.000Z',})@IsString()
  ...
  example: { success: true, data: 'screenshot_base64_data' },})@IsOptional()
  result?: unknown;

  @ApiPropertyOptional({
    description: 'Error message if job failed',
  example: 'Action execution failed: Timeout waiting for element',})@IsOptional()
  @IsString()
  errorMessage?: string;
  ...
  @ApiPropertyOptional({
    description: 'Additional job metadata',
  example: { userId: 'user_123', sessionId: 'session_456' },})@IsOptional()
  @IsObject()
  metadata?: Record<string, unknown>;
}
...
export class QueueMetricsDto {
  ...
  example: '2023-12-19T10:45:30.789Z',})@IsString()
  lastUpdated: string;
}
...
export class QueueOperationResultDto<T = unknown> {
  ...
  example: { jobId: 'job_123', queuePosition: 5 },})@IsOptional()
  data?: T;

  @ApiPropertyOptional({
    description: 'Error message if operation failed',
  example: 'Queue capacity exceeded',})@IsOptional()
  @IsString()
  error?: string;
  ...
  example: { priority: 'normal', retryAttempt: 1 },})@IsObject()
  metadata: Record<string, unknown>;
}
...
export class BatchJobSubmissionResultDto {
  ...
  example: [{ index: 3, error: 'Invalid priority value' },{ index: 7, error: 'Queue capacity exceeded' }],})
  @IsArray()
...
export class QueueHealthStatusDto {
  ...
  example: 'healthy',
  enum: ['healthy', 'degraded', 'critical', 'maintenance'],})@IsEnum(['healthy', 'degraded', 'critical', 'maintenance'])status: 'healthy' | 'degraded' | 'critical' | 'maintenance';@ApiProperty({description: 'Detailed health check results',
  example: {redis_connection: 'healthy',
  queue_capacity: 'healthy',
  lock_system: 'healthy',
  deadlock_detection: 'healthy'},})
  @IsObject()
  checks: Record<string, 'healthy' | 'degraded' | 'critical'>;@ApiProperty({description: 'Health check timestamp',
  example: '2023-12-19T10:45:30.789Z',})@IsString()
  lastChecked: string;
