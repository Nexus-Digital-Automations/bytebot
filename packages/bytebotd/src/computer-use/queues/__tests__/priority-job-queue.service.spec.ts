...
import {PriorityJobQueueService,
  EnhancedJobPriority,
  QueueOperation,
} from '../priority-job-queue.service';
import { JobStatus } from '../../dto/async-job.dto';
    // Mock Redis for testingjest.mock('ioredis');

describe('PriorityJobQueueService', () => {let service: PriorityJobQueueService;
    let module: TestingModule;
  let mockRedis: jest.Mocked<Redis>;
  let mockConfigService: jest.Mocked<ConfigService>;

  // Test data
  const testJobPayload = { action: 'screenshot', options: { format: 'png' } };
    const testJobId = 'job_test_123456789';beforeEach(async () => {// Create mock Redis instance
    mockRedis = {
  ...
  REDIS_DB: 0,
    ...
    jest.spyOn(Logger.prototype, 'log').mockImplementation();jest.spyOn(Logger.prototype, 'debug').mockImplementation();jest.spyOn(Logger.prototype, 'warn').mockImplementation();jest.spyOn(Logger.prototype, 'error').mockImplementation();});
  ...
  describe('Service Initialization', () => {it('should be defined', () => {expect(service).toBeDefined();});
    ...
    it('should initialize with correct configuration', async () => {await service.onModuleInit();
      expect(mockRedis.connect).toHaveBeenCalled();
      expect(mockConfigService.get).toHaveBeenCalledWith('REDIS_HOST', 'localhost');
      expect(mockConfigService.get).toHaveBeenCalledWith('QUEUE_MAX_SIZE', 10000);});

  it('should handle Redis connection errors during initialization', async () => {mockRedis.connect.mockRejectedValueOnce(new Error('Redis connection failed'));
    await expect(service.onModuleInit()).rejects.toThrow('Queue service initialization failed');});

  it('should cleanup resources on module destroy', async () => {await service.onModuleInit();
  ...
  describe('Job Enqueue Operations', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should enqueue a job with high priority', async () => {mockRedis.zcard.mockResolvedValue(0);mockRedis.pipeline().exec.mockResolvedValue([]);
        ...
        EnhancedJobPriority.HIGH,
        {
          ...
          maxRetries: 5,
          tags: ['high-priority', 'user-action'],
  userId: 'user123',});
      ...
      expect(result.data?.metadata.tags).toEqual(['high-priority', 'user-action']);
      expect(result.data?.metadata.userId).toBe('user123');});

  it('should reject jobs when queue capacity is exceeded', async () => {// Mock queue size at capacitymockRedis.zcard.mockResolvedValue(1000); // At max capacity
  ...
  it('should handle Redis errors during enqueue', async () => {mockRedis.zcard.mockResolvedValue(0);mockRedis.pipeline().exec.mockRejectedValueOnce(new Error('Redis error'));
    ...
    it('should calculate correct priority scores', async () => {mockRedis.zcard.mockResolvedValue(0);mockRedis.pipeline().exec.mockResolvedValue([]);
      ...
      await service.enqueue('urgent_job', testJobPayload, EnhancedJobPriority.URGENT);
    await service.enqueue('normal_job', testJobPayload, EnhancedJobPriority.NORMAL);
    await service.enqueue('background_job', testJobPayload, EnhancedJobPriority.BACKGROUND);
    // Verify that zadd was called with appropriate scoresconst pipeline = mockRedis.pipeline();
  ...
  submittedAt: new Date().toISOString(),
      ...
      expect(result.data?.metadata.jobId).toBe('urgent_job_123');
      expect(result.data?.metadata.priority).toBe(EnhancedJobPriority.URGENT);
      expect(result.data?.status).toBe(JobStatus.IN_PROGRESS);
    });
  ...
  it('should handle starvation prevention', async () => {// Mock scenario where low priority job has been waiting too longconst oldTimestamp = Date.now() - 400000; // 6.67 minutes ago (exceeds 5 minute threshold)
      ...
      expect(result.success).toBe(true);
      expect(result.data?.metadata.jobId).toBe('starved_job_123');});

  it('should handle Redis errors during dequeue', async () => {mockRedis.zrange.mockRejectedValue(new Error('Redis error'));
  ...
  submittedAt: new Date().toISOString(),
      ...
      expect(result.data?.metadata.jobId).toBe('high_job_123');
      expect(result.data?.status).toBe(JobStatus.PENDING);
    // Verify job was not removed from queue
  ...
  submittedAt: new Date().toISOString(),
    ...
    it('should return null when peeking at empty queue', async () => {mockRedis.zrange.mockResolvedValue([]);
  ...
  describe('Job Removal Operations', () => {
    beforeEach(async () => {
    ...
    it('should return error when trying to remove non-existent job', async () => {mockRedis.hgetall.mockResolvedValue({});

        const result = await service.removeJob('non_existent_job');
      expect(result.success).toBe(false);
  ...
  describe('Job Status Updates', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should return false when updating non-existent job', async () => {mockRedis.hgetall.mockResolvedValue({});
    ...
    it('should handle Redis errors during status update', async () => {const mockJobData = {metadata: JSON.stringify({
      ...
      mockRedis.hset.mockRejectedValue(new Error('Redis error'));
        ...
        const result = await service.updateJobStatus(testJobId, JobStatus.COMPLETED);
      expect(result).toBe(false);
    });
  });
  ...
  describe('Queue Metrics', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should handle metrics retrieval errors gracefully', async () => {mockRedis.hgetall.mockRejectedValue(new Error('Redis error'));
  ...
  payload: JSON.stringify(testJobPayload),
    ...
    it('should return null for non-existent job', async () => {mockRedis.hgetall.mockResolvedValue({});

        const job = await service.getJob('non_existent_job');
      expect(job).toBeNull();});

    it('should handle Redis errors during job retrieval', async () => {mockRedis.hgetall.mockRejectedValue(new Error('Redis error'));
  ...
  it('should retry lock acquisition with backoff', async () => {let callCount = 0;mockRedis.set.mockImplementation(() => {
        callCount++;
        if (callCount <= 3) {
  ...
  describe('Performance and Concurrency', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should handle priority ordering correctly under concurrent access', async () => {mockRedis.set.mockResolvedValue('OK');mockRedis.eval.mockResolvedValue(1);mockRedis.zcard.mockResolvedValue(0);
      ...
      const jobs = [
        { id: 'urgent_job', priority: EnhancedJobPriority.URGENT },{ id: 'normal_job', priority: EnhancedJobPriority.NORMAL },{ id: 'high_job', priority: EnhancedJobPriority.HIGH },{ id: 'low_job', priority: EnhancedJobPriority.LOW },{ id: 'background_job', priority: EnhancedJobPriority.BACKGROUND },];
  ...
  describe('Error Handling and Resilience', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should gracefully handle Redis connection failures', async () => {mockRedis.zcard.mockRejectedValue(new Error('Connection lost'));
  ...
  status: JobStatus.PENDING,
    ...
    it('should recover from transaction failures', async () => {mockRedis.set.mockResolvedValue('OK');mockRedis.eval.mockResolvedValue(1);mockRedis.zcard.mockResolvedValue(0);
      ...
      mockRedis.pipeline().exec.mockRejectedValueOnce(new Error('Transaction failed'));
