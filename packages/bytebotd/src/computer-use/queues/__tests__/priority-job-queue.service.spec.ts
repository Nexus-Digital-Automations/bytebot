...
describe('PriorityJobQueueService', () => {
  let service: PriorityJobQueueService;
  ...
  const testJobId = 'job_test_123456789';
  ...
  describe('Job Enqueue Operations', () => {
    beforeEach(async () => {
      await service.onModuleInit();
    });
    ...
    it('should enqueue a job with default priority', async () => {
      ...
      mockRedis.zcard.mockResolvedValue(0); // Queue size
    ...
    it('should reject jobs when queue capacity is exceeded', async () => {
      // Mock queue size at capacity
      mockRedis.zcard.mockResolvedValue(1000); // At max capacity
    ...
    it('should handle Redis errors during enqueue', async () => {
      mockRedis.zcard.mockResolvedValue(0);
      mockRedis.pipeline().exec.mockRejectedValueOnce(new Error('Redis error'));
    ...
    it('should calculate correct priority scores', async () => {
      mockRedis.zcard.mockResolvedValue(0);
      mockRedis.pipeline().exec.mockResolvedValue([]);
  ...
  describe('Job Dequeue Operations', () => {
    ...
    it('should handle starvation prevention', async () => {
      // Mock scenario where low priority job has been waiting too long
      const oldTimestamp = Date.now() - 400000; // 6.67 minutes ago (exceeds 5 minute threshold)

      mockRedis.zrange.mockImplementation((key: string, start: number, stop: number, ...args: any[]) => {
        if (key.includes('low') && args[0] === 'WITHSCORES') {
          return Promise.resolve(['starved_job_123', oldTimestamp.toString()]);
        }
        if (key.includes('low')) {
          return Promise.resolve(['starved_job_123']);
        }
        return Promise.resolve([]);
      });
    ...
    it('should handle Redis errors during dequeue', async () => {
  ...
  describe('Job Peek Operations', () => {
    beforeEach(async () => {
      await service.onModuleInit();
    });

    it('should peek at next job without removing it', async () => {
      // Mock a job in the high priority queue
      mockRedis.zrange.mockImplementation((key: string) => {
        if (key.includes('high')) {
          return Promise.resolve(['high_job_123']);
        }
        return Promise.resolve([]);
      });
    ...
    it('should peek at specific priority queue', async () => {
      mockRedis.zrange.mockImplementation((key: string) => {
        if (key.includes('normal')) {
          return Promise.resolve(['normal_job_123']);
        }
        return Promise.resolve([]);
      });
    ...
    it('should return null when peeking at empty queue', async () => {
      mockRedis.zrange.mockResolvedValue([]);
  ...
  describe('Job Removal Operations', () => {
    ...
    it('should return error when trying to remove non-existent job', async () => {
  ...
  describe('Job Status Updates', () => {
    ...
    it('should update job status successfully', async () => {
      ...
      const result = await service.updateJobStatus(
        testJobId,
        JobStatus.COMPLETED,
        { success: true, data: 'screenshot_data' },
        undefined
      );
    ...
    it('should return false when updating non-existent job', async () => {
      mockRedis.hgetall.mockResolvedValue({});
    ...
    it('should handle Redis errors during status update', async () => {
      const mockJobData = {
        metadata: JSON.stringify({
          jobId: testJobId,
          priority: EnhancedJobPriority.NORMAL,
  ...
  describe('Queue Metrics', () => {
    ...
    it('should return comprehensive queue metrics', async () => {
      ...
      mockRedis.hgetall.mockResolvedValue(mockMetricsData);
  ...
  describe('Job Retrieval', () => {
    ...
    it('should retrieve job details successfully', async () => {
      const mockJobData = {
        metadata: JSON.stringify({
          jobId: testJobId,
          priority: EnhancedJobPriority.NORMAL,
  ...
  describe('Distributed Locking', () => {
    ...
    it('should acquire and release locks properly during enqueue', async () => {
    ...
    it('should retry lock acquisition with backoff', async () => {
      ...
      mockRedis.set.mockImplementation(() => {
        ...
        return Promise.resolve('OK'); // Success on 4th attempt
      });
  ...
  describe('Performance and Concurrency', () => {
    ...
    it('should handle multiple concurrent enqueue operations', async () => {
    ...
    it('should handle priority ordering correctly under concurrent access', async () => {
      mockRedis.set.mockResolvedValue('OK');
      mockRedis.eval.mockResolvedValue(1);
      mockRedis.zcard.mockResolvedValue(0);
      mockRedis.pipeline().exec.mockResolvedValue([]);
      ...
      const jobs = [
        { id: 'urgent_job', priority: EnhancedJobPriority.URGENT },
        { id: 'normal_job', priority: EnhancedJobPriority.NORMAL },
        { id: 'high_job', priority: EnhancedJobPriority.HIGH },
        { id: 'low_job', priority: EnhancedJobPriority.LOW },
        { id: 'background_job', priority: EnhancedJobPriority.BACKGROUND },
      ];
  ...
  describe('Error Handling and Resilience', () => {
    ...
    it('should gracefully handle Redis connection failures', async () => {
      mockRedis.zcard.mockRejectedValue(new Error('Connection lost'));
  ...
  describe('Queue Configuration and Limits', () => {
    beforeEach(async () => {
      await service.onModuleInit();
    });

    it('should respect queue capacity limits', async () => {
      // Mock queue at 80% capacity (backpressure threshold)
      mockRedis.zcard.mockResolvedValue(800);
    ...
    it('should activate backpressure at configured threshold', async () => {
      // Mock queue at 90% capacity (above backpressure threshold)
      mockRedis.zcard.mockResolvedValue(900);
      mockRedis.set.mockResolvedValue('OK');
      mockRedis.eval.mockResolvedValue(1);
      mockRedis.pipeline().exec.mockResolvedValue([]);
