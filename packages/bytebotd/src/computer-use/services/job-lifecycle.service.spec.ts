...
describe('JobLifecycleService', () => {
  ...
  describe('Job State Machine', () => {
    it('should validate correct state transitions', () => {
      expect(stateMachine.validateTransition(JobLifecycleState.SUBMITTED, JobLifecycleState.QUEUED)).toBe(true);
    ...
    it('should reject invalid state transitions', () => {
      expect(stateMachine.validateTransition(JobLifecycleState.COMPLETED, JobLifecycleState.RUNNING)).toBe(false);
    ...
    it('should identify terminal states correctly', () => {
      expect(stateMachine.isTerminalState(JobLifecycleState.COMPLETED)).toBe(false); // Can transition to EXPIRED
      expect(stateMachine.isTerminalState(JobLifecycleState.EXPIRED)).toBe(true);
      expect(stateMachine.isTerminalState(JobLifecycleState.RUNNING)).toBe(false);
    });

    it('should return valid next states', () => {
      const validStates = stateMachine.getValidNextStates(JobLifecycleState.RUNNING);
  ...
  describe('Job Dependency Management', () => {
    it('should add and retrieve dependencies correctly', () => {
      const jobA = 'job-a';
      const jobB = 'job-b';
      const jobC = 'job-c';

      dependencyManager.addDependency(jobB, jobA);
      dependencyManager.addDependency(jobC, jobA);
  ...
  describe('Progress Tracking', () => {
    it('should update and track progress correctly', () => {
    ...
    it('should calculate ETA based on progress history', () => {
      const jobId = 'test-job-eta';

      // Simulate step-by-step progress
      progressTracker.updateProgress(jobId, 10, 'Step 1', 10, 1);

      const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

      return delay(50).then(() => {
        progressTracker.updateProgress(jobId, 20, 'Step 2', 10, 2);
        return delay(50);
      }).then(() => {
    ...
    it('should retrieve current progress', () => {
      const jobId = 'test-job-retrieve';

      progressTracker.updateProgress(jobId, 75, 'Almost done', 4, 3);
    ...
    it('should clear progress history', () => {
      ...
      progressTracker.updateProgress(jobId, 50, 'Halfway', 2, 1);
      expect(progressTracker.getProgress(jobId)).toBeDefined();
  ...
  describe('Result Management', () => {
    it('should store and retrieve results', async () => {
      ...
      await resultManager.storeResult(jobId, testResult, 3600, false);
    ...
    it('should handle compression', async () => {
      const jobId = 'test-job-compressed';
      const testResult = { large: 'data'.repeat(1000) };

      await resultManager.storeResult(jobId, testResult, 3600, true);
    ...
    it('should check result existence', async () => {
      const jobId = 'test-job-exists';

      // Mock Redis exists response
      const mockRedis = (resultManager as any).redis;
    ...
    it('should get result TTL', async () => {
    ...
    it('should delete results', async () => {
      ...
      await resultManager.deleteResult(jobId);
  ...
  describe('Job Lifecycle Operations', () => {
    it('should submit a job successfully', async () => {
      const result = await service.submitJob(mockAction, mockConfig);
    ...
    it('should reject jobs with circular dependencies', async () => {
      const jobA = await service.submitJob(mockAction, mockConfig);
    ...
    it('should get job status', async () => {
      const submissionResult = await service.submitJob(mockAction, mockConfig);
    ...
    it('should throw error for non-existent job status', () => {
      expect(() => service.getJobStatus('non-existent-job')).toThrow('Job not found');
    });

    it('should get job result for completed job', async () => {
      const submissionResult = await service.submitJob(mockAction, mockConfig);
    ...
    it('should handle job failures and retries', async () => {
      const submissionResult = await service.submitJob(mockAction, {
        ...mockConfig,
        maxRetries: 2,
      });
  ...
  describe('Batch Job Operations', () => {
    it('should submit batch jobs successfully', async () => {
      const batchConfig = {
        batchId: 'test-batch',
        name: 'Test Batch',
  ...
  describe('Job Statistics', () => {
    it('should provide comprehensive job statistics', async () => {
      // Submit various jobs
      await service.submitJob(mockAction, { ...mockConfig, priority: JobPriority.HIGH });
  ...
  describe('Event Emission', () => {
    ...
    it('should emit progress update events', async () => {
      ...
      expect(eventEmitter.emit).toHaveBeenCalledWith(
        JobLifecycleEvent.JOB_PROGRESS_UPDATED,
        expect.objectContaining({
          jobId: submissionResult.jobId,
  ...
  describe('Error Handling', () => {
    it('should handle service initialization errors gracefully', async () => {
      // Test that service can handle Redis connection failures
      const failingService = new JobLifecycleService(
        configService,
        eventEmitter,
        stateMachine,
        dependencyManager,
    ...
    it('should handle result storage failures gracefully', async () => {
      const submissionResult = await service.submitJob(mockAction, mockConfig);
      ...
      await expect(service.completeJob(submissionResult.jobId, { success: true })).resolves.not.toThrow();
    });

    it('should handle invalid job operations gracefully', async () => {
      // Test operations on non-existent jobs
      expect(await service.cancelJob('non-existent')).toBe(false);
      expect(await service.pauseJob('non-existent')).toBe(false);
      expect(await service.resumeJob('non-existent')).toBe(false);

      // Should not throw for progress updates on non-existent jobs
      await expect(service.updateJobProgress('non-existent', 50, 'test')).resolves.not.toThrow();
    });
  });

  describe('Memory Management and Cleanup', () => {
    it('should clean up progress tracking data', () => {
      const jobId = 'cleanup-test-job';

      progressTracker.updateProgress(jobId, 50, 'Test', 2, 1);
      expect(progressTracker.getProgress(jobId)).toBeDefined();
    ...
    it('should limit progress history size', () => {
      const jobId = 'history-limit-test';
    ...
    it('should handle service shutdown gracefully', async () => {
      const testService = new JobLifecycleService(
        configService,
        eventEmitter,
        stateMachine,
        dependencyManager,
  ...
  describe('Integration Scenarios', () => {
    it('should handle complex job dependency chain', async () => {
      ...
      const jobC = await service.submitJob(mockAction, {
        ...mockConfig,
        dependencies: [
          { jobId: jobB.jobId, type: 'completion' },
          { jobId: jobD.jobId, type: 'completion' },
        ],
      });
    ...
    it('should handle job lifecycle with progress tracking and completion', async () => {
      const job = await service.submitJob(mockAction, mockConfig);
      ...
      const finalResult = { success: true, data: 'completed' };
      await service.completeJob(job.jobId, finalResult);
    ...
    it('should handle batch processing with mixed outcomes', async () => {
      const batchConfig = {
