...
import { ComputerActionDto } from '../dto/computer-action.dto';
    // Mock Redis for testingjest.mock('ioredis', () => {return jest.fn().mockImplementation(() => ({setex: jest.fn().mockResolvedValue('OK'),
...
describe('JobLifecycleService', () => {let service: JobLifecycleService;
    let stateMachine: JobLifecycleStateMachine;
  ...
  describe('Job State Machine', () => {it('should validate correct state transitions', () => {expect(stateMachine.validateTransition(JobLifecycleState.SUBMITTED, JobLifecycleState.QUEUED)).toBe(true);
      expect(stateMachine.validateTransition(JobLifecycleState.QUEUED, JobLifecycleState.RUNNING)).toBe(false);
    ...
    it('should reject invalid state transitions', () => {expect(stateMachine.validateTransition(JobLifecycleState.COMPLETED, JobLifecycleState.RUNNING)).toBe(false);
      expect(stateMachine.validateTransition(JobLifecycleState.FAILED, JobLifecycleState.RUNNING)).toBe(false);
    ...
    it('should return valid next states', () => {const validStates = stateMachine.getValidNextStates(JobLifecycleState.RUNNING);
      expect(validStates).toContain(JobLifecycleState.PAUSED);
  ...
  describe('Job Dependency Management', () => {it('should add and retrieve dependencies correctly', () => {const jobA = 'job-a';
    const jobB = 'job-b';
    const jobC = 'job-c';dependencyManager.addDependency(jobB, jobA);dependencyManager.addDependency(jobC, jobA);
    ...
    it('should detect circular dependencies', () => {const jobA = 'job-a';
    const jobB = 'job-b';
    const jobC = 'job-c';dependencyManager.addDependency(jobB, jobA);dependencyManager.addDependency(jobC, jobB);
    ...
    it('should check dependency satisfaction correctly', () => {const jobA = 'job-a';
    const jobB = 'job-b';
    const jobC = 'job-c';dependencyManager.addDependency(jobC, jobA);dependencyManager.addDependency(jobC, jobB);
    ...
    it('should generate correct execution order', () => {const jobA = 'job-a';
    const jobB = 'job-b';
    const jobC = 'job-c';
    const jobD = 'job-d';dependencyManager.addDependency(jobB, jobA);dependencyManager.addDependency(jobC, jobA);
  ...
  describe('Progress Tracking', () => {it('should update and track progress correctly', () => {const jobId = 'test-job';
    const progress1 = progressTracker.updateProgress(jobId, 25, 'Step 1', 4, 1);
      expect(progress1.percentage).toBe(25);
      expect(progress1.currentStep).toBe('Step 1');
      expect(progress1.completedSteps).toBe(1);
      expect(progress1.totalSteps).toBe(4);

      // Wait a bit to simulate time passage
      const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

      return delay(100).then(() => {
        const progress2 = progressTracker.updateProgress(jobId, 50, 'Step 2', 4, 2);
      expect(progress2.percentage).toBe(50);
      expect(progress2.currentStep).toBe('Step 2');
      expect(progress2.averageStepTime).toBeGreaterThan(0);
      expect(progress2.estimatedTimeRemaining).toBeGreaterThan(0);
      });
    });

    it('should calculate ETA based on progress history', () => {const jobId = 'test-job-eta';
    // Simulate step-by-step progressprogressTracker.updateProgress(jobId, 10, 'Step 1', 10, 1);

        const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));return delay(50).then(() => {
        progressTracker.updateProgress(jobId, 20, 'Step 2', 10, 2);return delay(50);}).then(() => {
        const progress = progressTracker.updateProgress(jobId, 30, 'Step 3', 10, 3);
      expect(progress.averageStepTime).toBeGreaterThan(0);
    ...
    it('should retrieve current progress', () => {const jobId = 'test-job-retrieve';progressTracker.updateProgress(jobId, 75, 'Almost done', 4, 3);
      ...
      expect(currentProgress!.currentStep).toBe('Almost done');});

  it('should clear progress history', () => {const jobId = 'test-job-clear';progressTracker.updateProgress(jobId, 50, 'Halfway', 2, 1);
      expect(progressTracker.getProgress(jobId)).toBeDefined();progressTracker.clearProgress(jobId);
      expect(progressTracker.getProgress(jobId)).toBeNull();
    });
  });

  describe('Result Management', () => {it('should store and retrieve results', async () => {const jobId = 'test-job-result';
    const testResult = { success: true, data: 'test-data' };await resultManager.storeResult(jobId, testResult, 3600, false);
    ...
    it('should handle compression', async () => {const jobId = 'test-job-compressed';
    const testResult = { large: 'data'.repeat(1000) };await resultManager.storeResult(jobId, testResult, 3600, true);
    // Verify compression was used (store call should have base64 data)
      const mockRedis = (resultManager as any).redis;
      const storeCall = mockRedis.setex.mock.calls.find((call: any) => call[0] === jobId);
      expect(storeCall).toBeDefined();
      expect(typeof storeCall[2]).toBe('string');});

  it('should check result existence', async () => {const jobId = 'test-job-exists';
    // Mock Redis exists responseconst mockRedis = (resultManager as any).redis;
    ...
    it('should get result TTL', async () => {const jobId = 'test-job-ttl';
    const expectedTtl = 1800;
    // Mock Redis TTL response
      const mockRedis = (resultManager as any).redis;
    ...
    it('should delete results', async () => {const jobId = 'test-job-delete';await resultManager.deleteResult(jobId);
  ...
  describe('Job Lifecycle Operations', () => {it('should submit a job successfully', async () => {const result = await service.submitJob(mockAction, mockConfig);
    ...
    it('should reject jobs with circular dependencies', async () => {const jobA = await service.submitJob(mockAction, mockConfig);

        const configWithDep = {
        ...mockConfig,
        dependencies: [{ jobId: jobA.jobId, type: 'completion' as const }],};
    const jobB = await service.submitJob(mockAction, configWithDep);

      // Try to create circular dependency
      const circularConfig = {
        ...mockConfig,
        dependencies: [{ jobId: jobB.jobId, type: 'completion' as const }],};
    // Add circular dependency to dependency manager
      dependencyManager.addDependency(jobA.jobId, jobB.jobId);

      await expect(service.submitJob(mockAction, circularConfig)).rejects.toThrow('Circular dependency detected');});

  it('should get job status', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    ...
    it('should throw error for non-existent job status', () => {expect(() => service.getJobStatus('non-existent-job')).toThrow('Job not found');});

  it('should get job result for completed job', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);

        const testResult = { success: true, screenshot: 'base64-data' };
    // Complete the jobawait service.completeJob(submissionResult.jobId, testResult);
    ...
    it('should throw error for incomplete job result', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    await expect(service.getJobResult(submissionResult.jobId)).rejects.toThrow('has not completed yet');});

  it('should cancel jobs successfully', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    ...
    it('should not cancel already completed jobs', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    await service.completeJob(submissionResult.jobId, { success: true });

        const cancelled = await service.cancelJob(submissionResult.jobId);
      expect(cancelled).toBe(false);
    });

    it('should pause and resume jobs', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    // Manually transition to running state for testing
    ...
    it('should update job progress', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    await service.updateJobProgress(submissionResult.jobId, 50, 'Processing', 10, 5);

        const status = service.getJobStatus(submissionResult.jobId);
      expect(status.progress).toBe(50);
      expect(status.metadata!.currentStep).toBe('Processing');
      expect(status.metadata!.estimatedTimeRemaining).toBeGreaterThanOrEqual(0);});
    ...
    await service.failJob(submissionResult.jobId, testError);
  ...
  describe('Batch Job Operations', () => {it('should submit batch jobs successfully', async () => {const batchConfig = {batchId: 'test-batch',
  name: 'Test Batch',
  ...
  webhooks: [],
    ...
    // Verify all jobs have the batch ID
      result.jobIds.forEach(jobId => {
        const status = service.getJobStatus(jobId);
        expect(status.metadata!.batchId).toBe('test-batch');});
});

    it('should handle batch submission errors gracefully', async () => {const batchConfig = {batchId: 'error-batch',
  name: 'Error Batch',
  description: 'Batch that should fail',
  strategy: 'parallel' as const,
  ...
  webhooks: [],
      ...
      dependencyManager.addDependency(firstJob.jobId, 'non-existent-job');
  ...
  describe('Event Emission', () => {it('should emit lifecycle events', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    // Verify job submission event was emitted
    ...
    it('should emit progress update events', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    await service.updateJobProgress(submissionResult.jobId, 50, 'Processing');
      expect(eventEmitter.emit).toHaveBeenCalledWith(JobLifecycleEvent.JOB_PROGRESS_UPDATED,
        expect.objectContaining({
          jobId: submissionResult.jobId,
    ...
    it('should emit completion events', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
  ...
  describe('Error Handling', () => {it('should handle service initialization errors gracefully', async () => {// Test that service can handle Redis connection failuresconst failingService = new JobLifecycleService(
    ...
    it('should handle result storage failures gracefully', async () => {const submissionResult = await service.submitJob(mockAction, mockConfig);
    // Mock Redis failure
      const mockRedis = (resultManager as any).redis;
      mockRedis.setex.mockRejectedValueOnce(new Error('Redis connection failed'));
    // Should handle storage failure gracefullyawait expect(service.completeJob(submissionResult.jobId, { success: true })).resolves.not.toThrow();
    });

    it('should handle invalid job operations gracefully', async () => {// Test operations on non-existent jobsexpect(await service.cancelJob('non-existent')).toBe(false);
      expect(await service.pauseJob('non-existent')).toBe(false);
      expect(await service.resumeJob('non-existent')).toBe(false);
    // Should not throw for progress updates on non-existent jobsawait expect(service.updateJobProgress('non-existent', 50, 'test')).resolves.not.toThrow();});
});

  describe('Memory Management and Cleanup', () => {it('should clean up progress tracking data', () => {const jobId = 'cleanup-test-job';progressTracker.updateProgress(jobId, 50, 'Test', 2, 1);
      expect(progressTracker.getProgress(jobId)).toBeDefined();progressTracker.clearProgress(jobId);
      expect(progressTracker.getProgress(jobId)).toBeNull();
    });
  ...
  describe('Integration Scenarios', () => {it('should handle complex job dependency chain', async () => {// Create a complex dependency chain: A -> B -> C, A -> D -> Cconst jobA = await service.submitJob(mockAction, mockConfig);
        ...
        ...mockConfig,
        dependencies: [{ jobId: jobA.jobId, type: 'completion' }],});

        const jobD = await service.submitJob(mockAction, {
        ...mockConfig,
        dependencies: [{ jobId: jobA.jobId, type: 'completion' }],});

        const jobC = await service.submitJob(mockAction, {
        ...mockConfig,
        dependencies: [
    ...
    it('should handle job lifecycle with progress tracking and completion', async () => {const job = await service.submitJob(mockAction, mockConfig);
    // Simulate job progression
      await service.updateJobProgress(job.jobId, 25, 'Starting', 4, 1);
    await service.updateJobProgress(job.jobId, 50, 'Halfway', 4, 2);
    await service.updateJobProgress(job.jobId, 75, 'Almost done', 4, 3);

        const finalResult = { success: true, data: 'completed' };await service.completeJob(job.jobId, finalResult);

        const jobResult = await service.getJobResult(job.jobId);
      expect(jobResult.status).toBe(JobStatus.COMPLETED);
      expect(jobResult.result).toEqual(finalResult);
      expect(jobResult.executionTimeMs).toBeGreaterThan(0);
    });

    it('should handle batch processing with mixed outcomes', async () => {const batchConfig = {batchId: 'mixed-outcome-batch',
  name: 'Mixed Outcome Batch',
  description: 'Batch with successful and failed jobs',
  strategy: 'parallel' as const,
  failurePolicy: 'continue' as const,
  timeout: 60000,
  webhooks: [],
