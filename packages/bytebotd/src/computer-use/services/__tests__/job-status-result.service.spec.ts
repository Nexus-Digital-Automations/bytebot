...
import { JobStatusResultService } from '../job-status-result.service';
import { CacheService } from '../../../cache/cache.service';
import { MetricsService } from '../../../metrics/metrics.service';
import { JobStatus, JobPriority } from '../../dto/async-job.dto';
    // Mock dependenciesconst mockRedis = {
  pipeline: jest.fn(() => ({
...
describe('JobStatusResultService', () => {let service: JobStatusResultService;
    let module: TestingModule;

  const sampleJobId = 'job_1702983456789_abc123';
    const sampleEnhancedStatus = {jobId: sampleJobId,
  ...
  currentStepIndex: 2,
    ...
    timestamps: {
      submitted: new Date('2023-12-19T10:30:45.789Z'),
  started: new Date('2023-12-19T10:30:46.123Z'),
  lastUpdated: new Date('2023-12-19T10:31:10.456Z'),},
  performance: {
  ...
  describe('Service Initialization', () => {it('should be defined', () => {expect(service).toBeDefined();});
    ...
    it('should initialize with correct streaming configuration', () => {const streamingConfig = (service as any).streamingConfig;
      expect(streamingConfig).toEqual({
        chunkSize: 1024 * 1024,
    ...
    it('should initialize retention policies', () => {const retentionPolicies = (service as any).retentionPolicies;
      expect(retentionPolicies.size).toBeGreaterThan(0);
    });
  });
  ...
  describe('Job Status Management', () => {describe('updateJobStatus', () => {beforeEach(() => {mockCacheService.get.mockResolvedValue(sampleEnhancedStatus);
      ...
      it('should update job status successfully', async () => {const pipeline = {hset: jest.fn().mockReturnThis(),
        ...
        await service.updateJobStatus(
          ...
          JobStatus.COMPLETED,
          100,
          { currentStep: 'Completed successfully' },{ completedBy: 'system' });
      expect(pipeline.hset).toHaveBeenCalled();
        ...
        expect(mockEventEmitter.emit).toHaveBeenCalledWith('job.status.updated', expect.objectContaining({jobId: sampleJobId,
  status: JobStatus.COMPLETED,
          progress: 100,
        }));
      });

      it('should validate job ID', async () => {await expect(service.updateJobStatus('', JobStatus.COMPLETED, 100)).rejects.toThrow(BadRequestException);
    await expect(service.updateJobStatus(null as any, JobStatus.COMPLETED, 100))
          .rejects.toThrow(BadRequestException);
      });

      it('should validate progress range', async () => {mockCacheService.get.mockResolvedValue(sampleEnhancedStatus);
    await expect(service.updateJobStatus(sampleJobId, JobStatus.IN_PROGRESS, -10))
          .rejects.toThrow(BadRequestException);
      ...
      it('should clamp progress values to valid range', async () => {mockCacheService.get.mockResolvedValue(sampleEnhancedStatus);
  ...
  describe('Result Management', () => {const sampleResult = {screenshot: 'base64-encoded-data',
  success: true,
  metadata: { action: 'screenshot' },};describe('storeJobResult', () => {it('should store small result directly', async () => {const resultData = { test: 'data' };mockRedis.hset.mockResolvedValue('OK');mockRedis.expire.mockResolvedValue(1);

        const storageInfo = await service.storeJobResult(
          sampleJobId,
          resultData,
        ...
        expect(storageInfo.format).toBe('json');
      expect(storageInfo.compressed).toBe(false); // Small data not compressedexpect(mockRedis.hset).toHaveBeenCalled();
        expect(mockCacheService.set).toHaveBeenCalled();
      });

      it('should compress large results', async () => {const largeResult = { data: 'x'.repeat(2000) }; // Large enough to trigger compressionmockRedis.hset.mockResolvedValue('OK');mockRedis.expire.mockResolvedValue(1);
      ...
      it('should handle streaming for very large results', async () => {const veryLargeResult = { data: 'x'.repeat(6 * 1024 * 1024) }; // 6MBmockRedis.hset.mockResolvedValue('OK');mockRedis.expire.mockResolvedValue(1);
        ...
        expect(storageInfo.format).toBe('stream');});

  it('should generate and verify checksums', async () => {const testData = { test: 'checksum' };mockRedis.hset.mockResolvedValue('OK');mockRedis.expire.mockResolvedValue(1);
    ...
    describe('getJobResult', () => {const mockStorageInfo = {resultId: 'result_123',
  size: 1024,
  compressed: false,
        format: 'json',
  ...
  expiresAt: new Date().toISOString(),
      ...
      it('should retrieve result from Redis when cache misses', async () => {const storedResult = { test: 'redis' };mockCacheService.get.mockResolvedValue(null);(service as any).getResultStorageInfo = jest.fn().mockResolvedValue(mockStorageInfo);
        mockRedis.hget.mockResolvedValue(Buffer.from(JSON.stringify(storedResult)).toString('base64'));
      ...
      it('should handle compressed results', async () => {const originalData = { test: 'compressed' };
    const compressedStorageInfo = { ...mockStorageInfo, compressed: true };mockCacheService.get.mockResolvedValue(null);
        (service as any).getResultStorageInfo = jest.fn().mockResolvedValue(compressedStorageInfo);

        const zlib = require('zlib');

        const compressed = zlib.gzipSync(JSON.stringify(originalData));mockRedis.hget.mockResolvedValue(compressed.toString('base64'));
      ...
      it('should return stream for streaming request', async () => {const streamingStorageInfo = { ...mockStorageInfo, chunks: 5 };(service as any).getResultStorageInfo = jest.fn().mockResolvedValue(streamingStorageInfo);
        (service as any).createResultStream = jest.fn().mockReturnValue('mock-stream');

        const result = await service.getJobResult(sampleJobId, true);
      expect(result).toBe('mock-stream');
      expect((service as any).createResultStream).toHaveBeenCalledWith(sampleJobId, streamingStorageInfo);});

      it('should throw NotFoundException for missing result', async () => {(service as any).getResultStorageInfo = jest.fn().mockResolvedValue(null);
    await expect(service.getJobResult(sampleJobId, false))
          .rejects.toThrow(NotFoundException);
      });
    });
  });

  describe('History and Audit Trail', () => {describe('recordJobHistory', () => {it('should record history entry successfully', async () => {mockRedis.lpush.mockResolvedValue(1);mockRedis.ltrim.mockResolvedValue('OK');mockRedis.expire.mockResolvedValue(1);
    await service.recordJobHistory(
          sampleJobId,
      ...
      it('should not throw on history recording errors', async () => {mockRedis.lpush.mockRejectedValue(new Error('Redis error'));
    // Should not throwawait expect(service.recordJobHistory(sampleJobId, 'started', {})).resolves.not.toThrow();});
    });
  ...
  source: 'system',}),];
      ...
      it('should retrieve job history', async () => {mockRedis.lrange.mockResolvedValue(mockHistoryData);

        const history = await service.getJobHistory(sampleJobId, 100, 0);

        expect(history).toHaveLength(2);
        expect(history[0].event).toBe('created');
      expect(history[1].event).toBe('started');
  ...
  describe('Analytics and Monitoring', () => {describe('getJobAnalytics', () => {beforeEach(() => {mockCacheService.get.mockResolvedValue(sampleEnhancedStatus);
      ...
      it('should calculate comprehensive analytics', async () => {const analytics = await service.getJobAnalytics(sampleJobId);
    ...
    describe('getSystemMetrics', () => {it('should return system performance metrics', async () => {const metrics = await service.getSystemMetrics();
  ...
  describe('Data Retention and Cleanup', () => {describe('cleanupJob', () => {
    ...
    describe('performRetentionCleanup', () => {it('should perform scheduled cleanup', async () => {const oldJobKeys = ['bytebot:job:status:old_job_1','bytebot:job:status:old_job_2',];mockRedis.keys.mockResolvedValue(oldJobKeys);
        ...
        expect(mockRedis.keys).toHaveBeenCalledWith('bytebot:job:status:*');
      expect((service as any).deleteJobData).toHaveBeenCalledTimes(2);
  ...
  describe('Error Handling', () => {it('should handle Redis connection failures gracefully', async () => {mockRedis.pipeline.mockImplementation(() => {throw new Error('Redis connection failed');});
    await expect(service.updateJobStatus(sampleJobId, JobStatus.COMPLETED, 100))
        .rejects.toThrow(InternalServerErrorException);
    });

    it('should handle cache service failures gracefully', async () => {mockCacheService.get.mockRejectedValue(new Error('Cache error'));mockRedis.hget.mockResolvedValue(JSON.stringify(sampleEnhancedStatus));
    // Should still work by falling back to Redis
      const result = await service.getJobStatus(sampleJobId);
      expect(result).toBeDefined();
    });

    it('should handle JSON parsing errors', async () => {mockCacheService.get.mockResolvedValue(null);mockRedis.hget.mockResolvedValue('invalid-json');
    await expect(service.getJobStatus(sampleJobId)).rejects.toThrow(InternalServerErrorException);
    });
    ...
    it('should handle compression/decompression errors', async () => {const storageInfo = { compressed: true, format: 'json' };mockCacheService.get.mockResolvedValue(null);(service as any).getResultStorageInfo = jest.fn().mockResolvedValue(storageInfo);
      mockRedis.hget.mockResolvedValue('invalid-compressed-data');
    await expect(service.getJobResult(sampleJobId, false)).rejects.toThrow(InternalServerErrorException);
    });
  });

  describe('Performance and Concurrency', () => {it('should handle high-frequency status updates', async () => {mockCacheService.get.mockResolvedValue(sampleEnhancedStatus);
  ...
  describe('Data Integrity', () => {it('should maintain data consistency across Redis and cache', async () => {
    ...
    it('should verify result checksums on retrieval', async () => {const testData = { test: 'checksum' };
    const checksum = require('crypto').createHash('sha256').update(Buffer.from(JSON.stringify(testData), 'utf8')).digest('hex');

        const storageInfo = {compressed: false,
        checksum,
