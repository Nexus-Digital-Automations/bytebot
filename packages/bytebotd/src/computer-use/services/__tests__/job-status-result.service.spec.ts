...
const mockConfigService = {
  get: jest.fn((key: string, defaultValue?: any) => {
    const config = {
      ...
      'redis.db': 0,
      'job.streaming.chunkSize': 1024 * 1024,
      'job.streaming.maxConcurrentChunks': 5,
      'job.streaming.compression': true,
      'job.streaming.resumable': true,
      'job.streaming.cacheChunks': true,
      'job.streaming.thresholdMB': 5,
    };
...
describe('JobStatusResultService', () => {
  let service: JobStatusResultService;
  let module: TestingModule;

  const sampleJobId = 'job_1702983456789_abc123';
  const sampleEnhancedStatus = {
    jobId: sampleJobId,
  ...
  describe('Service Initialization', () => {
    ...
    it('should initialize with correct streaming configuration', () => {
      const streamingConfig = (service as any).streamingConfig;
    ...
    it('should initialize retention policies', () => {
      const retentionPolicies = (service as any).retentionPolicies;
  ...
  describe('Job Status Management', () => {
    describe('updateJobStatus', () => {
      ...
      it('should update job status successfully', async () => {
        ...
        expect(mockEventEmitter.emit).toHaveBeenCalledWith('job.status.updated', expect.objectContaining({
          jobId: sampleJobId,
      ...
      it('should validate job ID', async () => {
        await expect(service.updateJobStatus('', JobStatus.COMPLETED, 100))
  ...
  describe('Result Management', () => {
    ...
    describe('storeJobResult', () => {
      it('should store small result directly', async () => {
        ...
        expect(storageInfo.compressed).toBe(false); // Small data not compressed
        expect(mockRedis.hset).toHaveBeenCalled();
        expect(mockCacheService.set).toHaveBeenCalled();
      });

      it('should compress large results', async () => {
        const largeResult = { data: 'x'.repeat(2000) }; // Large enough to trigger compression
        mockRedis.hset.mockResolvedValue('OK');
        mockRedis.expire.mockResolvedValue(1);
      ...
      it('should handle streaming for very large results', async () => {
        const veryLargeResult = { data: 'x'.repeat(6 * 1024 * 1024) }; // 6MB
        mockRedis.hset.mockResolvedValue('OK');
        mockRedis.expire.mockResolvedValue(1);
      ...
      it('should validate job ID', async () => {
        await expect(service.storeJobResult('', sampleResult))
          .rejects.toThrow(BadRequestException);
      });
    });
    ...
    describe('getJobResult', () => {
      ...
      it('should retrieve result from cache', async () => {
        const cachedResult = { test: 'cached' };
      ...
      it('should retrieve result from Redis when cache misses', async () => {
        const storedResult = { test: 'redis' };
        mockCacheService.get.mockResolvedValue(null);
        (service as any).getResultStorageInfo = jest.fn().mockResolvedValue(mockStorageInfo);
      ...
      it('should handle compressed results', async () => {
        const originalData = { test: 'compressed' };
        const compressedStorageInfo = { ...mockStorageInfo, compressed: true };

        mockCacheService.get.mockResolvedValue(null);
        (service as any).getResultStorageInfo = jest.fn().mockResolvedValue(compressedStorageInfo);

        const zlib = require('zlib');
        const compressed = zlib.gzipSync(JSON.stringify(originalData));
        mockRedis.hget.mockResolvedValue(compressed.toString('base64'));
      ...
      it('should return stream for streaming request', async () => {
        const streamingStorageInfo = { ...mockStorageInfo, chunks: 5 };
        (service as any).getResultStorageInfo = jest.fn().mockResolvedValue(streamingStorageInfo);
      ...
      it('should throw NotFoundException for missing result', async () => {
        (service as any).getResultStorageInfo = jest.fn().mockResolvedValue(null);
  ...
  describe('History and Audit Trail', () => {
    describe('recordJobHistory', () => {
      it('should record history entry successfully', async () => {
      ...
      it('should not throw on history recording errors', async () => {
        mockRedis.lpush.mockRejectedValue(new Error('Redis error'));

        // Should not throw
        await expect(service.recordJobHistory(sampleJobId, 'started', {}))
    ...
    describe('getJobHistory', () => {
      ...
      it('should validate job ID', async () => {
        await expect(service.getJobHistory(''))
          .rejects.toThrow(BadRequestException);
      });
    });
  });
    ...
    describe('cleanupJob', () => {
      beforeEach(() => {
        mockRedis.keys.mockResolvedValue([
          `bytebot:job:status:${sampleJobId}`,
          `bytebot:job:result:${sampleJobId}`,
          `bytebot:job:history:${sampleJobId}`,
        ]);
      ...
      it('should archive before cleanup when requested', async () => {
        // Mock archiveJob method
        (service as any).archiveJob = jest.fn().mockResolvedValue(undefined);
      ...
      it('should validate job ID', async () => {
        await expect(service.cleanupJob(''))
          .rejects.toThrow(BadRequestException);
      });
    });

    describe('performRetentionCleanup', () => {
      it('should perform scheduled cleanup', async () => {
        const oldJobKeys = [
          'bytebot:job:status:old_job_1',
          'bytebot:job:status:old_job_2',
        ];
        mockRedis.keys.mockResolvedValue(oldJobKeys);

        // Mock methods used by retention cleanup
        (service as any).extractJobIdFromKey = jest.fn()
          .mockReturnValueOnce('old_job_1')
          .mockReturnValueOnce('old_job_2');
        (service as any).getRetentionPolicyForJob = jest.fn().mockReturnValue({
          retentionDays: 7,
  ...
  describe('Error Handling', () => {
    ...
    it('should handle JSON parsing errors', async () => {
      ...
      await expect(service.getJobStatus(sampleJobId))
        .rejects.toThrow(InternalServerErrorException);
    });

    it('should handle compression/decompression errors', async () => {
      const storageInfo = { compressed: true, format: 'json' };
      mockCacheService.get.mockResolvedValue(null);
  ...
  describe('Data Integrity', () => {
    it('should maintain data consistency across Redis and cache', async () => {
      const updatedStatus = {
        ...sampleEnhancedStatus,
        status: JobStatus.COMPLETED,
        progress: 100,
    ...
    it('should verify result checksums on retrieval', async () => {
      const testData = { test: 'checksum' };
      const checksum = require('crypto')
        .createHash('sha256')
        .update(Buffer.from(JSON.stringify(testData), 'utf8'))
        .digest('hex');
