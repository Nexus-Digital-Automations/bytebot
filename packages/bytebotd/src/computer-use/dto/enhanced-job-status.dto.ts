...
/**
 * Enhanced job progress tracking with subtask support
 */
...
export class JobErrorDetailsDto {
  @ApiProperty({
    ...
    example: 'Element not found: Unable to locate target element within timeout',
  })
  @IsString()
  message: string = '';
  ...
  @ApiPropertyOptional({
    ...
    example: 'Error: Element not found\\n    at ComputerUseService.findElement...',
  })
  @IsOptional()
  @IsString()
  stack?: string;
  ...
  @ApiPropertyOptional({
    ...
    example: ['Ensure the target element is visible', 'Check element selector accuracy', 'Increase timeout value'],
  })
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
...
export class JobHistoryEntryDto {
  ...
  @IsEnum(['created', 'started', 'progress', 'completed', 'failed', 'cancelled', 'retried'])
  event: string = '';
