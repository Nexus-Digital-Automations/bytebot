...
import { JobPriority, JobStatus } from './async-job.dto';
import { ComputerActionDto } from './computer-action.dto';
...
export class JobDependencyDto {
  @ApiProperty({
    description: 'Job ID that this job depends on',
  example: 'job_1702983456789_abc123',})@IsString()
  @IsUUID('4')dependsOnJobId: string = '';@ApiProperty({description: 'Type of dependency relationship',
  enum: DependencyType,
  example: DependencyType.SEQUENTIAL,
  })
  ...
  example: 'result.success === true',})@IsOptional()
  @IsString()
  condition?: string;
}
...
export class BatchJobSpecDto {
  @ApiProperty({
    description: 'Unique identifier for this job within the batch',
  example: 'screenshot-job-1',})@IsString()
  jobKey: string = '';@ApiProperty({description: 'Computer action to execute',
  ...
  @ApiPropertyOptional({
    description: 'Job dependencies within this batch',
  ...
  example: { stepName: 'capture-screen', retryable: true },})@IsOptional()
...
export class BatchJobSubmissionDto {
  @ApiProperty({
    description: 'Array of jobs to execute as a batch',
  ...
  example: BatchExecutionMode.MIXED,
  ...
  @ApiPropertyOptional({
    description: 'Maximum total execution time for entire batch',
  example: 300000,
  default: 300000,
  })
  @IsOptional()
  ...
  @ApiPropertyOptional({
    description: 'Whether to stop batch execution on first job failure',
  example: false,
  default: false,
  })
  @IsOptional()
  ...
  @ApiPropertyOptional({
    description: 'Custom metadata for batch tracking',
  example: { workflowName: 'ui-automation', userId: 'user123' },})@IsOptional()
  metadata?: Record<string, unknown>;
}
...
export class BatchJobSubmissionResponseDto {
  ...
  example: 'batch_1702983456789_xyz789',})@IsString()
  batchId: string = '';@ApiProperty({description: 'Individual job IDs within the batch',
  example: {'screenshot-job-1': 'job_1702983456789_abc123','click-job-2': 'job_1702983456790_def456',},})
  ...
  @ApiProperty({
    description: 'Batch execution mode used',
  enum: BatchExecutionMode,
  example: BatchExecutionMode.MIXED,
  })
  @IsEnum(BatchExecutionMode)
  ...
  @ApiProperty({
    description: 'Timestamp when batch was submitted',
  example: '2023-12-19T10:30:45.789Z',})@IsDateString()
  submittedAt: string = '';@ApiPropertyOptional({description: 'Estimated completion time for entire batch',
  example: '2023-12-19T10:35:45.789Z',})@IsOptional()
  @IsDateString()
...
export class JobSearchCriteriaDto {
  ...
  @ApiPropertyOptional({
    description: 'Filter by computer action type',
  example: 'screenshot',})@IsOptional()
  @IsString()
  actionType?: string;

  @ApiPropertyOptional({
    description: 'Filter by user ID',
  example: 'user123',})@IsOptional()
  @IsString()
  userId?: string;

  @ApiPropertyOptional({
    description: 'Filter jobs submitted after this date',
  example: '2023-12-19T00:00:00.000Z',})@IsOptional()
  @IsDateString()
  submittedAfter?: string;

  @ApiPropertyOptional({
    description: 'Filter jobs submitted before this date',
  example: '2023-12-19T23:59:59.999Z',})@IsOptional()
  @IsDateString()
  submittedBefore?: string;

  @ApiPropertyOptional({
    description: 'Filter jobs with execution time greater than (ms)',
  example: 5000,})
  @IsOptional()
  @Min(0)
  ...
  @ApiPropertyOptional({
    description: 'Filter jobs with execution time less than (ms)',
  example: 30000,})
  @IsOptional()
  @Min(0)
  ...
  @ApiPropertyOptional({
    description: 'Search term for job metadata or error messages',
  example: 'screenshot',})@IsOptional()
  @IsString()
  searchTerm?: string;
  ...
  example: 'batch_1702983456789_xyz789',})@IsOptional()
  @IsString()
  batchId?: string;
  ...
  example: 'submittedAt',
  enum: ['submittedAt', 'completedAt', 'executionTime', 'priority', 'status'],})@IsOptional()
  @IsString()
  sortBy?: string = 'submittedAt';@ApiPropertyOptional({description: 'Sort order',
  example: 'desc',
  enum: ['asc', 'desc'],})@IsOptional()
  @IsString()
...
export class JobSearchResultsDto {
  ...
  type: [Object], // Will be JobStatusResponseDto[]})
  jobs: unknown[] = [];
...
export class JobAnalyticsDto {
  @ApiProperty({
    description: 'Total number of jobs in timeframe',
  example: 1250,})
  @IsNumber()
  totalJobs: number = 0;
  ...
  @ApiProperty({
    description: 'Jobs per action type breakdown',
  example: {screenshot: 450,
      click: 320,
      type: 280,
      scroll: 200,
    },
  ...
  @ApiProperty({
    description: 'Jobs per priority breakdown',
  example: {urgent: 50,
      high: 200,
      normal: 800,
      low: 200,
  ...
  @ApiProperty({
    description: 'Performance trends over time (hourly)',
  example: [{ hour: '2023-12-19T10:00:00Z', jobCount: 45, avgExecutionTime: 2300 },{ hour: '2023-12-19T11:00:00Z', jobCount: 52, avgExecutionTime: 2150 },],})
...
export class JobProgressUpdateDto {
  ...
  example: 'job_1702983456789_abc123',})@IsString()
  @IsUUID('4')jobId: string = '';@ApiProperty({description: 'Current progress percentage (0-100)',
  example: 75,})
  @IsNumber()
  @Min(0)
  @Max(100)
  ...
  @ApiPropertyOptional({
    description: 'Current step or operation description',
  example: 'Processing image recognition...',})@IsOptional()
  @IsString()
  currentStep?: string;
  ...
  example: { stepsCompleted: 3, totalSteps: 5 },})
  @IsOptional()
  metadata?: Record<string, unknown>;

  @ApiProperty({
    description: 'Timestamp of this progress update',
  example: '2023-12-19T10:30:55.123Z',
  })
  @IsDateString()
