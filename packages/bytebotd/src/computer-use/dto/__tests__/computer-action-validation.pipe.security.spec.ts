...
describe('ComputerActionValidationPipe - Enhanced Security Pipeline', () => {let pipe: ComputerActionValidationPipe;beforeEach(async () => {
  ...
  describe('Multi-Stage Security Validation Pipeline', () => {it('should detect advanced XSS attacks with modern patterns', async () => {const maliciousInput = {action: 'type_text',
  text: '<img src=x onerror=alert(String.fromCharCode(88,83,83))>',};await expect(
      ...
      const advancedXSS = {
        action: 'type_text',
  text: 'javascript:eval(String.fromCharCode(97,108,101,114,116,40,39,88,83,83,39,41))',};await expect(
    ...
    it('should detect advanced SQL injection with database-specific patterns', async () => {const sqlInjectionInput = {action: 'type_text',
      ...
      const pgSqlInjection = {
        action: 'type_text',
  text: '1; SELECT version(); --',};await expect(
        pipe.transform(pgSqlInjection, createArgumentMetadata()),
      ).rejects.toThrow(BadRequestException);
    });

    it('should detect command injection attacks with platform analysis', async () => {const commandInjectionInput = {action: 'type_text',
  text: 'test; cat /etc/passwd',};await expect(
  ...
  path: '../../../etc/passwd',};await expect(
      ...
      const absolutePathInput = {
        action: 'write_file',
  path: '/etc/hosts',
  data: 'malicious content',};await expect(
  ...
  data: '<script>alert("XSS")</script>; DROP TABLE users;",};
    ...
    it('should handle legitimate requests without false positives', async () => {const legitimateInput = {action: 'move_mouse',
  coordinates: { x: 100, y: 200 },};
  ...
  it('should provide detailed security event logging', async () => {const maliciousInput = {action: 'type_text',
  ...
  text: 'A'.repeat(2 * 1024 * 1024), // 2MB payload};await expect(
    ...
    it('should validate action type security contexts correctly', async () => {// File actions should use 'file' contextconst fileInput = {action: 'write_file',
  path: 'test.txt',
  data: 'test content',};
    const result = await pipe.transform(fileInput, createArgumentMetadata());
      expect(result).toBeDefined();

      // Form actions should use 'form' contextconst formInput = {action: 'type_text',
  text: 'normal text',};
  ...
  describe('Performance and Reliability', () => {it('should complete security validation within reasonable time limits', async () => {const input = {action: 'move_mouse',
  coordinates: { x: 500, y: 300 },};
    ...
    it('should maintain consistent _error response format', async () => {const maliciousInput = {action: 'type_text',
  text: '<script>alert("test")</script>",};
