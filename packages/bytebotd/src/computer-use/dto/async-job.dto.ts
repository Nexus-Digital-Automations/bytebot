...
import { ApiProperty, ApiPropertyOptional } from '@nestjs/swagger';/*** Job status enumeration for async operations
 */
...
export class JobSubmissionResponseDto {
  ...
  @IsUUID('4', { message: 'Job ID must be a valid UUID v4' })jobId: string = '';@ApiProperty({description: 'Current job status at submission time',
  enum: JobStatus,
  ...
  submittedAt: string = '';@ApiPropertyOptional({description: 'Estimated completion time (optional)',
...
export class JobStatusResponseDto {
  ...
  @ApiProperty({
    description: 'Timestamp when job was submitted',
  example: '2023-12-19T10:30:45.789Z',})@IsString()
  submittedAt: string = '';@ApiPropertyOptional({description: 'Timestamp when job started processing',
  ...
  example: 'Action execution failed: Mouse movement timeout',})@IsOptional()
  @IsString()
  errorMessage?: string;

  @ApiPropertyOptional({
    description: 'Additional job metadata',
  example: { retryCount: 1, priority: 'normal' },})@IsOptional()
...
export class JobResultResponseDto {
  ...
  example: {image: 'base64-screenshot-data',
  ...
  @ApiProperty({
    description: 'Timestamp when job was submitted',
  example: '2023-12-19T10:30:45.789Z',})@IsString()
  submittedAt: string = '';@ApiProperty({description: 'Timestamp when job completed',
  example: '2023-12-19T10:31:15.456Z',})@IsString()
