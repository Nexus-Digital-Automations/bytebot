...
describe('BackgroundJobWorkerService', () => {let service: BackgroundJobWorkerService;
  ...
  beforeEach(async () => {
    ...
    mockFork.mockImplementation(() => {
      ...
      setTimeout(() => {
        mockProcess.emit('message', {type: WorkerMessageType.WORKER_READY,
  ...
  describe('Initialization', () => {it('should initialize with minimum workers', async () => {await service.onModuleInit();
    ...
    it('should configure worker pool with correct settings', async () => {await service.onModuleInit();
    ...
    it('should emit worker_pool_ready event after initialization', async () => {const eventPromise = new Promise((resolve) => {service.once('worker_pool_ready', resolve);});
  ...
  execArgv: expect.arrayContaining([
    ...
    it('should track worker information', () => {const workers = service.getWorkerInfo();
    ...
    it('should handle worker heartbeats', () => {const mockProcess = mockProcesses[0];
    const workerId = 'test-worker-' + mockProcess.pid;
    // Simulate heartbeat messagemockProcess.emit('message', {type: WorkerMessageType.HEARTBEAT,workerId,
        timestamp: new Date(),
        data: {
    ...
    it('should restart failed workers', async () => {const initialWorkerCount = mockProcesses.length;
    ...
    it('should handle worker disconnection', () => {const mockProcess = mockProcesses[0];
    const workerId = 'test-worker-' + mockProcess.pid;mockProcess.emit('disconnect');
  ...
  describe('Job Submission and Distribution', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should submit jobs with correct priority ordering', async () => {const lowPriorityAction: ComputerAction = { action: 'screenshot' };
    ...
    // Submit jobs in reverse priority orderconst jobId1 = await service.submitJob(lowPriorityAction, JobPriority.LOW);
  ...
  it('should handle job completion', async () => {const action: ComputerAction = { action: 'screenshot' };
    const jobId = await service.submitJob(action);jest.advanceTimersByTime(1000); // Allow job assignment

      const mockProcess = mockProcesses.find(p => !p.killed);

        const workerId = 'test-worker-' + mockProcess!.pid;
    // Simulate job completionmockProcess!.emit('message', {type: WorkerMessageType.JOB_COMPLETED,workerId,
        jobId,
        timestamp: new Date(),
        data: {
  ...
  metadata: { width: 1920, height: 1080 },},
    ...
    it('should handle job failures', async () => {const action: ComputerAction = { action: 'screenshot' };
    const jobId = await service.submitJob(action);jest.advanceTimersByTime(1000); // Allow job assignment

      const mockProcess = mockProcesses.find(p => !p.killed);

        const workerId = 'test-worker-' + mockProcess!.pid;
    // Simulate job failuremockProcess!.emit('message', {type: WorkerMessageType.JOB_FAILED,workerId,
        jobId,
        timestamp: new Date(),
    ...
    it('should emit job progress events', async () => {const action: ComputerAction = { action: 'write_file', path: '/test/file.txt', content: 'test' };
    const jobId = await service.submitJob(action);jest.advanceTimersByTime(1000); // Allow job assignment

      const mockProcess = mockProcesses.find(p => !p.killed);

        const workerId = 'test-worker-' + mockProcess!.pid;
    const progressPromise = new Promise((resolve) => {service.once('job_progress', resolve);});
  ...
  describe('Health Monitoring', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should monitor worker health and restart unhealthy workers', async () => {const initialWorkerCount = mockProcesses.length;
    const mockProcess = mockProcesses[0];

      // Stop sending heartbeats to simulate unhealthy worker
      mockProcess.removeAllListeners('message');
    // Advance time to trigger health checkjest.advanceTimersByTime(20000); // 4x health check interval

      // Should have restarted the unhealthy worker
      expect(mockFork).toHaveBeenCalledTimes(initialWorkerCount + 1);
    });

    it('should collect and emit metrics', () => {const metricsPromise = new Promise((resolve) => {service.once('metrics_collected', resolve);});
  ...
  describe('Performance Metrics', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should track performance metrics accurately', () => {const metrics = service.getWorkerPoolMetrics();
    ...
    it('should update execution time metrics on job completion', async () => {const action: ComputerAction = { action: 'screenshot' };
    const jobId = await service.submitJob(action);jest.advanceTimersByTime(1000); // Allow job assignment

      const mockProcess = mockProcesses.find(p => !p.killed);

        const workerId = 'test-worker-' + mockProcess!.pid;
    // Simulate job completion with execution timemockProcess!.emit('message', {type: WorkerMessageType.JOB_COMPLETED,workerId,
        jobId,
        timestamp: new Date(),
        data: { success: true },
      });
  ...
  describe('Resource Management', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should track worker resource usage', () => {const mockProcess = mockProcesses[0];
    const workerId = 'test-worker-' + mockProcess.pid;
    // Simulate resource usage updatemockProcess.emit('message', {type: WorkerMessageType.HEARTBEAT,workerId,
        timestamp: new Date(),
        data: {
    ...
    it('should enforce maximum jobs per worker', async () => {const maxJobsPerWorker = 5;
    const action: ComputerAction = { action: 'screenshot' };
    // Submit more jobs than max per workerconst jobs = [];
  ...
  describe('Graceful Shutdown', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should perform graceful shutdown', async () => {const shutdownPromise = service.shutdown();
    ...
    it('should wait for active jobs to complete during shutdown', async () => {// Submit a jobconst action: ComputerAction = { action: 'screenshot' };
    const jobId = await service.submitJob(action);jest.advanceTimersByTime(1000); // Allow job assignment
    ...
    it('should handle module destroy', async () => {const shutdownSpy = jest.spyOn(service, 'shutdown');
    await service.onModuleDestroy();
      expect(shutdownSpy).toHaveBeenCalled();
    });

    it('should handle application shutdown signals', async () => {const shutdownSpy = jest.spyOn(service, 'shutdown');
  ...
  describe('Error Handling', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should handle worker process errors gracefully', async () => {const initialWorkerCount = mockProcesses.length;
    const mockProcess = mockProcesses[0];
    ...
    it('should handle job assignment failures', async () => {// Mock a scenario where worker process is not foundconst action: ComputerAction = { action: 'screenshot' };
    // Remove all worker processes to simulate assignment failuremockProcesses.forEach(process => {
        process.kill();
      });
    ...
    it('should handle worker exit during job execution', async () => {const action: ComputerAction = { action: 'screenshot' };
    const jobId = await service.submitJob(action);jest.advanceTimersByTime(1000); // Allow job assignment

      const mockProcess = mockProcesses.find(p => !p.killed);

      // Simulate worker exit during job execution
      mockProcess!.emit('exit', 1, 'SIGKILL');jest.advanceTimersByTime(1000);
    // Job should be marked as failed
      expect(jobManagementService.updateJobStatus).toHaveBeenCalledWith(
        jobId,
        JobStatus.FAILED,
  ...
  describe('Edge Cases', () => {beforeEach(async () => {await service.onModuleInit();
    ...
    it('should handle rapid job submissions', async () => {const jobs = [];
    const action: ComputerAction = { action: 'screenshot' };
    // Submit 20 jobs rapidlyfor (let i = 0; i < 20; i++) {
    ...
    // All jobs should be queuedconst metrics = service.getWorkerPoolMetrics();
      expect(metrics.queueSize).toBeGreaterThan(0);
    });

    it('should maintain worker pool size within limits', async () => {const maxWorkers = 5;
    ...
    it('should handle worker message parsing errors', () => {const mockProcess = mockProcesses[0];
    // Simulate malformed message
      expect(() => {
        mockProcess.emit('message', null);}).not.toThrow();
      expect(() => {
