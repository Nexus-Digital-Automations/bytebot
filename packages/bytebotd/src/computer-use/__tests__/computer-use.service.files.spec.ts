...
jest.mock('@nut-tree-fork/nut-js', () => ({
  ...
  mouse: {
    ...
    getPosition: (
      jest.fn() as jest.MockedFunction<() => Promise<{ x: number; y: number }>>
    ).mockResolvedValue({
      x: 100,
      y: 200,
    }),
...
import { promisify } from 'util';
    // Import services after mockingimport {
  ComputerUseService,
  FileWriteResult,
  FileReadResult,
} from '../computer-use.service';
import { NutService } from '../../nut/nut.service';
    // Create mock implementations with proper typingconst mockFs = fs as jest.Mocked<typeof fs>;
...
describe('ComputerUseService - File Operations', () => {let service: ComputerUseService;
  ...
  describe('writeFile Method - Comprehensive File Writing Tests', () => {const validBase64Data = Buffer.from('test file content for writing operations',).toString('base64');

        const largeBase64Data = Buffer.from('a'.repeat(1024 * 1024)).toString('base64',); // 1MB test datadescribe('Successful File Writing Operations', () => {it('should write file successfully with absolute path', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/documents/test.txt',
  data: validBase64Data,};

        // Mock successful file operations
        mockExecAsync
          .mockResolvedValueOnce({ stdout: '', stderr: '' }) // mkdir command.mockResolvedValueOnce({ stdout: '', stderr: '' }) // cp command.mockResolvedValueOnce({ stdout: '', stderr: '' }) // chown command.mockResolvedValueOnce({ stdout: '', stderr: '' }); // chmod commandconst result = (await service.action(action)) as FileWriteResult;
    // Verify successful result structure
        expect(result).toMatchObject({
          success: true,
          path: '/home/user/documents/test.txt',
  size: Buffer.from(validBase64Data, 'base64').length,} as Partial<FileWriteResult>);
  ...
  fileSize: expect.any(Number) as number,}),
      ...
      it('should handle relative paths correctly', async () => {const action: WriteFileAction = {action: 'write_file',
  ...
  stderr: '',});
    ...
    // Verify path resolution logging
      ...
      it('should handle large file data successfully', async () => {const action: WriteFileAction = {action: 'write_file',
  ...
  stderr: '',});
      ...
      it('should handle empty file data correctly', async () => {const emptyBase64Data = Buffer.from('').toString('base64');
  ...
  stderr: '',});
    ...
    describe('Input Validation and Error Handling', () => {it('should reject empty or null file data', async () => {const action: WriteFileAction = {action: 'write_file',
  ...
  data: '',};
    ...
    // Verify error logging
      ...
      it('should reject empty or null file path', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '',
  data: validBase64Data,};
      ...
      it('should accept pseudo-invalid base64 data (Buffer.from is permissive)', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  ...
  stderr: '',});
      ...
      it('should accept malformed base64 with special characters (Buffer.from is permissive)', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  ...
  data: validBase64Data,
      ...
      it('should allow files in /tmp directory', async () => {const action: WriteFileAction = {action: 'write_file',
  ...
  it('should normalize paths with .. traversal attempts', async () => {const action: WriteFileAction = {action: 'write_file',
  ...
  data: validBase64Data,};
    ...
    describe('File System Error Handling', () => {it('should handle directory creation errors gracefully', async () => {const action: WriteFileAction = {action: 'write_file',
  ...
  data: validBase64Data,};
        ...
        mockExecAsync
          .mockRejectedValueOnce(
            new Error('Permission denied for directory creation'),).mockResolvedValueOnce({ stdout: '', stderr: '' }) // cp.mockResolvedValueOnce({ stdout: '', stderr: '' }) // chown.mockResolvedValueOnce({ stdout: '', stderr: '' }); // chmodconst result = (await service.action(action)) as FileWriteResult;
      expect(result.success).toBe(true);
      ...
      it('should handle file copy/move errors', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  data: validBase64Data,};

        // Mock file copy failure
        mockExecAsync
          .mockResolvedValueOnce({ stdout: '', stderr: '' }) // mkdir.mockRejectedValueOnce(new Error('Permission denied for file copy')); // cp failsconst result = (await service.action(action)) as FileWriteResult;
      expect(result.success).toBe(false);
        expect(result.message).toContain(
    ...
    // Verify temporary file cleanup was attempted
      ...
      it('should handle temporary file write errors', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  data: validBase64Data,};
  ...
  it('should handle cleanup errors without affecting main result', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  data: validBase64Data,};
  ...
  stderr: '',});(
    ...
    describe('Permission and Ownership Operations', () => {it('should handle permission setting errors', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  data: validBase64Data,};

        // Mock permission setting failure
        mockExecAsync
          .mockResolvedValueOnce({ stdout: '', stderr: '' }) // mkdir.mockResolvedValueOnce({ stdout: '', stderr: '' }) // cp.mockResolvedValueOnce({ stdout: '', stderr: '' }) // chown.mockRejectedValueOnce(new Error('Cannot set file permissions')); // chmod failsconst result = (await service.action(action)) as FileWriteResult;
      expect(result.success).toBe(false);
        expect(result.message).toContain(
          'Failed to move file to target location',);});

      it('should handle ownership setting errors', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  data: validBase64Data,};

        // Mock ownership setting failure
        mockExecAsync
          .mockResolvedValueOnce({ stdout: '', stderr: '' }) // mkdir.mockResolvedValueOnce({ stdout: '', stderr: '' }) // cp.mockRejectedValueOnce(new Error('Cannot change file ownership')); // chown failsconst result = (await service.action(action)) as FileWriteResult;
      expect(result.success).toBe(false);
        expect(result.message).toContain(
          'Failed to move file to target location',);});
    });
  ...
  describe('readFile Method - Comprehensive File Reading Tests', () => {const testFileContent = Buffer.from('test file content for reading operations',);

        const binaryFileContent = Buffer.from([
      0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
    ]); // PNG header

    describe('Successful File Reading Operations', () => {it('should read text file successfully with full metadata', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/documents/test.txt',};
  ...
  mediaType: 'text/plain',
      ...
      it('should handle relative paths correctly', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: 'documents/relative-test.txt',};mockExecAsync
  ...
  path: test.path,};
      ...
      it('should handle binary files correctly', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/image.png',};mockExecAsync
      ...
      it('should handle empty files correctly', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/empty.txt',};mockExecAsync
      ...
      it('should handle large files correctly', async () => {const largeContent = Buffer.alloc(1024 * 1024, 'a'); // 1MB fileconst action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/large-file.bin',};mockExecAsync
    ...
    describe('Input Validation and Error Handling', () => {it('should reject empty or null file path', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '',};
    const result = (await service.action(action)) as FileReadResult;
    ...
    // Verify error logging
      ...
      it('should handle non-string path parameter', async () => {const action = {action: 'read_file',
  path: null,} as unknown as ReadFileActionDto;
  ...
  path: dangerousPath,};
      ...
      it('should allow files in /tmp directory', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/tmp/test-file.txt',};mockExecAsync
    ...
    describe('File System Error Handling', () => {it('should handle file copy errors', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/test.txt',};
    // Mock file copy failure
        (
          mockExecAsync as jest.MockedFunction<
    ...
    // Verify temporary file cleanup was attempted
      ...
      it('should handle file read errors', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/test.txt',};
  ...
  it('should handle stat command errors', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/test.txt',};
    // Mock successful copy and read but stat failure
        mockExecAsync
          .mockResolvedValueOnce({ stdout: '', stderr: '' }).mockResolvedValueOnce({stdout: '',
  stderr: '',}).mockRejectedValue(new Error('Cannot get file stats'));(mockFs.readFile as jest.MockedFunction<typeof mockFs.readFile>
        ).mockResolvedValue(testFileContent);
      ...
      it('should handle malformed stat output', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/test.txt',};
    // Mock malformed stat output
      ...
      it('should handle cleanup _errors without affecting main error result', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/test.txt',};
    ...
    // Verify cleanup warning was loggedexpect(mockLogger.warn).toHaveBeenCalledWith(
          expect.stringMatching(/Failed to cleanup temp file on error/),
        );
      });

      it('should handle successful operation with cleanup failure', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/test.txt',};
    // Mock successful operation but cleanup failure
        mockExecAsync
          .mockResolvedValueOnce({ stdout: '', stderr: '' }).mockResolvedValueOnce({ stdout: '', stderr: '' }).mockResolvedValueOnce({ stdout: '10 1609459200', stderr: '' });(mockFs.readFile as jest.MockedFunction<typeof mockFs.readFile>
    ...
    describe('Base64 Encoding and Content Handling', () => {it('should properly encode various content types to base64', async () => {const testCases = [{ content: 'simple text', description: 'plain text' },{content: '{"key": "value", "number": 123}",
  description: 'JSON content',},{
            content: '<html><body>Test</body></html>',
  description: 'HTML content',},{ content: 'line1
line2
line3', description: 'multiline text' },{content: 'special chars: àáâãäåæçèéêë',
  description: 'Unicode characters',},{ content: '\x00\x01\x02\x03', description: 'binary data' },];for (const testCase of testCases) {
          jest.clearAllMocks();
    ...
    // Verify content can be decoded back correctlyif (!result.data) {
            throw new Error(
  ...
  describe('Logging and Operation Tracking', () => {it('should generate unique operation IDs for each file operation', async () => {const action1: WriteFileAction = {action: 'write_file',
  ...
  stderr: '',});(
    ...
    it('should log comprehensive operation metadata for successful writes', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  ...
  fileSize: expect.any(Number) as number,}),
    ...
    it('should log comprehensive operation metadata for successful reads', async () => {const action: ReadFileActionDto = {action: 'read_file',
  path: '/home/user/test.txt',};mockExecAsync
  ...
  base64Length: expect.any(Number) as number,}),
    ...
    it('should maintain consistent logging format for errors', async () => {const writeAction: WriteFileAction = {action: 'write_file',
  path: '/etc/passwd', // Unsafe pathdata: Buffer.from('test').toString('base64'),};
    const readAction: ReadFileActionDto = {
        action: 'read_file',
  path: '/etc/shadow', // Unsafe path};await service.action(writeAction);
  ...
  describe('Edge Cases and Boundary Conditions', () => {it('should handle extremely long file paths', async () => {const longPath = '/home/user/' + 'a'.repeat(255) + '.txt';
    const action: WriteFileAction = {action: 'write_file',
  path: longPath,
  ...
  stderr: '',});
    ...
    it('should handle paths with special characters', async () => {const specialPath ='/home/user/test file with spaces and "quotes" & symbols.txt";
    const action: ReadFileActionDto = {
        action: 'read_file',
  path: specialPath,};
    ...
    it('should handle concurrent file operations safely', async () => {const writeAction: WriteFileAction = {action: 'write_file',
  ...
  stderr: '',});(
    ...
    // Execute operations concurrentlyconst [writeResult, readResult] = await Promise.all([
  ...
  describe('Performance and Resource Management', () => {it('should cleanup all temporary resources in finally blocks', async () => {const action: WriteFileAction = {action: 'write_file',
  path: '/home/user/test.txt',
  data: Buffer.from('test').toString('base64'),};
      ...
      const failureScenarios = [
        () => {
          ...
          (
            ...
            .mockResolvedValueOnce({
              stdout: '',
  stderr: '',}).mockRejectedValue(new Error('cp failed')); // cp failure on second call},];
    ...
    it('should handle memory-efficient operations for large files', async () => {// Test with a very large base64 string (simulating 10MB file)const largeData = 'a'.repeat(10 * 1024 * 1024);
