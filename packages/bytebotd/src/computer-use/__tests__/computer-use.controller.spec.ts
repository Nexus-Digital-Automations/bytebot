...
describe('ComputerUseController', () => {
  ...
  describe('Initialization', () => {
    it('should initialize controller with required dependencies', () => {
    ...
    it('should have proper class decorators applied', () => {
      const metadata = Reflect.getMetadata('path', ComputerUseController);
      expect(metadata).toBeDefined();
    });
  });

  describe('Synchronous Action Execution', () => {
    describe('POST /action', () => {
      const mockMoveAction: MoveMouseActionDto = {
        action: 'move_mouse',
      ...
      it('should execute move mouse action successfully', async () => {
        computerUseService.action.mockResolvedValue(undefined);
      ...
      it('should execute click mouse action successfully', async () => {
        computerUseService.action.mockResolvedValue(undefined);
      ...
      it('should execute type text action successfully', async () => {
        computerUseService.action.mockResolvedValue(undefined);
      ...
      it('should execute screenshot action and return result', async () => {
        computerUseService.action.mockResolvedValue(mockScreenshotResult);

        const result = await controller.executeAction(
          mockScreenshotAction,
          mockUser,
        );
        ...
        expect(computerUseService.action).toHaveBeenCalledWith(
          mockScreenshotAction,
        );
      });

      it('should handle service errors gracefully', async () => {
        const serviceError = new Error('Computer service failed');
        computerUseService.action.mockRejectedValue(serviceError);
        await expect(
      ...
      it('should handle validation errors appropriately', async () => {
        ...
        computerUseService.action.mockRejectedValue(validationError);
        await expect(
          controller.executeAction(mockMoveAction, mockUser),
        ).rejects.toThrow(HttpException);
    ...
    describe('GET /screenshot', () => {
      it('should capture screenshot successfully', async () => {
        computerUseService.screenshot.mockResolvedValue(mockScreenshotResult);
      ...
      it('should handle screenshot service errors', async () => {
        ...
        computerUseService.screenshot.mockRejectedValue(screenshotError);
        await expect(controller.captureScreenshot(mockUser)).rejects.toThrow(
          HttpException,
        );
  ...
  describe('Asynchronous Job Management', () => {
    describe('POST /action/async', () => {
      const mockAsyncSubmission: ComputerActionDto & AsyncActionSubmissionDto =
        { action: 'screenshot', priority: JobPriority.NORMAL, timeout: 30000 };

      it('should submit async action successfully', async () => {
        (
          asyncJobService.submitJob as jest.MockedFunction<any>
        ).mockResolvedValue(mockJobSubmission);

        const result = await controller.submitAsyncAction(
          mockAsyncSubmission,
          mockUser,
        );
    ...
    describe('GET /jobs/:jobId/status', () => {
      ...
      it('should get job status successfully', async () => {
        (
          asyncJobService.getJobStatus as jest.MockedFunction<any>
      ...
      it('should handle job not found error', async () => {
        ...
        await expect(controller.getJobStatus(jobId, mockUser)).rejects.toThrow(
          HttpException,
        );
    ...
    describe('GET /jobs/:jobId/result', () => {
      ...
      it('should get job result successfully', async () => {
        (
          asyncJobService.getJobResult as jest.MockedFunction<any>
      ...
      it('should handle job result retrieval errors', async () => {
        ...
        (
          asyncJobService.getJobResult as jest.MockedFunction<any>
        ).mockRejectedValue(resultError);
        await expect(controller.getJobResult(jobId, mockUser)).rejects.toThrow(
          HttpException,
        );
    ...
    describe('DELETE /jobs/:jobId', () => {
      ...
      it('should cancel job successfully', async () => {
        (
          asyncJobService.cancelJob as jest.MockedFunction<any>
      ...
      it('should handle job cancellation errors', async () => {
        ...
        await expect(controller.cancelJob(jobId, mockUser)).rejects.toThrow(
          HttpException,
        );
  ...
  describe('Error Handling and Security', () => {
    it('should handle unknown errors gracefully', async () => {
      ...
      computerUseService.action.mockRejectedValue(unknownError);
    ...
    it('should preserve error types correctly', async () => {
      const forbiddenError = new HttpException(
        'Insufficient permissions',
        HttpStatus.FORBIDDEN,
      );
      computerUseService.action.mockRejectedValue(forbiddenError);
  ...
  describe('Request/Response Structure', () => {
    it('should include consistent timestamp format in responses', async () => {
      computerUseService.action.mockResolvedValue(undefined);
    ...
    it('should include action type in action responses', async () => {
      computerUseService.action.mockResolvedValue(undefined);
  ...
  describe('API Documentation and Decorators', () => {
    it('should have proper API operation metadata', () => {
      // Test that the controller methods have appropriate decorators// This verifies the Swagger documentation is properly configured
