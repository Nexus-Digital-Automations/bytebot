...
describe('Enhanced JobManagementService', () => {
    ...
    const mockConfigService = {
      get: jest.fn((key: string, defaultValue?: any) => {
        const config = {
          ...
          'REDIS_DB': 0,
          'JOB_ENCRYPTION_KEY': 'test-encryption-key-32-bytes-long',
          'JOB_DEFAULT_TIMEOUT': 30000,
          'JOB_MAX_RETRIES': 3,
          'JOB_WORKER_INTERVAL': 1000,
          'JOB_CLEANUP_INTERVAL': 300000,
  ...
  describe('Enhanced Job Creation and Priority Management', () => {
    beforeEach(() => {
      // Setup successful Redis operations
    ...
    it('should create jobs with different priorities and maintain queue order', async () => {
      const priorities = [JobPriority.LOW, JobPriority.HIGH, JobPriority.URGENT, JobPriority.NORMAL];
      const jobIds: string[] = [];
    ...
    it('should handle job creation with custom metadata and tags', async () => {
  ...
  describe('Enhanced Redis Operations and Failover', () => {
    it('should handle Redis connection failures during job creation', async () => {
      mockRedisInstance.setex.mockRejectedValue(new Error('Redis connection lost'));
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to save job'),
    ...
    it('should handle Redis cluster failover scenarios', async () => {
      ...
      mockRedisInstance.setex
        .mockRejectedValueOnce(new Error('CLUSTERDOWN'))
    ...
    it('should handle transaction failures gracefully', async () => {
      mockRedisPipeline.exec.mockResolvedValue([
        [new Error('Transaction failed'), null],
        [null, 1],
      ]);
  ...
  describe('Enhanced Job Execution and Worker Management', () => {
    let jobId: string;
    ...
    it('should handle concurrent job execution by multiple workers', async () => {
      // Mock multiple jobs in queue
      mockRedisInstance.smembers.mockResolvedValue([jobId, 'job2', 'job3']);
    ...
    it('should handle worker crashes and recovery', async () => {
      computerUseService.action
        .mockRejectedValueOnce(new Error('Worker crashed'))
        .mockResolvedValueOnce(mockResults.screenshot);
    ...
    it('should handle memory pressure during job execution', async () => {
    ...
    it('should handle job execution timeouts with proper cleanup', async () => {
  ...
  describe('Enhanced Error Handling and Retry Logic', () => {
    ...
    it('should handle different error types with appropriate retry strategies', async () => {
      const errors = [
        ...
        new Error('Invalid action'), // Non-retryable
        new Error('timeout'), // Retryable
      ];
  ...
  describe('Enhanced Performance and Monitoring', () => {
  ...
  describe('Enhanced Security and Data Protection', () => {
    it('should encrypt and decrypt job data correctly', async () => {
    ...
    it('should validate job access permissions', async () => {
      const jobId = await service.createJob(mockActions.screenshot, {
  ...
  describe('Enhanced Cleanup and Resource Management', () => {
    it('should perform efficient batch cleanup of expired jobs', async () => {
      const expiredJobs = Array.from({ length: 1000 }, (_, i) => `expired-job-${i}`);

      mockRedisInstance.keys.mockResolvedValue(
        expiredJobs.map(id => `bytebot:jobs:${id}`)
      );
    ...
    it('should maintain job statistics during cleanup', async () => {
  ...
  describe('Enhanced Edge Cases and Error Boundaries', () => {
    it('should handle malformed job data in Redis', async () => {
      mockRedisInstance.get.mockResolvedValue('invalid-json{');

      await expect(
        service.getJobStatus('malformed-job')
      ).rejects.toThrow();

      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to retrieve job'),
        expect.any(Object)
      );
    });

    it('should handle Redis connection drops during job execution', async () => {
      const jobId = await service.createJob(mockActions.screenshot);
