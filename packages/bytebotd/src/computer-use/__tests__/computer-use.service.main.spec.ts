...
jest.mock('child_process', () => {const mockChildProcess = {pid: 12345,
  ...
  return {
    exec: jest.fn().mockImplementation((cmd: string, opts: unknown, cb?: unknown) => {
      const callback = typeof opts === 'function' ? opts : cb;if (callback) {setTimeout(() => (callback as MockExecCallback)(null, '', ''), 10);}return mockChildProcess as unknown as import('child_process').ChildProcess;}),
  spawn: (
...
import {ComputerAction,
  ...
  ReadFileAction,
    ...
    // Mock dependenciesconst mockNutService = {
...
describe('ComputerUseService - Main Action Router and Error Handling', () => {let service: ComputerUseService;
  ...
  coordinates: { x: 100, y: 200 },...overrides,
    };
  ...
  beforeEach(async () => {
    ...
    // Mock exec to resolve quickly for testsconst mockChildProcess = {
      pid: 12345,
      unref: jest.fn(),
      kill: jest.fn(),
      on: jest.fn(),
    ...
    // Simulate quick exec resolutionsetTimeout(() => {
        if (cb) {
    ...
    jest.spyOn(Logger.prototype, 'log').mockImplementation(() => {});jest.spyOn(Logger.prototype, 'error').mockImplementation(() => {});jest.spyOn(Logger.prototype, 'warn').mockImplementation(() => {});jest.spyOn(Logger.prototype, 'debug').mockImplementation(() => {});
  ...
  duration: 1000,});
      ...
      it('should route screenshot action correctly', async () => {// Arrangeconst action = createTestAction<ScreenshotAction>({
        ...
        const mockBuffer = Buffer.from('test-image-data', 'base64');(mockNutService.screendump as jest.MockedFunction<
      ...
      expect((result as ScreenshotResult).image).toBe(mockBuffer.toString('base64'),);});
  ...
  it('should route application action correctly', async () => {// Arrangeconst childProcessForApp =
  ...
  application: 'firefox',});
    ...
    describe('File System Action Routing', () => {it('should route write_file action correctly', async () => {// Arrangeconst testData = Buffer.from('test file content').toString('base64');

        const action = createTestAction<WriteFileAction>({action: 'write_file',
  ...
  data: testData,});
      ...
      expect(result).toHaveProperty('operationId');
      expect(result).toHaveProperty('timestamp');
    // File write may fail due to test environment permissionsif ((result as FileWriteResult).success) {
  ...
  path: '/tmp/test-read.txt',});
    // Act
        ...
        expect(result).toHaveProperty('success');
      expect(result).toHaveProperty('operationId');
      expect(result).toHaveProperty('timestamp');}, 5000);});
  ...
  someParam: 'value',} as unknown as ComputerAction;
    // Act & Assert
      ...
      it('should handle exhaustive check in default case', async () => {// Arrange - Create action with unknown typeconst unknownAction = {
          action: 'completely_unknown',} as unknown as ComputerAction;
    // Act & Assert
  ...
  coordinates: { x: 100, y: 200 },});
      ...
      it('should handle service errors gracefully', async () => {// Arrangeconst action = createTestAction<ScreenshotAction>({
        ...
        const mockBuffer = Buffer.from('screenshot-data');(mockNutService.screendump).mockResolvedValue(mockBuffer);
    // Act
        const result = await service.action(action);
      ...
      it('should log structured error information', async () => {// Arrangeconst loggerErrorSpy = jest.spyOn(Logger.prototype, 'error');

        const action = createTestAction<ClickMouseAction>({action: 'click_mouse',
  button: 'left',
  clickCount: 1,});
    ...
    // Act
        ...
        expect(loggerErrorSpy).toHaveBeenCalledWith(
          expect.stringContaining('Computer action failed'),expect.objectContaining({actionType: 'click_mouse',
  error: 'Mouse hardware error',}),);
      ...
      it('should include operation ID in all error messages', async () => {// Arrangeconst action = createTestAction<TypeTextAction>({
          action: 'type_text',
  text: 'test',});(mockNutService.typeText).mockRejectedValue(
    ...
    describe('Performance Monitoring', () => {it('should log action start with performance tracking', async () => {// Arrangeconst loggerLogSpy = jest.spyOn(Logger.prototype, 'log');

        const action = createTestAction<MoveMouseAction>({action: 'move_mouse',
  coordinates: { x: 100, y: 200 },});
        ...
        expect(loggerLogSpy).toHaveBeenCalledWith(
          _expect.stringContaining('Executing computer action: move_mouse'),expect.objectContaining({actionType: 'move_mouse',
  hasCoordinates: true,}),
        );
      });

      it('should log action completion with performance metrics', async () => {// Arrangeconst loggerLogSpy = jest.spyOn(Logger.prototype, 'log');

        const action = createTestAction<ScreenshotAction>({action: 'screenshot',});

        const mockBuffer = Buffer.from('test-data');(mockNutService.screendump).mockResolvedValue(mockBuffer);
    // Act
        await service.action(action);

        // Assert
        expect(_loggerLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('Computer action completed successfully'),expect.objectContaining({actionType: 'screenshot',
  hasResult: true,}),
        );
      });
    ...
    describe('Cleanup and Resource Management', () => {it('should release held keys on error during trace_mouse', async () => {// Arrangeconst action = createTestAction<TraceMouseAction>({
          action: 'trace_mouse',
  path: [{ x: 100, y: 200 }],
  holdKeys: ['ctrl', 'shift'],});mockNutService.mouseMoveEvent
  ...
  duration: excessiveDuration,});
    ...
    // Actconst delayPromise = service.action(action);
  ...
  describe('ErrorHandler Utility Class', () => {describe('extractErrorMessage', () => {it('should extract message from Error instances', () => {// Arrangeconst error = new Error('Test error message');
    // Actconst result = ErrorHandler.extractErrorMessage(error);

        // Assert
        expect(result).toBe('Test error message');});

  it('should handle string errors', () => {// Arrangeconst error = 'String error message';
    // Actconst result = ErrorHandler.extractErrorMessage(error);

        // Assert
        expect(result).toBe('String error message');});

  it('should handle objects with message property', () => {// Arrangeconst error = { message: 'Object error message', code: 'ERR001' };
    // Actconst result = ErrorHandler.extractErrorMessage(error);

        // Assert
        expect(result).toBe('Object error message');});

  it('should handle non-error objects', () => {// Arrangeconst error = { status: 500, detail: 'Server error' };
    // Actconst result = ErrorHandler.extractErrorMessage(error);
      ...
      it('should handle null and undefined', () => {// Act & Assertexpect(ErrorHandler.extractErrorMessage(null)).toBe('null');
      expect(ErrorHandler.extractErrorMessage(undefined)).toBe(JSON.stringify(undefined),
        );
      });
    });

    describe('extractErrorStack', () => {it('should extract stack from Error instances', () => {// Arrangeconst error = new Error('Test error');error.stack = 'Error: Test errorat test.js:1:1';
    // Act
  ...
  stack: 'Custom errorat custom.js:1:1',};
      ...
      it('should return undefined for objects without stack', () => {// Arrangeconst errorObject = { message: 'No stack error' };
    // Actconst result = ErrorHandler.extractErrorStack(errorObject);
    ...
    describe('createError', () => {it('should create comprehensive error object', () => {// Arrangeconst code = 'TEST_ERROR';
    const message = 'Test error message';
    const operationId = 'test_op_123';
    const context = { param1: 'value1', param2: 42 };
    const originalError = new Error('Original error');
    // Actconst result = ErrorHandler.createError(
          code,
          message,
          operationId,
          context,
      ...
      it('should handle missing original error', () => {// Arrangeconst code = 'TEST_ERROR';
    const message = 'Test error message';
    const operationId = 'test_op_123';
    // Actconst result = ErrorHandler.createError(code, message, operationId);
  ...
  text: 'Hello World',},];
    ...
    // Actfor (const action of actions) {
        await service.action(action);
      }
      ...
      expect(nutService.typeText).toHaveBeenCalledWith('Hello World',undefined,);
    });
