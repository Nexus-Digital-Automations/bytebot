...
jest.mock('@nut-tree-fork/nut-js', () => ({keyboard: {pressKey: jest.fn().mockResolvedValue(
  ...
  mouse: {
    ...
    releaseButton: jest.fn().mockResolvedValue(
      undefined,
    ),
    ...
    scrollLeft: jest.fn().mockResolvedValue(
      undefined,
    ),
    scrollRight: jest.fn().mockResolvedValue(
      undefined,
    ),
...
jest.mock('util', () => {const originalUtil = jest.requireActual('util') as typeof import('util');return {...originalUtil,
...
jest.mock('fs/promises');
    // Mock axios and HTTP services that cause util.inherits issuesjest.mock('@nestjs/axios', () => ({HttpService: jest.fn().mockImplementation(() => ({
      axiosRef: {
        get: jest.fn(),
        post: jest.fn(),
        put: jest.fn(),
...
jest.mock('rxjs', () => ({firstValueFrom: jest.fn().mockResolvedValue({data: 'mocked',}),
  of: jest.fn(),
  from: jest.fn(),
...
import { Readable } from 'stream';describe('ComputerUseService - Application Management', () => {let service: ComputerUseService;
  ...
  describe('Desktop Activation', () => {/*** Test desktop activation - special case that doesn't follow the standard application pattern
     ...
     */
    it('should activate desktop using wmctrl command', async () => {const action: ApplicationActionDto = {action: 'application',
  application: ApplicationName.DESKTOP,};
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        'sudo',['-u', 'user', 'wmctrl', '-k', 'on'],expect.objectContaining({env: expect.objectContaining({
            DISPLAY: ':0.0',}) as NodeJS.ProcessEnv,
  stdio: 'ignore',
  detached: true,}) as SpawnOptions,
      );
  ...
  describe('Application Launch Operations', () => {/*** Test firefox application launch when not currently running
     ...
     */
    it('should launch firefox when not running', async () => {const action: ApplicationActionDto = {action: 'application',
...
/**
    ...
    it('should launch all supported applications when not running', async () => {const appConfigs: Array<[ApplicationName, string, string]> = [[ApplicationName.FIREFOX, 'firefox-esr', 'Navigator.firefox-esr'],[ApplicationName.ONEPASSWORD, '1password', '1password.1Password'],[ApplicationName.THUNDERBIRD, 'thunderbird', 'Mail.thunderbird'],[ApplicationName.VSCODE, 'code', 'code.Code'],[ApplicationName.TERMINAL,
        ...
        expect(mockSpawn).toHaveBeenCalledWith(
          'sudo',['-u', 'user', 'nohup', command],expect.objectContaining({env: expect.objectContaining({ DISPLAY: ':0.0' }) as SpawnOptions['env'],
  stdio: 'ignore',
  detached: true,}),
        );
      }
  ...
  describe('Application Activation and Window Management', () => {/*** Test activation of already running applications
    ...
    it('should activate and maximize running applications', async () => {const appConfigs: Array<[ApplicationName, string]> = [[ApplicationName.FIREFOX, 'Navigator.firefox-esr'],[ApplicationName.VSCODE, 'code.Code'],[ApplicationName.THUNDERBIRD, 'Mail.thunderbird'],];for (const [app, processName] of appConfigs) {
        ...
        expect(mockSpawn).toHaveBeenNthCalledWith(
          _1,
          'sudo',['-u', 'user', 'wmctrl', '-x', '-a', processName],expect.objectContaining({env: expect.objectContaining({ DISPLAY: ':0.0' }) as SpawnOptions['env'],
  stdio: 'ignore',
  detached: true,}),
        );
  ...
  describe('Error Handling and Edge Cases', () => {/*** Test handling of unsupported applications
     */
    it('should throw _error for unsupported application', async () => {const action = {action: 'application' as const,
  application: 'unsupported-app' as ApplicationName,};await expect(service.action(action)).rejects.toThrow(
...
/**
     ...
     */
    it('should handle wmctrl timeout _error gracefully', async () => {const action: ApplicationActionDto = {action: 'application',
  application: ApplicationName.FIREFOX,};
...
/**
     ...
     */
    it('should handle spawn errors during application launch', async () => {const action: ApplicationActionDto = {action: 'application',
  ...
  describe('Process Management and Security', () => {/*** Test proper process unreferencing for detached processes
     */
    it('should properly unref spawned processes', async () => {const action: ApplicationActionDto = {action: 'application',
...
/**
     ...
     */
    it('should run all operations with sudo user context', async () => {const action: ApplicationActionDto = {action: 'application',
...
/**
     ...
     */
    it('should set secure DISPLAY environment variable', async () => {const action: ApplicationActionDto = {action: 'application',
  ...
  describe('Timeout and Performance', () => {/*** Test timeout configuration for status checks
     */
    it('should apply timeout to application status checks', async () => {const action: ApplicationActionDto = {action: 'application',
