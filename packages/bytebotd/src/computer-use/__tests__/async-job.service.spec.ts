...
jest.mock('uuid', () => ({v4: jest.fn(() => 'mock-uuid-12345'),}));
...
describe('AsyncJobService', () => {let service: AsyncJobService;
  ...
  describe('Initialization', () => {it('should initialize service with required dependencies', () => {expect(service).toBeDefined();
  ...
  describe('Job Submission', () => {const userId = 'user-123';
    const mockOptions = {priority: 'high' as JobPriority,
  timeout: 30000,
  useCache: true,
      metadata: { source: 'test' },};it('should submit job successfully with default options', async () => {const result = await service.submitAction(mockComputerAction, userId);
  ...
  estimatedDuration: expect.any(Number),
    ...
    it('should submit job with custom options', async () => {const result = await service.submitAction(mockComputerAction, mockOptions);
      expect(result).toMatchObject({
        jobId: 'mock-uuid-12345',
  status: 'queued',
  submittedAt: expect.any(String),
  estimatedDuration: expect.any(Number),
    ...
    it('should handle multiple job submissions correctly', async () => {const result1 = await service.submitAction(mockComputerAction, userId);
  ...
  describe('Job Execution', () => {const userId = 'user-123';
    let jobId: string;beforeEach(async () => {
      const submission = await service.submitAction(mockComputerAction, userId);
      jobId = submission.jobId;
    });

    it('should execute job and update status', async () => {computerUseService.action.mockResolvedValue(mockScreenshotResult);
    ...
    it('should handle job execution errors', async () => {const executionError = new Error('Computer action failed');computerUseService.action.mockRejectedValue(executionError);
    // Allow time for job processing
      await new Promise(resolve => setTimeout(resolve, 100));
  ...
  describe('Job Status and Progress Tracking', () => {const userId = 'user-123';
    let jobId: string;beforeEach(async () => {
      const submission = await service.submitAction(mockComputerAction, userId);
      jobId = submission.jobId;
    });

    it('should get job status successfully', async () => {const status = await service.getJobStatus(jobId);
  ...
  it('should handle unauthorized job status requests', async () => {const unauthorizedUserId = 'other-user-456';await expect(service.getJobStatus(jobId)
      ).rejects.toThrow('Unauthorized');});

  it('should track progress updates during execution', async () => {computerUseService.action.mockImplementation(async () => {// Simulate progress updates
  ...
  describe('Job Result Retrieval', () => {const userId = 'user-123';
    let jobId: string;beforeEach(async () => {
      const submission = await service.submitAction(mockComputerAction, userId);
      jobId = submission.jobId;
    ...
    it('should get job result after completion', async () => {// Allow time for job completionawait new Promise(resolve => setTimeout(resolve, 200));
      ...
      if (result.status === 'completed') {expect(result.result).toBeDefined();
  ...
  it('should handle non-existent job result requests', async () => {const invalidJobId = 'non-existent-job';await expect(service.getJobResult(invalidJobId)
      ).rejects.toThrow('Job not found');});
});
  ...
  describe('Job Cancellation', () => {const userId = 'user-123';
    ...
    it('should cancel queued job successfully', async () => {const result = await service.cancelJob(jobId);
  ...
  it('should handle cancellation of running jobs', async () => {// Mock a long-running operationcomputerUseService.action.mockImplementation(
    ...
    it('should handle cancellation of completed jobs', async () => {computerUseService.action.mockResolvedValue(mockScreenshotResult);
  ...
  it('should handle cancellation of non-existent jobs', async () => {const invalidJobId = 'non-existent-job';await expect(service.cancelJob(invalidJobId)
      ).rejects.toThrow('Job not found');});
});
  ...
  describe('Priority Queue Management', () => {const userId = 'user-123';it('should handle priority-based job ordering', async () => {// Submit jobs with different prioritiesconst lowPriorityJob = await service.submitAction(mockComputerAction, {
    ...
    // Allow time for processingawait new Promise(resolve => setTimeout(resolve, 300));
  ...
  describe('Error Handling and Edge Cases', () => {const userId = 'user-123';it('should handle service unavailability gracefully', async () => {computerUseService.action.mockRejectedValue(new Error('Service unavailable'));
    ...
    it('should handle cache service failures', async () => {cacheService.get.mockRejectedValue(new Error('Cache unavailable'));cacheService.set.mockRejectedValue(new Error('Cache unavailable'));

        const submission = await service.submitAction(mockComputerAction, {useCache: true,
      });
    ...
    it('should handle metrics service failures', async () => {metricsService.recordJobSubmission.mockImplementation(() => {throw new Error('Metrics unavailable');});
  ...
  describe('Performance and Metrics', () => {const userId = 'user-123';it('should record performance metrics correctly', async () => {computerUseService.action.mockResolvedValue(mockScreenshotResult);
