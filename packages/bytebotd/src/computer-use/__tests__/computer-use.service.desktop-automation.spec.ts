...
jest.mock('util', () => ({...jest.requireActual('util') as typeof import('util'),
  promisify: jest.fn((fn: (...args: unknown[]) => unknown) =>
...
jest.mock('path', () => ({...jest.requireActual('path') as typeof import('path'),
...
import { ScreenshotActionDto } from '../dto/computer-action.dto';
    // Mock services for comprehensive testingconst mockNutService = {
...
describe('ComputerUseService - Advanced Desktop Automation', () => {let service: ComputerUseService;
  ...
  describe('Complex Mouse Automation Workflows', () => {it('should perform complex drag and drop operations with multiple waypoints', async () => {const dragAction: DragMouseAction = {action: 'drag_mouse',
  path: [{ x: 100, y: 100 },
          { x: 400, y: 300 },
        ],
        button: 'left',};
  ...
  coordinates: { x: 1920, y: 1080 },},
    ...
    it('should perform trace mouse operations with smooth path generation', async () => {const traceAction: TraceMouseAction = {action: 'trace_mouse',
  path: [{ x: 0, y: 0 },
          { x: 100, y: 50 },
          { x: 200, y: 100 },
          { x: 300, y: 150 },
  ...
  keys: ['ctrl', 'a'],
  press: 'down' as const,},// Type replacement text
        { action: 'type_text' as const, text: 'New content for document' },// Save document{
          action: 'press_keys' as const,
  keys: ['ctrl', 's'],
  press: 'down' as const,},// Undo last action
        {
          action: 'press_keys' as const,
  keys: ['ctrl', 'z'],
  press: 'down' as const,},// Redo action
        {
          action: 'press_keys' as const,
  keys: ['ctrl', 'y'],
  press: 'down' as const,},];
    ...
    it('should perform advanced clipboard operations with formatting preservation', async () => {const clipboardOperations = [// Copy formatted text
        {
          action: 'press_keys' as const,
  keys: ['ctrl', 'c'],
  press: 'down' as const,},// Move to new location
        {
          action: 'press_keys' as const,
  keys: ['tab'],
  press: 'down' as const,},// Paste with formatting
        {
          action: 'press_keys' as const,
  keys: ['ctrl', 'v'],
  press: 'down' as const,},// Paste as plain text
        {
          action: 'press_keys' as const,
  keys: ['ctrl', 'shift', 'v'],
  press: 'down' as const,},];
  ...
  describe('Cross-Platform Desktop Environment Integration', () => {it('should adapt operations for different operating systems', async () => {const platforms = ['darwin', 'linux', 'win32'];for (const platform of platforms) {// Mock platform detection
        Object.defineProperty(_process, 'platform', {value: platform,
  configurable: true,
        });
  ...
  press: 'down',};
    ...
    it('should handle desktop environment specific features', async () => {// Test desktop-specific features that might vary by environmentconst desktopFeatures = [
        // Window management
        {
          action: 'press_keys' as const,
  keys: ['win', 'up'],
  press: 'down' as const,}, // Maximize (Windows){
          action: 'press_keys' as const,
  keys: ['ctrl', 'alt', 't'],
  press: 'down' as const,}, // Terminal (Linux){
          action: 'press_keys' as const,
  keys: ['cmd', 'space'],
  press: 'down' as const,}, // Spotlight (macOS)];
  ...
  describe('Performance Optimization Testing', () => {it('should maintain performance under high-frequency operations', async () => {const performanceData = createPerformanceTestData();
  ...
  coordinates: { x: i % 1920, y: (i * 2) % 1080 },};
    ...
    it('should handle concurrent operations without race conditions', async () => {const performanceData = createPerformanceTestData();
  ...
  describe('Screenshot and Basic Vision Functionality', () => {it('should capture screenshots successfully', async () => {// Mock a screenshot capture(mockNutService.screendump).mockResolvedValue('base64-screenshot-data');

        const screenshotAction: ScreenshotActionDto = {action: 'screenshot',};
  ...
  coordinates: { x: 100, y: 100 },};
    ...
    it('should implement retry logic for transient failures', async () => {// Mock a transient failure followed by successnutService.mouseClickEvent = (jest
        ...
        const clickAction: ClickMouseAction = {action: 'click_mouse',
  coordinates: { x: 200, y: 200 },
  button: 'left',
  clickCount: 1,};
    ...
    it('should maintain operation state during partial failures', async () => {// Test that other operations continue working even if one failsconst mixedActions = [
        { action: 'move_mouse' as const, coordinates: { x: 100, y: 100 } },{action: 'click_mouse' as const,
  coordinates: { x: 200, y: 200 },
  button: 'left' as const,
  clickCount: 1,},
    ...
    // Make the middle action fail
      nutService.mouseClickEvent = (jest
        ...
        const results = [];for (const action of mixedActions) {
        try {
  ...
  describe('Memory Usage Optimization', () => {it('should properly clean up resources after operations', async () => {const initialMemory = process.memoryUsage().heapUsed;
  ...
  press: 'down' as const,},{
          action: 'press_keys' as const,
  keys: ['win', 'r'],
  press: 'down' as const,},{ action: 'type_text' as const, text: 'shutdown /s /t 0' },];for (const operation of systemOperations) {
  ...
  path: [{ x: 100, y: 100 },
    ...
    it('should maintain service state consistency across operations', async () => {// Perform a sequence of operations that require state managementconst operationSequence = [
        { action: 'move_mouse' as const, coordinates: { x: 0, y: 0 } },{action: 'press_mouse' as const,
  button: 'left' as const,
  coordinates: { x: 0, y: 0 },
  press: 'down' as const,},{ action: 'move_mouse' as const, coordinates: { x: 100, y: 100 } },{action: 'press_mouse' as const,
  button: 'left' as const,
  coordinates: { x: 100, y: 100 },
  press: 'up' as const,
        },
