...
} from './browser-orchestration-session.service';
import {
...
/**
 * Create Orchestration Session DTO
 */
 ...
 */
@ApiTags('Browser Orchestration Sessions')
@ApiBearerAuth()
@Controller('browser-orchestration/sessions')
@UseGuards(JwtAuthGuard, RolesGuard)
export class BrowserOrchestrationSessionController {
  ...
  @ApiOperation({
    summary: 'Create orchestrated browser session',
    description: 'Create a new browser session with orchestration capabilities including pool management, agent coordination, and performance monitoring.',
  })
  @ApiBody({
    type: CreateOrchestrationSessionDto,
    description: 'Session creation configuration with orchestration parameters',
  })
  @ApiResponse({
    status: 201,
  ...
  async createOrchestrationSession(
    ...
    this.logger.log(`[${operationId}] Creating orchestration session`, {
      ...
      sessionType: createSessionDto.sessionType || SessionType.STANDARD,
    ...
    try {
      ...
      if (error.message.includes('pool at capacity') || error.message.includes('capacity')) {
        throw new HttpException(error.message, HttpStatus.CONFLICT);
      }

      throw new HttpException(
        error.message || 'Failed to create orchestration session',
  ...
  @Roles(UserRole.OPERATOR, UserRole.ADMIN)
  @ApiOperation({
    summary: 'Get available session for agent',
    description: 'Get an available browser session for an agent with intelligent allocation, reuse optimization, and sharing capabilities.',
  })
  @ApiBody({
    type: GetAvailableSessionDto,
    description: 'Agent session request parameters',
  })
  @ApiResponse({
    status: 200,
    description: 'Available session found and assigned',
    type: BrowserSessionDto,
  })
  @ApiResponse({
    status: 404,
  ...
  async getAvailableSessionForAgent(
    ...
    this.logger.log(`[${operationId}] Finding available session for agent`, {
      agentId: requestDto.agentId,
      sessionType: requestDto.sessionType || SessionType.STANDARD,
    ...
    try {
      ...
      if (!session) {
        throw new HttpException(
          `No available sessions for agent ${requestDto.agentId}`,
          HttpStatus.NOT_FOUND,
        );
      }

      this.logger.log(`[${operationId}] Available session found for agent`, {
        sessionId: session.sessionId,
        agentId: requestDto.agentId,
      });
      ...
      throw new HttpException(
        error.message || 'Failed to get available session',
        HttpStatus.BAD_REQUEST,
      );
    }
  }
   ...
   */
  @Put(':sessionId/assign')
  @Roles(UserRole.OPERATOR, UserRole.ADMIN)
  @ApiOperation({
    summary: 'Assign session to agent',
    description: 'Assign a specific browser session to an agent with tracking and coordination capabilities.',
  })
  @ApiParam({
    name: 'sessionId',
  ...
  @ApiBody({
    ...
    description: 'Agent assignment parameters',
  })
  @ApiResponse({
    status: 200,
    description: 'Session assigned successfully',
    type: SessionAssignmentResponseDto,
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 409,
    description: 'Session not available for assignment',
  })
  async assignSessionToAgent(
    ...
    this.logger.log(`[${operationId}] Assigning session to agent`, {
      sessionId,
    ...
    try {
      ...
      return {
        success: false,
        error: error.message || 'Failed to assign session',
        sessionId,
        agentId: assignDto.agentId,
      };
    }
   ...
   */
  @Put(':sessionId/release/:agentId')
  @Roles(UserRole.OPERATOR, UserRole.ADMIN)
  @ApiOperation({
    summary: 'Release session from agent',
    description: 'Release a browser session from an agent with intelligent cleanup and lifecycle management.',
  })
  @ApiParam({
    name: 'sessionId',
    description: 'Session identifier',
  ...
  @ApiQuery({
    name: 'keepAlive',
    description: 'Keep session alive after release',
    type: 'boolean',
    required: false,
  })
  @ApiResponse({
    ...
    type: SessionReleaseResponseDto,
  })
  async releaseSessionFromAgent(
    @Param('sessionId') sessionId: string,
    @Param('agentId') agentId: string,
    @Query('keepAlive') keepAlive: boolean = true,
  ): Promise<SessionReleaseResponseDto> {
    const operationId = this.generateOperationId();

    this.logger.log(`[${operationId}] Releasing session from agent`, {
      sessionId,
      agentId,
      keepAlive,
    });
  ...
  @ApiOperation({
    ...
    description: 'Get comprehensive status and metrics for browser session orchestration including pools, assignments, and performance data.',
  })
  @ApiResponse({
    status: 200,
  ...
  async getOrchestrationStatus(): Promise<OrchestrationStatusResponseDto> {
    ...
    this.logger.debug(`[${operationId}] Getting orchestration status`);

    try {
      const status = await this.orchestrationSessionService.getOrchestrationStatus();

      this.logger.debug(`[${operationId}] Orchestration status retrieved successfully`, {
        totalSessions: status.totalSessions,
        agentAssignments: status.agentAssignments,
      });
      ...
      throw new HttpException(
        error.message || 'Failed to get orchestration status',
  ...
  @ApiOperation({
    summary: 'Optimize session pools',
    description: 'Trigger session pool optimization based on usage patterns, utilization rates, and performance metrics.',
  })
  @ApiResponse({
    status: 200,
  ...
  async optimizeSessionPools(): Promise<{ success: boolean; message: string; timestamp: string }> {
    ...
    try {
      await this.orchestrationSessionService.optimizeSessionPools();
      ...
      throw new HttpException(
        error.message || 'Session pool optimization failed',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  @Roles(UserRole.OPERATOR, UserRole.ADMIN)
  @ApiOperation({
    summary: 'Close orchestration session',
    description: 'Close a browser session with comprehensive cleanup, agent release, and coordination notification.',
  })
  @ApiParam({
    name: 'sessionId',
    description: 'Session identifier',
  ...
  async closeOrchestrationSession(
    ...
    this.logger.log(`[${operationId}] Closing orchestration session`, { sessionId });

    try {
      await this.orchestrationSessionService.closeOrchestrationSession(sessionId);
  ...
  @ApiOperation({
    ...
    description: 'Get detailed performance metrics for a specific browser session including operation counts, response times, and resource usage.',
  })
  @ApiParam({
    name: 'sessionId',
    description: 'Session identifier',
  ...
  @ApiResponse({
    status: 404,
    description: 'Session or metrics not found',
  })
  async getSessionMetrics(
    ...
    this.logger.debug(`[${operationId}] Getting session metrics`, { sessionId });
    ...
    try {
      ...
      if (!metrics) {
        throw new HttpException(
          `Metrics not found for session ${sessionId}`,
          HttpStatus.NOT_FOUND,
        );
      }

      this.logger.debug(`[${operationId}] Session metrics retrieved successfully`, {
        sessionId,
      ...
      throw new HttpException(
        error.message || 'Failed to get session metrics',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  @Roles(UserRole.OPERATOR, UserRole.ADMIN)
  @ApiOperation({
    summary: 'Get session assignments for agent',
    description: 'Get all current session assignments for a specific agent including session details and assignment metadata.',
  })
  @ApiParam({
    name: 'agentId',
    description: 'Agent identifier',
  ...
  async getAgentAssignments(
    ...
    try {
      ...
      this.logger.debug(`[${operationId}] Agent assignments retrieved successfully`, {
        agentId,
