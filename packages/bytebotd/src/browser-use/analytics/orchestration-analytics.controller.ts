...
import { OrchestrationAnalyticsService } from './orchestration-analytics.service';
import { OrchestrationOperationType } from '../errors/orchestration-errors';
import { OrchestrationResponseInterceptor } from '../interceptors/orchestration-response.interceptor';
 ...
 */
@ApiTags('Orchestration Analytics')
@Controller('orchestration-analytics')
@ApiBearerAuth()
@UseInterceptors(ClassSerializerInterceptor, OrchestrationResponseInterceptor)
export class OrchestrationAnalyticsController {
  ...
  @Get('overview')
  @ApiOperation({
    summary: 'Get orchestration analytics overview',
    description: 'Retrieves comprehensive analytics overview including performance metrics, error analytics, real-time monitoring, and recommendations',
  })
  ...
  @ApiQuery({ name: 'operationType', enum: OrchestrationOperationType, required: false })
  @ApiQuery({ name: 'timeWindow', enum: ['1h', '6h', '24h', '7d', '30d'], required: false })
  @ApiResponse({ status: HttpStatus.OK, description: 'Analytics overview retrieved successfully' })
  async getAnalyticsOverview(
    @Query() query: AnalyticsQueryParams
  ) {
    ...
    try {
      ...
      this.logger.error('Failed to get analytics overview', {
  ...
  @ApiOperation({
    ...
    description: 'Retrieves real-time monitoring data for active orchestrations, system resources, and performance indicators',
  })
  @ApiResponse({ status: HttpStatus.OK, description: 'Real-time monitoring data retrieved successfully' })
  async getRealTimeMonitoring() {
    ...
    try {
      ...
      this.logger.error('Failed to get real-time monitoring data', {
        error: error instanceof Error ? error.message : 'Unknown error',
        duration: Date.now() - startTime,
      });
      throw error;
   ...
   */
  @Get('performance/:operationType')
  @ApiOperation({
    summary: 'Get performance metrics by operation type',
    description: 'Retrieves detailed performance metrics for a specific orchestration operation type',
  })
  @ApiParam({ name: 'operationType', enum: OrchestrationOperationType })
  @ApiQuery({ name: 'startDate', type: String, required: false })
  @ApiQuery({ name: 'endDate', type: String, required: false })
  @ApiResponse({ status: HttpStatus.OK, description: 'Performance metrics retrieved successfully' })
  async getPerformanceMetrics(
    @Param('operationType', new ParseEnumPipe(OrchestrationOperationType)) operationType: OrchestrationOperationType,
    @Query() query: AnalyticsQueryParams
  ) {
    const startTime = Date.now();
  ...
  @ApiOperation({
    summary: 'Get orchestration error analytics',
    description: 'Retrieves comprehensive error analytics including categorization, trends, and recovery statistics',
  })
  @ApiQuery({ name: 'startDate', type: String, required: false })
  @ApiQuery({ name: 'endDate', type: String, required: false })
  @ApiQuery({ name: 'operationType', enum: OrchestrationOperationType, required: false })
  @ApiResponse({ status: HttpStatus.OK, description: 'Error analytics retrieved successfully' })
  ...
  @Get('recommendations')
  @ApiOperation({
    summary: 'Get analytics recommendations',
    description: 'Retrieves intelligent recommendations for performance optimization and error reduction',
  })
  @ApiQuery({ name: 'startDate', type: String, required: false })
  @ApiQuery({ name: 'endDate', type: String, required: false })
  @ApiQuery({ name: 'operationType', enum: OrchestrationOperationType, required: false })
  ...
  @ApiOperation({
    ...
    description: 'Exports analytics data in specified format for external analysis or reporting',
  })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        ...
        operationType: { enum: Object.values(OrchestrationOperationType) },
      },
  ...
  async exportAnalyticsData(
    @Body(ValidationPipe) exportRequest: ExportRequest
  ) {
    const startTime = Date.now();

    this.logger.log('Exporting analytics data', {
      format: exportRequest.format,
      dateRange: `${exportRequest.startDate} - ${exportRequest.endDate}`,
      includeRawData: exportRequest.includeRawData,
      operationType: exportRequest.operationType,
    });
  ...
  @Get('health')
  @ApiOperation({
    summary: 'Get orchestration health status',
    description: 'Retrieves current health status and system indicators for orchestration operations',
  })
  @ApiResponse({ status: HttpStatus.OK, description: 'Health status retrieved successfully' })
  async getHealthStatus() {
    const startTime = Date.now();

    this.logger.log('Getting orchestration health status');
    ...
    try {
      ...
      const healthStatus = {
        ...
        status: recommendations.systemHealth.overallScore > 80 ? 'healthy' :
                recommendations.systemHealth.overallScore > 60 ? 'warning' : 'critical',
