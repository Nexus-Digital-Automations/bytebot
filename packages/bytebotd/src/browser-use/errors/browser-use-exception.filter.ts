...
export class BrowserUseExceptionFilter implements ExceptionFilter {
  ...
  private async handleGenericError(
    ...
    const browserUseError = await this.browserUseErrorClassification.classifyBrowserUseError(
      error,
      {
        ...
        selector: context.request.body?.selector || context.request.query?.selector,
  ...
  private sendBrowserUseErrorResponse(
    ...
    if (errorResponse.recovery.retryable && errorResponse.recovery.retryAfterMs) {
      response.setHeader('Retry-After', Math.ceil(errorResponse.recovery.retryAfterMs / 1000));
      response.setHeader('X-RateLimit-Limit', '100');
      response.setHeader('X-RateLimit-Remaining', '0');
      response.setHeader('X-RateLimit-Reset', String(Math.ceil(Date.now() / 1000) + 60));
    }
    ...
    this.logger.debug(`[${context.requestId}] Browser-Use error response sent`, {
      requestId: context.requestId,
  ...
  private sendFallbackErrorResponse(context: ExceptionContext): void {
    const fallbackResponse = {
      ...
      error: {
        ...
        userMessage: 'The system encountered an unexpected error. Please try again later.',
      ...
      recovery: {
        attempted: false,
        retryable: true,
        recommendations: ['Check system status', 'Retry the operation', 'Contact support']
      },
      support: {
        contactInfo: 'support@bytebot.com'
      }
  ...
  getErrorAnalytics(timeRange?: { start: Date; end: Date }): {
    ...
    const filteredMetrics = timeRange
      ? this.errorMetrics.filter(m => m.timestamp >= timeRange.start && m.timestamp <= timeRange.end)
      : this.errorMetrics;
  ...
  private mapBrowserUseErrorToHttpStatus(browserUseError: BrowserUseError): HttpStatus {
    const categoryToStatus = {
      [BrowserUseErrorCategory.AUTHENTICATION_REQUIRED]: HttpStatus.UNAUTHORIZED,
      ...
      [BrowserUseErrorCategory.TASK_EXECUTION_TIMEOUT]: HttpStatus.REQUEST_TIMEOUT,
      [BrowserUseErrorCategory.PAGE_LOAD_TIMEOUT]: HttpStatus.REQUEST_TIMEOUT,
      [BrowserUseErrorCategory.RATE_LIMIT_DETECTED]: HttpStatus.TOO_MANY_REQUESTS,
      [BrowserUseErrorCategory.SESSION_CONFLICT]: HttpStatus.CONFLICT,
      [BrowserUseErrorCategory.INVALID_TASK_CONFIGURATION]: HttpStatus.UNPROCESSABLE_ENTITY,
      [BrowserUseErrorCategory.BROWSER_LAUNCH_FAILED]: HttpStatus.SERVICE_UNAVAILABLE,
      [BrowserUseErrorCategory.MEMORY_EXHAUSTED]: HttpStatus.SERVICE_UNAVAILABLE
    };

    return categoryToStatus[browserUseError.browserUseCategory] || HttpStatus.INTERNAL_SERVER_ERROR;
  }
  ...
  private setSecurityHeaders(response: Response, errorResponse: BrowserUseErrorResponse): void {
    response.setHeader('X-Content-Type-Options', 'nosniff');
    response.setHeader('X-Frame-Options', 'DENY');
    response.setHeader('X-XSS-Protection', '1; mode=block');
    response.setHeader('Strict-Transport-Security', 'max-age=31536000; includeSubDomains');

    if (errorResponse.error.category === BrowserUseErrorCategory.BOT_DETECTION) {
      response.setHeader('X-Bot-Detection', 'true');
      response.setHeader('X-Challenge-Required', 'human-verification');
    }

    if (errorResponse.error.category === BrowserUseErrorCategory.RATE_LIMIT_DETECTED) {
  ...
  private getUserFriendlyMessage(browserUseError: BrowserUseError): string {
    const userMessages = {
      [BrowserUseErrorCategory.BROWSER_LAUNCH_FAILED]: 'Unable to start the browser session. Please try again.',
      [BrowserUseErrorCategory.BROWSER_CRASH]: 'The browser session crashed. A new session will be created automatically.',
      [BrowserUseErrorCategory.SESSION_NOT_FOUND]: 'The browser session was not found. Please create a new session.',
      [BrowserUseErrorCategory.SESSION_EXPIRED]: 'Your browser session has expired. Please start a new session.',
      [BrowserUseErrorCategory.NAVIGATION_FAILED]: 'Unable to navigate to the requested page. Please check the URL and try again.',
      [BrowserUseErrorCategory.PAGE_LOAD_TIMEOUT]: 'The page took too long to load. Please try again or check your connection.',
      [BrowserUseErrorCategory.ELEMENT_NOT_FOUND]: 'The requested element was not found on the page. The page content may have changed.',
      [BrowserUseErrorCategory.ELEMENT_NOT_INTERACTABLE]: 'The element cannot be interacted with. It may be hidden or disabled.',
      [BrowserUseErrorCategory.BOT_DETECTION]: 'The website detected automated activity. Manual verification may be required.',
      [BrowserUseErrorCategory.RATE_LIMIT_DETECTED]: 'Request rate limit exceeded. Please wait before making more requests.',
      [BrowserUseErrorCategory.MEMORY_EXHAUSTED]: 'System resources are temporarily exhausted. Please try again in a few minutes.',
      [BrowserUseErrorCategory.TASK_EXECUTION_TIMEOUT]: 'The automation task took too long to complete. Please try again with a simpler task.'
    };

    return userMessages[browserUseError.browserUseCategory] || 'An error occurred during browser automation. Please try again.';
  }

  private getRecoveryRecommendations(browserUseError: BrowserUseError, recoveryAction: any): string[] {
    ...
    return recommendations.length > 0 ? recommendations : ['Check system status and retry the operation'];
  }
  ...
  private groupBy(metrics: ErrorMetrics[], field: keyof ErrorMetrics): Record<string, number> {
  ...
  private calculateAverage(metrics: ErrorMetrics[], field: keyof ErrorMetrics): number {
  ...
  private calculateErrorRate(metrics: ErrorMetrics[]): number {
    ...
    const now = Date.now();
    const recentErrors = metrics.filter(m => now - m.timestamp.getTime() < timeWindow);
    return recentErrors.length;
  }

  private getTopUrls(metrics: ErrorMetrics[]): Array<{ url: string; count: number }> {
  ...
  private startMetricsCleanup(): void {
    ...
    setInterval(() => {
      ...
      if (initialLength > this.errorMetrics.length) {
        this.logger.debug(`Cleaned up ${initialLength - this.errorMetrics.length} old error metrics`);
      }
    }, 3600000); // Every hour
  }
}