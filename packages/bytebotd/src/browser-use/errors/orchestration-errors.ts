...
export interface OrchestrationError extends BaseError {
  ...
  readonly distributedContext: {
    ...
    readonly coordinationState: 'initializing' | 'executing' | 'aggregating' | 'finalizing' | 'failed';
  };
...
export interface DistributedOperationError extends OrchestrationError {
  ...
  readonly distributedError: {
    ...
    readonly taskDistribution: Array<{
      readonly nodeId: string;
...
export interface AggregationError extends OrchestrationError {
  ...
  readonly aggregationError: {
    ...
    readonly partialResults: Array<{
      readonly operationId: string;
      readonly status: 'success' | 'partial' | 'failed';
      readonly dataSize: number;
      readonly dataType: string;
      readonly mergeStatus: 'pending' | 'merged' | 'failed';
    }>;
    readonly mergeConflicts: Array<{
      readonly field: string;
      readonly conflictType: 'type_mismatch' | 'value_conflict' | 'structure_mismatch';
      readonly resolution: 'manual' | 'automatic' | 'skip';
    }>;
...
export function isOrchestrationError(error: unknown): error is OrchestrationError {
  return (
    ...
    error !== null &&
    'name' in error &&
    (error as { name: string }).name === 'OrchestrationError' &&
    'orchestrationId' in error &&
    'operationType' in error &&
    'distributedContext' in error
  );
}
...
export function isDistributedOperationError(error: unknown): error is DistributedOperationError {
  return (
    ...
    'distributedError' in error &&
    (error.category === OrchestrationErrorCategory.DISTRIBUTED_TASK_ERROR ||
     error.category === OrchestrationErrorCategory.PARALLEL_EXECUTION_ERROR)
  );
}
...
export function isWorkflowCoordinationError(error: unknown): error is WorkflowCoordinationError {
  return (
    isOrchestrationError(error) &&
    'workflowError' in error &&
    error.category === OrchestrationErrorCategory.WORKFLOW_COORDINATION_ERROR
  );
}
...
export function isResourceAllocationError(error: unknown): error is ResourceAllocationError {
  return (
    isOrchestrationError(error) &&
    'resourceError' in error &&
    (error.category === OrchestrationErrorCategory.RESOURCE_ALLOCATION_ERROR ||
     error.category === OrchestrationErrorCategory.BROWSER_POOL_ERROR ||
...
export function isAggregationError(error: unknown): error is AggregationError {
  return (
    isOrchestrationError(error) &&
    'aggregationError' in error &&
    (error.category === OrchestrationErrorCategory.RESULT_AGGREGATION_ERROR ||
     error.category === OrchestrationErrorCategory.DATA_MERGE_ERROR ||
...
export class OrchestrationErrorFactory {
  ...
  static createDistributedOperationError(
    ...
    distributedContext: OrchestrationError['distributedContext'],
    distributedError: DistributedOperationError['distributedError'],
    additionalContext?: Record<string, unknown>
  ): DistributedOperationError {
    return {
      name: 'OrchestrationError',
      message,
      code: 'DISTRIBUTED_OPERATION_FAILED',
      timestamp: new Date(),
  ...
  static createWorkflowCoordinationError(
    ...
    workflowId: string,
    operationType: OrchestrationOperationType,
    workflowError: WorkflowCoordinationError['workflowError'],
    additionalContext?: Record<string, unknown>
  ): WorkflowCoordinationError {
    return {
      name: 'OrchestrationError',
      ...
      timestamp: new Date(),
      category: OrchestrationErrorCategory.WORKFLOW_COORDINATION_ERROR,
  ...
  static createResourceAllocationError(
    ...
    orchestrationId: string,
    operationType: OrchestrationOperationType,
    resourceError: ResourceAllocationError['resourceError'],
    additionalContext?: Record<string, unknown>
  ): ResourceAllocationError {
    return {
      name: 'OrchestrationError',
      ...
      timestamp: new Date(),
      category: OrchestrationErrorCategory.RESOURCE_ALLOCATION_ERROR,
      severity: OrchestrationErrorSeverity.HIGH,
      operationType,
      orchestrationId,
      distributedContext: {
  ...
  static createAggregationError(
    ...
    orchestrationId: string,
    operationType: OrchestrationOperationType,
    aggregationError: AggregationError['aggregationError'],
    additionalContext?: Record<string, unknown>
  ): AggregationError {
    return {
      name: 'OrchestrationError',
      ...
      timestamp: new Date(),
      category: OrchestrationErrorCategory.RESULT_AGGREGATION_ERROR,
      severity: OrchestrationErrorSeverity.MEDIUM,
      operationType,
      orchestrationId,
      distributedContext: {
        totalOperations: aggregationError.partialResults.length,
        completedOperations: aggregationError.partialResults.filter(r => r.status === 'success').length,
        failedOperations: aggregationError.partialResults.filter(r => r.status === 'failed').length,
        remainingOperations: aggregationError.partialResults.filter(r => r.status === 'partial').length,
        parallelExecutions: aggregationError.partialResults.length,
...
export class OrchestrationErrorAnalyzer {
  ...
  static analyzeErrorImpact(error: OrchestrationErrorType): {
    ...
    recoveryComplexity: 'simple' | 'moderate' | 'complex' | 'manual';
