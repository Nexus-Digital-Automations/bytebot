import {
  ...
  ApiResponse,
  ApiBody,
} from '@nestjs/swagger';import { ExtractionService } from './extraction.service';import {TextExtractionRequestDto,
...
} from './dto/data-extraction.dto';/*** Data Extraction Controller
 *
 ...
 */
@ApiTags('Data Extraction')@Controller('browser/extract')export class ExtractionController {private readonly logger = new Logger(ExtractionController.name);
   ...
   */
  @Post('text')@HttpCode(HttpStatus.OK)@ApiOperation({
    summary: 'Extract text content',
    description: `Extract text content from web page elements using CSS selectors.Supports multiple selectors, text cleaning, metadata extraction, and content length limits.
...
`,
  ...
  @ApiBody({
    ...
    description: 'Text extraction configuration',examples: {basic: {
        summary: 'Basic text extraction',description: 'Extract headings and paragraphs from a page',value: {sessionId: 'session_123456789',config: {selectors: ['h1', 'h2', 'p', '.content'],includeMetadata: true,cleanText: true,
          },
          format: 'json',timeout: 30000,},
      },
      advanced: {
        summary: 'Advanced text extraction with attributes',description: 'Extract text with attributes and length limits',value: {sessionId: 'session_123456789',config: {selectors: ['[data-content]', '.article-text', 'blockquote'],includeAttributes: true,includeMetadata: true,
            cleanText: true,
            maxTextLength: 500,
          },
          format: 'json',waitForSelector: '.content-loaded',timeout: 45000,},
      },
    },
  })
  @ApiResponse({
    status: 200,
    description: 'Text content extracted successfully',type: TextExtractionResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid extraction configuration',})@ApiResponse({
    status: 500,
    description: 'Text extraction failed',
  })
  async extractText(@Body() dto: TextExtractionRequestDto): Promise<TextExtractionResponseDto> {
    this.logger.log(`Text extraction request for session: ${dto.sessionId}`, {sessionId: dto.sessionId,selectorCount: dto.config.selectors.length,
      format: dto.format,
    ...
    try {
      ...
      this.logger.log(`Text extraction completed for session: ${dto.sessionId}`, {sessionId: dto.sessionId,success: result.success,
      ...
      throw new InternalServerErrorException({
        message: 'Text extraction failed',error: error instanceof Error ? error.message : String(error),});
    }
  }
   ...
   */
  @Post('table')@HttpCode(HttpStatus.OK)@ApiOperation({
    summary: 'Extract table data',
    description: `Extract structured table data from HTML tables with advanced formatting options.Supports header extraction, column mapping, row limits, and multiple output formats.
...
`,
  ...
  @ApiBody({
    ...
    description: 'Table extraction configuration',examples: {basic: {
        summary: 'Basic table extraction',description: 'Extract data from a simple table',value: {sessionId: 'session_123456789',config: {selector: 'table.data-table',includeHeaders: true,skipEmptyRows: true,
          },
          format: 'json',timeout: 30000,},
      },
      advanced: {
        summary: 'Advanced table extraction with mapping',description: 'Extract table with column mapping and limits',value: {sessionId: 'session_123456789',config: {selector: '#products-table',includeHeaders: true,columnMapping: {
              'Product Name': 'name','Price ($)': 'price','In Stock': 'available',},maxRows: 100,
            skipEmptyRows: true,
          },
          format: 'csv',waitForSelector: '.table-loaded',timeout: 45000,},
      },
    },
  })
  @ApiResponse({
    status: 200,
    description: 'Table data extracted successfully',type: TableExtractionResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid table extraction configuration',})@ApiResponse({
    status: 500,
  ...
  async extractTable(@Body() dto: TableExtractionRequestDto): Promise<TableExtractionResponseDto> {
    this.logger.log(`Table extraction request for session: ${dto.sessionId}`, {sessionId: dto.sessionId,selector: dto.config.selector,
      format: dto.format,
    ...
    try {
      ...
      this.logger.log(`Table extraction completed for session: ${dto.sessionId}`, {sessionId: dto.sessionId,success: result.success,
      ...
      throw new InternalServerErrorException({
        message: 'Table extraction failed',error: error instanceof Error ? error.message : String(error),});
    }
  }
   ...
   */
  @Post('links')@HttpCode(HttpStatus.OK)@ApiOperation({
    summary: 'Extract page links',
    description: `Extract all links from a web page with advanced filtering and categorization.Supports internal/external link detection, pattern filtering, and metadata extraction.
...
`,
  ...
  @ApiBody({
    ...
    description: 'Link extraction configuration',examples: {basic: {
        summary: 'Basic link extraction',description: 'Extract all links from a page',value: {sessionId: 'session_123456789',config: {includeInternal: true,
      ...
      filtered: {
        summary: 'Filtered link extraction',description: 'Extract links with pattern filtering',value: {sessionId: 'session_123456789',config: {containerSelector: 'nav, .menu',includeInternal: true,includeExternal: false,
            includeMetadata: true,
            filterPattern: '^/products/.*',excludePattern: '\\.(pdf|doc|zip)$',},waitForSelector: '.navigation-loaded',timeout: 30000,},
      },
    },
  })
  @ApiResponse({
    status: 200,
    description: 'Links extracted successfully',type: LinkExtractionResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid link extraction configuration',})@ApiResponse({
    status: 500,
    description: 'Link extraction failed',
  })
  async extractLinks(@Body() dto: LinkExtractionRequestDto): Promise<LinkExtractionResponseDto> {
    this.logger.log(`Link extraction request for session: ${dto.sessionId}`, {sessionId: dto.sessionId,containerSelector: dto.config?.containerSelector,
    ...
    try {
      ...
      this.logger.log(`Link extraction completed for session: ${dto.sessionId}`, {sessionId: dto.sessionId,success: result.success,
      ...
      throw new InternalServerErrorException({
        message: 'Link extraction failed',error: error instanceof Error ? error.message : String(error),});
    }
  }
   ...
   */
  @Post('images')@HttpCode(HttpStatus.OK)@ApiOperation({
    summary: 'Extract page images',
    description: `Extract image sources and comprehensive metadata from web pages.Supports dimension filtering, format detection, and metadata extraction.
...
`,
  ...
  @ApiBody({
    ...
    description: 'Image extraction configuration',examples: {basic: {
        summary: 'Basic image extraction',description: 'Extract all images from a page',value: {sessionId: 'session_123456789',config: {includeMetadata: true,
      ...
      filtered: {
        summary: 'Filtered image extraction',description: 'Extract images with size and format filters',value: {sessionId: 'session_123456789',config: {containerSelector: '.gallery, .product-images',includeMetadata: true,includeDataUrls: false,
            minDimensions: { width: 200, height: 200 },
            fileExtensions: ['jpg', 'png', 'webp'],},waitForSelector: '.images-loaded',timeout: 45000,},
      },
    },
  })
  @ApiResponse({
    status: 200,
    description: 'Images extracted successfully',type: ImageExtractionResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid image extraction configuration',})@ApiResponse({
    status: 500,
  ...
  async extractImages(@Body() dto: ImageExtractionRequestDto): Promise<ImageExtractionResponseDto> {
    this.logger.log(`Image extraction request for session: ${dto.sessionId}`, {sessionId: dto.sessionId,containerSelector: dto.config?.containerSelector,
    ...
    try {
      ...
      this.logger.log(`Image extraction completed for session: ${dto.sessionId}`, {sessionId: dto.sessionId,success: result.success,
      ...
      throw new InternalServerErrorException({
        message: 'Image extraction failed',error: error instanceof Error ? error.message : String(error),});
    }
  }
   ...
   */
  @Post('structured-data')@HttpCode(HttpStatus.OK)@ApiOperation({
    summary: 'Extract structured data',
    description: `Extract structured data from web pages including JSON-LD, Microdata, and OpenGraph.Supports schema.org markup, social media metadata, and SEO structured data.
...
`,
  ...
  @ApiBody({
    ...
    description: 'Structured data extraction configuration',examples: {basic: {
  ...
  @ApiResponse({
    status: 200,
    description: 'Structured data extracted successfully',type: StructuredDataExtractionResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid structured data extraction configuration',})@ApiResponse({
    status: 500,
  ...
  async extractStructuredData(
    ...
    this.logger.log(`Structured data extraction request for session: ${dto.sessionId}`, {sessionId: dto.sessionId,jsonLdSelectors: dto.jsonLdSelectors?.length,
    ...
    try {
      ...
      this.logger.log(`Structured data extraction completed for session: ${dto.sessionId}`, {sessionId: dto.sessionId,success: result.success,
      ...
      throw new InternalServerErrorException({
        message: 'Structured data extraction failed',error: error instanceof Error ? error.message : String(error),});
    }
  }
   ...
   */
  @Post('xpath')@HttpCode(HttpStatus.OK)@ApiOperation({
    summary: 'Extract data using XPath',
    description: `Extract data using XPath expressions for complex element selection and data extraction.Supports advanced DOM querying, attribute extraction, and text content retrieval.
...
`,
  ...
  @ApiResponse({
    status: 200,
    description: 'XPath data extracted successfully',type: XPathExtractionResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid XPath extraction configuration',})@ApiResponse({
    status: 500,
  ...
  async extractXPath(@Body() dto: XPathExtractionRequestDto): Promise<XPathExtractionResponseDto> {
    this.logger.log(`XPath extraction request for session: ${dto.sessionId}`, {sessionId: dto.sessionId,xpathCount: dto.xpaths.length,
    ...
    try {
      ...
      this.logger.log(`XPath extraction completed for session: ${dto.sessionId}`, {sessionId: dto.sessionId,success: result.success,
      ...
      throw new InternalServerErrorException({
        message: 'XPath extraction failed',error: error instanceof Error ? error.message : String(error),});
    }
  }
   ...
   */
  @Post('batch')@HttpCode(HttpStatus.OK)@ApiOperation({
    summary: 'Perform batch extraction',
    description: `Perform multiple extraction operations in a single request for efficiency.Supports combining different extraction types with individual configurations.
...
`,
  ...
  @ApiBody({
    ...
    description: 'Batch extraction configuration',examples: {comprehensive: {
        summary: 'Comprehensive page extraction',description: 'Extract multiple data types from a page',value: {sessionId: 'session_123456789',operations: [{
              type: 'text',name: 'page_content',config: {selectors: ['h1', 'h2', 'p', '.content'],includeMetadata: true,cleanText: true,
              },
            },
            {
              type: 'links',name: 'navigation',config: {containerSelector: 'nav',includeInternal: true,includeExternal: false,
              },
            },
            {
              type: 'images',name: 'gallery',config: {containerSelector: '.gallery',includeMetadata: true,minDimensions: { width: 100, height: 100 },
              },
            },
          ],
          timeout: 60000,
  ...
  @ApiResponse({
    status: 200,
    description: 'Batch extraction completed',type: BatchExtractionResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid batch extraction configuration',})@ApiResponse({
    status: 500,
  ...
  async extractBatch(@Body() dto: BatchExtractionRequestDto): Promise<BatchExtractionResponseDto> {
    ...
    try {
      ...
      this.logger.log(`Batch extraction completed for session: ${dto.sessionId}`, {sessionId: dto.sessionId,success: result.success,
