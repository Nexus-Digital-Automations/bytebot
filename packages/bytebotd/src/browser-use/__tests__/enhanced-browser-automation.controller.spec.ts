...
import { BrowserTaskStatus, BrowserSessionStatus } from '../dto/browser-session.dto';
import {
...
describe('EnhancedBrowserAutomationController', () => {
  let controller: EnhancedBrowserAutomationController;
  ...
  const mockElementDetectionResult: ElementDetectionResultDto = {
    ...
    sessionId: 'session-123',
    success: true,
    strategy: DetectionStrategy.CSS_SELECTOR,
    elementsFound: 1,
    elements: [
      {
  ...
  const mockRealtimeConnectionStatus: RealtimeConnectionStatusDto = {
    ...
    sessionId: 'session-123',
    state: 'connected',
    activeSubscriptions: ['page-changes', 'dom-mutations'],
    eventsReceived: 25,
  ...
  describe('captureScreenshot', () => {
    const captureDto: ScreenshotCaptureDto = {
      sessionId: 'session-123',
      type: ScreenshotType.FULLPAGE,
      format: ScreenshotFormat.PNG,
      quality: 90,
    ...
    it('should capture a screenshot successfully', async () => {
      browserUseService.captureEnhancedScreenshot.mockResolvedValue(mockScreenshotResult);

      const result = await controller.captureScreenshot(captureDto);
    ...
    it('should handle screenshot capture failure', async () => {
      ...
      browserUseService.captureEnhancedScreenshot.mockRejectedValue(error);

      await expect(controller.captureScreenshot(captureDto)).rejects.toThrow(
  ...
  describe('batchCaptureScreenshots', () => {
    const batchDto: BatchScreenshotCaptureDto = {
      sessionId: 'session-123',
      screenshots: [
        {
          type: ScreenshotType.FULLPAGE,
    ...
    it('should capture batch screenshots successfully', async () => {
  ...
  describe('performDOMInteraction', () => {
    ...
    it('should perform DOM interaction successfully', async () => {
      browserUseService.performDOMInteraction.mockResolvedValue(mockDOMInteractionResult);

      const result = await controller.performDOMInteraction(interactionDto);
    ...
    it('should handle DOM interaction failure', async () => {
      const error = new Error('DOM interaction failed');
      browserUseService.performDOMInteraction.mockRejectedValue(error);

      await expect(controller.performDOMInteraction(interactionDto)).rejects.toThrow(
        InternalServerErrorException,
      );
    });
  });

  describe('performBatchDOMInteractions', () => {
    const batchDto: BatchDOMInteractionDto = {
      sessionId: 'session-123',
      interactions: [
        {
          action: DOMActionType.CLICK,
        ...
        },
        {
          ...
          selector: { value: '#input1', type: 'css' },
          value: 'test input',
        },
      ],
      continueOnError: true,
    ...
    it('should perform batch DOM interactions successfully', async () => {
  ...
  describe('detectElements', () => {
    const detectionDto: ElementDetectionDto = {
      sessionId: 'session-123',
      criteria: {
        strategy: DetectionStrategy.CSS_SELECTOR,
    ...
    it('should detect elements successfully', async () => {
      browserUseService.detectElements.mockResolvedValue(mockElementDetectionResult);

      const result = await controller.detectElements(detectionDto);
    ...
    it('should handle element detection failure', async () => {
      const error = new Error('Element detection failed');
      browserUseService.detectElements.mockRejectedValue(error);

      await expect(controller.detectElements(detectionDto)).rejects.toThrow(
        InternalServerErrorException,
      );
    });
  });
  ...
  describe('extractTextOCR', () => {
    ...
    it('should extract text using OCR successfully', async () => {
      browserUseService.extractTextOCR.mockResolvedValue(mockOCRResult);

      const result = await controller.extractTextOCR(ocrDto);
    ...
    it('should handle OCR extraction failure', async () => {
      const error = new Error('OCR extraction failed');
      browserUseService.extractTextOCR.mockRejectedValue(error);

      await expect(controller.extractTextOCR(ocrDto)).rejects.toThrow(
        InternalServerErrorException,
      );
    });
  });
  ...
  describe('subscribeToRealtimeEvents', () => {
    const subscriptionDto: RealtimeSubscriptionDto = {
      sessionId: 'session-123',
      subscriptionTypes: ['page-changes', 'dom-mutations'],
      includeScreenshots: true,
      clientId: 'client-123',
      eventFilters: {
        elementSelectors: ['.important'],
        urlPatterns: ['*/api/*'],
      },
    };

    it('should subscribe to real-time events successfully', async () => {
      browserUseService.subscribeToRealtimeEvents.mockResolvedValue(mockRealtimeConnectionStatus);

      const result = await controller.subscribeToRealtimeEvents(subscriptionDto);
    ...
    it('should handle subscription failure', async () => {
      ...
      browserUseService.subscribeToRealtimeEvents.mockRejectedValue(error);

      await expect(controller.subscribeToRealtimeEvents(subscriptionDto)).rejects.toThrow(
        InternalServerErrorException,
      );
    });
  });
  ...
  describe('getConnectionStatus', () => {
    it('should return connection status', async () => {
      browserUseService.getConnectionStatus.mockResolvedValue(mockRealtimeConnectionStatus);

      const result = await controller.getConnectionStatus('conn-123');

      expect(result).toEqual(mockRealtimeConnectionStatus);
      expect(browserUseService.getConnectionStatus).toHaveBeenCalledWith('conn-123');
    });

    it('should throw NotFoundException when connection not found', async () => {
      browserUseService.getConnectionStatus.mockResolvedValue(null);

      await expect(controller.getConnectionStatus('invalid-conn')).rejects.toThrow(
        NotFoundException,
      );
    });
  });

  describe('closeRealtimeConnection', () => {
    it('should close real-time connection successfully', async () => {
      browserUseService.closeRealtimeConnection.mockResolvedValue(undefined);

      await controller.closeRealtimeConnection('conn-123');

      expect(browserUseService.closeRealtimeConnection).toHaveBeenCalledWith('conn-123');
    });

    it('should throw NotFoundException when connection not found', async () => {
      const error = new Error('Connection not found');
      browserUseService.closeRealtimeConnection.mockRejectedValue(error);

      await expect(controller.closeRealtimeConnection('invalid-conn')).rejects.toThrow(
        NotFoundException,
      );
    });
  });

  describe('streamRealtimeEvents', () => {
    it('should create SSE stream successfully', async () => {
      const mockObservable = new Observable((subscriber) => {
        subscriber.next({ data: 'test event' });
        subscriber.complete();
      });
      ...
      expect(browserUseService.createSSEStream).toHaveBeenCalledWith(
        expect.objectContaining({
          sessionId: 'session-123',
          eventTypes: ['page-changes', 'dom-mutations'],
          metadata: { includeScreenshots: true },
        }),
      );
    });
  });

  describe('getRealtimeEventStats', () => {
    it('should return real-time event statistics', async () => {
      ...
      const result = await controller.getRealtimeEventStats('session-123', 60);

      expect(result).toEqual(mockStats);
      expect(browserUseService.getRealtimeEventStats).toHaveBeenCalledWith('session-123', 60);
    });

    it('should throw NotFoundException when session not found', async () => {
      browserUseService.getRealtimeEventStats.mockResolvedValue(null);

      await expect(controller.getRealtimeEventStats('invalid-session', 60)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('getEnhancedHealth', () => {
    it('should return enhanced health information', async () => {
      const mockSessions = [
        { sessionId: 'session-1', status: BrowserSessionStatus.ACTIVE },
      ];
      const mockRunningTasks = [
        { taskId: 'task-1', status: BrowserTaskStatus.RUNNING },
      ];
      ...
      expect(result.service).toBe('Enhanced Browser Automation Controller');
      expect(result.version).toBe('3.0.0');
      expect(result.capabilities.multiFormatScreenshots).toBe(true);
  ...
  describe('getAutomationCapabilities', () => {
