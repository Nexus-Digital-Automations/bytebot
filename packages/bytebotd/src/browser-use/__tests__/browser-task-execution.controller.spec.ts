...
describe('BrowserTaskExecutionController', () => {
  let controller: BrowserTaskExecutionController;
  let taskExecutionService: jest.Mocked<BrowserTaskExecutionService>;
  ...
  const mockBatchExecutionResponse: BatchTaskExecutionResponseDto = {
    ...
    executions: [mockTaskExecutionResponse],
  ...
  describe('executeTask', () => {
    const executionRequest: TaskExecutionRequestDto = {
      taskDefinition: {
        name: 'Test Task',
    ...
    it('should execute a task successfully', async () => {
      taskExecutionService.executeTask.mockResolvedValue(mockTaskExecutionResponse);
    ...
    it('should handle task execution failure', async () => {
      ...
      taskExecutionService.executeTask.mockRejectedValue(error);

      await expect(controller.executeTask(executionRequest)).rejects.toThrow(
    ...
    it('should validate task definition before execution', async () => {
      ...
      const validationError = new Error('Task definition validation failed: No steps provided');
      taskExecutionService.executeTask.mockRejectedValue(validationError);
  ...
  describe('executeBatchTasks', () => {
    const batchRequest: BatchTaskExecutionRequestDto = {
      sessionId: 'session-123',
      tasks: [
        {
          taskDefinition: {
            ...
            steps: [{ type: 'navigate', url: 'https://example1.com' }],
          },
          executionOptions: { captureScreenshots: true },
        },
        {
          taskDefinition: {
            name: 'Task 2',
            description: 'Second task',
            steps: [{ type: 'navigate', url: 'https://example2.com' }],
          },
          executionOptions: { captureScreenshots: false },
        },
      ],
      batchOptions: {
        parallel: false,
    ...
    it('should execute batch tasks successfully', async () => {
      taskExecutionService.executeBatchTasks.mockResolvedValue(mockBatchExecutionResponse);

      const result = await controller.executeBatchTasks(batchRequest);
    ...
    it('should handle batch execution failure', async () => {
      ...
      taskExecutionService.executeBatchTasks.mockRejectedValue(error);

      await expect(controller.executeBatchTasks(batchRequest)).rejects.toThrow(
        InternalServerErrorException,
      );
    });
  });
  ...
  describe('getTaskExecution', () => {
    it('should return task execution details', async () => {
      taskExecutionService.getTaskExecution.mockResolvedValue(mockTaskExecutionResponse);

      const result = await controller.getTaskExecution('exec-123');

      expect(result).toEqual(mockTaskExecutionResponse);
      expect(taskExecutionService.getTaskExecution).toHaveBeenCalledWith('exec-123');
    });

    it('should throw NotFoundException when execution not found', async () => {
      taskExecutionService.getTaskExecution.mockResolvedValue(null);

      await expect(controller.getTaskExecution('invalid-exec')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('cancelTaskExecution', () => {
    it('should cancel task execution successfully', async () => {
      ...
      const result = await controller.cancelTaskExecution('exec-123');

      expect(result).toEqual(canceledExecution);
      expect(taskExecutionService.cancelTaskExecution).toHaveBeenCalledWith('exec-123');
    });

    it('should throw NotFoundException when execution not found', async () => {
      const error = new Error('Execution not found');
      taskExecutionService.cancelTaskExecution.mockRejectedValue(error);

      await expect(controller.cancelTaskExecution('invalid-exec')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('pauseTaskExecution', () => {
    it('should pause task execution successfully', async () => {
      ...
      const result = await controller.pauseTaskExecution('exec-123');

      expect(result).toEqual(pausedExecution);
      expect(taskExecutionService.pauseTaskExecution).toHaveBeenCalledWith('exec-123');
    });
  });

  describe('resumeTaskExecution', () => {
    it('should resume task execution successfully', async () => {
      ...
      const result = await controller.resumeTaskExecution('exec-123');

      expect(result).toEqual(resumedExecution);
      expect(taskExecutionService.resumeTaskExecution).toHaveBeenCalledWith('exec-123');
    });
  });

  describe('getTaskExecutionHistory', () => {
    it('should return task execution history with filters', async () => {
      const history = [mockTaskExecutionResponse];
      taskExecutionService.getTaskExecutionHistory.mockResolvedValue(history);
    ...
    it('should return all history when no filters provided', async () => {
      ...
      const result = await controller.getTaskExecutionHistory('session-123');
  ...
  describe('getTaskExecutionMetrics', () => {
    it('should return task execution metrics', async () => {
      const metrics = {
        totalExecutions: 100,
      ...
      const result = await controller.getTaskExecutionMetrics('session-123', 30);

      expect(result.status).toBe('success');
      expect(result.data).toEqual(metrics);
      expect(taskExecutionService.getTaskExecutionMetrics).toHaveBeenCalledWith(
        'session-123',
        30,
      );
    });
  ...
  describe('validateTaskDefinition', () => {
    it('should validate task definition successfully', async () => {
      ...
      expect(taskExecutionService.validateTaskDefinition).toHaveBeenCalledWith(taskDefinition);
    });

    it('should return validation errors for invalid task definition', async () => {
      const invalidTaskDefinition = {
        name: '',
        description: '',
        steps: [],
      };
      ...
      const validationResult = {
        ...
        warnings: [],
        suggestions: ['Consider adding more descriptive step names'],
      };
  ...
  describe('getTaskExecutionHealth', () => {
    it('should return task execution health status', async () => {
      const healthData = {
        status: 'healthy',
        service: 'Browser Task Execution Controller',
        ...
        capabilities: {
          ...
          supportedTaskTypes: ['navigation', 'interaction', 'extraction', 'validation'],
          featuresEnabled: [
            'screenshot_capture',
            'error_recovery',
            'batch_execution',
