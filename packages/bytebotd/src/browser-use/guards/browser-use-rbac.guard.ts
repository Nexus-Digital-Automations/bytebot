 ...
 */
export const RequireConversationalApproval = (riskLevel: RiskLevel = RiskLevel.HIGH) =>
...
export class BrowserUseRbacGuard implements CanActivate {
  ...
  private async evaluateAuthorization(
    ...
    this.logger.debug(
      `[${decisionId}] Evaluating browser use authorization`,
      {
    ...
    try {
      ...
      this.logger.debug(
        `[${decisionId}] Browser use authorization granted`,
        {
          userId: request.user.userId,
      ...
      this.logger.error(
        `[${decisionId}] Browser use authorization failed`,
        {
  ...
  private extractAuthorizationRequirements(context: ExecutionContext) {
    const requiredPermissions = this.reflector.get<BrowserPermission[]>('permissions', context.getHandler()) || [];
    const requiredRoles = this.reflector.get<string[]>('roles', context.getHandler()) || [];
    const conversationalApprovalLevel = this.reflector.get<RiskLevel>('conversationalApproval', context.getHandler());
  ...
  private async evaluateConversationalValidation(
    ...
    if (!requiredRiskLevel) {
      ...
      if (evaluationContext.operation.estimatedRiskLevel < RiskLevel.HIGH) {
    ...
    try {
      ...
      return {
        required: true,
        approved: false,
        reasoning: 'Conversational validation service unavailable',
  ...
  private getRequiredPermissionsForOperation(operation: BrowserOperationContext): BrowserPermission[] {
    ...
    const operationPermissionMap: Record<string, BrowserPermission[]> = {
      ...
      'DELETE /parlant/browser-use/tasks/:taskId': [BrowserPermission.DELETE_TASK],
  ...
  private generateAuthorizationConditions(context: PermissionEvaluationContext): AuthorizationCondition[] {
    ...
    if (context.operation.estimatedRiskLevel >= RiskLevel.HIGH) {
      conditions.push({
        type: 'MONITORING',
  ...
  private determineOperationType(method: string): 'READ' | 'WRITE' | 'DELETE' | 'ADMIN' {
  ...
  private estimateOperationRisk(request: AuthenticatedRequest): RiskLevel {
    ...
    if (request.method === 'DELETE') return RiskLevel.HIGH;
    if (request.url.includes('/admin')) return RiskLevel.CRITICAL;
    if (request.body?.actions?.length > 10) return RiskLevel.MEDIUM;
  ...
  private getRequiredSecurityLevel(operation: BrowserOperationContext): SecurityLevel {
    if (operation.operationType === 'ADMIN') return SecurityLevel._CRITICAL;
    if (operation.operationType === 'DELETE') return SecurityLevel._HIGH;
    if (operation.businessImpact === 'CRITICAL') return SecurityLevel._HIGH;
  ...
  private determineEscalationLevel(businessImpact: string): 'STANDARD' | 'SUPERVISOR' | 'SECURITY_TEAM' | 'EMERGENCY' {
    switch (businessImpact) {
      case 'CRITICAL': return 'SECURITY_TEAM';
      case 'HIGH': return 'SUPERVISOR';
  ...
  private async logAuthorizationDecision(decision: AuthorizationDecision): Promise<void> {
    ...
    this.logger.log(`Authorization Decision: ${decision.auditTrail.decision}`, {
      decisionId: decision.auditTrail.decisionId,
      userId: decision.auditTrail.userId,
