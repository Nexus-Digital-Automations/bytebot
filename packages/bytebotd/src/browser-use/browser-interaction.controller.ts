 ...
 */
@ApiTags('Browser Interaction')
@Controller('browser')
export class BrowserInteractionController {
  private readonly logger = new Logger(BrowserInteractionController.name);
  ...
  @ApiOperation({
    ...
    description: 'Click an element by selector or coordinates with configurable click type',
  })
  @ApiBody({ type: ClickInteractionDto })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Click interaction completed',
  ...
  async clickElement(@Body() clickDto: ClickInteractionDto): Promise<InteractionResultDto> {
    this.logger.log(`Click element request for session: ${clickDto.sessionId}`, {
    ...
    try {
      ...
      this.logger.log(`Click element completed: ${result.interactionId}`, {
      ...
      throw new InternalServerErrorException({
        message: 'Click interaction failed',
  ...
  @ApiOperation({
    summary: 'Type text',
    description: 'Type text into an element with configurable typing options',
  })
  @ApiBody({ type: TypeInteractionDto })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Type interaction completed',
    type: InteractionResultDto,
  })
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
  ...
  async typeText(@Body() typeDto: TypeInteractionDto): Promise<InteractionResultDto> {
    this.logger.log(`Type text request for session: ${typeDto.sessionId}`, {
    ...
    try {
      ...
      this.logger.log(`Type text completed: ${result.interactionId}`, {
        success: result.success,
        durationMs: result.durationMs,
      });
      ...
      throw new InternalServerErrorException({
        message: 'Type interaction failed',
        error: error instanceof Error ? error.message : String(error),
  ...
  @ApiOperation({
    ...
    description: 'Scroll the page or a specific element with direction and distance control',
  })
  @ApiBody({ type: ScrollInteractionDto })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Scroll interaction completed',
    type: InteractionResultDto,
  })
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
  ...
  async scrollElement(@Body() scrollDto: ScrollInteractionDto): Promise<InteractionResultDto> {
    this.logger.log(`Scroll request for session: ${scrollDto.sessionId}`, {
    ...
    try {
      ...
      this.logger.log(`Scroll completed: ${result.interactionId}`, {
        success: result.success,
        durationMs: result.durationMs,
      });
      ...
      throw new InternalServerErrorException({
        message: 'Scroll interaction failed',
        error: error instanceof Error ? error.message : String(error),
  ...
  @ApiOperation({
    ...
    description: 'Hover over an element with configurable hover duration',
  })
  @ApiBody({ type: HoverInteractionDto })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Hover interaction completed',
    type: InteractionResultDto,
  })
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
  ...
  async hoverElement(@Body() hoverDto: HoverInteractionDto): Promise<InteractionResultDto> {
    this.logger.log(`Hover request for session: ${hoverDto.sessionId}`, {
    ...
    try {
      ...
      this.logger.log(`Hover completed: ${result.interactionId}`, {
        success: result.success,
        durationMs: result.durationMs,
      });
      ...
      throw new InternalServerErrorException({
        message: 'Hover interaction failed',
        error: error instanceof Error ? error.message : String(error),
  ...
  @ApiOperation({
    ...
    description: 'Find elements by selector with optional screenshot capture',
  })
  @ApiQuery({
    name: 'sessionId',
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Element search completed',
    type: ElementFindResultDto,
  })
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
    description: 'Invalid request parameters',
  ...
  async findElements(
    ...
    @Query('selectorType') selectorType?: SelectorStrategy,
    @Query('findAll') findAll?: boolean,
    @Query('waitForElement') waitForElement?: boolean,
    @Query('timeout') timeout?: number,
    @Query('includeScreenshot') includeScreenshot?: boolean,
  ): Promise<ElementFindResultDto> {
    this.logger.log(`Find elements request for session: ${sessionId}`, {
      selector,
      selectorType,
      findAll,
    });
    ...
    try {
      ...
      this.logger.log(`Find elements completed for session: ${sessionId}`, {
        count: result.count,
      });
  ...
  @ApiOperation({
    ...
    description: 'Get detailed information about an element including attributes, styles, and properties',
  })
  @ApiQuery({
    name: 'sessionId',
    description: 'Browser session identifier',
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Element information retrieved',
  ...
  async getElementInfo(
    ...
    @Query('selector') selector: string,
    @Query('selectorType') selectorType?: SelectorStrategy,
    @Query('infoType') infoType?: ElementInfoType,
    @Query('includeStyles') includeStyles?: boolean,
    @Query('includeAttributes') includeAttributes?: boolean,
  ): Promise<ElementInfoResultDto> {
    this.logger.log(`Get element info request for session: ${sessionId}`, {
      selector,
      selectorType,
      infoType,
    });
    ...
    try {
      ...
      this.logger.log(`Get element info completed for session: ${sessionId}`, {
        found: result.found,
      ...
      throw new InternalServerErrorException({
        message: 'Element info retrieval failed',
        error: error instanceof Error ? error.message : String(error),
        sessionId,
      });
    }
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Element text content extracted',
    schema: {
      type: 'object',
      properties: {
  ...
  async getElementText(
    ...
    this.logger.log(`Get element text request for session: ${sessionId}`, {
      selector,
      selectorType,
    });
    ...
    try {
      ...
      this.logger.log(`Get element text completed for session: ${sessionId}`, {
        found: result.found,
      ...
      throw new InternalServerErrorException({
        message: 'Element text extraction failed',
        error: error instanceof Error ? error.message : String(error),
        sessionId,
      });
    }
  ...
  @ApiOperation({
    ...
    description: 'Extract the HTML source code of the current page',
  })
  @ApiQuery({
    name: 'sessionId',
    description: 'Browser session identifier',
  ...
  @ApiQuery({
    name: 'formatted',
    description: 'Return formatted HTML',
    type: Boolean,
    required: false,
  })
  @ApiQuery({
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Page source extracted',
    type: PageSourceResultDto,
  })
  @ApiResponse({
    status: HttpStatus.BAD_REQUEST,
    description: 'Invalid request parameters',
  ...
  async getPageSource(
    ...
    this.logger.log(`Get page source request for session: ${sessionId}`, {
      formatted,
      includeMetadata,
    });
    ...
    try {
      ...
      this.logger.log(`Get page source completed for session: ${sessionId}`, {
      ...
      throw new InternalServerErrorException({
        message: 'Page source extraction failed',
        error: error instanceof Error ? error.message : String(error),
        sessionId,
      });
    }
  ...
  @Get('capabilities')
  @ApiOperation({
    summary: 'Get browser interaction capabilities',
    description: 'Get information about available browser interaction capabilities',
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Browser interaction capabilities',
    schema: {
      type: 'object',
      properties: {
        interactions: {
  ...
  async getCapabilities(): Promise<{
    ...
    this.logger.log('Get browser interaction capabilities request');

    const capabilities = {
      interactions: ['click', 'type', 'scroll', 'hover'],
      selectorTypes: Object.values(SelectorStrategy),
      clickTypes: ['left', 'right', 'double', 'middle'],
      scrollDirections: ['up', 'down', 'left', 'right', 'top', 'bottom'],
      infoTypes: Object.values(ElementInfoType),
      version: '1.0.0',
      timestamp: new Date(),
    };
