...
/**
 * Multi-service workflow definition with validation checkpoints
 */
...
export interface WorkflowValidationRequirement {
  ...
  type: 'CROSS_SERVICE_TRANSACTION' | 'RESOURCE_ALLOCATION' | 'SECURITY_CHECKPOINT' | 'BUSINESS_APPROVAL';
  condition: string;
...
export interface ConversationMessage {
  ...
  content: string;
  messageType: 'VALIDATION_REQUEST' | 'APPROVAL_REQUEST' | 'STATUS_UPDATE' | 'ERROR_NOTIFICATION';
  metadata: Record<string, unknown>;
}
...
export interface OrchestrationError {
  errorId: string;
  timestamp: Date;
  errorType: 'VALIDATION_FAILED' | 'APPROVAL_TIMEOUT' | 'SERVICE_FAILURE' | 'RESOURCE_EXHAUSTED' | 'BUSINESS_RULE_VIOLATION';
  message: string;
...
export interface CircuitBreakerConfig {
  ...
  timeout: number;
  fallbackStrategy: 'FAIL_FAST' | 'GRACEFUL_DEGRADATION' | 'ALTERNATIVE_SERVICE';
