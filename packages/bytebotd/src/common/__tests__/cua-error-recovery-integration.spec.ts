...
import { MoveMouseAction, ScreenshotAction, RiskLevel } from '@bytebot/shared';
...
type MockedComputerUseTools = {
  [K in keyof ComputerUseTools]: ComputerUseTools[K] extends (
    ...args: infer Args
  ) => infer Return
    ...
    for (let attempt = 1; attempt <= options.maxAttempts; attempt++) {
      try {
        ...
        if (
          ...
          !this.isRetryableError(lastError, options.retryableErrors)
        ) {
          throw lastError;
        }
...
describe('CUA Error Recovery and Failover Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    context = {
      ...
      mcpTools: testModule.get<ComputerUseTools>(
        ComputerUseTools,
      ) as ComputerUseTools,
      ...
      circuitBreakerService: testModule.get<CircuitBreakerService>(
  ...
  beforeEach(() => {
    ...
    ['NutService', 'ParlantService', 'CacheService', 'McpService'].forEach(
      (serviceId) => {
  ...
  describe('NUT Service Error Recovery', () => {
    ...
    it('should activate circuit breaker for persistent NUT service failures', async () => {
      const scenario = createFailureScenario({
        ...
        description:
          'NUT service persistent failure triggering circuit breaker',
        failureType: 'service-unavailable',
        affectedServices: ['NutService'],
      ...
      jest
        .spyOn(context.nutService, 'screendump')
      ...
      const failurePromises = Array.from({ length: 6 }, () =>
        context.circuitBreakerService
          .executeWithCircuitBreaker('NutService', () =>
            context.computerUseService.action(action),
          )
      ...
      const circuitBreakerState =
        context.circuitBreakerService.getCircuitBreakerState('NutService');
    ...
    it('should implement graceful degradation for NUT service outages', async () => {
      ...
      jest
        .spyOn(context.nutService, 'getCursorPosition')
        .mockRejectedValue(
          new Error('NUT service unavailable for cursor position'),
        );

      // Implement graceful degradation (return cached or default position)
      const gracefulCursorPosition = async (): Promise<{
        x: number;
        y: number;
      }> => {
        try {
  ...
  describe('Parlant Validation Service Recovery', () => {
    it('should handle Parlant service timeouts with fallback validation', async () => {
      ...
      const validateSpy = jest.spyOn(
        context.parlantIntegrationService,
        'validateFunctionExecution',
      );
      (
        validateSpy as jest.MockedFunction<
          (request: any) => Promise<ValidationResponse>
        >
      ).mockImplementation(
        () =>
          new Promise<ValidationResponse>((_, reject) =>
            setTimeout(
              () => reject(new Error('Parlant validation timeout')),
              10000,
            ),
          ),
      );
  ...
  describe('MCP Server Connection Recovery', () => {
    it('should handle MCP server disconnection and reconnection', async () => {
      ...
      const originalMoveMouse: (
        params: MouseMoveParams,
      ) => Promise<McpToolResponse> = jest.fn().mockResolvedValue({
        content: [{ type: 'text' as const, text: 'mouse moved' }],
      });
      const mouseMoveSpy = jest.spyOn(context.mcpTools as any, 'moveMouse');
      mouseMoveSpy.mockImplementation(
        async (params: MouseMoveParams): Promise<McpToolResponse> => {
          connectionAttempts++;
      ...
      const result =
        await context.circuitBreakerService.executeWithCircuitBreaker(
          'McpService',
    ...
    it('should implement MCP tool fallback for critical operations', async () => {
      ...
      const screenshotSpy = jest.spyOn(
        context.mcpTools,
        'screenshot',
      ) as jest.MockedFunction<typeof context.mcpTools.screenshot>;
  ...
  describe('Cache Service Recovery', () => {
    it('should continue operations during cache service failures', async () => {
      ...
      const cacheGetSpy = jest.spyOn(
        context.cacheService,
        'get',
      ) as jest.MockedFunction<typeof context.cacheService.get>;
      const cacheSetSpy = jest.spyOn(
        context.cacheService,
        'set',
      ) as jest.MockedFunction<typeof context.cacheService.set>;
    ...
    it('should implement cache warming after service recovery', async () => {
      ...
      const cacheSetAsyncSpy = jest.spyOn(
        context.cacheService,
        'set',
      ) as jest.MockedFunction<typeof context.cacheService.set>;
  ...
  describe('System-Wide Recovery Coordination', () => {
    it('should coordinate recovery across multiple service failures', async () => {
      ...
      const nutMouseCascadeSpy = jest.spyOn(
        context.nutService,
        'mouseMoveEvent',
      ) as jest.MockedFunction<typeof context.nutService.mouseMoveEvent>;
      nutMouseCascadeSpy
        .mockRejectedValueOnce(new Error('NUT service unavailable'))
        .mockResolvedValue({ success: true });

      const cacheGetSpy2 = jest.spyOn(
        context.cacheService,
        'get',
      ) as jest.MockedFunction<typeof context.cacheService.get>;
      cacheGetSpy2
      ...
      const parlantCascadeSpy2 = jest.spyOn(
        context.parlantIntegrationService,
        'validateFunctionExecution',
      ) as jest.MockedFunction<
        typeof context.parlantIntegrationService.validateFunctionExecution
      >;
      parlantCascadeSpy2
      ...
      expect(
        recoveryEvents.filter((e) => e.includes('recovery-started')),
      ).toHaveLength(3);
      expect(
        recoveryEvents.filter((e) => e.includes('recovery-completed')),
      ).toHaveLength(3);
      recordRecoveryMetrics(scenario, startTime, endTime, {
        totalFailures: 3,
    ...
    it('should implement health check-driven automated recovery', async () => {
      ...
      const typedCheckHealthSpy = checkHealthSpy as jest.MockedFunction<
        () => Promise<{ status: string; details: Record<string, string> }>
      >;
      typedCheckHealthSpy.mockImplementation(
        (): Promise<{ status: string; details: Record<string, string> }> => {
          healthCheckAttempts++;
      ...
      const automatedRecovery = async () => {
        ...
        while (!isHealthy && attempts < maxAttempts) {
          ...
          const healthResult: {
            ...
            context.healthService.checkHealth as () => Promise<{
              status: string;
              details: Record<string, string>;
            }>
          )();
  ...
  function createFailureScenario(
    params: Partial<FailureScenario>,
  ): FailureScenario {
    return {
  ...
  function recordRecoveryMetrics(
    ...
    const metrics: RecoveryMetrics = {
      ...
      gracefulDegradationActivated:
        results.gracefulDegradationActivated ?? false,
  ...
  function createMockNutService(): Partial<NutService> {
    return {
      ...
      screendump: jest
        .fn()
        .mockResolvedValue(Buffer.from('mocked-recovery-screenshot')),
