...
class TestSession extends EventEmitter {
      ...
      this.ws.on('message', (data: WebSocket.RawData) => {
        try {
          ...
          if (message.type === ConversationalMessageType.VALIDATION_RESPONSE) {
            this.sessionState.validations.set(
              message.payload.validationId,
              message.payload,
            );
          }
        } catch (error) {
  ...
  async waitForValidationResponse(
    validationId: string,
    timeout = 5000,
  ): Promise<ValidationResponsePayload> {
    ...
    throw new Error(
      `Validation ${validationId} response timeout in session ${this.sessionId}`,
    );
  }
...
class ConcurrentSessionManager {
        ...
        session
          ...
          .then(() => {
            ...
            this.metrics.averageConnectionTime =
              this.metrics.connectionTimes.reduce(
                (sum, time) => sum + time,
                0,
  ...
  private async testSessionIsolation(sessions: TestSession[]): Promise<{
    ...
    violations: Array<{
      sessionId: string;
      violation: string;
      details: IsolationViolationDetails;
    }>;
  }> {
    ...
    for (const session of sessions) {
      if (session.isConnected()) {
        const uniqueData = `unique-data-${session.sessionId}-${Date.now()}`;
        uniqueMessages.set(session.sessionId, uniqueData);

        await session.sendMessage({
          type: ConversationalMessageType.CONNECTION_STATUS,
          payload: { uniqueData, testType: 'isolation' } as Record<
            string,
            unknown
          >,
        });
    ...
    for (const session of sessions) {
      if (session.isConnected()) {
        ...
        const hasOwnMessage = receivedMessages.some(
          (msg) => msg.payload?.uniqueData === uniqueData,
        );
        ...
        for (const [otherSessionId, otherUniqueData] of uniqueMessages) {
          if (otherSessionId !== session.sessionId) {
            const hasOtherMessage = receivedMessages.some(
              (msg) => msg.payload?.uniqueData === otherUniqueData,
            );
    ...
    for (const session of sessions) {
      if (session.isConnected()) {
        ...
        const hasSessionSpecificMessages = state.messagesReceived.every(
          (msg) =>
        ...
        if (hasSessionSpecificMessages) {
          ...
          violations.push({
            ...
            details: {
              ...
              foundMessages: state.messagesReceived.filter(
                (msg) =>
      ...
      if (session.isConnected()) {
        ...
        const validationPromise = (async () => {
          ...
          try {
            ...
            const response =
              await session.waitForValidationResponse(validationId);
...
const mockConfigService = {
  get: jest.fn((key: string, defaultValue?: unknown) => {
    const config: Record<string, unknown> = {
      ...
      WEBSOCKET_SESSION_TIMEOUT: 300000, // 5 minutes'WEBSOCKET_CLEANUP_INTERVAL': 60000, // 1 minute'WEBSOCKET_MEMORY_LIMIT_PER_SESSION': 10485760, // 10MB
    };
...
describe('Concurrent Session Management Tests', () => {
  ...
  beforeAll(async () => {
    ...
    wsServer.on('connection', (ws: WebSocket.WebSocket, req) => {
      ...
      console.log(
        `New session: ${sessionId} for user ${userId} on device ${deviceId}`,
      );
      ...
      ws.on('message', (data: WebSocket.RawData) => {
        try {
          ...
          if (message.sessionId && message.sessionId !== sessionId) {
            console.warn(
              `Session isolation violation: Message for ${
                message.sessionId
          ...
          switch (message.type) {
            ...
            default: {
              ...
              const ackMessage: ConversationalMessage = {
                ...
                payload: { acknowledgedMessageId: message.messageId } as Record<
                  string,
                  unknown
                >,
                metadata: {
        ...
        () => {
          const response: ConversationalMessage = {
            ...
            payload: {
              ...
              approved: true,
              confidence: 0.9,
              reasoning: `Validation approved for session ${
  ...
  describe('Concurrent Session Creation', () => {
    it('should create and manage 100 concurrent sessions successfully', async () => {
      ...
      const sessions = await sessionManager.createConcurrentSessions(
        sessionCount,
        'multiple',
      );
...
ms avg`);
      ...
      const averagePerformance =
        performanceResults.reduce((sum, r) => sum + r.averageTime, 0) /
        testRounds;
      const averageSuccessRate =
        performanceResults.reduce((sum, r) => sum + r.successRate, 0) /
        testRounds;
  ...
  describe('Session Isolation and Data Integrity', () => {
    it('should maintain perfect session isolation across concurrent sessions', async () => {
      ...
      const isolationResults =
        await sessionManager.testSessionIsolation(connectedSessions);

      console.log('Session Isolation Test Results:', {
        testedSessions: connectedSessions.length,
        isolationIntegrity: `${(
          isolationResults.isolationIntegrity * 100
        ).toFixed(2)}%`,
      ...
      expect(isolationResults.violations.length).toBeLessThan(
        connectedSessions.length * 0.01,
      ); // <1% violations

      if (isolationResults.violations.length > 0) {
        console.warn(
          'Session isolation violations detected:',
          isolationResults.violations,
        );
      }
    });
    ...
    it('should prevent cross-session data leakage', async () => {
      ...
      for (const session of connectedSessions) {
        ...
        for (const message of receivedMessages) {
          if (
            message.payload?.sensitiveData &&
            message.payload.sensitiveData !== ownSecretData
          ) {
            leakageDetected++;
            leakageDetails.push({
      ...
      console.log('Data Leakage Prevention Test:', {
        ...
        leakageDetected,
        leakageRate: `${(
          (leakageDetected / connectedSessions.length) *
          100
        ).toFixed(3)}%`,
    ...
    it('should handle concurrent validations without interference', async () => {
      ...
      const validationResults =
        await sessionManager.performConcurrentValidations(connectedSessions);
      ...
      expect(validationResults.completedValidations).toBeGreaterThan(
        connectedSessions.length * 0.95,
      ); // 95%+ success
  ...
  describe('Multi-user Session Management', () => {
    it('should handle multiple sessions per user correctly', async () => {
      ...
      for (let userId = 0; userId < usersCount; userId++) {
        const userSessions = await sessionManager.createConcurrentSessions(
          sessionsPerUser,
          'single',
        );
        ...
        userSessions.forEach((session) => {
          ...
          (session as TestSession & { userId: string }).userId =
            `multi-user-${userId}`;
        });
      ...
      console.log('Multi-user Session Distribution:', {
        ...
        sessionDistribution: Array.from(userSessionCounts.entries()).slice(
          0,
          5,
        ),
      });
    ...
    it('should synchronize session state across devices for same user', async () => {
      ...
      const hasSyncResponse = firstDeviceMessages.some(
  ...
  describe('Resource Management and Cleanup', () => {
    it('should manage memory usage efficiently across sessions', async () => {
      ...
      const averageMemoryPerSession =
        totalMemoryEstimate / connectedSessions.length;

      console.log('Memory Usage Analysis:', {
        connectedSessions: connectedSessions.length,
        totalMessages,
        totalMemoryEstimate: `${(totalMemoryEstimate / 1024 / 1024).toFixed(
          2,
        )} MB`,
        averageMemoryPerSession: `${(averageMemoryPerSession / 1024).toFixed(2)} KB`,
        memoryEfficiency: `${((1024 * 1024) / averageMemoryPerSession).toFixed(1)}
x target`,
      });
    ...
    it('should handle session cleanup and resource release', async () => {
      ...
      const sessionsToDisconnect = sessions.slice(
        0,
      ...
      console.log('Session Cleanup Test:', {
        ...
        cleanupEfficiency: `${(
          (remainingActiveSessions /
            (sessions.length - sessionsToDisconnect.length)) *
          100
        ).toFixed(1)}%`,
      });

      expect(remainingActiveSessions).toBeLessThanOrEqual(
        sessions.length - sessionsToDisconnect.length + 2,
      ); // Allow small variance
      expect(afterDisconnectMetrics.activeSessions).toBeLessThan(
