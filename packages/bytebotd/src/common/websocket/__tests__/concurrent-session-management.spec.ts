      ...
      ws.on('message', async (data: WebSocket.RawData) => {
        try {
          ...
          if (message.sessionId && message.sessionId !== sessionId) {
            console.warn(`Session isolation violation: Message for ${message.sessionId} received in ${sessionId}`);
          }
  ...
  describe('Concurrent Session Creation', () => {
    ...
    it('should maintain session creation performance under concurrent load', async () => {
      ...
      for (let round = 0; round < testRounds; round++) {
        ...
        console.log(`Round ${round + 1}: ${connectedCount}/${sessionsPerRound} sessions, ${averageTime.toFixed(2)}ms avg`);
