...
class SecurityTestClient extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('message', (data: WebSocket.RawData) => {
        try {
          ...
          this.recordSecurityEvent('message_received', {
            messageType: message.type,
          });
          this.emit('message', message);
        } catch (error) {
          this.recordSecurityEvent('message_parse_error', {
            error: error.message,
          });
      ...
      this.ws.on('close', (code: number, reason: Buffer) => {
        clearTimeout(timeout);
        this.connected = false;
        this.recordSecurityEvent('connection_closed', {
          code,
  ...
  sendMaliciousPayload(payload: string): void {
    ...
    this.recordSecurityEvent('malicious_payload_sent', {
...
class TokenGenerator {
  ...
  generateTamperedToken(userId: string): string {
    ...
    parts[2] = parts[2].slice(0, -5) + 'XXXXX'; // Corrupt signaturereturn parts.join('.');
  }
...
class AttackSimulator {
  ...
  async simulateXSSAttack(client: SecurityTestClient): Promise<boolean> {
    ...
    for (const payload of xssPayloads) {
      try {
        ...
        this.recordAttack('xss_injection', false, {
          payload,
          error: error.message,
        });
      }
  ...
  async simulateSQLInjectionAttack(
    ...
    for (const payload of sqlPayloads) {
      try {
        ...
        this.recordAttack('sql_injection', false, {
          payload,
          error: error.message,
        });
      }
  ...
  async simulateBufferOverflowAttack(
    client: SecurityTestClient,
  ): Promise<boolean> {
    const largePayloads = [
  ...
  async simulateRateLimitingBypass(
    client: SecurityTestClient,
    requestCount = 1000,
  ): Promise<boolean> {
...
class RateLimiter {
  ...
  isAllowed(clientId: string): boolean {
    ...
    if (
      requestsLastSecond >= this.limits.perSecond ||
  ...
  getStats(clientId: string): {
    ...
    return {
      perSecond: clientRequests.filter((timestamp) => timestamp > now - 1000)
        .length,
      perMinute: clientRequests.filter((timestamp) => timestamp > now - 60000)
        .length,
      perHour: clientRequests.filter((timestamp) => timestamp > now - 3600000)
        .length,
    };
  }
  ...
  get: jest.fn((key: string, defaultValue?: unknown) => {
    const config: Record<string, unknown> = {
      ...
      WEBSOCKET_ALLOWED_ORIGINS:
        'https://localhost:3000,https://trusted-domain.com',
    };
...
describe('WebSocket Security Validation Tests', () => {
  ...
  beforeAll(async () => {
    ...
    wsServer.on('connection', (ws: WebSocket.WebSocket, req) => {
      ...
      console.log(
        `Secure WebSocket connection: ${clientId} (User: ${userContext.sub})`,
      );
      ...
      ws.on('message', (data: WebSocket.RawData) => {
        try {
          ...
          const message = JSON.parse(
            Buffer.from(data as ArrayBuffer).toString('utf8'),
          ) as ConversationalMessage; // Input validation and sanitizationconst validationResult = validateMessageContent(message);
  ...
  describe('Authentication and Authorization', () => {
    it('should reject connections without valid authentication tokens', async () => {
      ...
      const connectionEvents = securityEvents.filter(
        (e) => e.eventType === 'connection_error',
      );
      expect(connectionEvents.length).toBeGreaterThan(0);
    });

    it('should accept connections with valid JWT tokens', async () => {
      const validToken = tokenGenerator.generateValidToken('test-user-123', [
        'read',
        'write',
      ]);
      ...
      expect(
        securityEvents.some((e) => e.eventType === 'connection_established'),
      ).toBe(true);
      expect(securityEvents.some((e) => e.eventType === 'message_sent')).toBe(
        true,
      );
    ...
    it('should reject expired JWT tokens', async () => {
      const expiredToken =
        tokenGenerator.generateExpiredToken('test-user-expired');
    ...
    it('should reject malformed or tampered JWT tokens', async () => {
      ...
      const tamperedToken =
        tokenGenerator.generateTamperedToken('test-user-tampered');
  ...
  describe('Input Validation and Injection Prevention', () => {
    it('should prevent XSS attacks through message content', async () => {
      ...
      console.log('XSS Prevention Test Results:', {
        ...
        attackAttempts: attackLog.filter(
          (a) => a.attackType === 'xss_injection',
        ).length,
    ...
    it('should prevent SQL injection attempts through validation payloads', async () => {
      ...
      const sqlBlocked =
        !(await attackSimulator.simulateSQLInjectionAttack(client));
      const attackLog = attackSimulator.getAttackLog();

      console.log('SQL Injection Prevention Test Results:', {
        sqlAttackBlocked: sqlBlocked,
        attackAttempts: attackLog.filter(
          (a) => a.attackType === 'sql_injection',
        ).length,
        connectionStillActive: client.isConnected(),
      });
    ...
    it('should enforce message size limits to prevent buffer overflow attacks', async () => {
      ...
      const bufferOverflowBlocked =
        !(await attackSimulator.simulateBufferOverflowAttack(client));
      const attackLog = attackSimulator.getAttackLog();

      console.log('Buffer Overflow Prevention Test Results:', {
        bufferOverflowBlocked: bufferOverflowBlocked,
        attackAttempts: attackLog.filter(
          (a) => a.attackType === 'buffer_overflow',
        ).length,
        connectionStillActive: client.isConnected(),
      });
  ...
  describe('Rate Limiting and DDoS Protection', () => {
    it('should enforce rate limits per connection', async () => {
      const validToken = tokenGenerator.generateValidToken(
      ...
      const rateLimitBypassed =
        await attackSimulator.simulateRateLimitingBypass(client, 100);
      const attackLog = attackSimulator.getAttackLog();

      console.log('Rate Limiting Test Results:', {
        rateLimitBypassed,
        attackLog: attackLog.filter(
          (a) => a.attackType === 'rate_limiting_bypass',
        ),
        connectionStillActive: client.isConnected(),
      });
    ...
    it('should handle multiple concurrent connections within limits', async () => {
      ...
      console.log('Concurrent Connections Test Results:', {
        ...
        connectionSuccess: `${(
          (successfulConnections / maxClients) *
          100
        ).toFixed(1)}%`,
  ...
  describe('Origin Validation and CSRF Protection', () => {
    ...
    it('should prevent CSRF attacks through origin validation', async () => {
      ...
      const csrfOrigins = [
        'https://evil-site.com',
        'http://localhost:3000', // Wrong protocol'https://fake-localhost.com','javascript:alert(1)',
      ];
      ...
      console.log('CSRF Protection Test Results:', {
        ...
        blockedOrigins,
        protectionRate: `${(
          (blockedOrigins / csrfOrigins.length) *
          100
        ).toFixed(1)}%`,
      });
  ...
  describe('Message Integrity and Confidentiality', () => {
    it('should maintain message integrity during transmission', async () => {
      ...
      console.log('Message Integrity Test Results:', {
        ...
        integrityValidated: responses.every(
          (r) => r.payload?.securityValidated,
        ),
      });
