...
class MockParlantService {
  ...
  private activeValidations = new Map<
    string,
    {
      validationId: string;
      startTime: number;
  ...
  private setupRoutes(): void {
    this.httpServer.on('request', (req, res) => {
      ...
      if (req.url === '/api/validate' && req.method === 'POST') {
        ...
        req.on('end', () => {
          try {
            const validationRequest = JSON.parse(body);
      ...
      if (req.url === '/api/conversation' && req.method === 'POST') {
        ...
        req.on('end', () => {
          try {
            const conversationRequest = JSON.parse(body);
            const response =
              this.processConversationRequest(conversationRequest);
            res.writeHead(200);
            res.end(JSON.stringify(response));
          } catch (error) {
            res.writeHead(400);
  ...
  private setupWebSocketServer(): void {
    ...
    this.wsServer.on('connection', (ws: WebSocket.WebSocket) => {
      ...
      console.log('PARLANT WebSocket client connected');
      ws.on('message', async (data: WebSocket.RawData) => {
        ...
        try {
          ...
          this.connectionMetrics.averageProcessingTime =
            this.connectionMetrics.processingTimes.reduce(
              (sum, time) => sum + time,
              0,
            ) / this.connectionMetrics.processingTimes.length;
        } catch (error) {
          console.error('PARLANT WebSocket message error:', error);
          ws.send(
            JSON.stringify({
      ...
      ws.on('close', () => {
        this.connectionMetrics.activeConnections--;
        console.log('PARLANT WebSocket client disconnected');
      });
      ws.on('error', (error) => {
        console.error('PARLANT WebSocket error:', error);
        this.connectionMetrics.activeConnections--;
      });
    });
  }
  ...
  private async handleWebSocketMessage(message: any): Promise<any> {
    switch (message.type) {
      ...
      case 'user_confirmation':
        return this.handleUserConfirmation(message);
  ...
  private handleUserConfirmation(message: any): any {
    ...
    const result = {
      ...
      timestamp: Date.now(),
      parlantDecision:
        validation.status === 'completed' ? 'processed' : 'pending',
    };
  ...
  private handleConversationMessage(message: any): any {
    ...
    return {
      ...
      conversationId,
      response: this.generateConversationalResponse(message.content),
      context: {
  ...
  private shouldApproveAction(action: any): boolean {
    ...
    const highRiskActions = ['delete', 'remove', 'destroy', 'format'];
...
class ParlantIntegrationTestClient extends EventEmitter {
  ...
  private validationTracking = new Map<
    string,
    {
      startTime: number;
      completed: boolean;
  ...
  async waitForValidationCompletion(
    ...
    throw new Error(
      `Validation ${validationId} did not complete within ${timeout}ms`,
    );
  }
  ...
  async sendUserConfirmation(
    ...
    const confirmationMessage: UserConfirmationMessage = {
      ...
      payload: {
        ...
        reasoning:
          reasoning ||
          (approved ? 'User approved action' : 'User rejected action'),
        confidence: 0.95,
      },
      metadata: {
        priority: 'high',
  ...
  getValidationMetrics() {
    ...
    return {
      ...
      averageDuration:
        completed.length > 0
          ? completed.reduce(
              (sum, v) => sum + (performance.now() - v.startTime),
              0,
            ) / completed.length
          : 0,
      totalProgressUpdates: validations.reduce(
        (sum, v) => sum + v.progressUpdates.length,
        0,
      ),
    };
  }
...
const mockConfigService = {
  get: jest.fn((key: string, defaultValue?: unknown) => {
    const config: Record<string, unknown> = {
      CONVERSATIONAL_WEBSOCKET_PORT: 8195,
      PARLANT_WEBSOCKET_PORT: 8196,
      PARLANT_SERVICE_URL: 'http://localhost:8196',
      PARLANT_WEBSOCKET_URL: 'ws://localhost:8196/ws',
      PARLANT_API_TIMEOUT: 10000,
      PARLANT_RETRY_ATTEMPTS: 3,
      PARLANT_HEALTH_CHECK_INTERVAL: 30000,
    };
...
describe('PARLANT Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    parlantBridgeService = module.get<ParlantWebSocketBridgeService>(
      ParlantWebSocketBridgeService,
    );
          ...
          switch (message.type) {
            case ConversationalMessageType.VALIDATION_REQUEST:
              await handleValidationRequest(
                ws,
                message as ValidationRequestMessage,
              );
              break;
            ...
            case ConversationalMessageType.USER_CONFIRMATION:
              await handleUserConfirmation(
                ws,
                message as UserConfirmationMessage,
              );
              break;
    ...
    async function handleValidationRequest(
      ...
      try {
        ...
        const parlantResponse = await axios.post(
          `http://localhost:${PARLANT_PORT}/api/validate`,
          {
            validationId,
            action,
        ...
        setTimeout(
          ...
          (streamingOptions?.maxUpdateCount || 3) *
            (streamingOptions?.updateInterval || 200) +
            100,
        );
      } catch (error) {
        console.error('PARLANT integration error:', error);
        const errorResponse: ConversationalMessage = {
          type: ConversationalMessageType.ERROR,
          messageId: randomUUID(),
          sessionId: request.sessionId,
          timestamp: Date.now(),
  ...
  describe('PARLANT Service Integration', () => {
    it('should detect PARLANT service availability', async () => {
      try {
        const healthResponse = await axios.get(
          `http://localhost:${PARLANT_PORT}/health`,
          {
            timeout: 5000,
          },
        );
        ...
        expect(healthResponse.data.capabilities).toContain('validation');
        console.log('PARLANT Service Health Check:', {
    ...
    it('should establish WebSocket connection with PARLANT service', async () => {
      const parlantWs = new WebSocket.WebSocket(
        `ws://localhost:${PARLANT_PORT}/ws`,
      );
    ...
    it('should handle PARLANT service communication errors gracefully', async () => {
      ...
      try {
        await axios.post(
          'http://localhost:99999/api/validate',
          {
            validationId: 'test',
  ...
  describe('Real-time Validation Workflows', () => {
    ...
    it('should handle progressive validation streaming with PARLANT', async () => {
      ...
      expect(
        result.progressUpdates[result.progressUpdates.length - 1].payload
          .progress,
      ).toBe(100);
    ...
    it('should correlate validation responses with original requests', async () => {
      ...
      const actions: ValidationAction[] = [
        ...
        },
        {
          ...
          expectedOutcome: 'Query executed successfully',
          reversible: true,
          impact: {
            scope: 'local',
            dataAccess: true,
        ...
        },
        {
          ...
          expectedOutcome: 'API data retrieved',
          reversible: true,
          impact: {
            scope: 'external',
            dataAccess: true,
      ...
      results.forEach((result, index) => {
        expect(result.result.response.payload.validationId).toBe(
          result.validationId,
        );
  ...
  describe('User Interaction Event Handling', () => {
    it('should handle user confirmation events with PARLANT', async () => {
      ...
      const validationId =
        await client.startValidationWorkflow(confirmationAction);
      const validationResult =
        await client.waitForValidationCompletion(validationId);
      ...
      ); // Wait for confirmation resultawait new Promise(resolve => setTimeout(resolve, 1000));

      console.log('User Confirmation Results:', {
        validationId,
        initialApproval: validationResult.response.payload.approved,
        userConfirmationSent: true,
        parlantProcessingTime:
          validationResult.response.payload.metadata?.parlantProcessingTime,
        integrationLatency:
          validationResult.response.payload.metadata?.integrationLatency,
      });

      expect(validationResult.response.payload.approved).toBeDefined();
      expect(
        validationResult.response.payload.metadata?.parlantService,
      ).toBeTruthy();
    ...
    it('should handle rejection scenarios appropriately', async () => {
      ...
      const validationId =
        await client.startValidationWorkflow(dangerousAction);
      const validationResult =
        await client.waitForValidationCompletion(validationId);
      ...
      await client.sendUserConfirmation(
        validationId,
        false,
        'Action too dangerous - user rejected',
      );
      console.log('Rejection Handling Results:', {
        validationId,
  ...
  describe('Performance and Reliability', () => {
    it('should maintain sub-1000ms validation response times', async () => {
      ...
      const performanceActions: ValidationAction[] = Array.from(
        { length: 10 },
        (_, i) => ({
          actionType: `performance_test_${i}`,
      ...
      const maxDuration = Math.max(
        ...performanceResults.map((r) => r.duration),
      );
      const successRate =
        performanceResults.filter((r) => r.approved !== undefined).length /
        performanceResults.length;
    ...
    it('should handle concurrent validation requests efficiently', async () => {
      ...
      const validationPromises = clients.map(async (client, index) => {
        const validationId =
          await client.startValidationWorkflow(concurrentAction);
