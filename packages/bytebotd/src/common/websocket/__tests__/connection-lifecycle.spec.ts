  ...
  private scheduleReconnection(): void {
    ...
    setTimeout(async () => {
      try {
        ...
        this.emit('reconnection-failed', { attempt: this.reconnectionAttempts, error });
      }
    }, delay);
  }
...
describe('WebSocket Connection Lifecycle Tests', () => {
  ...
  describe('Connection Pool Management', () => {
    ...
    it('should track connection pool performance metrics', async () => {
      ...
      console.log('Connection Pool Performance:', {
        poolSize,
        totalSetupTime: `${totalSetupTime.toFixed(2)}ms`,
  ...
  describe('Session Correlation and Tracking', () => {
    ...
    it('should track connection lifecycle events per session', async () => {
      ...
      expect(sessionEvents[2].event).toBe('disconnected');

      // Verify event timing
      expect(sessionEvents[1].timestamp).toBeGreaterThan(sessionEvents[0].timestamp);
  ...
  describe('Performance and Reliability', () => {
    it('should maintain sub-100ms connection establishment target', async () => {
      ...
      console.log('Connection Performance Metrics:', {
        ...
        p95ConnectionTime: `${p95ConnectionTime.toFixed(2)}ms`,
        target: '100ms',
        fastestConnection: `${Math.min(...connectionTimes).toFixed(2)}ms`,
        slowestConnection: `${Math.max(...connectionTimes).toFixed(2)}ms`,
      });
