...
class ConnectionLifecycleTestClient extends EventEmitter {
      ...
      try {
        ...
        const connectionTimeout = setTimeout(() => {
          if (this.connectionState === 'connecting') {
            ...
            this.connectionMetrics.lastConnectionError = error;
  ...
  private scheduleReconnection(): void {
    ...
    this.connectionMetrics.reconnectionCount++;

    const delay =
      this.reconnectionDelay * Math.pow(2, this.reconnectionAttempts - 1); // Exponential backoff
  ...
  isConnected(): boolean {
    return (
      this.connectionState === 'connected' &&
      this.ws?.readyState === WebSocket.WebSocket.OPEN
    );
  }
}
...
class ConnectionPoolTester {
  ...
  async createConnectionPool(poolSize: number, url: string): Promise<void> {
    ...
    for (let i = 0; i < poolSize; i++) {
      ...
      client.on('connected', (metrics: ConnectionMetrics) => {
        ...
        if (metrics.connectionTime) {
          ...
          this.poolMetrics.averageConnectionTime =
            this.poolMetrics.connectionTimes.reduce(
              (sum, time) => sum + time,
              0,
      ...
      connectionPromises.push(
        client.connect().catch((): Promise<void> => {
  ...
  getPoolMetrics() {
    return {
      ...this.poolMetrics,
      successRate:
        this.poolMetrics.totalConnections > 0
          ? (this.poolMetrics.totalConnections -
              this.poolMetrics.failedConnections) /
            this.poolMetrics.totalConnections
          : 0,
...
describe('WebSocket Connection Lifecycle Tests', () => {
  ...
  beforeAll(async () => {
    ...
    wsServer.on('connection', (ws: WebSocket.WebSocket, req) => {
      console.log(
        `New WebSocket connection from ${req.connection.remoteAddress}`,
      );
  ...
  describe('Connection State Management', () => {
    ...
    it('should maintain connection metrics accurately', async () => {
      ...
      expect(metricsAfterDisconnect.disconnectionTime).toBeGreaterThan(
        metricsAfterConnect.connectionTime!,
      );
    });
  });
  ...
  describe('Reconnection and Failover', () => {
    it('should handle automatic reconnection after connection loss', async () => {
      ...
      client.on('connected', () => {
        if (reconnectionEvents > 0) {
          // This is a reconnection
          reconnectionEvents++;
        }
      });

      client.on('reconnection-failed', () => {
...
ms`,
  ...
  describe('Session Correlation and Tracking', () => {
    it('should correlate WebSocket connections with sessions', async () => {
      ...
      if (receivedMessage?.payload) {
        ...
        if (
          ...
          const originalMessageValue = (payload as Record<string, unknown>)
            .originalMessage;
          if (
            originalMessageValue &&
            typeof originalMessageValue === 'object'
          ) {
            const originalMessage = originalMessageValue as ParsedMessage;
    ...
    it('should track connection lifecycle events per session', async () => {
      ...
      client.on('connected', () => trackEvent(sessionId, 'connected'));
      client.on('disconnected', () => trackEvent(sessionId, 'disconnected'));
      trackEvent(sessionId, 'connection-attempt');
      await client.connect();
      await client.disconnect();
      ...
      expect(sessionEvents[2].timestamp).toBeGreaterThan(
        sessionEvents[1].timestamp,
      );
    });
  });
    ...
    it('should maintain sub-100ms connection establishment target', async () => {
      ...
      const p95ConnectionTime =
        connectionTimes.sort((a, b) => a - b)[
          Math.floor(connectionTimes.length * 0.95)
        ] ?? 0;
...
ms`,
        p95ConnectionTime: `${p95ConnectionTime.toFixed(2)}
ms`,
        target: '100ms',
        fastestConnection: `${Math.min(...connectionTimes).toFixed(2)}
ms`,
        slowestConnection: `${Math.max(...connectionTimes).toFixed(2)}
ms`,
      });
