...
class LoadTestingWebSocketClient extends EventEmitter {
  ...
  getStats(): ConnectionStats {
    return {
      ...
      connectionDuration: this.isConnected
...
class LoadTestMetrics {
  ...
  getAggregatedMetrics(): AggregatedMetrics {
    ...
    const activeConnections = connectionMetrics.filter(
      (c) => c.isActive,
    ).length;
    const totalMessages = connectionMetrics.reduce(
      (sum, c) => sum + c.messagesSent,
      0,
    );
    const totalBytes = connectionMetrics.reduce(
    ...
    return {
      ...
      averageMessagesPerConnection:
        connectionMetrics.length > 0
          ? totalMessages / connectionMetrics.length
          : 0,
...
class ProgressiveLoadGenerator {
  ...
  async executeLoadTest(config: LoadTestConfig): Promise<LoadTestResults> {
    ...
    try {
      ...
      await this.executePhase(
        'baseline',
        config.baselineConnections,
        config.baselineRPS,
        60000,
      );
      results.phases.push(this.capturePhaseResults('baseline')); // Phase 2: Ramp Upawait this.executeRampUp(config.baselineConnections, config.targetConnections, config.rampUpDuration);
  ...
  private async executeRampUp(
    ...
    const connectionIncrement = Math.max(
      1,
      Math.floor((endConnections - startConnections) / (duration / 5000)),
    );
    const intervalDuration = Math.floor(
      duration /
        Math.ceil((endConnections - startConnections) / connectionIncrement),
    );

    let currentConnections = startConnections;
    while (currentConnections < endConnections && this.isRunning) {
      const nextTarget = Math.min(
        currentConnections + connectionIncrement,
        endConnections,
      );
  ...
  private async generateLoad(targetRPS: number): Promise<void> {
    const activeClients = Array.from(this.clients.values()).filter(
      (client) => client.getStats().isConnected,
    );
    if (activeClients.length === 0) return;

    const messagesPerClient = Math.max(
      1,
      Math.floor(targetRPS / activeClients.length),
    );
    const promises: Promise<void>[] = [];
...
class MemoryLeakDetector {
  ...
  private isConsistentGrowth(): boolean {
    ...
    for (let i = 1; i < this.memorySnapshots.length; i++) {
      if (
        this.memorySnapshots[i].heapUsed > this.memorySnapshots[i - 1].heapUsed
      ) {
        growthCount++;
      }
    }
  ...
  private generateMemoryAnalysis(
    ...
    if (hasLeak) {
      return `Potential memory leak detected: ${growthPercentage.toFixed(
        2,
      )}% heap growth with consistent increase pattern`;
  ...
  async testConnectionPoolExhaustion(): Promise<ExhaustionTestResult> {
    ...
    try {
      ...
      for (let i = 0; i < 50000; i++) {
        const client = new LoadTestingWebSocketClient(
          `exhaust_${i}`,
          this.metrics,
        );
...
describe('WebSocket Load Testing and Stress Testing', () => {
  ...
  describe('Resource Exhaustion Testing', () => {
    ...
    test('should perform sustained load test with memory monitoring', async () => {
      ...
      expect(safeToNumber(safeGet(typedResult, 'sustainedDuration', 0))).toBe(
        sustainedDuration,
      );
    }, 180000);
  });
    ...
    test('should detect consistent memory growth patterns', async () => {
      ...
      if (analysis.hasMemoryLeak) {
        ...
        expect(analysis.recommendations).toContain(
          'Investigate connection cleanup procedures',
        );
      }
  ...
  describe('Performance Under Load', () => {
    ...
    test('should handle concurrent message bursts', async () => {
      ...
      const totalSent = clients.reduce(
        (sum, client) => sum + client.getStats().messagesSent,
        0,
      );
  ...
  describe('System Recovery Testing', () => {
    test('should recover from overload conditions', async () => {
      ...
      expect(
        finalMetrics!.totalErrors / finalMetrics!.totalConnections,
      ).toBeLessThan(0.1); // <10% error rate
    }, 120000);
  ...
  describe('Resource Cleanup Validation', () => {
    ...
    test('should validate proper connection cleanup', async () => {
      ...
      const finalActiveCount = clients.filter(
        (c) => c.getStats().isConnected,
      ).length;
