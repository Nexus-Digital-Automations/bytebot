...
import { MetricsCollection, safeGet, safeToNumber } from '../websocket-types';
import {
...
class TestMessageFactory {
  ...
  static createTestMessage(
    ...
    const actualSessionId =
      sessionId ?? `test_session_${this.sessionCounter++}`;
...
describe('MessageOrderingDeliveryValidationService', () => {
  ...
  describe('Message Sequence Validation', () => {
    it('should validate correct message sequence ordering', () => {
      ...
      expect(
        results.every((result) => result.validationErrors.length === 0),
      ).toBe(true);
    });
    ...
    it('should detect out-of-order messages', () => {
      ...
      expect(
        results[1].validationErrors.some(
          (err) => err.errorCode === 'SEQUENCE_OUT_OF_ORDER',
        ),
      ).toBe(true); // Third message should be invalid (out of order)expect(results[2].sequenceValid).toBe(false);
    });
    ...
    it('should detect duplicate messages', async () => {
      ...
      expect(
        secondResult.validationErrors.some(
          (err) => err.errorCode === 'DUPLICATE_MESSAGE',
        ),
      ).toBe(true);
    });
  ...
  describe('Delivery Acknowledgment and Confirmation', () => {
    ...
    it('should track acknowledgment performance metrics', async () => {
      ...
      for (let i = 1; i <= messageCount; i++) {
        ...
        const ack = service.processDeliveryAcknowledgment(
          message.messageId,
          sessionId,
          latency,
        );
  ...
  describe('Message Deduplication and Duplicate Detection', () => {
    ...
    it('should track duplicate detection metrics', async () => {
      ...
      expect(finalMetrics.duplicatesDetected).toBe(
        initialDuplicates + duplicateCount,
      );
    });
    ...
    it('should handle high-volume duplicate detection efficiently', async () => {
      ...
      const originalMessages = Array.from(
        { length: Math.floor(messageCount * (1 - duplicateRatio)) },
        (_, i) =>
          TestMessageFactory.createTestMessage(
      ...
      for (let i = 0; i < duplicateCount; i++) {
        const randomOriginal =
          originalMessages[Math.floor(Math.random() * originalMessages.length)];
  ...
  describe('Priority Queue Management and Processing', () => {
    it('should process messages according to priority order', async () => {
      ...
      const criticalIndex = processedMessages.findIndex((msg) =>
        msg.includes(':0'),
      ); // MessagePriority.CRITICAL = 0const lowIndex = processedMessages.findIndex(msg => msg.includes(':3')); // MessagePriority.LOW = 3expect(criticalIndex).toBeLessThan(lowIndex);});
      ...
      expect(
        droppedMessages.some((msg) => msg.includes('buffer_overflow')),
      ).toBe(true);
    });
    ...
    it('should maintain message ordering within same priority level', async () => {
      ...
      for (let i = 1; i < queuedMessages.length; i++) {
        expect(queuedMessages[i].timestamp).toBeGreaterThanOrEqual(
          queuedMessages[i - 1].timestamp,
        );
      }
    });
  });
  ...
  describe('Timeout and Retry Mechanism Testing', () => {
    ...
    it('should move messages to dead letter queue after max retries', async () => {
      ...
      expect(metrics.failedDeliveries).toBeGreaterThanOrEqual(
    ...
    it('should implement exponential backoff for retries', async () => {
      ...
      if (retryDelays.length > 1) {
        ...
        expect(sortedDelays[sortedDelays.length - 1]).toBeGreaterThan(
          sortedDelays[0],
        );
      }
    });
  ...
  describe('Conversational Flow Ordering Validation with PARLANT Integration', () => {
    it('should validate complete PARLANT validation workflow', async () => {
      ...
      const userConfirmation = TestMessageFactory.createUserConfirmationMessage(
        sessionId,
        validationId,
        true,
        3,
      ...
      (validationResponse as { messageId: string }).messageId =
        `validation_response_${validationId}`;
      (confirmationResult as { messageId: string }).messageId =
        `confirmation_result_${validationId}`;
      ...
      // Validate the flow
      const flowValidation = service.validateParlantIntegrationFlow(
        sessionId,
        validationId,
      );
    ...
    it('should validate progress update ordering in streaming flow', async () => {
      ...
      const progressUpdates = [0, 25, 50, 75, 100].map((progress, index) =>
        TestMessageFactory.createProgressUpdateMessage(
          sessionId,
          operationId,
          progress,
          index + 1,
    ...
    it('should handle concurrent PARLANT validation flows', async () => {
      ...
      for (let i = 1; i <= validationCount; i++) {
        ...
        const validationRequest =
          TestMessageFactory.createValidationRequestMessage(
            sessionId,
            validationId,
            i * 10,
          );
        const userConfirmation =
          TestMessageFactory.createUserConfirmationMessage(
            sessionId,
            validationId,
            true,
            i * 10 + 1,
      ...
      expect(flowValidations.every((flow) => flow.integrityScore > 0.5)).toBe(
        true,
      );
    });
  });
  ...
  describe('Message Integrity and Checksum Validation', () => {
    ...
    it('should detect message integrity violations', async () => {
      ...
      const verified = service.verifyMessageIntegrity(
        originalMessage,
        'invalid_checksum',
      );
    ...
    it('should handle high-volume checksum validation efficiently', async () => {
      ...
      const messageCount = 1000;
      const { throughput, averageLatency } =
        await PerformanceTestUtils.measureThroughput(
          async () => {
            const message = TestMessageFactory.createTestMessage(
              ConversationalMessageType.HEARTBEAT,
              sessionId,
  ...
  describe('Performance Metrics Collection and Validation', () => {
    ...
    it('should track latency percentiles accurately', async () => {
      ...
      for (let i = 1; i <= messageCount; i++) {
        ...
        service.processDeliveryAcknowledgment(
          message.messageId,
          sessionId,
          latency,
        );
  ...
  describe('Integration and End-to-End Validation', () => {
    it('should handle complete end-to-end message lifecycle', async () => {
      ...
      (messages[1] as { messageId: string }).messageId =
        `validation_response_${validationId}`;
      (messages[3] as { messageId: string }).messageId =
        `confirmation_result_${validationId}`;
      ...
      expect(
        validationResults.every((result) => result.deliveryGuaranteed),
      ).toBe(true);
      ...
      const metrics = service.getPerformanceMetrics();
      expect(metrics.successfulDeliveries).toBeGreaterThanOrEqual(
        messages.length,
      );
    });
    ...
    it('should maintain consistency across concurrent operations', async () => {
      ...
      const operationPromises = Array.from(
        ...
        async (_, operationIndex) => {
          ...
          const operationMessages = Array.from(
            { length: messagesPerOperation },
            (_, msgIndex) =>
              TestMessageFactory.createTestMessage(
                ConversationalMessageType.PROGRESS_UPDATE,
                sessionId,
      ...
      expect(
        finalReport.performanceMetrics.totalMessages,
      ).toBeGreaterThanOrEqual(concurrentOperations * messagesPerOperation);
    });
  });
});
