...
class SystemResourceMonitor {
  ...
  getResourceAnalysis(): {
    ...
    const duration =
      this.resourceSnapshots[this.resourceSnapshots.length - 1].timestamp -
      this.resourceSnapshots[0].timestamp;
    ...
    const memoryUsages = this.resourceSnapshots.map(
      (snapshot) => snapshot.memoryUsage.heapUsed / 1024 / 1024,
    );

    // Calculate system metrics
    const systemLoads = this.resourceSnapshots.map(
      (snapshot) => snapshot.systemLoad[0],
    );
    const memoryUtilizations = this.resourceSnapshots.map(
      (snapshot) =>
        ((snapshot.totalMemory - snapshot.freeMemory) / snapshot.totalMemory) *
        100,
    );
    ...
    return {
      ...
      cpu: {
        ...
        total:
          (this.resourceSnapshots[this.resourceSnapshots.length - 1].cpuUsage
            .user +
            this.resourceSnapshots[this.resourceSnapshots.length - 1].cpuUsage
              .system -
            this.initialCpuUsage.user -
            this.initialCpuUsage.system) /
      ...
      system: {
        averageLoad:
          systemLoads.reduce((sum, load) => sum + load, 0) / systemLoads.length,
        memoryUtilization:
          memoryUtilizations.reduce((sum, util) => sum + util, 0) /
...
class ThroughputBenchmark {
  ...
  startBenchmark(): void {
    ...
    this.snapshotInterval = setInterval(() => {
      ...
      const instantThroughput =
        elapsed > 0 ? (this.receivedMessages / elapsed) * 1000 : 0;
  ...
  getResults(): {
    ...
    const messagesPerSecond =
      duration > 0 ? (this.receivedMessages / duration) * 1000 : 0;
    ...
    const throughputValues = this.throughputSnapshots.map(
      (s) => s.instantThroughput,
    );
    const avgThroughput =
      throughputValues.reduce((sum, val) => sum + val, 0) /
    ...
    const throughputStability =
      avgThroughput > 0 ? 1 - throughputStdDev / avgThroughput : 0;
    ...
    return {
      ...
      averageMessageSize:
        this.messageSizes.length > 0
          ? this.messageSizes.reduce((sum, size) => sum + size, 0) /
            this.messageSizes.length
          : 0,
      totalDataTransferred: this.messageSizes.reduce(
        (sum, size) => sum + size,
        0,
      ),
      deliveryRate:
        this.sentMessages > 0 ? this.receivedMessages / this.sentMessages : 0,
      throughputStability,
    };
  }
...
class ConnectionScalabilityTester {
  ...
  async createConcurrentConnections(
    ...
    for (let i = 0; i < count; i += batchSize) {
      ...
      for (let j = i; j < batchEnd; j++) {
        ...
        const connectionPromise = this.createSingleConnection(
          connectionId,
          url,
          connectionTimeout,
        );
      ...
      const currentConcurrent = Array.from(this.connections.values()).filter(
        (conn) => conn.connected,
      ).length;
      this.connectionMetrics.concurrentPeak = Math.max(
        this.connectionMetrics.concurrentPeak,
        currentConcurrent,
      );
    ...
    this.connectionMetrics.connectionRate =
      totalTime > 0
        ? (this.connectionMetrics.successfulConnections / totalTime) * 1000
        : 0;

    // Calculate average connection time
    if (this.connectionMetrics.connectionTimes.length > 0) {
      this.connectionMetrics.averageConnectionTime =
        this.connectionMetrics.connectionTimes.reduce(
          (sum, time) => sum + time,
          0,
  ...
  async testConnectionLoadDistribution(messageCount: number): Promise<{
    ...
    if (activeConnections.length === 0) {
      throw new Error(
        'No active connections available for load distribution test',
      );
    }
    const messagesPerConnection = Math.floor(
      messageCount / activeConnections.length,
    );
    const sendPromises: Promise<void>[] = [];

    // Send messages to each connection
    for (const connection of activeConnections) {
      const sendPromise = this.sendMessagesToConnection(
        connection,
    ...
    const variance =
      messageCounts.reduce(
        (sum, count) => sum + Math.pow(count - averageMessages, 2),
        0,
      ) / messageCounts.length;
  ...
  getScalabilityMetrics() {
    ...
    return {
      ...
      successRate:
        this.connectionMetrics.totalAttempts > 0
          ? this.connectionMetrics.successfulConnections /
            this.connectionMetrics.totalAttempts
          : 0,
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('message', (data: WebSocket.RawData) => {
        ...
        try {
          ...
          ) as ConversationalMessage; // Calculate latency if we sent this messageconst sentTime = this.sentMessageTimestamps.get(message.messageId);
          if (sentTime) {
          ...
          this.emit('message', { message, receiveTime });
        } catch (error) {
...
_${Date.now()}`,
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, Math.max(5000, messageCount * 2)),
    );
...
const mockConfigService = {
  get: jest.fn((key: string, defaultValue?: unknown) => {
    const config: Record<string, unknown> = {
      CONVERSATIONAL_WEBSOCKET_PORT: 8189,
      PARLANT_WEBSOCKET_PORT: 8190,
      WEBSOCKET_PERFORMANCE_MONITORING: true,
      WEBSOCKET_COMPRESSION_LEVEL: 6,
      WEBSOCKET_MAX_PAYLOAD_SIZE: 1048576, // 1MB'WEBSOCKET_QUEUE_SIZE': 10000,
    };
    ...
    wsServer.on('connection', (ws: WebSocket.WebSocket, req) => {
      ...
      ws.on('message', (data: WebSocket.RawData) => {
        try {
          ...
          connectionInfo.lastActivity = performance.now();
...
ms`,
        p95Latency: `${results.p95Latency.toFixed(2)}
ms`,
        deliveryRate: `${(results.deliveryRate * 100).toFixed(2)}%`,
        throughputStability: `${(results.throughputStability * 100).toFixed(1)}%`,
        target: '5000 messages/second',
      });
      console.log('Resource Utilization:', {
        cpuAverage: `${resourceAnalysis.cpu.average.toFixed(1)}%`,
        cpuPeak: `${resourceAnalysis.cpu.peak.toFixed(1)}%`,
        memoryAverage: `${resourceAnalysis.memory.average.toFixed(1)} MB`,
        memoryPeak: `${resourceAnalysis.memory.peak.toFixed(1)} MB`,
    ...
    it('should maintain sub-50ms P95 latency under load', async () => {
      ...
      console.log('Latency Benchmark Results:', {
        ...
        averageLatency: `${results.averageLatency.toFixed(2)}
ms`,
        p50Latency: `${results.p50Latency.toFixed(2)}
ms`,
        p95Latency: `${results.p95Latency.toFixed(2)}
ms`,
        p99Latency: `${results.p99Latency.toFixed(2)}
ms`,
        minLatency: `${results.minLatency.toFixed(2)}
ms`,
        maxLatency: `${results.maxLatency.toFixed(2)}
ms`,
        target: 'P95 < 50ms',
      });
    ...
    it('should handle various message sizes efficiently', async () => {
      ...
      resultsPerSize.forEach(({ size, results }) => {
        console.log(
          `  ${size} bytes: ${Math.floor(results.messagesPerSecond)} msg/s, ` +
            `${results.averageLatency.toFixed(1)}
ms avg, ` +
            `${results.resourceUsage.cpu.toFixed(1)}% CPU`,
        );
      });

      // Verify performance degrades gracefully with size
      const latencies = resultsPerSize.map((r) => r.results.averageLatency);
      const throughputs = resultsPerSize.map(
        (r) => r.results.messagesPerSecond,
      );
  ...
  describe('Connection Scalability Testing', () => {
    it('should support 1000+ concurrent connections', async () => {
      ...
      await scalabilityTester.createConcurrentConnections(
        targetConnections,
        TEST_URL,
        {
          batchSize: 50,
...
ms`,
        connectionRate: `${scalabilityMetrics.connectionRate.toFixed(1)} conn/s`,
      ...
      console.log('Resource Usage for Scalability:', {
        cpuAverage: `${resourceAnalysis.cpu.average.toFixed(1)}%`,
        memoryAverage: `${resourceAnalysis.memory.average.toFixed(1)} MB`,
        memoryPerConnection: `${(resourceAnalysis.memory.average / scalabilityMetrics.currentActiveConnections).toFixed(2)} MB/conn`,
      });

      expect(scalabilityMetrics.successfulConnections).toBeGreaterThan(
        targetConnections * 0.8,
      ); // 80%+ success
      expect(scalabilityMetrics.averageConnectionTime).toBeLessThan(500); // Sub-500ms average
      expect(
        resourceAnalysis.memory.average /
          scalabilityMetrics.currentActiveConnections,
      ).toBeLessThan(2); // <2MB per connection
    ...
    it('should distribute load evenly across connections', async () => {
      ...
      const messageCount = 5000;
      const loadResults =
        await scalabilityTester.testConnectionLoadDistribution(messageCount);
      ...
      console.log('Load Distribution Results:', {
        ...
        averageMessagesPerConnection:
          loadResults.averageMessagesPerConnection.toFixed(1),
        distributionVariance:
          loadResults.messageDistributionVariance.toFixed(2),
        distributionEfficiency: `${(
          100 -
          (loadResults.messageDistributionVariance /
            loadResults.averageMessagesPerConnection) *
            100
        ).toFixed(1)}%`,
      });
      ...
      expect(loadResults.activeConnections).toBeGreaterThan(
        connectionCount * 0.9,
      ); // 90%+ active
      expect(loadResults.totalMessages).toBeGreaterThan(messageCount * 0.9); // 90%+ delivered
      expect(loadResults.messageDistributionVariance).toBeLessThan(
        loadResults.averageMessagesPerConnection * 0.2,
      ); // <20% variance
  ...
  describe('Resource Utilization Analysis', () => {
    it('should maintain CPU usage under 70% at maximum load', async () => {
      ...
      const totalMessages = results.reduce(
        (sum, result) => sum + result.deliveryRate * 2000,
        0,
      );
      const averageThroughput = results.reduce(
        (sum, result) => sum + result.messagesPerSecond,
        0,
      );
      const averageLatency =
        results.reduce((sum, result) => sum + result.averageLatency, 0) /
        results.length;
...
ms`,
        cpuUsage: {
          average: `${resourceAnalysis.cpu.average.toFixed(1)}%`,
          peak: `${resourceAnalysis.cpu.peak.toFixed(1)}%`,
          target: '<70%',
        },
        memoryUsage: {
          average: `${resourceAnalysis.memory.average.toFixed(1)} MB`,
          peak: `${resourceAnalysis.memory.peak.toFixed(1)} MB`,
        },
        systemMetrics: {
          load: resourceAnalysis.system.averageLoad.toFixed(2),
          memoryUtilization: `${resourceAnalysis.system.memoryUtilization.toFixed(
            1,
          )}%`,
        },
      });
    ...
    it('should demonstrate memory efficiency per connection', async () => {
      ...
      for (const batchSize of connectionBatches) {
        ...
        const memoryPerConnection =
          currentMemory / scalabilityMetrics.currentActiveConnections;
        ...
        console.log(
          `Memory at ${scalabilityMetrics.currentActiveConnections} connections: ` +
  ...
  MB(${memoryPerConnection.toFixed(3)} MB/conn)`,
      ...
      const memoryGrowthRate =
        (memoryResults[memoryResults.length - 1].memoryUsage -
          memoryResults[0].memoryUsage) /
        (memoryResults[memoryResults.length - 1].connections -
          memoryResults[0].connections);

      console.log('Memory Efficiency Analysis:', {
        finalConnections: finalResult.connections,
        finalMemoryUsage: `${finalResult.memoryUsage.toFixed(1)} MB`,
        memoryPerConnection: `${finalResult.memoryPerConnection.toFixed(3)} MB`,
        memoryGrowthRate: `${memoryGrowthRate.toFixed(3)} MB/conn`,
  ...
  describe('Compression Performance Evaluation', () => {
    it('should evaluate compression efficiency vs performance trade-offs', async () => {
      ...
      const largeMessage = JSON.stringify({
        data: 'This is a test message that contains repeated data. '.repeat(
          100,
        ), // ~5KB of repeated text
