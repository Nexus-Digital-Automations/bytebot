...
import { MessageOrderingDeliveryValidationService } from '../message-ordering-delivery-validation.service';
import {
...
class AdvancedPerformanceAnalyzer {
  ...
  private readonly resourceUsage: Array<{
    timestamp: number;
    memory: number;
    cpu: number;
  }> = [];
  ...
  private analyzePerformance(
    ...
    const variance =
      measurements.reduce((acc, val) => acc + Math.pow(val - average, 2), 0) /
      measurements.length;
...
class LoadSimulator {
  ...
  async simulateConcurrentLoad(
    service: MessageOrderingDeliveryValidationService,
    config: {
    ...
    return {
      ...
      totalMessages: sessionResults.reduce(
        (sum, result) => sum + result.messagesProcessed,
        0,
      ),
      duration: endTime - startTime,
      sessionResults,
      averageLatency:
        sessionResults.reduce((sum, result) => sum + result.averageLatency, 0) /
      ...
      errorRate:
        sessionResults.reduce((sum, result) => sum + result.errors, 0) /
        sessionResults.reduce(
          (sum, result) => sum + result.messagesProcessed,
          0,
        ),
  ...
  private async simulateSession(
    ...
    while (
      Date.now() < endTime &&
      messages.length < config.messagesPerSession &&
      this.isRunning
    ) {
      try {
      ...
      if (config.messageInterval > 0) {
        await new Promise((resolve) =>
          setTimeout(resolve, config.messageInterval),
        );
      }
    }
...
ms`,
        p95Latency: `${result.p95Latency.toFixed(3)}
ms`,
        memoryUsage: `${result.memoryUsage.toFixed(2)}
MB`,
      });
      ...
      expect(result.memoryUsage / messageCount).toBeLessThan(
        PERFORMANCE_TARGETS.maxMemoryPerMessage,
      );
    });
    ...
    it('should maintain throughput under high priority message load', () => {
      ...
      for (let i = 1; i <= messageCount; i++) {
        ...
        const message = createBenchmarkMessage(
          'priority_session',
          i,
          'critical',
        );
...
ms`,
      ...
      expect(result.throughput).toBeGreaterThan(
        PERFORMANCE_TARGETS.minThroughput * 0.8,
      );
      expect(result.p95Latency).toBeLessThan(
        PERFORMANCE_TARGETS.maxP95Latency * 0.5,
      );
    });
    ...
    it('should handle burst traffic efficiently', async () => {
      ...
      expect(result.throughput).toBeGreaterThan(
        PERFORMANCE_TARGETS.minThroughput * 0.6,
      );
    });
  });
  ...
  describe('Latency Performance Benchmarks', () => {
    it('should maintain low latency under steady load', async () => {
      ...
      console.log('Steady Load Latency Results:', {
        ...
        averageLatency: `${result.averageLatency.toFixed(3)}
ms`,
        medianLatency: `${result.medianLatency.toFixed(3)}
ms`,
        p95Latency: `${result.p95Latency.toFixed(3)}
ms`,
        p99Latency: `${result.p99Latency.toFixed(3)}
ms`,
...
ms`,
      ...
      expect(result.p95Latency).toBeLessThan(PERFORMANCE_TARGETS.maxP95Latency);
      expect(result.p99Latency).toBeLessThan(
        PERFORMANCE_TARGETS.maxP95Latency * 2,
      );
    ...
    it('should handle latency spikes gracefully', async () => {
      ...
      expect(result.maxLatency).toBeLessThan(
        PERFORMANCE_TARGETS.maxP95Latency * 5,
      );
    });
  });
  ...
  describe('Concurrent Session Scalability Benchmarks', () => {
    it('should handle target concurrent sessions efficiently', async () => {
      ...
      const result = await loadSimulator.simulateConcurrentLoad(
        service,
        config,
      );
...
ms`,
    ...
    it('should scale linearly with session count', async () => {
      ...
      expect(scalabilityResults[2].throughput).toBeGreaterThan(
        scalabilityResults[1].throughput * 1.5,
      );

      // Latency should not degrade significantly
      const maxLatencyIncrease = 2.0;
      expect(scalabilityResults[2].latency).toBeLessThan(
        scalabilityResults[0].latency * maxLatencyIncrease,
      );
    });
  });
...
MB`,
        memoryPerMessage: `${(memoryPerMessage * 1024).toFixed(3)}
KB`,
      });

      expect(memoryPerMessage).toBeLessThan(
        PERFORMANCE_TARGETS.maxMemoryPerMessage,
      );
    });
...
ms`,
      ...
      expect(deliverySuccessRate).toBeGreaterThan(
        PERFORMANCE_TARGETS.minDeliverySuccessRate,
      );
      expect(result.averageLatency).toBeLessThan(
        PERFORMANCE_TARGETS.maxP95Latency,
      );
  ...
  describe('Stress and Endurance Testing', () => {
    it('should handle extreme load without degradation', async () => {
      ...
      console.log('Extreme Load Results:', {
        ...
        throughput: `${result.throughput.toFixed(0)} msg/sec`,
        errorRate: `${(result.errorRate * 100).toFixed(3)}%`,
        memoryUsage: `${analysisResult.memoryUsage.toFixed(2)}
MB`,
      });

      // Should maintain reasonable performance even under extreme load
      expect(result.throughput).toBeGreaterThan(
        PERFORMANCE_TARGETS.minThroughput * 0.2,
      );
      expect(result.errorRate).toBeLessThan(
        PERFORMANCE_TARGETS.maxErrorRate * 10,
    ...
    it('should demonstrate system stability over time', async () => {
      ...
      expect(finalResult.throughput).toBeGreaterThan(
        PERFORMANCE_TARGETS.minThroughput * 0.1,
      );
      expect(finalResult.standardDeviation).toBeLessThan(
        finalResult.averageLatency * 2,
      ); // Reasonable variance
    });
  });
});
