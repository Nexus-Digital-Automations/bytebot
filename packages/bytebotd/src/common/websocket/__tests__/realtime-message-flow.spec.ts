      ...
      try {
        ...
        if (sentMessage) {
          ...
          this.flowMetrics.averageLatency =
            this.flowMetrics.latencies.reduce((sum, lat) => sum + lat, 0) /
            this.flowMetrics.latencies.length;
        }
    ...
    return new Promise((resolve, reject) => {
      const checkMessage = () => {
        ...
        if (Date.now() - startTime >= timeout) {
          reject(
            new Error(`Timeout waiting for message matching predicate after ${timeout}
ms`),
          );
          return;
        }
  ...
  getFlowMetrics() {
    const acknowledgedCount = Array.from(this.acknowledgments.values()).filter(
      (ack) => ack.acknowledged,
    ).length;

    const unacknowledgedCount = Array.from(
      this.acknowledgments.values(),
    ).filter((ack) => !ack.acknowledged).length;

    const deliverySuccessRate =
      this.flowMetrics.totalSent > 0
        ? (this.flowMetrics.totalSent - this.flowMetrics.deliveryFailures) /
          this.flowMetrics.totalSent
        : 0;

    const acknowledgmentRate =
      this.acknowledgments.size > 0
    ...
    return {
      ...
      acknowledgmentRate,
      p95Latency:
        this.flowMetrics.latencies.length > 0
          ? (this.flowMetrics.latencies.sort((a, b) => a - b)[
              Math.floor(this.flowMetrics.latencies.length * 0.95)
            ] ?? 0)
          : 0,
      p50Latency:
        this.flowMetrics.latencies.length > 0
          ? (this.flowMetrics.latencies.sort((a, b) => a - b)[
              Math.floor(this.flowMetrics.latencies.length * 0.5)
            ] ?? 0)
          : 0,
    };
  }
...
class StreamingValidationTester {
  ...
  private validationWorkflows = new Map<
    string,
    {
      startTime: number;
      endTime?: number;
  ...
  private setupProgressTracking(): void {
    this.messageValidator.on('message-received', ({ message }) => {
      if (message.type === ConversationalMessageType.PROGRESS_UPDATE) {
        ...
        const workflow = this.validationWorkflows.get(
          progressMsg.payload.operationId,
        );
        if (workflow) {
...
_${Math.random().toString(36).substring(7)}`;
  ...
  async waitForWorkflowCompletion(
    ...
    throw new Error(`Workflow ${validationId} did not complete within ${timeout}
ms`);
  }

  getStreamingMetrics() {
    const completedWorkflows = Array.from(
      this.validationWorkflows.values(),
    ).filter((workflow) => workflow.status === 'completed');
    const averageDuration =
      completedWorkflows.length > 0
        ? completedWorkflows.reduce(
            (sum, workflow) =>
              sum + ((workflow.endTime || Date.now()) - workflow.startTime),
            0,
    ...
    const averageProgressCount =
      completedWorkflows.length > 0
        ? completedWorkflows.reduce(
            (sum, workflow) => sum + workflow.progressCount,
            0,
          ) / completedWorkflows.length
        : 0;
    ...
    return {
      ...
      failedWorkflows: Array.from(this.validationWorkflows.values()).filter(
        (w) => w.status === 'failed',
      ).length,
      pendingWorkflows: Array.from(this.validationWorkflows.values()).filter(
        (w) => w.status === 'pending',
      ).length,
      averageDuration,
...
const mockConfigService = {
  get: jest.fn((key: string, defaultValue?: unknown) => {
    const config: Record<string, unknown> = {
      ...
      WEBSOCKET_MAX_MESSAGE_SIZE: 1048576, // 1MB'WEBSOCKET_COMPRESSION_ENABLED': true,'WEBSOCKET_HEARTBEAT_INTERVAL': 30000,
    };
...
describe('Real-time Message Flow Tests', () => {
  ...
  beforeAll(async () => {
    ...
    streamingService = module.get<ParlantWebSocketStreamingBridgeService>(
      ParlantWebSocketStreamingBridgeService,
    );
...
_${updateCount}`,
            ...
            payload: {
              ...
              estimatedTimeRemaining:
                progress < 100
                  ? updateInterval * (maxUpdates - updateCount)
                  : 0,
            },
            metadata: {
              priority: 'normal',
  ...
  describe('Bidirectional Message Exchange', () => {
    it('should handle bidirectional message flow correctly', async () => {
      ...
      expect(ackMessage.payload.acknowledgedMessageId).toBe(
    ...
    it('should maintain message ordering in bidirectional flow', async () => {
      ...
      const sentMessages = messageLog.filter(
        (entry) => entry.direction === 'sent',
      );
  ...
  describe('Message Serialization and Performance', () => {
    it('should handle message serialization/deserialization efficiently', () => {
      ...
      console.log('Serialization Performance:', {
        ...
        serializationTime: `${serializationTime.toFixed(3)}
ms`,
        deserializationTime: `${deserializationTime.toFixed(3)}
ms`,
      });
    });
    ...
    it('should achieve sub-50ms message delivery latency target', async () => {
      ...
      console.log('Message Delivery Performance:', {
        ...
        deliverySuccessRate: `${(metrics.deliverySuccessRate * 100).toFixed(
          2,
        )}%`,
...
ms`,
  ...
  describe('Streaming Validation Workflows', () => {
    it('should handle real-time streaming validation workflow', async () => {
      ...
      const validationId =
        await streamingTester.startValidationWorkflow(testAction);

      // Wait for workflow completion
      const workflowResult =
        await streamingTester.waitForWorkflowCompletion(validationId);
      ...
      const sortedUpdates = workflowResult.progressUpdates.sort(
        (a, b) => a.sequence - b.sequence,
      );
      for (let i = 1; i < sortedUpdates.length; i++) {
        expect(sortedUpdates[i].payload.progress).toBeGreaterThanOrEqual(
          sortedUpdates[i - 1].payload.progress,
        );
      }
...
ms`,
    ...
    it('should handle multiple concurrent streaming workflows', async () => {
      ...
      for (let i = 0; i < workflowCount; i++) {
        ...
        workflows.push(
          streamingTester
            .startValidationWorkflow(testAction)
            .then((validationId) =>
              streamingTester.waitForWorkflowCompletion(validationId),
            ),
        );
      }
      ...
      console.log('Concurrent Streaming Metrics:', {
        ...
        averageDuration: `${streamingMetrics.averageDuration.toFixed(2)}
ms`,
  ...
  describe('Flow Control and Backpressure', () => {
    it('should handle high-throughput message flow without loss', async () => {
      ...
      const testMessages: ConversationalMessage[] = Array.from(
        { length: messageCount },
        (_, i) => ({
      ...
      console.log('High-throughput Flow Results:', {
        messageCount,
        sendDuration: `${sendDuration.toFixed(2)}
ms`,
    ...
    it('should implement proper backpressure handling', async () => {
      ...
      const burstSize = 500;
      const largeMessages: ConversationalMessage[] = Array.from(
        { length: burstSize },
        (_, i) => ({
      ...
      console.log('Backpressure Test Results:', {
        ...
        maxQueueSize: `${(queueSize / 1024).toFixed(2)}
KB`,
        deliverySuccessRate: `${(metrics.deliverySuccessRate * 100).toFixed(2)}%`,
      });
