 ...
 */
@Injectable()
export class ParlantWebSocketIntegrationService
  ...
  implements OnModuleInit, OnApplicationShutdown
{
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.log(
      `[${operationId}] Initializing ParlantWebSocketIntegrationService`,
      {
        operationId,
        maxConcurrentValidations:
          this.performanceTargets.maxConcurrentValidations,
    ...
    try {
      ...
      this.logger.error(
        `[${operationId}] Failed to initialize ParlantWebSocketIntegrationService`,
        {
          operationId,
  ...
  private setupConversationalBridgeListeners(): void {
    ...
    this.conversationalBridge.on(
      ...
      (event: { sessionId: string; code: number; reason: string }) => {
  ...
  private async handleConversationalValidationRequest(event: {
    ...
    this.logger.log(
      `[${operationId}] Processing conversational validation request`,
      {
        operationId,
    ...
    try {
      ...
      const parlantRequest = await this.convertToParlantValidation(
        event.sessionId,
        event.message,
      );
      ...
      const result = await this.processParlantValidation(
        parlantRequest,
        operationId,
      );

      // Send result back through conversational bridge
      await this.sendValidationResult(
        event.sessionId,
        event.validationId,
        result,
      );
      ...
      this.logger.error(
        `[${operationId}] Failed to process conversational validation`,
        {
          operationId,
          validationId: event.validationId,
          error: error instanceof Error ? error.message : String(error),
  ...
  private async convertToParlantValidation(
    ...
    const conversationId =
      this.conversationMappings.get(sessionId) ??
    ...
    const parlantAction: ParlantValidationAction = {
      ...
      parlantActionId: `action_${Date.now()}
_${Math.random().toString(36).substring(7)}`,
      conversationalContext: await this.generateConversationalContext(
        message.payload.action,
      ),
      naturalLanguageDescription: await this.generateNaturalLanguageDescription(
        message.payload.action,
      ),
      expectedUserResponse: await this.generateExpectedUserResponse(
        message.payload.action,
      ),
      fallbackActions: await this.generateFallbackActions(
        message.payload.action,
      ),
    };
    ...
    const parlantStreamingOptions: ParlantStreamingOptions = {
      enableConversationalUpdates:
        message.payload.streamingOptions.enableProgressUpdates,
  ...
  private async processParlantValidation(
    ...
    const result: ParlantValidationResult = {
      ...
      conversationalResponse:
        await this.generateConversationalResponse(validationResult),
   ...
   */
  private async performParlantValidationLogic(
    request: ParlantValidationRequest,
  ): Promise<{
    approved: boolean;
    confidence: number;
...
_${Date.now()}`,
   ...
   */
  private handleSessionConnected(event: {
    sessionId: string;
    clientId: string;
    session: ConversationalSession;
  }): void {
...
_${Math.random().toString(36).substring(7)}`;
   ...
   */
  private handleSessionDisconnected(event: {
    sessionId: string;
    code: number;
    reason: string;
  }): void {
...
_metrics`,
  ...
  private calculateAverageValidationTime(): number {
    const times = Array.from(this.validationResults.values()).map(
      (result) => result.performanceMetrics.processingTime,
    );
  ...
  private generateConversationalContext(
    ...
    return Promise.resolve(
      `User is requesting to perform: ${action.actionType}`,
    );
  } /**
  ...
  private generateNaturalLanguageDescription(
    ...
    return Promise.resolve(
      `This action will ${action.actionType} with the following parameters: ${JSON.stringify(action.parameters)}`,
    );
  } /**
   * Generate expected user response (mock implementation)
   */
  private generateExpectedUserResponse(
    action: ValidationAction,
  ): Promise<string> {
    return Promise.resolve(
      `Please confirm if you want to proceed with ${action.actionType}`,
    );
  }
  ...
  private generateConversationalResponse(validationResult: {
    ...
    if (validationResult.approved) {
      return Promise.resolve(
        `✅ Action approved with ${Math.round(validationResult.confidence * 100)}% confidence. ${validationResult.reasoning}`,
      );
    } else {
      return Promise.resolve(
        `❌ Action rejected. ${validationResult.reasoning}`,
      );
    }
  }
  ...
  private mapRiskToPriority(riskLevel: string): ValidationPriority {
    switch (riskLevel) {
      case 'critical':
        return ValidationPriority.EMERGENCY;
      case 'high':
  ...
  async createIntegratedValidationRequest(
    ...
    const validationId = `integrated_validation_${Date.now()}
_${Math.random().toString(36).substring(7)}`;
