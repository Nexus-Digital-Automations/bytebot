 ...
 */
@Injectable()
export class ConversationalWebSocketBridgeService
  extends EventEmitter
  implements OnApplicationShutdown
{
  private readonly logger = new Logger(
    ConversationalWebSocketBridgeService.name,
  );
  ...
  private initializeConversationalBridge(): void {
    ...
    this.logger.log(
      `[${operationId}] Initializing ConversationalWebSocketBridge`,
      {
        operationId,
    ...
    try {
      ...
      this.logger.log(
        `[${operationId}] ConversationalWebSocketBridge initialized successfully`,
        {
          operationId,
          port: this.getConversationalPort(),
      ...
      this.logger.error(
        `[${operationId}] Failed to initialize ConversationalWebSocketBridge`,
        {
          operationId,
  ...
  private setupConversationalEventHandlers(): void {
    ...
    this.webSocketServer.on('connection', (ws: WebSocket.WebSocket, req) => {
      this.handleNewConversationalConnection(
        ws,
   ...
   */
  private handleNewConversationalConnection(
   ...
   */
  private handleConversationalMessage(
    sessionId: string,
    data: WebSocket.RawData,
    _startTime: number,
  ): void {
    const operationId = `conv_message_${sessionId}
_${Date.now()}`;

    try {
      const rawMessage = Buffer.from(data as ArrayBuffer).toString('utf8');
      const message = JSON.parse(rawMessage) as ConversationalMessage; // Validate message structure
      ...
      this.routeConversationalMessage(
        sessionId,
        message,
        operationId,
        routingStartTime,
      ...
      this.logger.error(
        `[${operationId}] Failed to process conversational message`,
        {
          operationId,
          sessionId,
          error: error instanceof Error ? error.message : String(error),
  ...
  private async handleValidationRequest(
    ...
    if (message.payload.streamingOptions.enableProgressUpdates) {
      this.startValidationProgressStreaming(
        sessionId,
        validationId,
        message.payload.streamingOptions,
      );
    }
  ...
  private startValidationProgressStreaming(
    ...
    const progressInterval = setInterval(async () => {
      ...
      const progressStatus: 'pending' | 'active' | 'completed' | 'failed' =
   ...
   */
  private async sendValidationResponse(
    sessionId: string,
    response: {
      validationId: string;
      status: string;
   ...
   */
  private async sendConfirmationResult(
    sessionId: string,
    result: {
   ...
   */
  private async sendStreamingComplete(
    sessionId: string,
    operationId: string,
  ): Promise<void> {
    const message: ConversationalMessage = {
   ...
   */
  private async sendSessionReady(
    sessionId: string,
  ...
  private async handleHeartbeat(
    ...
    const ackMessage: ConversationalMessage = {
      ...
      payload: {
        ...
        serverTime: Date.now(),
        latency:
          Date.now() - ((message.payload.clientTime as number) || Date.now()),
      },
      metadata: {
        priority: 'low',
        requiresAck: false,
  ...
  private collectPerformanceMetrics(): void {
    ...
    const sessionMetrics: SessionPerformanceMetrics[] = Array.from(
      this.sessions.values(),
    ).map((s) => s.performanceMetrics);

    if (sessionMetrics.length > 0) {
      metrics.averageLatency =
        sessionMetrics.reduce((sum, m) => sum + m.averageLatency, 0) /
        sessionMetrics.length;
      metrics.averageMessageRate =
        sessionMetrics.reduce((sum, m) => sum + m.messageRate, 0) /
        sessionMetrics.length;
      metrics.averageErrorRate =
        sessionMetrics.reduce((sum, m) => sum + m.errorRate, 0) /
        sessionMetrics.length;
    }
  ...
  private validateMessageStructure(
    message: unknown,
  ): message is ConversationalMessage {
   ...
   */
  private handleSessionDisconnection(
    sessionId: string,
    code: number,
    reason: Buffer,
  ): void {
...
_${this.generateId()}`;
   ...
   */
  async broadcastToAllSessions(
    message: Omit<
      ConversationalMessage,
      'sessionId' | 'messageId' | 'sequence'
    >,
  ): Promise<void> {
    const activeSessions = Array.from(this.sessions.keys()).filter(
      (sessionId) =>
        this.sessions.get(sessionId)?.status === SessionStatus.ACTIVE,
    );
