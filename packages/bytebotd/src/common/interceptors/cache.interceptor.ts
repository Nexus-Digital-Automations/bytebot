...
import { createHash } from 'crypto';/*** Cache configuration for different endpoint patterns
 */
interface CacheRule {
  pattern: RegExp;
...
export class CacheInterceptor implements NestInterceptor {
  ...
  this.logger.log('Cache Interceptor initialized');
    this.logger.log(`Configured ${this.cacheRules.length
} cache rules`);
  ...
  // Cache error - proceed without cache
          this.logger.error(
            `[${operationId
}] Cache retrieval error: ${_error instanceof Error ? _error.message : 'Unknown error'}`,
          );
  ...
  const duration = Date.now() - startTime;
    ...
    this.logger.debug(
      `[${operationId}] Cache HIT: ${cachedResponse.metadata.url} (${duration}
ms)`,
      {
...
this.updateStats();
  ...
  const keyParts: string[] = [request.method, request.url];
  ...
  const now = Date.now();
    ...
    const ifNoneMatch = request.get('If-None-Match');if (ifNoneMatch && cachedResponse.metadata.etag) {
  return ifNoneMatch === cachedResponse.metadata.etag;
    
}

    // Check Last-Modified if provided in request
    const ifModifiedSince = request.get('If-Modified-Since');if (ifModifiedSince && cachedResponse.metadata.lastModified) {
  ...
  // Check status code
    ...
    const cacheControl = response.get('Cache-Control');if (cacheControl) {
  if (
  ...
  try {
      ...
      if (resourcePattern) {
        ...
        this.logger.debug(
          `Cache invalidation triggered by ${request.method
} ${request.url}`,{ pattern: resourcePattern },);
      }
    } catch (_error) {
  this.logger.error(
...
return basePath.replace(/\/\d+/g, '/:id') // Replace IDs.replace(/\/[a-f0-9-]{36}/g, '/:uuid'); // Replace UUIDs}/**
  ...
  this.stats.totalRequests =
      this.stats.cacheHits + this.stats.cacheMisses + this.stats.cacheSkips;
...
}%`,topEndpoints: Array.from(this.stats.endpointStats.entries()).sort((a, b) => b[1].hits + b[1].misses - (a[1].hits + a[1].misses))
            .slice(0, 5)
            .map(([url, stats]) => ({
  url,
