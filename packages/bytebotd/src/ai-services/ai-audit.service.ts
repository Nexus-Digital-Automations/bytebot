...
export class AIAuditService {
  ...
  constructor(
    ...
    this.logger.log(
      `[${operationId}] AI Audit Service initialized with comprehensive monitoring`,
      {
        parlantEnabled: true,
   ...
   */
  recordAIOperation(
    entry: Omit<AIOperationAuditEntry, 'id' | 'timestamp'>,
  ): void {
    ...
    if (
      entry.riskLevel === RiskLevel._CRITICAL ||
      entry.validationResult === 'denied' ||
      entry.executionResult === 'failure'
    ) {
    ...
    if (this.auditTrail.length > maxSize) {
      const removedEntries = this.auditTrail.splice(
        0,
        this.auditTrail.length - maxSize,
      );
      this.logger.debug(
        `Removed ${removedEntries.length} old audit entries to maintain size limit`,
      );
    }
  }
  ...
  generateAuditReport(_period?: { from: Date; to: Date }): AIAuditReport {
    ...
    const relevantEntries = this.auditTrail.filter(
      (entry) =>
        entry.timestamp >= reportPeriod.from &&
        entry.timestamp <= reportPeriod.to,
    );
    ...
    const operationsByService =
      this.calculateOperationsByService(relevantEntries);
    const validationStatistics =
      this.calculateValidationStatistics(relevantEntries);
    const performanceStatistics =
      this.calculatePerformanceStatistics(relevantEntries);
  ...
  getComplianceViolations(
    ...
    const violations = this.auditTrail.filter((entry) => {
      const hasCriticalFlags = entry.complianceFlags.some(
        (flag) => flag.includes('violation') || flag.includes('non_compliant'),
      );
      const hasHighRisk =
        entry.riskLevel === RiskLevel._CRITICAL ||
        entry.riskLevel === RiskLevel._HIGH;
      const wasDenied = entry.validationResult === 'denied';
      const failed = entry.executionResult === 'failure';
      if (severity === 'critical') {
        return (
          hasCriticalFlags &&
          (entry.riskLevel === RiskLevel._CRITICAL || wasDenied)
        );
      }
      if (severity === 'high') {
  ...
  private calculateOperationsByService(
    entries: AIOperationAuditEntry[],
  ...
  private calculateValidationStatistics(entries: AIOperationAuditEntry[]): {
    ...
    const approved = entries.filter(
      (e) => e.validationResult === 'approved',
    ).length;
    const denied = entries.filter(
      (e) => e.validationResult === 'denied',
    ).length;
    const errors = entries.filter((e) => e.validationResult === 'error').length;
    const totalValidationTime = entries.reduce(
      (sum, e) => sum + e.performanceMetrics.validationTimeMs,
      0,
    );
    return {
  ...
  private calculatePerformanceStatistics(entries: AIOperationAuditEntry[]): {
    ...
    const successful = entries.filter(
      (e) => e.executionResult === 'success',
    ).length;
    const timeouts = entries.filter(
      (e) => e.executionResult === 'timeout',
    ).length;
    const totalExecutionTime = entries.reduce(
      (sum, e) => sum + e.performanceMetrics.executionTimeMs,
      0,
    ...
    const avgResourceUsage =
      entries
        ...
        .reduce(
          (sum, e) => sum + (e.performanceMetrics.resourceUsage?.cpu ?? 0),
          0,
        ) / total;
  ...
  private calculateComplianceStatus(entries: AIOperationAuditEntry[]): {
    ...
    const flagged = entries.filter((e) =>
      e.complianceFlags.some(
        (flag) => flag.includes('violation') || flag.includes('flagged'),
      ),
    ).length;
    const critical = entries.filter(
      (e) =>
        e.riskLevel === RiskLevel._CRITICAL &&
        (e.validationResult === 'denied' || e.executionResult === 'failure'),
    ).length;
    const overallScore = Math.max(
      0,
      100 - (flagged / total) * 50 - (critical / total) * 30,
    );

    const recommendedActions = [];
    if (flagged > 0)
      recommendedActions.push(
        'Review flagged operations for compliance violations',
      );
    if (critical > 0)
      recommendedActions.push(
        'Address critical security incidents immediately',
      );
    if (overallScore < 80)
      recommendedActions.push('Implement additional validation controls');
    return {
      overallScore,
  ...
  private calculateSecurityAnalysis(entries: AIOperationAuditEntry[]): {
    ...
    const securityIncidents = entries.filter((e) =>
      e.securityFlags.some(
        (flag) => flag.includes('incident') || flag.includes('threat'),
      ),
    ).length;
    ...
    const criticalIncidents = riskDistribution[RiskLevel._CRITICAL] ?? 0;
    const highIncidents = riskDistribution[RiskLevel._HIGH] ?? 0;

    if (criticalIncidents > entries.length * 0.1) threatLevel = 'critical';
    else if (criticalIncidents > 0 || highIncidents > entries.length * 0.2)
      threatLevel = 'high';
    else if (securityIncidents > 0 || anomaliesDetected > 0)
      threatLevel = 'medium';
  ...
  private performAuditMaintenance(): void {
    ...
    const remainingEntries = this.auditTrail.filter(
    ...
    if (remainingEntries.length < initialSize) {
      ...
      this.logger.debug(
        `Audit maintenance: removed ${initialSize - remainingEntries.length} expired entries`,
      );
    }
  }
    ...
    const stats = {
      ...
      averageValidationTime:
        this.auditOperationsCount > 0
          ? this.totalValidationTime / this.auditOperationsCount
          : 0,
      averageExecutionTime:
        this.auditOperationsCount > 0
          ? this.totalExecutionTime / this.auditOperationsCount
          : 0,
      recentComplianceViolations: this.getComplianceViolations(
        'critical',
      ).filter((e) => e.timestamp > new Date(Date.now() - 60 * 60 * 1000))
        .length,
      recentSecurityIncidents: this.getSecurityIncidents().filter(
  ...
  private isComplianceMonitoringEnabled(): boolean {
    return this.configService.get<boolean>(
      'AI_COMPLIANCE_MONITORING_ENABLED',
      true,
    );
  }
  ...
  getServiceHealth(): {
    ...
    const recentViolations = this.getComplianceViolations('critical').filter(
      (e) => e.timestamp > new Date(Date.now() - 24 * 60 * 60 * 1000),
    ).length;

    const recentIncidents = this.getSecurityIncidents().filter(
      (e) => e.timestamp > new Date(Date.now() - 24 * 60 * 60 * 1000),
    ).length;
    ...
    return {
      ...
      metrics: {
        ...
        averageValidationTime:
          this.auditOperationsCount > 0
            ? `${(this.totalValidationTime / this.auditOperationsCount).toFixed(2)}ms`
            : '0ms',
        averageExecutionTime:
          this.auditOperationsCount > 0
            ? `${(this.totalExecutionTime / this.auditOperationsCount).toFixed(2)}ms`
            : '0ms',
        recentViolations,
        recentIncidents,
        complianceMonitoringEnabled: this.isComplianceMonitoringEnabled(),
  ...
  clearAuditTrail(): void {
    ...
    this.logger.warn(
      'AI Audit trail cleared - this should only be used for testing',
    );
  }
}
