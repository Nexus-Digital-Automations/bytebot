...
export class AnthropicService {
  ...
  constructor(
    ...
    if (!this.apiKey) {
      this.logger.warn(
        `[${operationId}] Anthropic API key not configured - service will operate in validation-only mode`,
      );
    }

    this.logger.log(
      `[${operationId}] Anthropic Service initialized with MAXIMUM Parlant integration`,
      {
        parlantEnabled: true,
  ...
  async executeChatCompletion(
    ...
    try {
      ...
      this.logger.log(
        `[${request.operationId}] Requesting Parlant validation for Claude AI interaction`,
      );
      ...
      this.logger.log(
        `[${request.operationId}] Parlant validation approved - proceeding with Claude API call`,
      );
  ...
  async executeStreamingChat(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.logger.log(
        `[${operationId}] Claude streaming completed successfully`,
        {
          operationId,
          duration,
          validationId: validationResponse.conversationId,
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `[${operationId}] Claude streaming failed: ${error instanceof Error ? error.message : String(error)}`,
        {
          operationId,
          error: error instanceof Error ? error.message : String(error),
          duration,
  ...
  async executeToolCalling(
    ...
    try {
      ...
      if (!validationResponse.approved) {
        throw new Error(
          `Tool calling operation blocked: ${validationResponse.reasoning}`,
        );
      ...
      this.logger.log(
        `[${operationId}] Claude tool calling completed successfully`,
        {
          operationId,
          responseId: response.id,
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `[${operationId}] Claude tool calling failed: ${error instanceof Error ? error.message : String(error)}`,
        {
          operationId,
          error: error instanceof Error ? error.message : String(error),
          duration,
   ...
   */
  private async performClaudeAPICall(
    request: ClaudeChatRequest,
  ): Promise<ClaudeChatResponse> {
  ...
  private async performStreamingClaudeCall(
    ...
    const chunks: ClaudeStreamChunk[] = [
      ...
      {
        type: 'content_block_delta',
        delta: { text: 'from Claude with validation.' },
      },
      { type: 'message_stop' },
    ];
   ...
   */
  private async performClaudeToolCall(
    request: ClaudeChatRequest,
  ): Promise<ClaudeChatResponse> {
    ...
    this.logger.log('Anthropic Service Performance Metrics', {
      ...
      tokenRatio:
        this.tokenUsage.input > 0
          ? (this.tokenUsage.output / this.tokenUsage.input).toFixed(2)
          : '0',
    });
  }
