...
export class GoogleService {
  ...
  constructor(
    ...
    if (!this.apiKey) {
      this.logger.warn(
        `[${operationId}] Google AI API key not configured - service will operate in validation-only mode`,
      );
    }
    this.logger.log(
      `[${operationId}] Google AI Service initialized with MAXIMUM Parlant integration`,
      {
        parlantEnabled: true,
  ...
  async executeChatCompletion(
    ...
    try {
      ...
      this.logger.log(
        `[${request.operationId}] Requesting Parlant validation for Gemini AI interaction`,
      );
      ...
      this.logger.log(
        `[${request.operationId}] Parlant validation approved - proceeding with Gemini API call`,
      );
  ...
  async executeStreamingChat(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.logger.log(
        `[${operationId}] Gemini streaming completed successfully`,
        {
          operationId,
          duration,
          validationId: validationResponse.conversationId,
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `[${operationId}] Gemini streaming failed: ${error instanceof Error ? error.message : String(error)}`,
        {
          operationId,
          error: error instanceof Error ? error.message : String(error),
          duration,
  ...
  async executeFunctionCalling(
    ...
    try {
      ...
      const firstCandidate =
        response.candidates.length > 0 ? response.candidates[0] : null;
      const functionCalls =
        firstCandidate?.content.parts.filter((part) => part.functionCall) ?? [];

      this.logger.log(
        `[${operationId}] Gemini function calling completed successfully`,
        {
          operationId,
          modelVersion: response.modelVersion,
          functionCallsCount: functionCalls.length,
          functionNames: functionCalls
            .map((call) => call.functionCall?.name)
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `[${operationId}] Gemini function calling failed: ${error instanceof Error ? error.message : String(error)}`,
        {
          operationId,
          error: error instanceof Error ? error.message : String(error),
          duration,
  ...
  async executeMultimodalProcessing(
    ...
    try {
      ...
      const validationRequest: ParlantValidationRequest = {
        ...
        context: request.context,
        riskLevel: RiskLevel._HIGH, // Multimodal processing is HIGH risk
        operationId,
      };
      ...
      if (!validationResponse.approved) {
        throw new Error(
          `Multimodal processing operation blocked: ${validationResponse.reasoning}`,
        );
      ...
      this.logger.log(
        `[${operationId}] Gemini multimodal processing completed successfully`,
        {
          operationId,
          modelVersion: response.modelVersion,
          promptTokens: response.usageMetadata.promptTokenCount,
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `[${operationId}] Gemini multimodal processing failed: ${error instanceof Error ? error.message : String(error)}`,
        {
          operationId,
          error: error instanceof Error ? error.message : String(error),
          duration,
   ...
   */
  private async performGeminiAPICall(
    request: GeminiChatRequest,
  ): Promise<GeminiChatResponse> {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, 180 + Math.random() * 320),
    );
   ...
   */
  private async performGeminiFunctionCall(
    request: GeminiChatRequest,
  ): Promise<GeminiChatResponse> {
   ...
   */
  private async performGeminiMultimodalCall(
    request: GeminiMultimodalRequest,
  ): Promise<GeminiChatResponse> {
    ...
    const mockResponse: GeminiChatResponse = {
      ...
      usageMetadata: {
        promptTokenCount:
          this.estimatePromptTokens(request) + (request.imageCount ?? 0) * 200, // Images add tokens
        candidatesTokenCount: 150,
        totalTokenCount:
          this.estimatePromptTokens(request) +
          (request.imageCount ?? 0) * 200 +
          150,
      },
      modelVersion: request.config.model,
    };
  ...
  private estimatePromptTokens(request: GeminiChatRequest): number {
    ...
    return Math.ceil(
      (textContent.length + systemContent.length + toolsContent.length) / 4,
    );
  }
    ...
    this.logger.log('Google AI Service Performance Metrics', {
      ...
      tokenRatio:
        this.tokenUsage.prompt > 0
          ? (this.tokenUsage.candidates / this.tokenUsage.prompt).toFixed(2)
          : '0',
    });
  }
