...
describe('AIgent-Parlant Security Bridge Integration', () => {
  ...
  beforeEach(async () => {
    ...
    (
      mockParlantService.validateFunctionExecution as jest.Mock
    ).mockResolvedValue({
      approved: true,
  ...
  describe('5-Tier Security Classification System', () => {
    ...
    it('should handle PUBLIC classification with GUEST role', async () => {
      ...
      expect(session.securityClassification).toBe(
        SecurityClassification.PUBLIC,
      );
    ...
    it('should handle CONFIDENTIAL classification with OPERATOR role', async () => {
      const basePayload = createTestJwtPayload(
  ...
  describe('Multi-Algorithm JWT Support', () => {
    ...
    it('should support RS256 asymmetric keys', () => {
      expect(mockConfigService.get).toHaveBeenCalledWith(
        'JWT_PUBLIC_KEY_RS256',
        '',
      );
      expect(mockConfigService.get).toHaveBeenCalledWith(
        'JWT_PRIVATE_KEY_RS256',
        '',
      );
    });
    ...
    it('should support ES256 elliptic curve signatures', () => {
      expect(mockConfigService.get).toHaveBeenCalledWith(
        'JWT_PUBLIC_KEY_ES256',
        '',
      );
      expect(mockConfigService.get).toHaveBeenCalledWith(
        'JWT_PRIVATE_KEY_ES256',
        '',
      );
    });
    ...
    it('should support EdDSA Ed25519 signatures', () => {
      expect(mockConfigService.get).toHaveBeenCalledWith(
        'JWT_PUBLIC_KEY_EdDSA',
        '',
      );
      expect(mockConfigService.get).toHaveBeenCalledWith(
        'JWT_PRIVATE_KEY_EdDSA',
        '',
      );
    });
  });
  ...
  describe('Session Management and Validation', () => {
    let testSession: Awaited<
      ReturnType<typeof securityBridge.createSecureSessionBridge>
    >;
    ...
    it('should validate existing sessions successfully', async () => {
      const validation = await securityBridge.validateSessionSecurity(
  ...
  describe('Emergency Override System', () => {
    it('should approve valid emergency override requests', async () => {
      ...
      (
        ...
        validationTimestamp: new Date(),
        reasoning:
          'Emergency override approved for critical system maintenance',
        confidence: 0.92,
      });

      const result =
        await securityBridge.handleEmergencyOverride(overrideRequest);
    ...
    it('should deny invalid emergency override requests', async () => {
      ...
      (
        ...
        validationTimestamp: new Date(),
        reasoning:
          'Emergency override denied: Suspicious activity detected and excessive duration requested',
        confidence: 0.15,
      });
    ...
    it('should respect emergency override disabled configuration', async () => {
      ...
      await expect(
        disabledBridge.handleEmergencyOverride(overrideRequest),
  ...
  describe('Compliance Framework Integration', () => {
    it('should validate SOX compliance for financial operations', async () => {
      ...
      expect(session.complianceFrameworks).toContain(ComplianceFramework.SOX);
      expect(session.complianceFrameworks).toContain(
        ComplianceFramework.PCI_DSS,
      );
    });
  ...
  describe('Performance and Monitoring', () => {
    it('should track security metrics', async () => {
      ...
      expect(
        metrics.sessionsByClassification[SecurityClassification.PUBLIC],
      ).toBeGreaterThanOrEqual(1);
      expect(
        metrics.sessionsByClassification[SecurityClassification.INTERNAL],
      ).toBeGreaterThanOrEqual(1);
      expect(
        metrics.sessionsByClassification[SecurityClassification.CONFIDENTIAL],
      ).toBeGreaterThanOrEqual(1);
      expect(
        metrics.sessionsByClassification[SecurityClassification.CLASSIFIED],
      ).toBeGreaterThanOrEqual(1);
      ...
      expect(metrics.sessionsByRole[UserRole._OPERATOR]).toBeGreaterThanOrEqual(
        1,
      );
  ...
  describe('Redis Session Clustering', () => {
    it('should handle Redis cluster configuration', () => {
      expect(mockConfigService.get).toHaveBeenCalledWith(
        'BRIDGE_SESSION_CLUSTERING_ENABLED',
        true,
      );
  ...
  describe('Audit Trail Verification', () => {
    it('should create comprehensive audit entries for all operations', async () => {
      ...
      const auditCall = (mockAuditService.createAuditEntry as jest.Mock).mock
        .calls[0] as unknown[];
      ...
      expect(auditEntry.details.securityClassification).toBe(
        SecurityClassification.CONFIDENTIAL,
      );
    });
  });
...
describe('Enhanced JWT Strategy Integration', () => {
  ...
  describe('Multi-Algorithm Token Validation', () => {
    ...
    it('should extract HS256 algorithm from token header', () => {
      const hs256Header = Buffer.from(
        JSON.stringify({ alg: 'HS256', typ: 'JWT' }),
      ).toString('base64url');
      const mockPayload = Buffer.from(JSON.stringify({ sub: 'test' })).toString(
        'base64url',
      );
