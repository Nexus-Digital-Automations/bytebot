...
{
  "actionType": "fill_form","url": "https://example.com/contact","formData": {"name": "John Doe","email": "john@example.com"},"configuration": {"waitForLoad": 3000,"validateFields": true
  }
}
\`\`\`### Extract Structured Data\`\`\`json
POST /data-extraction/extract
{
  "url": "https://example.com/products","extractionType": "table_data","selector": ".products-table","outputFormat": "json","configuration": {"includeHeaders": true,"pagination": true
  }
}
\`\`\`### Monitor Content Changes\`\`\`json
POST /content-monitoring/monitors
{
  "id": "price-monitor-123","name": "Product Price Monitor","url": "https://store.example.com/product/123","type": "text_change","selector": ".price","frequency": { "interval": 300000 },"notifications": [{ "method": "email", "target": "alerts@company.com" }
  ]
}
\`\`\`## 📚 Additional Resources- **API Reference**: Complete endpoint documentation with examples
    ...
    app.use(
      helmet({
        contentSecurityPolicy: {
          directives: {
            defaultSrc: ["'self'"],scriptSrc: ["'self'',"'unsafe-inline'', // Required for VNC viewer"'unsafe-eval'', // Required for noVNC client'https://cdn.jsdelivr.net',
            ],
            styleSrc: ["'self'', "'unsafe-inline'"], // Required for VNC viewerfontSrc: ["'self'', 'data:'],
            imgSrc: ["'self'', 'data:', 'blob:', 'http://localhost:*'],
            connectSrc: [
              "'self'','ws:','wss:','http://localhost:*','https://localhost:*',...(environment === 'production'? ['wss://app.bytebot.ai', 'https://api.bytebot.ai']
                : []),
            ],
            objectSrc: ["'none'"],mediaSrc: ["'self'', 'blob:'],
