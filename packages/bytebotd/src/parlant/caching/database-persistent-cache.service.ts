...
export type DatabaseType = 'postgresql' | 'sqlite' | 'mongodb';
...
export class DatabasePersistentCacheService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.logger.log('Database Persistent Cache Service initializing...', {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const operationId = `db_cache_init${Date.now()}`;

    try {
      this.logger.log(`[${operationId}] Initializing Database Persistent Cache...`);
      ...
      this.logger.log(`[${operationId}] Database Persistent Cache initialized successfully`, {
        database: this.config.database,
        tableName: this.config.schema.tableName,
        indexes: this.config.schema.indexes.length,
        compressionEnabled: this.config.compression.enabled,
      });

    } catch (error) {
      this.logger.error(`[${operationId}] Database cache initialization failed:`, error);
      throw error;
    }
  }

  async onModuleDestroy(): Promise<void> {
    const operationId = `db_cache_shutdown${Date.now()}`;

    try {
      this.logger.log(`[${operationId}] Shutting down Database Persistent Cache...`);

      // Final cleanup if enabled
      if (this.config.maintenance.cleanup.enabled && !this.cleanupInProgress) {
      ...
      this.logger.log(`[${operationId}] Database Persistent Cache shutdown completed`);

    } catch (error) {
      this.logger.error(`[${operationId}] Database cache shutdown error:`, error);
    }
  }
  ...
  async get<T>(key: string, options: {
    ...
    try {
      ...
      this.logger.debug(`[${operationId}] Database cache GET successful: ${key} (${queryTime.toFixed(2)}ms)`, {
      ...
      this.logger.error(`[${operationId}] Database cache GET error:`, {
        key,
  ...
  async set<T>(
    ...
    try {
      ...
      const entry: Partial<DatabaseCacheEntry> = {
        ...
        risk_level: options.riskLevel || 'unknown',
      ...
      this.logger.error(`[${operationId}] Database cache SET error:`, {
        key,
        error: error instanceof Error ? error.message : String(error),
        queryTime: `${queryTime.toFixed(2)}ms`,
      });
  ...
  async del(key: string): Promise<DatabaseQueryResult<void>> {
    ...
    try {
      ...
      this.logger.error(`[${operationId}] Database cache DEL error:`, {
        key,
        error: error instanceof Error ? error.message : String(error),
        queryTime: `${queryTime.toFixed(2)}ms`,
      });
  ...
  async performCleanup(): Promise<{ cleanedEntries: number; freedSpace: number }> {
    ...
    const operationId = `db_cleanup${Date.now()}`;
    this.cleanupInProgress = true;

    try {
      this.logger.log(`[${operationId}] Starting database cache cleanup...`);

      const { deletedCount, freedBytes } = await this.executeCleanupQuery();

      this.metrics.maintenance.lastCleanup = new Date();
      this.metrics.maintenance.cleanedEntries = deletedCount;

      this.logger.log(`[${operationId}] Cache cleanup completed: ${deletedCount} entries cleaned, ${(freedBytes / 1024 / 1024).toFixed(2)}MB freed`);

      return {
        cleanedEntries: deletedCount,
  ...
  private loadDatabaseCacheConfig(): DatabaseCacheConfig {
    const databaseType = this.configService.get<DatabaseType>('DB_CACHE_TYPE', 'postgresql');

    return {
      enabled: this.configService.get<boolean>('DB_CACHE_ENABLED', true),
      database: databaseType,
      connection: this.loadConnectionConfig(databaseType),
      schema: {
        tableName: this.configService.get<string>('DB_CACHE_TABLE_NAME', 'parlant_cache_entries'),
        indexes: [
          { name: 'cache_key_idx', columns: ['cache_key'], unique: true, type: 'btree' },
          { name: 'expires_at_idx', columns: ['expires_at'], unique: false, type: 'btree' },
          { name: 'function_name_idx', columns: ['function_name'], unique: false, type: 'btree' },
          { name: 'created_at_idx', columns: ['created_at'], unique: false, type: 'btree' },
        ],
        partitioning: {
          enabled: this.configService.get<boolean>('DB_CACHE_PARTITIONING_ENABLED', false),
          strategy: 'time',
          intervalHours: this.configService.get<number>('DB_CACHE_PARTITION_INTERVAL_HOURS', 24),
        },
        constraints: {
          maxKeyLength: this.configService.get<number>('DB_CACHE_MAX_KEY_LENGTH', 500),
          maxValueSize: this.configService.get<number>('DB_CACHE_MAX_VALUE_SIZE', 10 * 1024 * 1024), // 10MB
          maxTtlHours: this.configService.get<number>('DB_CACHE_MAX_TTL_HOURS', 168), // 1 week
        },
      },
      performance: {
        batchSize: this.configService.get<number>('DB_CACHE_BATCH_SIZE', 1000),
        queryTimeout: this.configService.get<number>('DB_CACHE_QUERY_TIMEOUT', 30000),
        connectionPool: {
          min: this.configService.get<number>('DB_CACHE_POOL_MIN', 2),
          max: this.configService.get<number>('DB_CACHE_POOL_MAX', 20),
          acquireTimeoutMs: this.configService.get<number>('DB_CACHE_POOL_ACQUIRE_TIMEOUT', 5000),
          idleTimeoutMs: this.configService.get<number>('DB_CACHE_POOL_IDLE_TIMEOUT', 30000),
        },
        transactions: {
          enabled: this.configService.get<boolean>('DB_CACHE_TRANSACTIONS_ENABLED', true),
          isolation: 'READ_COMMITTED',
          timeoutMs: this.configService.get<number>('DB_CACHE_TRANSACTION_TIMEOUT', 10000),
        },
        optimization: {
          preparedStatements: this.configService.get<boolean>('DB_CACHE_PREPARED_STATEMENTS', true),
          queryPlanning: this.configService.get<boolean>('DB_CACHE_QUERY_PLANNING', true),
          statisticsCollection: this.configService.get<boolean>('DB_CACHE_STATISTICS', true),
        },
      },
      maintenance: {
        cleanup: {
          enabled: this.configService.get<boolean>('DB_CACHE_CLEANUP_ENABLED', true),
          intervalMinutes: this.configService.get<number>('DB_CACHE_CLEANUP_INTERVAL_MINUTES', 60),
          batchSize: this.configService.get<number>('DB_CACHE_CLEANUP_BATCH_SIZE', 1000),
          maxAge: this.configService.get<number>('DB_CACHE_CLEANUP_MAX_AGE_HOURS', 72),
        },
        vacuum: {
          enabled: this.configService.get<boolean>('DB_CACHE_VACUUM_ENABLED', true),
          intervalHours: this.configService.get<number>('DB_CACHE_VACUUM_INTERVAL_HOURS', 24),
          analyze: this.configService.get<boolean>('DB_CACHE_VACUUM_ANALYZE', true),
        },
        monitoring: {
          slowQueryThresholdMs: this.configService.get<number>('DB_CACHE_SLOW_QUERY_THRESHOLD', 100),
          deadlockDetection: this.configService.get<boolean>('DB_CACHE_DEADLOCK_DETECTION', true),
          lockTimeoutMs: this.configService.get<number>('DB_CACHE_LOCK_TIMEOUT', 5000),
        },
      },
      compression: {
        enabled: this.configService.get<boolean>('DB_CACHE_COMPRESSION_ENABLED', true),
        threshold: this.configService.get<number>('DB_CACHE_COMPRESSION_THRESHOLD', 5120), // 5KB
        algorithm: 'gzip',
        level: this.configService.get<number>('DB_CACHE_COMPRESSION_LEVEL', 6),
        autoAnalysis: this.configService.get<boolean>('DB_CACHE_COMPRESSION_AUTO_ANALYSIS', true),
      },
      analytics: {
        enabled: this.configService.get<boolean>('DB_CACHE_ANALYTICS_ENABLED', true),
        retention: {
          accessPatterns: this.configService.get<number>('DB_CACHE_ANALYTICS_ACCESS_RETENTION_DAYS', 30),
          performanceMetrics: this.configService.get<number>('DB_CACHE_ANALYTICS_PERFORMANCE_RETENTION_DAYS', 7),
          errorLogs: this.configService.get<number>('DB_CACHE_ANALYTICS_ERROR_RETENTION_DAYS', 14),
        },
        aggregation: {
          enabled: this.configService.get<boolean>('DB_CACHE_ANALYTICS_AGGREGATION_ENABLED', true),
          intervalMinutes: this.configService.get<number>('DB_CACHE_ANALYTICS_AGGREGATION_INTERVAL_MINUTES', 15),
        },
      },
    };
  }
  ...
  private loadConnectionConfig(databaseType: DatabaseType): DatabaseConnectionConfig {
    switch (databaseType) {
      case 'postgresql':
        return {
          postgresql: {
            host: this.configService.get<string>('DB_CACHE_PG_HOST', 'localhost'),
            port: this.configService.get<number>('DB_CACHE_PG_PORT', 5432),
            database: this.configService.get<string>('DB_CACHE_PG_DATABASE', 'parlant_cache'),
            username: this.configService.get<string>('DB_CACHE_PG_USERNAME', 'postgres'),
            password: this.configService.get<string>('DB_CACHE_PG_PASSWORD', ''),
            ssl: this.configService.get<boolean>('DB_CACHE_PG_SSL', false),
            poolSize: this.configService.get<number>('DB_CACHE_PG_POOL_SIZE', 20),
            idleTimeoutMs: this.configService.get<number>('DB_CACHE_PG_IDLE_TIMEOUT', 30000),
            connectionTimeoutMs: this.configService.get<number>('DB_CACHE_PG_CONNECTION_TIMEOUT', 5000),
          },
        };
      ...
      case 'sqlite':
        return {
          sqlite: {
            filePath: this.configService.get<string>('DB_CACHE_SQLITE_PATH', './cache/parlant_cache.db'),
            inMemory: this.configService.get<boolean>('DB_CACHE_SQLITE_IN_MEMORY', false),
            busyTimeoutMs: this.configService.get<number>('DB_CACHE_SQLITE_BUSY_TIMEOUT', 5000),
            pragma: {
              journal_mode: 'WAL',
              synchronous: 'NORMAL',
      ...
      case 'mongodb':
        return {
          mongodb: {
            uri: this.configService.get<string>('DB_CACHE_MONGO_URI', 'mongodb://localhost:27017'),
            database: this.configService.get<string>('DB_CACHE_MONGO_DATABASE', 'parlant_cache'),
            collection: this.configService.get<string>('DB_CACHE_MONGO_COLLECTION', 'cache_entries'),
            poolSize: this.configService.get<number>('DB_CACHE_MONGO_POOL_SIZE', 20),
            serverSelectionTimeoutMs: this.configService.get<number>('DB_CACHE_MONGO_SELECTION_TIMEOUT', 5000),
          },
        };
  ...
  private async createSchema(): Promise<void> {
    ...
    this.logger.debug('Creating database schema and indexes');
  }
  ...
  private async initializePreparedStatements(): Promise<void> {
    ...
    this.logger.debug('Initializing prepared statements');
  }

  private async executeSelectQuery(key: string): Promise<DatabaseCacheEntry | null> {
  ...
  private createFailureResult<T>(error: string, startTime: number): DatabaseQueryResult<T> {
    return {
      success: false,
      error,
      metadata: {
  ...
  private recordCacheHit(queryTime: number): void {
    this.recordQueryHistory('HIT', queryTime, true);
  ...
  private recordCacheMiss(queryTime: number): void {
    this.recordQueryHistory('MISS', queryTime, true);
  }

  private recordQueryHistory(type: string, duration: number, success: boolean): void {
  ...
  private startPerformanceMonitoring(): void {
    ...
    setInterval(() => {
      ...
      this.logger.log('Database Cache Performance Report', {
        ...
        hitRate: `${analytics.metrics.cache.hitRate.toFixed(2)}%`,
        avgQueryTime: `${analytics.metrics.performance.avgQueryTime.toFixed(2)}ms`,
        p95QueryTime: `${analytics.metrics.performance.p95QueryTime.toFixed(2)}ms`,
        storageSize: `${(analytics.metrics.cache.storageSize / 1024 / 1024).toFixed(2)}MB`,
        compressionRate: `${(analytics.metrics.cache.compressionRate * 100).toFixed(1)}%`,
  ...
  private async performVacuum(): Promise<void> {
    ...
    const operationId = `db_vacuum${Date.now()}`;
    this.vacuumInProgress = true;

    try {
      this.logger.log(`[${operationId}] Starting database vacuum...`);

      // TODO: Execute VACUUM ANALYZE on PostgreSQL
      await new Promise(resolve => setTimeout(resolve, 5000)); // Mock vacuum time

      this.metrics.maintenance.lastVacuum = new Date();

      this.logger.log(`[${operationId}] Database vacuum completed`);

    } catch (error) {
      this.logger.error(`[${operationId}] Database vacuum error:`, error);
    } finally {
  ...
  private assessCacheHealth(): string {
    ...
    if (metrics.performance.avgQueryTime > 100) return 'POOR';
    if (metrics.cache.hitRate < 10) return 'POOR';
    if (metrics.connections.errors > 100) return 'POOR';

    if (metrics.performance.avgQueryTime > 50) return 'FAIR';
    if (metrics.cache.hitRate < 15) return 'FAIR';
    if (metrics.connections.errors > 10) return 'FAIR';

    if (metrics.performance.avgQueryTime > 25) return 'GOOD';
    if (metrics.cache.hitRate < 20) return 'GOOD';
  ...
  private generateRecommendations(): string[] {
    ...
    if (metrics.performance.avgQueryTime > 50) {
      recommendations.push(`Average query time ${metrics.performance.avgQueryTime.toFixed(2)}ms exceeds 50ms target - consider database optimization`);
    }

    if (metrics.cache.hitRate < 15) {
      recommendations.push(`Cache hit rate ${metrics.cache.hitRate.toFixed(2)}% is below 15% target - review TTL settings`);
    }

    if (metrics.performance.slowQueries > 100) {
      recommendations.push(`${metrics.performance.slowQueries} slow queries detected - consider index optimization`);
    }

    if (metrics.cache.storageSize > 1024 * 1024 * 1024) { // 1GB
  ...
  private generateStatistics(): {
    ...
    return {
      topFunctions: [
        { name: 'computer_use_click', count: 1500 },
        { name: 'security_validation', count: 1200 },
        { name: 'database_query', count: 800 },
      ],
      riskDistribution: {
        MINIMAL: 40,
        LOW: 30,
