...
import {
  ...
  CurrentUser,
  ByteBotdUser,
} from '../auth/decorators/roles.decorator';import { ParlantValidatedComputerUseService,
  ComputerActionValidationContext 
} from './parlant-validated-computer-use.service';import { ParlantIntegrationService,
  ConversationalValidationError,
  RiskLevel 
} from './parlant-integration.service';import { ComputerActionValidationPipe } from '../computer-use/dto/computer-action-validation.pipe';import { ComputerAction } from '@bytebot/shared';// ===== PARLANT-ENHANCED INTERFACES =====/**
...
export class ParlantComputerActionDto {
  ...
  userSecurityLevel?: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';/** Additional context for validation */validationContext?: {
...
export class ParlantSystemStatusDto {
  ...
  status!: 'HEALTHY' | 'DEGRADED' | 'FAILED';/** Performance metrics */metrics!: {
  ...
  health!: {
    validationService: 'healthy' | 'degraded' | 'failed';cacheService: 'healthy' | 'degraded' | 'failed';auditService: 'healthy' | 'degraded' | 'failed';};/** Last health check timestamp */
  lastHealthCheck!: Date;
}

// ===== PARLANT-ENHANCED CONTROLLER =====

@ApiTags('Parlant Computer Use - Conversational AI Validation')@Controller('parlant/computer-use')@UseGuards(JwtAuthGuard, RolesGuard, EnterpriseRateLimitGuard)@UsePipes(SecuritySanitizationPipes.MAXIMUM_SECURITY)
...
export class ParlantComputerUseController {
  ...
  constructor(
    ...
    this.logger.log('Parlant Computer Use Controller initialized - Conversational validation active');}// ===== VALIDATED COMPUTER ACTION ENDPOINTS =====
   ...
   */
  @Post('action/validated')@OperatorOrAdmin()@ForVersion(SUPPORTED_API_VERSIONS.V1)
  @ApiOperation({
    summary: 'Execute validated computer action',description: 'Execute computer action with Parlant conversational AI validation. Every action is validated through natural language conversation for safety and compliance.',operationId: 'executeValidatedComputerAction',})@ApiBody({
    type: ParlantComputerActionDto,
    description: 'Computer action parameters with conversation context for validation',})@ApiResponse({
    status: 200,
    description: 'Action executed successfully after validation',type: ParlantComputerActionResultDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid action parameters or conversation context',})@ApiResponse({
    status: 401,
    description: 'Authentication required',})@ApiResponse({
    status: 403,
    description: 'Action denied by conversational validation or insufficient permissions',})@ApiResponse({
    status: 409,
    description: 'Conversational validation failed - user intent unclear or high-risk operation denied',})@ApiResponse({
    status: 429,
    description: 'Rate limit exceeded',})@ApiResponse({
    status: 503,
    description: 'Parlant validation service temporarily unavailable',})async executeValidatedAction(
    @Body(new ComputerActionValidationPipe()) params: ParlantComputerActionDto,
    @CurrentUser() user: ByteBotdUser,
    @Headers('x-conversation-id') conversationId?: string,@Headers('x-session-context') sessionContext?: string,
  ): Promise<ParlantComputerActionResultDto> {
    const operationId = `parlant_validated${Date.now()}${Math.random().toString(36).substring(7)}`;const startTime = Date.now();this.logger.log(
      `[${operationId}] Executing Parlant-validated computer action: ${params.action}`,{operationId,
    ...
    try {
      ...
      const validationContext: ComputerActionValidationContext = {
        userId: user.id,
        sessionId: conversationId ?? params.conversationContext?.sessionId ?? `session${Date.now()}`,agentRole: user.role,securityLevel: this.mapUserRoleToSecurityLevel(user.role),
      ...
      this.logger.log(
        `[${operationId}] Parlant-validated action completed successfully`,{operationId,
          actionType: params.action,
          executionTime,
          totalTime,
          userId: user.id,
      ...
      return {
        ...
        validation: {
          ...
          reasoning: 'Action approved through conversational validation',confidence: 0.95, // High confidence for successful execution},
        performance: {
        ...
        throw new HttpException(
          {
            ...
            message: 'Action denied by conversational validation',error: 'Conversational Validation Failed',
            details: {
              reasoning: error.reasoning,
              conversationId: error.conversationId,
      ...
      throw new HttpException(
        {
          ...
          message: 'Computer action execution failed',error: error instanceof Error ? error.message : String(error),},
   ...
   */
  @Post('action/validate')@OperatorOrAdmin()@ForVersion(SUPPORTED_API_VERSIONS.V1)
  @ApiOperation({
    summary: 'Pre-validate computer action',description: 'Validate computer action through Parlant conversational AI without execution. Check if action would be approved and get recommendations.',operationId: 'preValidateComputerAction',})@ApiBody({
    type: ParlantComputerActionDto,
    description: 'Computer action parameters with conversation context for validation',})@ApiResponse({
    status: 200,
    description: 'Validation completed - check approved field for result',type: ParlantValidationResponseDto,})
  @ApiResponse({
    status: 400,
    description: 'Invalid action parameters or conversation context',})@ApiResponse({
    status: 401,
    description: 'Authentication required',})@ApiResponse({
    status: 403,
    description: 'Insufficient permissions',})async preValidateAction(
    @Body(new ComputerActionValidationPipe()) params: ParlantComputerActionDto,
    @CurrentUser() user: ByteBotdUser,
    @Headers('x-conversation-id') conversationId?: string,
  ): Promise<ParlantValidationResponseDto> {
    const operationId = `parlant_prevalidate${Date.now()}${Math.random().toString(36).substring(7)}`;this.logger.log(`[${operationId}] Pre-validating computer action: ${params.action}`,{operationId,
        actionType: params.action,
        userId: user.id,
        conversationId,
      }
    ...
    try {
      ...
      const validationContext: ComputerActionValidationContext = {
        userId: user.id,
        sessionId: conversationId ?? `prevalidate_session${Date.now()}`,agentRole: user.role,securityLevel: this.mapUserRoleToSecurityLevel(user.role),
        conversationHistory: params.conversationContext?.conversationHistory?.map(entry => ({
          timestamp: new Date(entry.timestamp),
          speaker: entry.speaker,
          message: entry.message,
      ...
      const validationRequest = {
        functionName: `ComputerUseService.action.${params.action}`,functionParams: this.sanitizeParamsForValidation(params),actionDescription: this.generateActionDescription(params),
      ...
      this.logger.log(
        `[${operationId}] Pre-validation completed: ${validationResponse.approved ? 'APPROVED' : 'DENIED'}`,{operationId,
      ...
      throw new HttpException(
        {
          statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
          message: 'Validation service error',error: error instanceof Error ? error.message : String(error),},
        HttpStatus.INTERNAL_SERVER_ERROR
      );
    }
  }
   ...
   */
  @Get('status')@OperatorOrAdmin()@ForVersion(SUPPORTED_API_VERSIONS.V1)
  @ApiOperation({
    summary: 'Get Parlant system status',description: 'Get current status and performance metrics for Parlant conversational validation system.',operationId: 'getParlantSystemStatus',})@ApiResponse({
    status: 200,
    description: 'Parlant system status retrieved successfully',type: ParlantSystemStatusDto,})
  async getSystemStatus(): Promise<ParlantSystemStatusDto> {
    try {
      ...
      return {
        ...
        status: 'HEALTHY', // TODO: Implement actual health checkmetrics: {totalOperations: performanceMetrics.totalOperations,
        ...
        health: {
          validationService: 'healthy',cacheService: 'healthy',auditService: 'healthy',},lastHealthCheck: new Date(),
      };
    } catch (error) {
      this.logger.error('Failed to get Parlant system status', { error: error instanceof Error ? error.message : String(error) });throw new HttpException('Failed to retrieve system status',HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
