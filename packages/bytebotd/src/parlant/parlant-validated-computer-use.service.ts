...
import { Injectable, Logger } from '@nestjs/common';import { ComputerUseService } from '../computer-use/computer-use.service';import { ParlantIntegrationService, 
  ParlantValidationRequest,
  ParlantConversationContext,
  RiskLevel,
  ConversationalValidationError
} from './parlant-integration.service';import {ComputerAction,
  MoveMouseAction,
  ClickMouseAction,
  TypeTextAction,
  ApplicationAction,
  WriteFileAction,
  ReadFileAction,
} from '@bytebot/shared';// ===== PARLANT VALIDATION INTERFACES =====/**
...
export class ParlantValidatedComputerUseService {
  ...
  constructor(
    ...
    const operationId = `parlant_computer_init${Date.now()}${Math.random().toString(36).substring(7)}`;this.logger.log(`[${operationId}] Initializing Parlant-Validated Computer Use Service`, {hasOriginalService: !!this.originalComputerUseService,hasParlantService: !!this.parlantIntegrationService,
  ...
  async action(
    ...
    const operationId = `parlant_action${Date.now()}${Math.random().toString(36).substring(7)}`;const startTime = Date.now();this.totalOperations++;
    ...
    try {
      ...
      this.logger.log(
        `[${operationId}] Risk assessment completed: ${riskAssessment.riskLevel}`,{operationId,
      ...
      this.logger.error(
        `[${operationId}] Parlant-validated computer action failed`,{operationId,
          actionType: params.action,
  ...
  private assessActionRisk(params: ComputerAction, context: ComputerActionValidationContext): ActionRiskAssessment {
    ...
    switch (params.action) {
      case 'screenshot':riskLevel = RiskLevel._MINIMAL;break;

      case 'move_mouse':case 'cursor_position':riskLevel = RiskLevel._LOW;break;

      case 'click_mouse':case 'press_mouse':case 'scroll':riskLevel = RiskLevel._LOW;riskFactors.push('user_interface_modification');break;case 'drag_mouse':case 'trace_mouse':riskLevel = RiskLevel._MODERATE;riskFactors.push('complex_mouse_interaction');break;case 'type_keys':case 'press_keys':case 'type_text':case 'paste_text':riskLevel = RiskLevel._MODERATE;riskFactors.push('keyboard_input', 'potential_data_entry');break;case 'application':riskLevel = RiskLevel._HIGH;riskFactors.push('application_control', 'system_state_change');break;case 'write_file':riskLevel = RiskLevel._HIGH;riskFactors.push('file_system_modification', 'data_persistence');break;case 'read_file':riskLevel = RiskLevel._MODERATE;riskFactors.push('file_system_access', 'data_access');break;default:
        riskLevel = RiskLevel._MODERATE;
        riskFactors.push('unknown_action_type');}// Assess context-based risk escalation
    if (context.systemState.securityAlerts.length > 0) {
      riskLevel = this.escalateRiskLevel(riskLevel);
      riskFactors.push('active_security_alerts');}if (context.systemState.maintenanceMode) {
      riskLevel = this.escalateRiskLevel(riskLevel);
      riskFactors.push('maintenance_mode_active');}if (this.hasRecentHighRiskActions(context.recentActions)) {
      riskLevel = this.escalateRiskLevel(riskLevel);
      riskFactors.push('recent_high_risk_activity');}// Check for file operations with sensitive paths
    if (this.involvesSystemFiles(params)) {
      riskLevel = RiskLevel._CRITICAL;
      riskFactors.push('system_file_access');}const mitigationStrategies = this.generateMitigationStrategies(riskLevel, riskFactors);
  ...
  private hasRecentHighRiskActions(recentActions: ComputerActionAuditEntry[]): boolean {
    ...
    return recentActions.some(action => 
      [RiskLevel._HIGH, RiskLevel._CRITICAL].includes(action.riskLevel) &&
  ...
  private involvesSystemFiles(params: ComputerAction): boolean {
    if (params.action === 'write_file' || params.action === 'read_file') {const fileAction = params as WriteFileAction | ReadFileAction;const systemPaths = ['/etc/', '/sys/', '/proc/', '/boot/', 'C:\\Windows\\', 'C:\\System32\\'];return systemPaths.some(path => fileAction.path?.startsWith(path));}
    return false;
  }
  ...
  private generateMitigationStrategies(riskLevel: RiskLevel, riskFactors: string[]): string[] {
    ...
    if (riskFactors.includes('file_system_modification')) {strategies.push('backup_original_file', 'verify_write_permissions');}if (riskFactors.includes('application_control')) {strategies.push('verify_application_state', 'monitor_system_resources');}if (riskFactors.includes('active_security_alerts')) {strategies.push('security_review_required', 'additional_authorization');}if (riskLevel === RiskLevel._CRITICAL) {
  ...
  private generateActionDescription(params: ComputerAction): string {
    switch (params.action) {
      case 'screenshot':return 'Capture screenshot of current screen';case 'move_mouse': {
      ...
      case 'application': {
        ...
        return `Control application: ${appAction.application} (${appAction.action})`;}default:
  ...
  private sanitizeParamsForValidation(params: ComputerAction): Record<string, unknown> {
    ...
    if ('text' in sanitized && typeof sanitized.text === 'string') {// Truncate long text and mask potential passwordslet text = sanitized.text.substring(0, 100);
      if (this.looksLikePassword(text)) {
        text = '[PASSWORD MASKED]';}sanitized.text = text;
    }

    if ('content' in sanitized && typeof sanitized.content === 'string') {// Truncate file content for validationsanitized.content = sanitized.content.substring(0, 200) + (sanitized.content.length > 200 ? '...' : '');
    }
