...
export class ParlantEnvironmentConfigService implements OnModuleInit {
  ...
  private readonly configUpdateCallbacks: Array<(config: ParlantEnvironmentConfig) => void> = [];
  ...
  async onModuleInit(): Promise<void> {
    const operationId = `config_init_${Date.now()}_${Math.random().toString(36).substring(7)}`;try {this.logger.log(`[${operationId}] Loading Parlant environment configuration`);// Load and validate configurationconst config = await this.loadConfiguration();
      ...
      this.logger.log(`[${operationId}] Parlant configuration loaded successfully`, {
      ...
      this.logger.error(`[${operationId}] Failed to initialize Parlant configuration`, {
   ...
   */
  onConfigurationUpdate(callback: (config: ParlantEnvironmentConfig) => void): void {
   ...
   */
  private validateConfiguration(config: ParlantEnvironmentConfig): ConfigValidationResult {
    const errors: string[] = [];
    ...
    for (const configFile of configFiles) {
      ...
      if (fs.existsSync(filePath)) {
        fs.watchFile(filePath, { interval: 1000 }, () => {
          this.logger.log(`Configuration file ${configFile} changed, reloading...`);
          this.reloadConfiguration().catch(error => {
            this.logger.error('Failed to reload configuration after file change', {
              error: error instanceof Error ? error.message : String(error),
            });
          });
        });
