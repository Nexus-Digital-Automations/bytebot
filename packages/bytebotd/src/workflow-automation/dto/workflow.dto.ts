...
export class DataTransformationDto {
  ...
  @ApiPropertyOptional({
    ...
    example: 'data => data.filter(row => row.length > 0).map(row => ({ name: row[0], value: row[1] }))'
  })
  @IsOptional()
  @IsString()
...
export class WorkflowStepDto {
  ...
  @ApiPropertyOptional({
    ...
    example: { loginSuccess: 'result.success', userProfile: 'result.data.profile' }
  })
  @IsOptional()
  @IsObject()
...
export class WorkflowDto {
  ...
  @ApiPropertyOptional({
    ...
    example: 'Automated workflow to collect product data from e-commerce websites'
  })
  @IsOptional()
  @IsString()
  description?: string;
