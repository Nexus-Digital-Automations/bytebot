...
export class SessionController {
  ...
  @ApiBody({
    ...
    description:
      'Browser session configuration with security and performance options',
  })
  @ApiResponse({
  ...
  async getAllSessions(
    ...
    this.logger.debug('Sessions retrieved', {
      ...
      activeSessions: result.sessions.filter(
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('running tasks')) {
        throw new ConflictException({
          ...
          sessionId,
          suggestion:
            'Use force=true to force close or wait for tasks to complete',
        });
      }
  ...
  async bulkSessionOperation(
    ...
    this.logger.log(
      `Executing bulk session operation: ${bulkOperation.operation}`,
      {
        operation: bulkOperation.operation,
        sessionCount:
          bulkOperation.sessionIds?.length ??
          bulkOperation.sessionConfigs?.length ??
          0,
    ...
    try {
      const result =
        await this.sessionService.executeBulkOperation(bulkOperation);
      ...
      this.logger.error(
        `Bulk operation failed: ${bulkOperation.operation}`,
        error,
      );
      throw new InternalServerErrorException({
