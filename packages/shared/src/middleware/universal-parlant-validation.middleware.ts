...
export class UniversalParlantValidationMiddleware implements NestMiddleware {
  private readonly logger = new Logger(
    UniversalParlantValidationMiddleware.name,
  );
  ...
  async use(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      if (
        this.config.bypassDecoratedEndpoints &&
        this.hasExplicitParlantDecorator(req)
      ) {
        this.logger.debug(
          `Bypassing endpoint with explicit Parlant decorator: ${metadata.endpoint}`,
        );
        metadata.bypassReason = "explicit_decorator";
        this.logValidationEvent(metadata, {
          approved: true,
      ...
      const validationResult = await this.performUniversalValidation(
        req,
        metadata,
      );
      ...
      this.logger.warn(
        `Allowing request through due to validation service error: ${metadata.endpoint}`,
      );
      next();
    }
  }
  ...
  private determineSecurityLevel(
    ...
    for (const rule of this.config.riskBasedAssignment.rules) {
      if (
        ...
        new RegExp(rule.pattern, "i").test(endpoint)
      ) {
  ...
  private async performUniversalValidation(
    ...
    if (this.config.enableCaching) {
      ...
      if (cached) {
        this.logger.debug(
          `Using cached validation result: ${metadata.operationId}`,
        );
        return cached;
      }
    }
    ...
    if (
      ...
      this.logger.warn(
        `Validation concurrency limit reached, allowing request: ${metadata.endpoint}`,
      );
      return {
        approved: true,
    ...
    try {
      const validationRequest: ParlantValidationRequest = {
        ...
        parameters: {
          ...
          query: req.query,
          body:
            metadata.method !== "GET" ? this.sanitizeBody(req.body) : undefined,
        },
        userContext: {
  ...
  private setInCache(
    ...
    const rule = this.config.riskBasedAssignment.rules.find(
      (r) =>
