 ...
 * (Renamed to avoid conflict with imported ThreatDetectionRule from express-extensions)
 */
interface EnhancedThreatDetectionRule {
  ruleId: string;
  name: string;
  description: string;
...
interface ThreatAnalysisResult {
  threatsDetected: EnhancedThreatDetectionRule[];
  riskScore: number;
...
interface IPTrackingData {
  ...
  lastSeen: Date;
  requestCount: number;
  requests: number; // Added for compatibility - tracks total requests (same as requestCount)
...
export class EnhancedSecurityMiddleware implements NestMiddleware {
  ...
  private threatDetectionRules: EnhancedThreatDetectionRule[] = [];
  ...
  constructor(
    ...
    this.logger.log(
      "Enhanced Security Middleware initialized with threat detection",
    );
  }
  ...
  use(req: Request, res: Response, next: NextFunction): void {
    const operationId = `enhanced-security-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
    const startTime = Date.now();

    // Set correlation ID
    const extendedReq = req as ExtendedRequest;
    extendedReq.correlationId = operationId;
    extendedReq.securityContext = {};
    ...
    try {
      ...
      const threatAnalysis = this.performThreatDetection(
        req,
        clientIP,
        operationId,
      );
  ...
  private initializeThreatDetectionRules(): void {
    ...
    this.logger.log(
      `Initialized ${this.threatDetectionRules.length} threat detection rules`,
    );
  }
  ...
  private extractClientIP(req: Request): string {
    ...
    const reqWithConnection = req as Request & {
  ...
  private updateIPTracking(
    ...
    if (!ipData) {
      ipData = {
        ...
        lastSeen: now,
        requestCount: 0,
        requests: 0, // Initialize requests to match requestCount
        securityEvents: 0,
        reputation: 0,
        userAgents: new Set(),
        endpoints: new Set(),
    ...
    ipData.lastSeen = now;
    ipData.requestCount++;
    ipData.requests = ipData.requestCount; // Keep requests in sync with requestCount
  ...
  private updateIPReputation(ip: string, ipData: IPTrackingData): void {
    ...
    const timeSpan =
      (ipData.lastSeen.getTime() - ipData.firstSeen.getTime()) / 1000; // seconds
    if (timeSpan > 0) {
    ...
    ipData.reputation = Math.max(
      -100,
      Math.min(100, ipData.reputation + reputationAdjustment),
    );
  ...
  private performThreatDetection(
    ...
    const threatsDetected: EnhancedThreatDetectionRule[] = [];
    let riskScore = 0;
    ...
    for (const rule of this.threatDetectionRules) {
      ...
      try {
        if (rule.pattern instanceof RegExp) {
          ...
          const stringPattern = rule.pattern;
          isMatch = Object.values(requestData).some(
            (data) => typeof data === "string" && data.includes(stringPattern),
          );
        ...
        this.logger.error(
          `[${operationId}] Error in threat detection rule: ${rule.ruleId}`,
          {
            operationId,
            ruleId: rule.ruleId,
            error: err instanceof Error ? err.message : String(err),
  ...
  private countRecentRequests(ip: string, seconds: number): number {
    ...
    return this.recentSecurityEvents.filter(
      (event) => event.ipAddress === ip && event.timestamp > cutoffTime,
    ).length;
  }
  ...
  private createEnhancedSecurityEvent(
    ...
    const baseEvent = this.createSecurityEvent(
      ...
      false,
      `Threat detected: ${threatAnalysis.threatsDetected.map((t: EnhancedThreatDetectionRule) => t.name).join(", ")}`,
      operationId,
    );
    ...
    const enhancedEvent: EnhancedSecurityEvent = {
      ...
      rulesTriggered: threatAnalysis.threatsDetected.map(
        (r: EnhancedThreatDetectionRule) => r.ruleId,
      ),
      responseActions: [],
      correlationData: {
        relatedEvents: this.findCorrelatedEvents(clientIP),
        attackPatterns: this.identifyAttackPatterns(
          threatAnalysis.threatsDetected,
        ),
  ...
  private findCorrelatedEvents(ip: string): string[] {
    ...
    return this.recentSecurityEvents
      .filter(
        (event) => event.ipAddress === ip && event.timestamp > recentCutoff,
      )
  ...
  private createSecurityEvent(
    ...
    const event = createSecurityEvent(
      ...
      {
        operationId,
        correlationId: (req as ExtendedRequest).correlationId,
  ...
  private logSecurityEvent(event: EnhancedSecurityEvent): void {
    ...
    this._eventEmitter.emit("security.threat.detected", event);
  ...
  private executeAutomatedResponse(
    ...
    for (const ruleId of event.rulesTriggered) {
    ...
    switch (action) {
      ...
      case SecurityAction.TEMPORARY_BLOCK:
        this.blockIP(
          clientIP,
          15 * 60 * 1000,
          "Temporary block due to security threat",
        ); // 15 minutes
        this.logger.error(
          `[${operationId}] Temporarily blocked IP: ${clientIP}`,
        );
        break;
      ...
      case SecurityAction.PERMANENT_BLOCK:
        this.blockIP(
          clientIP,
          24 * 60 * 60 * 1000,
          "Permanent block due to critical security threat",
        ); // 24 hours
        this.logger.error(
          `[${operationId}] Permanently blocked IP: ${clientIP}`,
        );
        break;
      ...
      default:
        this.logger.warn(
          `[${operationId}] Unknown security action: ${String(action)}`,
        );
    }
  }
  ...
  private alertSecurityTeam(
    ...
    this._eventEmitter.emit("security.alert.critical", {
      event,
      operationId,
  ...
  private triggerEmergencyLockdown(
    ...
    this._eventEmitter.emit("security.emergency.lockdown", {
      event,
      operationId,
  ...
  private handleBlockedRequest(
    ...
    res.status(403).json({
      error: "Access Denied",
      code: SecurityErrorCode._ACCESS_DENIED,
  ...
  > & {
    ...
    return {
      ...
      responseActionsTriggered: Object.fromEntries(
        this.securityMetrics.responseActionsTriggered,
      ),
    };
  }
   ...
   */
  getIPTrackingData(): Record<
    string,
    Omit<IPTrackingData, "userAgents" | "endpoints"> & {
      userAgents: string[];
      endpoints: string[];
      requests: number;
    }
  > {
    ...
    for (const [ip, data] of Array.from(this.ipTrackingData.entries())) {
      result[ip] = {
        ...
        requests: data.requests, // Include the requests property for compatibility
  ...
  getBlockedIPs(): Record<string, { until: string; reason: string }> {
    const result: Record<string, { until: string; reason: string }> = {};
