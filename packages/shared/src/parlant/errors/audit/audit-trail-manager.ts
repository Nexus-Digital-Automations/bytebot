...
export class EnterpriseAuditTrailManager {
  ...
  private readonly activeInvestigations = new Map<
    string,
  ...
  private readonly complianceFrameworks = new Map<
    string,
    ComplianceFramework
  >();
  ...
  async recordErrorAudit(
    ...
    return await this.recordAuditEvent(
      ...
      },
      {
        ...
        sensitivity: this.mapSecurityLevelToSensitivity(
          errorContext.security?.securityLevel,
        ),
      },
      {
        action: eventType,
  ...
  async collectForensicEvidence(
    ...
    try {
      for (const evidenceType of evidenceTypes) {
        const evidenceId = await this.collectEvidenceByType(
          errorContext,
          evidenceType,
        );
  ...
  async startForensicInvestigation(
    ...
    try {
      ...
      this.logger.error(
        `Failed to start investigation ${investigationId}:`,
        error,
      );
      throw error;
    }
    ...
    try {
      ...
      if (query.actors) {
        filteredRecords = filteredRecords.filter((record) => {
          if (
            query.actors!.types &&
            !query.actors!.types.includes(record.actor.type)
          ) {
            return false;
          }
          if (
            query.actors!.ids &&
            !query.actors!.ids.includes(record.actor.id)
          ) {
            return false;
          }
          if (
            query.actors!.roles &&
            record.actor.role &&
            !query.actors!.roles.includes(record.actor.role)
          ) {
            return false;
          }
          return true;
        ...
        filteredRecords = filteredRecords.filter(
          (record) =>
            ...
            (record.event.errorMessage &&
              record.event.errorMessage.toLowerCase().includes(searchTerm)),
        );
      }
      ...
      const details = complianceFramework.requirements.map((requirement) => {
        ...
        const lastViolation =
          requirementViolations.length > 0
            ? new Date(
                Math.max(
                  ...requirementViolations.map((v) => v.timestamp.getTime()),
                ),
              )
            : undefined;
  ...
  private async createAuditRecord(
    ...
    const riskAssessment = await this.assessRiskAndImpact(
      eventType,
      event,
      context,
    );
    ...
    const retention = await this.determineRetentionRequirements(
      eventType,
      riskAssessment,
    );
  ...
  private mapSecurityLevelToSensitivity(
    securityLevel?: any,
  ...
  private async assessRiskAndImpact(
    eventType: AuditEventType,
    event: any,
    context?: any,
  ): Promise<AuditRecord["riskAssessment"]> {
    return {
      riskLevel: "MEDIUM",
      impactLevel: "MEDIUM",
  ...
  private async determineRetentionRequirements(
    eventType: AuditEventType,
    riskAssessment: any,
  ): Promise<AuditRecord["retention"]> {
    return {
