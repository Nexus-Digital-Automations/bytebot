...
export interface ComplianceAuditEntry
  extends Omit<PreExecutionAuditEntry, "compliance"> {
...
export class ComplianceAuditTrailService implements OnApplicationShutdown {
  ...
  >();
  private readonly complianceReports = new Map<
    string,
  ...
  async createComplianceAuditEntry(
    ...
    try {
      ...
      this.logger.debug("Creating compliance audit entry", {
        ...
        frameworks:
          validationRequest.riskMetadata.compliance.complianceFrameworks,
      });
  ...
  async verifyAuditIntegrity(auditId: string): Promise<{
    ...
    try {
      ...
      const overallValid =
        hashValid && hmacValid && chainValid && signatureValid;
  ...
  async searchAuditTrail(searchQuery: {
    ...
    try {
      ...
      if (searchQuery.query) {
        candidateEntries = candidateEntries.filter((entry) =>
          entry.searchMetadata.searchableContent
            .toLowerCase()
      ...
      candidateEntries.sort(
        (a, b) => b.auditTimestamp.getTime() - a.auditTimestamp.getTime(),
      );
    ...
    return {
      ...
      compliance: {
        framework:
          validationRequest.riskMetadata.compliance.complianceFrameworks,
        requirements:
          validationResponse.riskAssessment.validationRequirements.map(
            (req) => req.description,
          ),
        evidence: {
  ...
  private async buildComplianceMetadata(
    ...
    const retentionPeriodYears = this.calculateRetentionPeriod(
      frameworks,
      dataSensitivity,
    );
    ...
    return {
      ...
      consentStatus: await this.getConsentStatus(
        validationRequest.userContext.userId,
      ),
  ...
  private async applyPrivacyProtection(
    ...
    return {
      ...
      dataSubjectId: this.generateDataSubjectId(
        validationRequest.userContext.userId,
      ),
    };
  }
  ...
  private async generateIntegrityProof(
    ...
    const entryHash = createHash("sha256")
      .update(entryJson + salt)
      .digest("hex");

    // Generate HMAC signature
    const hmacKey = this.configService.get<string>(
      "PARLANT_AUDIT_HMAC_KEY",
      "default-hmac-key",
    );
  ...
  private async updateAuditIndex(
    auditEntry: ComplianceAuditEntry,
  ): Promise<void> {
  ...
  private async performComplianceMonitoring(
    auditEntry: ComplianceAuditEntry,
  ): Promise<void> {
    ...
    switch (condition.type) {
      ...
      case "custom":
        return condition.customCondition
          ? condition.customCondition(auditEntry)
          : false;
      default:
        return false;
    }
  ...
  private verifyHmacSignature(auditEntry: ComplianceAuditEntry): boolean {
    ...
    return (
      computedHmac === auditEntry.immutableMetadata.integrityProof.hmacSignature
    );
  }
  ...
  private generateDigitalSignature(auditEntry: PreExecutionAuditEntry): string {
    ...
    return createHash("sha256")
      .update(entryData + "signature-salt")
      .digest("hex");
  }
  ...
  private async buildReportData(
    ...
    const reportData: any = {
      ...
      entries: auditEntries.map((entry) =>
        this.sanitizeForReport(entry, reportConfig.framework),
      ),
  ...
  private calculateReportStatistics(auditEntries: ComplianceAuditEntry[]): any {
    ...
    const approvedEntries = auditEntries.filter(
      (e) => e.response.decision === "APPROVED",
    ).length;
    const rejectedEntries = auditEntries.filter(
      (e) => e.response.decision === "REJECTED",
    ).length;

    const riskDistribution = auditEntries.reduce(
      (dist, entry) => {
    ...
    return {
      ...
      riskDistribution,
      averageRiskScore:
        auditEntries.reduce((sum, e) => sum + e.riskAssessment.riskScore, 0) /
        totalEntries,
    };
  }
  ...
  private assessComplianceStatus(
    ...
    const violations = auditEntries.filter(
      (entry) =>
        entry.compliance.frameworks.includes(framework) &&
  ...
  private hasComplianceViolation(
    entry: ComplianceAuditEntry,
    framework: string,
  ): boolean {
    ...
    switch (framework) {
      case "GDPR":
        return (
          !entry.compliance.consentStatus?.consentGiven &&
          entry.compliance.dataSensitivity === "personal"
        );
      case "HIPAA":
        return (
          entry.compliance.dataSensitivity === "health" &&
          entry.riskAssessment.riskScore > 70
        );
      case "SOC2":
        return (
          !entry.compliance.frameworks.includes("SOC2") &&
  ...
  private calculateRetentionPeriod(
    ...
    for (const framework of frameworks) {
      const frameworkRetention =
        this.config.retention.frameworkPolicies[framework];
  ...
  private determineProcessingPurpose(
  ...
  private async getConsentStatus(
    userId: string,
  ): Promise<ConsentStatus | undefined> {
  ...
  private evaluateAnonymizationCondition(
    condition: string,
    request: PreExecutionValidationRequest,
  ): boolean {
    ...
    if (
      condition === "gdpr_applicable" &&
      request.riskMetadata.compliance.complianceFrameworks.includes("GDPR")
    ) {
      return true;
    }
    return false;
  ...
  private applyHashAnonymization(
    auditEntry: PreExecutionAuditEntry,
    rule: AnonymizationRule,
  ): void {
  ...
  private applyMaskAnonymization(
    auditEntry: PreExecutionAuditEntry,
    rule: AnonymizationRule,
  ): void {
    ...
    if (auditEntry.request.userContext.sessionContext.ipAddress) {
      ...
      auditEntry.request.userContext.sessionContext.ipAddress = ip
        .split(".")
  ...
  private applyGeneralizationAnonymization(
    auditEntry: PreExecutionAuditEntry,
    rule: AnonymizationRule,
  ): void {
    ...
    if (auditEntry.request.userContext.sessionContext.userAgent) {
      ...
      if (userAgent.includes("Chrome")) {
        auditEntry.request.userContext.sessionContext.userAgent =
          "Chrome Browser";
  ...
  private applySuppressionAnonymization(
    auditEntry: PreExecutionAuditEntry,
    rule: AnonymizationRule,
  ): void {
  ...
  private generateDataSubjectId(userId: string): string {
    return createHash("sha256")
      .update(userId + "data-subject-salt")
      .digest("hex")
      .substring(0, 16);
  }
  ...
  private loadComplianceConfiguration(): ComplianceAuditTrailConfig {
    return {
      enabled: this.configService.get<boolean>(
        "PARLANT_COMPLIANCE_AUDIT_ENABLED",
        true,
      ),
      supportedFrameworks: [
        {
        ...
        },
        {
          ...
          retentionRequirements: {
            minimumYears: 3,
            archivalAfterYears: 1,
            destructionAfterYears: 7,
          },
          privacyRequirements: {
            dataMinimization: true,
