...
export class UniversalAPIMiddleware {
  ...
  constructor(conversationalController: ConversationalAPIController) {
    ...
    this.logger.log(
      "UniversalAPIMiddleware initialized with multi-framework support",
    );
   ...
   */
  conversationalAPI(
    config: { enabled?: boolean; monitoring?: boolean; caching?: boolean } = {},
  ) {
    return (
      target: any,
      propertyName: string,
      ...
      descriptor.value = async function (...args: any[]) {
        ...
        try {
          ...
          if (
            requestContext &&
            this.shouldProcessConversationally(requestContext)
          ) {
            // Process through conversational controller
          ...
          this.logger.error(
            `Conversational decorator error on ${propertyName}`,
            error.stack,
          );
  ...
  private async handleConversationalRequest(
    ...
    try {
      ...
      const naturalLanguageRequest =
        this.extractNaturalLanguageRequest(context);
      ...
      const conversationalResponse =
        await this.conversationalController.processNaturalLanguageRequest(
          apiRequest,
        );
  ...
  private shouldHandleConversationally(path: string): boolean {
    ...
    return this.config.conversationalRoutes.some((route) =>
   ...
   */
  private extractNaturalLanguageRequest(
    context: MiddlewareContext,
  ): string | null {
  ...
  private async extractUserContextFromExpress(req: any): Promise<any> {
    return {
      ...
      preferences: {
        ...
        notificationMethod:
          req.user?.preferences?.notificationMethod || "IMMEDIATE",
  ...
  private recordMetrics(
    ...
    existing.averageProcessingTime =
      existing.totalProcessingTime / existing.totalRequests;
...
export const createExpressConversationalMiddleware = (
  controller: ConversationalAPIController,
...
export const createFastAPIConversationalMiddleware = (
  controller: ConversationalAPIController,
  config: MiddlewareConfig,
) => {
  const middleware = new UniversalAPIMiddleware(controller);
...
export const createNextJSConversationalMiddleware = (
  controller: ConversationalAPIController,
  config: MiddlewareConfig,
) => {
  const middleware = new UniversalAPIMiddleware(controller);
...
export const createKoaConversationalMiddleware = (
  controller: ConversationalAPIController,
  config: MiddlewareConfig,
) => {
  const middleware = new UniversalAPIMiddleware(controller);
