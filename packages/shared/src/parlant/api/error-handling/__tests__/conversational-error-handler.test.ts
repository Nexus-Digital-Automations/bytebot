...
describe("ConversationalErrorHandler", () => {
  ...
  beforeEach(async () => {
    ...
    errorHandler = module.get<ConversationalErrorHandler>(
      ConversationalErrorHandler,
    );
    nlpProcessor = module.get<ErrorNaturalLanguageProcessor>(
      ErrorNaturalLanguageProcessor,
    );
  ...
  describe("Error Processing", () => {
    const createTestContext = (
      overrides: Partial<ConversationalErrorContext> = {},
    ): ConversationalErrorContext => ({
    ...
    it("should process UnauthorizedException correctly", async () => {
      ...
      expect(result.guidance.immediateActions[0].action).toContain(
        "logging in",
      );
    });
    ...
    it("should handle processing errors gracefully", async () => {
      ...
      jest
        .spyOn(nlpProcessor, "processErrorToNaturalLanguage")
        .mockRejectedValue(new Error("NLP failed"));
  ...
  describe("Guidance Generation", () => {
    ...
    it("should generate escalation guidance", async () => {
      ...
      expect(result.guidance.escalationGuidance?.expectedResponse).toContain(
        "hours",
      );
    });
  ...
  describe("Recovery Recommendations", () => {
    it("should generate recovery recommendations sorted by confidence", async () => {
      ...
      for (let i = 0; i < result.recoveryRecommendations.length - 1; i++) {
        expect(
          result.recoveryRecommendations[i].confidence,
        ).toBeGreaterThanOrEqual(
          result.recoveryRecommendations[i + 1].confidence,
        );
      }
...
describe("ErrorNaturalLanguageProcessor", () => {
  ...
  describe("Natural Language Processing", () => {
    it("should process BadRequestException to natural language", async () => {
      ...
      // Act
      const result = await nlpProcessor.processErrorToNaturalLanguage(
        error,
        context,
      );
    ...
    it("should adapt language for different expertise levels", async () => {
      ...
      // Act
      const beginnerResult = await nlpProcessor.processErrorToNaturalLanguage(
        error,
        beginnerContext,
      );
      const expertResult = await nlpProcessor.processErrorToNaturalLanguage(
        error,
        expertContext,
      );
