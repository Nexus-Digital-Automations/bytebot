...
export class EnterpriseIntegration {
  ...
  async processEnterpriseRequest(request: any): Promise<any> {
    this.logger.debug("Processing enterprise request", {
   ...
   */
  async validateInterventionPermission(
    context: any,
    interventionRequest?: any,
  ): Promise<any> {
    this.logger.debug("Validating intervention permission", {
      userId: context?.userId,
      interventionRequest,
    });
    return {
