...
export class ParlantValidationInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return this.performValidation(context, validationConfig, operationId)
      .pipe(
        switchMap((validationResult) => {
          ...
          this.logger.log(
            `[${operationId}] PARLANT validation approved`,
            {
              operationId,
        ...
        catchError((error) => {
          ...
          if (error instanceof ConversationalValidationError) {
            this.logger.warn(
              `[${operationId}] PARLANT validation denied`,
              {
                operationId,
                intent: validationConfig.intent,
            ...
            const httpError = new HttpException(
              {
                ...
                guidance: this.generateConversationalGuidance(error, validationConfig),
                metadata: {
                  operationId,
          ...
          this.logger.error(
            `[${operationId}] PARLANT validation error`,
            {
              operationId,
              intent: validationConfig.intent,
  ...
  private async executeValidation(
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('Validation timeout')), config.timeout || 10000);
    });
    ...
    if (config.cacheable && validationResult.approved) {
      const cacheKey = this.generateCacheKey(config, user, request);
      this.setCachedValidation(cacheKey, validationResult, config.timeout || 10000);
    }
   ...
   */
  private generateCacheKey(config: ParlantValidationConfig, user: any, request: any): string {
    const keyData = {
