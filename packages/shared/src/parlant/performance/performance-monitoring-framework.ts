...
class TimeSeriesCollector {
  ...
  calculateStatistics(metricName: string, since?: Date): {
    count: number;
    mean: number;
    min: number;
    max: number;
    ...
    if (values.length === 0) {
      return {
        ...
        percentiles: {
          ...
          mean: 0, median: 0, min: 0, max: 0, standardDeviation: 0
        }
      };
    }
  ...
  private calculatePercentiles(sortedValues: number[]): ResponseTimeMetrics {
    ...
    return {
      ...
      standardDeviation: Math.sqrt(
        sortedValues.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / sortedValues.length
      )
    };
  }
}
...
class PerformanceBenchmarker {
  ...
  async runBenchmark(
    ...
    // Detect outliers
    const outliers = config.outlierDetection ? this.detectOutliers(times, standardDeviation, averageTime) : [];
    ...
    this.logger.log(`Benchmark ${name} completed: ${averageTime.toFixed(2)}ms avg, ${result.operationsPerSecond.toFixed(0)} ops/sec`);

    return result;
  }

  saveBaseline(name: string, metrics: PerformanceMetrics, benchmarks: Map<string, BenchmarkResult>): void {
  ...
  compareWithBaseline(name: string, currentResult: BenchmarkResult): {
    ...
    const percentageChange = ((currentResult.averageTime - baselineResult.averageTime) / baselineResult.averageTime) * 100;
  ...
  private calculateConfidenceInterval(values: number[], confidenceLevel: number): {
    lower: number;
    upper: number;
    level: number;
  } {
  ...
  private detectOutliers(values: number[], standardDeviation: number, mean: number): number[] {
    const threshold = 2 * standardDeviation;
    return values.filter(value => Math.abs(value - mean) > threshold);
  }

  private detectRegression(benchmarkName: string, currentTime: number): {
    detected: boolean;
    severity: 'none' | 'minor' | 'major' | 'critical';
    percentageChange: number;
  } {
    ...
    const percentageChange = ((currentTime - baselineTime) / baselineTime) * 100;
...
class PerformanceAlertManager {
  ...
  checkMetricsForAlerts(metrics: PerformanceMetrics, config: AlertingConfig): PerformanceAlert[] {
  ...
  private checkResourceAlerts(
    ...
    if (resourceMetrics.memory.utilization > config.resourceThresholds.memoryCritical) {
      alerts.push({
...
export class PerformanceMonitoringFramework {
  ...
  async runBenchmark(
    ...
    this.recordMetric(`benchmark.${name}.operations_per_second`, result.operationsPerSecond);
   ...
   */
  async generateReport(type: 'summary' | 'detailed' | 'benchmark', period?: {
    start: Date;
    end: Date;
  }): Promise<any> {
    const reportData = {
  ...
  private collectCurrentMetrics(): void {
    ...
    // Check for alerts
    this.alertManager.checkMetricsForAlerts(this.currentMetrics, this.config.alerting);
  }

  private calculateResponseTimeMetrics(): ResponseTimeMetrics {
    const responseTimeData = this.timeSeriesCollector.getTimeSeriesData('response_time');
    const statistics = this.timeSeriesCollector.calculateStatistics('response_time');

    return statistics.percentiles;
  }

  private calculateThroughputMetrics(): ThroughputMetrics {
    const rpsData = this.timeSeriesCollector.getTimeSeriesData('requests_per_second');
    const latestRps = rpsData.length > 0 ? rpsData[rpsData.length - 1].value : 0;
  ...
  private startDashboardUpdates(): void {
    ...
    this.dashboardInterval = setInterval(() => {
      const dashboardData = {
        ...
        timeSeries: {
          responseTime: this.timeSeriesCollector.getTimeSeriesData('response_time'),
          throughput: this.timeSeriesCollector.getTimeSeriesData('requests_per_second'),
    ...
    return {
      ...
      detailed: {
        ...
        timeSeries: this.timeSeriesCollector.getAllMetrics().map(metric => ({
          metric,
          data: this.timeSeriesCollector.getTimeSeriesData(metric, reportData.period.start)
        })),
        alerts: Array.from(this.alertManager.activeAlerts.values()),
  ...
  private calculateAverageResponseTime(metrics: PerformanceMetrics[]): number {
    return metrics.reduce((sum, m) => sum + m.responseTime.mean, 0) / metrics.length;
  }

  private calculateAverageThroughput(metrics: PerformanceMetrics[]): number {
    return metrics.reduce((sum, m) => sum + m.throughput.requestsPerSecond, 0) / metrics.length;
  }
  ...
  private initializeMetrics(): PerformanceMetrics {
    return {
      ...
      resource: {
        ...
        memory: {
          used: 0, total: totalmem(), utilization: 0,
          heapUsed: 0, heapTotal: 0, external: 0, rss: 0
        },
        disk: {
          reads: 0, writes: 0, readThroughput: 0,
          writeThroughput: 0, utilization: 0
        },
        network: {
          bytesIn: 0, bytesOut: 0, packetsIn: 0,
          packetsOut: 0, connectionsActive: 0, connectionsWaiting: 0
        }
      },
      error: {
        totalErrors: 0, errorRate: 0, errorsByType: new Map(),
        errorsByEndpoint: new Map(), criticalErrors: 0,
        warningErrors: 0, timeoutErrors: 0, connectionErrors: 0
      },
      custom: {}
    };
  }
