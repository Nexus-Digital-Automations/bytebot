...
export class FunctionDiscoveryService implements IFunctionDiscovery {
  ...
  async discover(
    ...
    logger.debug(
      `Discovery configuration: ${JSON.stringify(configuration, null, 2)}`,
    );
    ...
    try {
      ...
      const processPromises = configuration.parallel
        ? filesToScan.map((filePath) =>
            this.processFile(filePath, configuration, errors),
          )
        : [];
      ...
      if (configuration.parallel) {
        ...
        for (const filePath of filesToScan) {
          try {
            const functions = await this.processFile(
              filePath,
              configuration,
              errors,
            );
      ...
      statistics.functionsRegistered = discoveredFunctions.filter(
        (f) => f.confidence >= configuration.confidenceThreshold,
      ).length;
      statistics.functionsSkipped =
        discoveredFunctions.length - statistics.functionsRegistered;
      statistics.errors = errors.length;
      statistics.averageConfidence =
        discoveredFunctions.length > 0
          ? discoveredFunctions.reduce((sum, f) => sum + f.confidence, 0) /
            discoveredFunctions.length
          : 0;
      ...
      logger.log(
        `Discovery completed in ${duration}ms. Found ${discoveredFunctions.length} functions with ${errors.length} errors`,
      );

      return {
        functions: discoveredFunctions.filter(
          (f) => f.confidence >= configuration.confidenceThreshold,
        ),
        metadata,
        statistics,
  ...
  async discoverInFile(filePath: string): Promise<FunctionDiscoveryEntry[]> {
    ...
    try {
      const defaultConfig: DiscoveryConfiguration = {
        methods: [
          DiscoveryMethod._AST_PARSING,
          DiscoveryMethod._STATIC_ANALYSIS,
        ],
      ...
      this.logger.error(
        `Failed to discover functions in file ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return [];
    }
  }
  ...
  async discoverInDirectory(
    ...
    this.logger.debug(
      `Discovering functions in directory: ${directoryPath} (recursive: ${recursive})`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `Discovered ${allFunctions.length} functions in directory ${directoryPath}`,
      );
      return allFunctions;
    } catch (error) {
      this.logger.error(
        `Failed to discover functions in directory ${directoryPath}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return [];
    }
  }
  ...
  async rediscover(functionIds: string[]): Promise<RediscoveryResult> {
    ...
    this.logger.log(
      `Rediscovery completed: ${result.successfulRediscoveries} successful, ${result.failedRediscoveries} failed`,
    );
    return result;
  }
  ...
  validateConfiguration(
    ...
    if (
      configuration.confidenceThreshold < 0 ||
      configuration.confidenceThreshold > 1
    ) {
      errors.push({
    ...
    if (configuration.parallel && configuration.timeout < 60000) {
      recommendations.push(
        "Consider increasing timeout for parallel processing",
      );
    }
  ...
  private async getFilesToScan(
    ...
    for (const basePath of scope.paths) {
      const scopedFiles = await this.scanDirectory(
        basePath,
        scope.maxDepth > 1,
      );
      files.push(
        ...scopedFiles.filter((file) => this.matchesScope(file, scope)),
      );
    }
  ...
  private matchesScope(filePath: string, scope: DiscoveryScope): boolean {
    ...
    const includeMatch = scope.includePatterns.some((pattern) =>
      this.matchesPattern(filePath, pattern),
    );
    if (!includeMatch) return false;

    // Check if file matches exclude patterns
    const excludeMatch = scope.excludePatterns.some((pattern) =>
      this.matchesPattern(filePath, pattern),
    );
    return !excludeMatch;
  }
  ...
  private async processFile(
    ...
    try {
      ...
      for (const method of configuration.methods) {
        const methodFunctions = await this.discoverWithMethod(
          filePath,
          content,
          method,
        );
  ...
  private async discoverWithAST(
    ...
    try {
      ...
      this.logger.warn(
        `AST parsing failed for ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
   ...
   */
  private async discoverWithStaticAnalysis(
    filePath: string,
    content: string,
  ): Promise<FunctionDiscoveryEntry[]> {
    const functions: FunctionDiscoveryEntry[] = [];
   ...
   */
  private async discoverWithAnnotations(
    filePath: string,
    content: string,
  ): Promise<FunctionDiscoveryEntry[]> {
    const functions: FunctionDiscoveryEntry[] = [];
    ...
    const jsdocPattern =
      /\/\*\*[\s\S]*?\*\/\s*(?:export\s+)?(?:async\s+)?function\s+([a-zA-Z_$][a-zA-Z0-9_$]*)/g;
   ...
   */
  private async discoverWithReflection(
    filePath: string,
    content: string,
  ): Promise<FunctionDiscoveryEntry[]> {
    const functions: FunctionDiscoveryEntry[] = [];
    ...
    this.logger.debug(
      `Runtime reflection discovery not implemented for security reasons: ${filePath}`,
    );
  ...
  private extractFunctionInfo(
    ...
    try {
      ...
      this.logger.warn(
        `Failed to extract function info from AST node: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
   ...
   */
  private extractSignatureFromAST(
    node: any,
    functionName: string,
  ): FunctionSignature {
   ...
   */
  private mergeFunctionDiscoveries(
