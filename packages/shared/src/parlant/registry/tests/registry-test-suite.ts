...
export class MockDataGenerator {
  static generateFunctionRegistryEntry(
    overrides?: Partial<FunctionRegistryEntry>,
  ): FunctionRegistryEntry {
    return {
  ...
  static generateFunctionDiscoveryEntry(
    overrides?: Partial<FunctionDiscoveryEntry>,
  ): FunctionDiscoveryEntry {
    return {
...
export class RegistryTestSuite {
  ...
  async runUnitTests(): Promise<void> {
    ...
    await this.runTest(
      "function-discovery-unit",
      TestCaseType._UNIT,
      async (context) => {
        const discoveryService =
          this.testingModule.get<FunctionDiscoveryService>(
            FunctionDiscoveryService,
          );
        ...
        return {
          assertions: [
            ...
            {
              description:
                "Discovery service should support parallel processing",
              expected: true,
    ...
    await this.runTest(
      "auto-registration-unit",
      TestCaseType._UNIT,
      async (context) => {
        const registrationService =
          this.testingModule.get<AutoRegistrationService>(
            AutoRegistrationService,
          );
        ...
        const result = await registrationService.registerFunction(
          mockFunction,
          mockConfig,
        );
  ...
  async runIntegrationTests(): Promise<void> {
    ...
    await this.runTest(
      "discovery-registration-integration",
      TestCaseType._INTEGRATION,
      async (context) => {
        const discoveryService =
          this.testingModule.get<FunctionDiscoveryService>(
            FunctionDiscoveryService,
    ...
    await this.runTest(
      ...
      async (context) => {
        ...
        const sensitiveFunction =
          MockDataGenerator.generateFunctionDiscoveryEntry({
            name: "deleteAllUsers",
          });
  ...
  async runEndToEndTests(): Promise<void> {
    ...
    await this.runTest(
      ...
      async (context) => {
        ...
        const configService =
          this.testingModule.get<ConfigurationManagerService>(
            ConfigurationManagerService,
          );
        const healthService =
          this.testingModule.get<HealthMonitorService>(HealthMonitorService);
        const versionService = this.testingModule.get<VersionManagerService>(
          VersionManagerService,
        );
        ...
        const registrationResult = await registrationService.registerFunction(
          mockFunction,
          registrationConfig,
        );
        ...
        const configResult = await configService.updateConfiguration(
          functionId,
          configUpdates,
        );
  ...
  private async runTest(
    ...
    try {
      ...
      if (failedAssertions.length > 0) {
        ...
        console.log(
          `    ❌ ${testName} - ${failedAssertions.length} assertion(s) failed`,
        );
      } else {
  ...
  private generateSummaryReport(): void {
    ...
    const passedTests = Array.from(this.testResults.values()).filter(
      (r) => r.status === TestStatus._PASSED,
    ).length;
    const failedTests = Array.from(this.testResults.values()).filter(
      (r) => r.status === TestStatus._FAILED,
    ).length;
    const errorTests = Array.from(this.testResults.values()).filter(
      (r) => r.status === TestStatus._ERROR,
    ).length;
    ...
    // Performance summary
    const performanceTests = Array.from(this.testResults.values()).filter(
      (r) => r.testType === TestCaseType._PERFORMANCE,
    );
    if (performanceTests.length > 0) {
      const avgExecutionTime =
        performanceTests.reduce(
          (sum, test) => sum + test.performance.executionTime,
          0,
    ...
    if (this.config.coverage) {
      const avgCoverage =
        Array.from(this.testResults.values()).reduce(
          (sum, test) => sum + test.coverage.percentage,
          0,
        ) / totalTests;
      console.log(`\nCoverage Summary:`);
