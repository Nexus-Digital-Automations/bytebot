...
export class ImmutableAuditTrail {
  ...
  private readonly eventIndex: Map<
    string,
    { blockNumber: number; eventIndex: number }
  > = new Map();
    ...
    try {
      ...
      const auditEvent: AuditEvent = {
        ...
        compliance: {
          ...
          dataClassification:
            eventData.compliance?.dataClassification || "internal",
      ...
      this.logger.debug(
        `Audit event recorded: ${eventId} - Type: ${auditEvent.eventType}`,
      );
  ...
  private async createAuditBlock(): Promise<string> {
    ...
    try {
      ...
      this.logger.log(
        `Audit block created: ${auditBlock.blockId} - Events: ${eventsToInclude.length}, Block: ${blockNumber}`,
      );
  ...
  public async queryEvents(query: AuditQuery): Promise<AuditQueryResult> {
    ...
    try {
      ...
      this.logger.debug(
        `Audit query completed - Results: ${paginatedEvents.length}/${totalCount}, Time: ${queryTime}ms`,
      );
    ...
    try {
      ...
      const integrityScore =
        this.auditChain.length > 0
          ? verifiedBlocks / this.auditChain.length
          : 1;
      ...
      this.logger.log(
        `Chain integrity verification completed - Score: ${(integrityScore * 100).toFixed(1)}%, Time: ${verificationTime}ms`,
      );
  ...
  private async verifyBlock(
    ...
    try {
      ...
      if (calculatedHash !== block.blockHash) {
        this.logger.warn(
          `Block hash mismatch in block ${block.blockNumber}: ${block.blockId}`,
        );
        return false;
      }
      ...
      if (!isSignatureValid) {
        this.logger.warn(
          `Block signature invalid in block ${block.blockNumber}: ${block.blockId}`,
        );
        return false;
      }
      ...
      if (calculatedMerkleRoot !== block.merkleRoot) {
        this.logger.warn(
          `Merkle root mismatch in block ${block.blockNumber}: ${block.blockId}`,
        );
        return false;
      }
      ...
      this.logger.error(
        `Block verification failed for block ${block.blockNumber}`,
        error,
      );
      return false;
    }
  ...
  private applyFilters(events: AuditEvent[], query: AuditQuery): AuditEvent[] {
    return events.filter((event) => {
      ...
      // Actor filter
      if (query.actorIds && !query.actorIds.includes(event.actor.id))
        return false;
      ...
      if (
        query.targetIds &&
        event.target &&
        !query.targetIds.includes(event.target.id)
      )
        return false;

      // Source filter
      if (query.sources && !query.sources.includes(event.context.source))
        return false;
      ...
      if (
        ...
        !query.correlationIds.includes(event.context.correlationId)
      )
        return false;
      ...
      if (
        query.frameworks &&
        !query.frameworks.some((f) => event.compliance.frameworks.includes(f))
      )
        return false;
      ...
      if (
        ...
        !query.dataClassifications.includes(event.compliance.dataClassification)
      )
        return false;
