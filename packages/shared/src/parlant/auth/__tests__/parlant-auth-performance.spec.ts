...
import { ParlantSessionManager, ParlantSession } from '../parlant-session-manager.service';
...
describe('PARLANT Authentication Performance Testing Suite', () => {
  ...
  beforeAll(async () => {
    ...
    jwtBridgeService = module.get<ParlantJWTBridgeService>(ParlantJWTBridgeService);
    sessionManager = module.get<ParlantSessionManager>(ParlantSessionManager);
    securityValidator = module.get<ParlantSecurityValidator>(ParlantSecurityValidator);
   ...
   */
  function calculatePerformanceStats(responseTimes: number[]): PerformanceTestResult['responseTimeStats'] {
  ...
  describe('Token Exchange Performance Testing', () => {
    it('should validate token exchange latency under load', async () => {
      ...
      for (const concurrentUsers of performanceConfig.loadTestParams.concurrentUsers) {
        ...
        const responseTimeStats = calculatePerformanceStats(loadTestResult.responseTimes);
        ...
        const result: PerformanceTestResult = {
          ...
          errorStats: {
            ...
            errorRate: loadTestResult.errors.length / loadTestResult.totalRequests,
            errorTypes: {},
          },
          resourceStats: {
          ...
          complianceResults: {
            meetsP95Target: responseTimeStats.p95 < performanceConfig.responseTimeTargets.tokenExchange,
            meetsP99Target: responseTimeStats.p99 < performanceConfig.responseTimeTargets.tokenExchange * 2,
            meetsThroughputTarget: throughputStats.averageRps > 100, // 100 RPS minimum
            meetsErrorRateTarget: (loadTestResult.errors.length / loadTestResult.totalRequests) < 0.01,
          },
        };

        testResults.push(result);

        console.log(`  P95: ${responseTimeStats.p95.toFixed(2)}ms, RPS: ${throughputStats.averageRps.toFixed(2)}, Errors: ${loadTestResult.errors.length}`);

        // Validate performance targets
        expect(responseTimeStats.p95).toBeLessThan(performanceConfig.responseTimeTargets.tokenExchange);
        expect(responseTimeStats.p99).toBeLessThan(performanceConfig.responseTimeTargets.tokenExchange * 2);
        expect(loadTestResult.errors.length / loadTestResult.totalRequests).toBeLessThan(0.01);
      }
    ...
    it('should maintain performance under sustained load', async () => {
      ...
      try {
        ...
        const finalSnapshot = performanceSnapshots[performanceSnapshots.length - 1];

        if (initialSnapshot && finalSnapshot) {
          // Check for performance degradation
          const responseDegradation = (finalSnapshot.p95ResponseTime - initialSnapshot.p95ResponseTime) / initialSnapshot.p95ResponseTime;
          const memoryGrowth = finalSnapshot.memoryUsage - initialSnapshot.memoryUsage;

          expect(responseDegradation).toBeLessThan(0.2); // Less than 20% degradation
          expect(memoryGrowth).toBeLessThan(performanceConfig.memoryTargets.maxHeapIncrease);

          console.log(`  Performance degradation: ${(responseDegradation * 100).toFixed(2)}%`);
    ...
    it('should validate concurrent session handling performance', async () => {
      ...
      const sessionPromises = Array.from({ length: maxConcurrentSessions }, async (_, index) => {
        const startTime = performance.now();
      ...
      // Validate performance targets
      expect(creationStats.p95).toBeLessThan(performanceConfig.responseTimeTargets.sessionCreation);
      expect(validationStats.p95).toBeLessThan(performanceConfig.responseTimeTargets.sessionValidation);

      console.log(`  Session creation P95: ${creationStats.p95.toFixed(2)}ms`);
      console.log(`  Session validation P95: ${validationStats.p95.toFixed(2)}ms`);
  ...
  describe('Cache Performance Testing', () => {
    it('should validate cache performance and hit rates', async () => {
      ...
      expect(cacheHitRate).toBeGreaterThan(performanceConfig.cacheTargets.hitRate);
      expect(cacheStats.p95).toBeLessThan(performanceConfig.cacheTargets.lookupTime);
    ...
    it('should test cache performance under memory pressure', async () => {
      ...
      try {
        ...
        for (let i = 0; i < 1000; i++) {
          largeDataSets.push(new Array(10000).fill(`memory-pressure-data-${i}`));
        }
        ...
        const pressureStats = calculatePerformanceStats(cachePerformanceUnderPressure);

        console.log(`  P95 under memory pressure: ${pressureStats.p95.toFixed(2)}ms`);

        // Should still meet performance targets under pressure
        expect(pressureStats.p95).toBeLessThan(performanceConfig.responseTimeTargets.tokenExchange * 1.5); // 50% tolerance
  ...
  describe('Security Validation Performance Testing', () => {
    it('should validate security validation performance under load', async () => {
      ...
      for (const testCase of securityTestCases) {
        ...
        const securityStats = calculatePerformanceStats(securityValidationTimes);

        console.log(`    P95: ${securityStats.p95.toFixed(2)}ms, Expected: < ${testCase.expectedTime}ms`);
    ...
    it('should test concurrent security validations', async () => {
      ...
      const validationPromises = Array.from({ length: concurrentValidations }, async (_, index) => {
        const startTime = performance.now();
      ...
      console.log(`  Concurrent validations P95: ${concurrentStats.p95.toFixed(2)}ms`);
      console.log(`  Total validations: ${validationTimes.length}`);

      // Should handle concurrent validations efficiently
      expect(concurrentStats.p95).toBeLessThan(performanceConfig.responseTimeTargets.securityValidation * 2);
  ...
  describe('Memory Usage and Resource Management', () => {
    it('should validate memory usage patterns', async () => {
      ...
      const initialMemory = process.memoryUsage();
      const memorySnapshots: Array<{ iteration: number; heapUsed: number; timestamp: number }> = [];
      ...
      if (memorySnapshots.length >= 2) {
        ...
        const growthRate = (lastSnapshot.heapUsed - firstSnapshot.heapUsed) / (testIterations / 100);

        console.log(`  Memory growth rate: ${growthRate.toFixed(4)}MB per 100 operations`);
    ...
    it('should detect and prevent memory leaks', async () => {
      ...
      for (let phase = 0; phase < leakDetectionPhases; phase++) {
        console.log(`  Memory leak detection phase ${phase + 1}/${leakDetectionPhases}`);
        ...
        const phaseMemoryGrowth = (phaseEndMemory - phaseStartMemory) / 1024 / 1024; // MB

        memoryBaselines.push(phaseMemoryGrowth);
        console.log(`    Phase ${phase + 1} memory growth: ${phaseMemoryGrowth.toFixed(2)}MB`);
      }

      // Analyze memory growth trend across phases
      if (memoryBaselines.length >= 2) {
        const avgGrowthPerPhase = memoryBaselines.reduce((sum, growth) => sum + growth, 0) / memoryBaselines.length;

        console.log(`  Average memory growth per phase: ${avgGrowthPerPhase.toFixed(2)}MB`);
        ...
        const growthTrend = memoryBaselines.slice(1).every((growth, index) =>
          growth <= memoryBaselines[index] * 2 // Each phase shouldn't be more than 2x previous
        );
  ...
  describe('Performance Benchmarking and Optimization', () => {
    it('should benchmark core operations', async () => {
      ...
      for (const benchmark of benchmarkOperations) {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          try {
            ...
            console.error(`Benchmark ${benchmark.name} iteration ${i} failed:`, error);
          }
        }
      ...
      const tokenExchangeBenchmark = benchmarkResults.find(r => r.operation === 'Token Exchange');
      if (tokenExchangeBenchmark) {
        expect(tokenExchangeBenchmark.p95Time).toBeLessThan(performanceConfig.responseTimeTargets.tokenExchange);
        expect(tokenExchangeBenchmark.throughput).toBeGreaterThan(100); // 100 ops/sec minimum
      }

      const sessionBenchmark = benchmarkResults.find(r => r.operation === 'Session Creation');
      if (sessionBenchmark) {
        expect(sessionBenchmark.p95Time).toBeLessThan(performanceConfig.responseTimeTargets.sessionCreation);
      }
    ...
    it('should validate enterprise-grade performance under realistic load', async () => {
      console.log('🏢 Testing enterprise-grade performance under realistic load...');
      ...
      console.log(`  Running enterprise load test for ${enterpriseLoadTest.duration / 1000} seconds with ${enterpriseLoadTest.concurrentUsers} concurrent users...`);
        ...
        try {
          switch (selectedOperation) {
            case 'token_exchange':
              ...
              const context = generateTestContext();
              await jwtBridgeService.exchangeTokens(token, context);
              performanceMetrics.tokenExchange.push(performance.now() - startTime);
              break;

            case 'session_create':
              const createContext = generateTestContext();
              await sessionManager.createSession(createContext);
              performanceMetrics.sessionCreate.push(performance.now() - startTime);
              break;

            case 'session_validate':
              const validateContext = generateTestContext();
              const session = await sessionManager.createSession(validateContext);
              await sessionManager.getSession(session.sessionId);
              performanceMetrics.sessionValidate.push(performance.now() - startTime);
              break;

            case 'session_terminate':
              const terminateContext = generateTestContext();
              const sessionToTerminate = await sessionManager.createSession(terminateContext);
              await sessionManager.terminateSession(sessionToTerminate.sessionId);
              performanceMetrics.sessionTerminate.push(performance.now() - startTime);
              break;
          }
      ...
      // Analyze results
      const results = {
        tokenExchange: calculatePerformanceStats(performanceMetrics.tokenExchange),
        sessionCreate: calculatePerformanceStats(performanceMetrics.sessionCreate),
        sessionValidate: calculatePerformanceStats(performanceMetrics.sessionValidate),
        sessionTerminate: calculatePerformanceStats(performanceMetrics.sessionTerminate),
      };

      const totalThroughput = totalOperations / (enterpriseLoadTest.duration / 1000);
      const errorRate = errors.length / totalOperations;

      console.log(`  Total operations: ${totalOperations}`);
      console.log(`  Overall throughput: ${totalThroughput.toFixed(2)} ops/sec`);
      console.log(`  Error rate: ${(errorRate * 100).toFixed(3)}%`);
      console.log(`  Memory growth: ${loadTestResult.resourceUsage.memory.toFixed(2)}MB`);

      console.log('  Operation Performance:');
      Object.entries(results).forEach(([operation, stats]) => {
        if (stats.mean > 0) {
          console.log(`    ${operation}: P95=${stats.p95.toFixed(2)}ms, P99=${stats.p99.toFixed(2)}ms`);
        }
      });

      // Validate enterprise requirements
      expect(results.tokenExchange.p95).toBeLessThan(performanceConfig.responseTimeTargets.tokenExchange);
      expect(results.sessionCreate.p95).toBeLessThan(performanceConfig.responseTimeTargets.sessionCreation);
      expect(results.sessionValidate.p95).toBeLessThan(performanceConfig.responseTimeTargets.sessionValidation);
      ...
      expect(loadTestResult.resourceUsage.memory).toBeLessThan(performanceConfig.memoryTargets.maxHeapIncrease);
  ...
  describe('Performance Regression Detection', () => {
    it('should detect performance regressions', async () => {
      ...
      const originalExchangeTokens = jwtBridgeService.exchangeTokens.bind(jwtBridgeService);

      // Add 200ms delay to simulate regression
      const regressedExchangeTokens = async (token: string, context: ParlantContext) => {
      ...
      try {
        ...
        const regressionStats = calculatePerformanceStats(regressionResponseTimes);
        console.log(`  Regression P95: ${regressionStats.p95.toFixed(2)}ms`);

        // Detect regression
        const regressionPercentage = ((regressionStats.p95 - baselineStats.p95) / baselineStats.p95) * 100;
        console.log(`  Detected regression: ${regressionPercentage.toFixed(2)}%`);
