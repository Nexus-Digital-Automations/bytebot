...
export class ConversationalAuthService {
  ...
  constructor(private readonly jwtBridgeService: ParlantJWTBridgeService) {
    this.logger.log(
      "🗣️ Conversational Authentication Service initialized with natural language processing",
    );
  }
  ...
  async authenticateWithConversation(
    ...
    try {
      ...
      const authResult = await this.executeConversationalFlow(
        request,
        authFlow,
        securityValidation,
      );
      ...
      if (authResult.authLevel === "authenticated") {
        sessionToken = await this.generateSessionTokenWithConversationalContext(
          request,
          nlpIntent,
        );
      }
      ...
      this.logger.log(
        `Conversational auth completed in ${totalProcessingTime.toFixed(2)}ms for conversation: ${request.conversationId}`,
      );
      return result;
    } catch (error) {
      this.logger.error(
        `Conversational authentication failed for conversation: ${request.conversationId}`,
        error,
      );
      throw error;
    }
  ...
  private async performSecurityValidation(
    ...
    if (
      suspiciousKeywords.some((keyword) =>
        request.context.message.toLowerCase().includes(keyword),
      )
    ) {
      riskScore += 0.4;
  ...
  private async executeConversationalFlow(
    ...
    authLevel: ConversationAuthResult["authLevel"];
    nextSteps?: ConversationStep[];
  }> {
    ...
    switch (authFlow) {
      ...
      case "enhanced-conversation-auth":
        return this.executeEnhancedConversationAuth(
          request,
          securityValidation,
        );

      case "multi-step-verification-with-conversation":
        return this.executeMultiStepConversationAuth(
          request,
          securityValidation,
        );

      default:
        return this.executeAdaptiveConversationFlow(
          request,
          securityValidation,
        );
    }
  ...
  private async executeSimpleConversationAuth(
    ...
    this.logger.debug(
      `Executing simple conversation auth for user: ${request.userId}`,
    );
  ...
  private async executeStandardConversationAuth(
    ...
    this.logger.debug(
      `Executing standard conversation auth for user: ${request.userId}`,
    );
    ...
    const nextSteps: ConversationStep[] = [
      {
        ...
        type: "verification",
        message:
          'To complete authentication, please confirm your identity by saying "I am [your full name]"',
  ...
  private async executeEnhancedConversationAuth(
    ...
    this.logger.debug(
      `Executing enhanced conversation auth for user: ${request.userId}`,
    );
    ...
    const nextSteps: ConversationStep[] = [
      {
        ...
        type: "challenge",
        message:
          "For enhanced security, please answer: What is the name of your first pet?",
        timeout: 45000,
        securityLevel: "HIGH",
      },
      {
  ...
  private async executeMultiStepConversationAuth(
    ...
    this.logger.debug(
      `Executing multi-step conversation auth for user: ${request.userId}`,
    );
    ...
    const nextSteps: ConversationStep[] = [
      {
        ...
        type: "verification",
        message:
          'Please say the following phrase for voice verification: "Alpha Beta Gamma Authentication"',
        timeout: 60000,
      ...
      },
      {
        ...
        type: "confirmation",
        message:
          "Critical access requires administrator approval. Please wait for confirmation.",
  ...
  private async executeAdaptiveConversationFlow(
    ...
    this.logger.debug(
      `Executing adaptive conversation flow for user: ${request.userId}`,
    );
  ...
  private async generateSessionTokenWithConversationalContext(
    ...
    try {
      const bridgeResult = await this.jwtBridgeService.exchangeTokens(
        aigentToken,
        parlantContext,
      );
      ...
      this.logger.error(
        "Failed to generate session token via JWT bridge",
        error,
      );
  ...
  private updatePerformanceMetrics(
    ...
    this.performanceMetrics.averageProcessingTime =
      (this.performanceMetrics.averageProcessingTime *
        (this.performanceMetrics.totalAuthRequests - 1) +
        totalTime) /
      this.performanceMetrics.totalAuthRequests;
  ...
  async processVoiceAuthentication(
    ...
    this.logger.debug(
      `Processing voice authentication for conversation: ${conversationId}`,
    );
  ...
  async healthCheck(): Promise<{ status: string; metrics: any }> {
    try {
      ...
      this.logger.error(
        "Conversational authentication service health check failed",
        error,
      );
      return {
        status: "unhealthy",
