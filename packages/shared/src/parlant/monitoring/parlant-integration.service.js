...
let ParlantIntegrationService = ParlantIntegrationService_1 = class ParlantIntegrationService {
    ...
    async validateFunction(request) {
        ...
        try {
            ...
            throw new ConversationalValidationError(undefined, `PARLANT validation service error: ${error instanceof Error ? error.message : String(error)}`, [
                "Check service connectivity",
                "Retry operation",
                "Contact system administrator",
            ], 0.0, parlant_types_1.RiskLevel._HIGH, "SERVICE_ERROR", { originalError: error, request });
        }
    }
    ...
    mapRiskLevelToSecurityLevel(riskLevel) {
        switch (riskLevel) {
            case parlant_types_1.RiskLevel._MINIMAL:
    ...
    calculateRiskScore(request) {
        ...
        if (request.functionName.includes("delete") ||
    ...
    identifyRiskFactors(request) {
        const factors = [];
        if (request.securityLevel === parlant_integration_types_1.SecurityLevel._HIGH ||
            request.securityLevel === parlant_integration_types_1.SecurityLevel._CRITICAL) {
