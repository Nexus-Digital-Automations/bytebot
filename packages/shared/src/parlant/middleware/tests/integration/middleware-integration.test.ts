...
describe("PARLANT Universal Middleware Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    console.log(
      `   🔄 Standard Operations: <${PERFORMANCE_TARGETS.STANDARD}ms`,
    );
    console.log(`   🔧 Complex Operations: <${PERFORMANCE_TARGETS.COMPLEX}ms`);
    console.log(
      `   🎯 Critical Operations: <${PERFORMANCE_TARGETS.CRITICAL}ms`,
    );
  ...
  afterAll(async () => {
    ...
    Object.entries(performanceMetrics).forEach(([category, times]) => {
      if (times.length > 0) {
        ...
        console.log(
          `   ${category}: avg=${avg.toFixed(2)}ms, min=${min.toFixed(2)}ms, max=${max.toFixed(2)}ms`,
        );
      }
    });
  });
  ...
  describe("Performance Optimization Tests", () => {
    ...
    it("should handle concurrent requests efficiently", async () => {
      ...
      console.log(
        `✅ Concurrent requests test: ${avgDuration.toFixed(2)}ms avg`,
      );
    });
    ...
    it("should maintain performance under load", async () => {
      ...
      for (let batch = 0; batch < loadTestRequests / batchSize; batch++) {
        ...
        for (let i = 0; i < batchSize; i++) {
          batchPromises.push(
            request(app.getHttpServer()).get("/basic-tasks").expect(200),
          );
        }
      ...
      console.log(
        `✅ Load test: avg=${avgDuration.toFixed(2)}ms, max=${maxDuration.toFixed(2)}ms`,
      );
    });
  });
  ...
  describe("Cross-Service Integration Tests", () => {
    ...
    it("should handle complex data transformations", async () => {
      ...
      console.log(
        `✅ Complex data transformation test: ${duration.toFixed(2)}ms`,
      );
    });
    ...
    it("should maintain middleware chain integrity", async () => {
      ...
      console.log(
        `✅ Middleware chain integrity test: ${duration.toFixed(2)}ms`,
      );
    });
  });
  ...
  describe("Configuration Integration Tests", () => {
    it("should respect performance configuration", async () => {
      ...
      console.log(
        `✅ Performance configuration test: ${duration.toFixed(2)}ms`,
      );
    });
...
console.log(
  "🎯 Performance Targets: Sub-1000ms processing, 95% cache hit ratio",
);
