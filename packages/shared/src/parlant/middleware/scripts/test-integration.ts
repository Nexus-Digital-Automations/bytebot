...
class ParlantIntegrationTestRunner {
  ...
  constructor() {
    ...
    this.testOutputDir = join(this.projectRoot, 'test-output', 'parlant-integration');
  }
  ...
  private async checkNodeVersion(): Promise<void> {
    return new Promise((resolve, reject) => {
      exec('node --version', (error, stdout) => {
        ...
        if (majorVersion < 18) {
          reject(new Error(`Node.js ${majorVersion} detected. Minimum required: 18`));
          return;
        }
  ...
  private async runIntegrationTests(): Promise<void> {
    ...
    const integrationTestConfig = {
      ...
      testTimeout: 60000,
      setupFilesAfterEnv: ['<rootDir>/packages/shared/src/parlant/middleware/tests/setup.ts'],
    };

    const result = await this.runJestTests('Integration Tests', integrationTestConfig);
    this.testResults.push(result);
  }
  ...
  private async runE2ETests(): Promise<void> {
    ...
    const e2eTestConfig = {
      ...
      setupFilesAfterEnv: ['<rootDir>/packages/shared/src/parlant/middleware/tests/e2e-setup.ts'],
    };
  ...
  private async runPerformanceTests(): Promise<void> {
    ...
    const result = await this.runJestTests('Performance Tests', performanceTestConfig);
    this.testResults.push(result);
   ...
   */
  private async testServiceCompatibility(config: ServiceTestConfig): Promise<void> {
    ...
    try {
      ...
      const testFile = join(this.testOutputDir, `${config.serviceName}-compatibility.test.js`);
      ...
      console.log(`  ❌ ${config.serviceName} compatibility: FAILED - ${error.message}`);
...
`;
   ...
   */
  private async runJestTests(suiteName: string, config: any): Promise<TestResult> {
    const startTime = performance.now();
    ...
    return new Promise((resolve, reject) => {
      ...
      const jestConfigFile = join(this.testOutputDir, `jest.${suiteName.toLowerCase().replace(/\s+/g, '-')}.config.js`);
      const jestConfigContent = `module.exports = ${JSON.stringify(jestConfig, null, 2)};`;

      fs.writeFile(jestConfigFile, jestConfigContent).then(() => {
        const jestProcess = spawn('npx', ['jest', '--config', jestConfigFile, '--json'], {
        ...
        jestProcess.on('close', (code) => {
          ...
          try {
            ...
            const jsonLine = outputLines.find(line => line.startsWith('{') && line.includes('testResults'));
   ...
   */
  private async runNodeScript(scriptPath: string): Promise<{ duration: number; exitCode: number }> {
    const startTime = performance.now();
...
<body>
    ...
    <div class="summary">
        ...
        <div class="card success">
            ...
            ${Object.entries(report.performanceCompliance).map(([key, value]) =>
    ...
    <div class="card">
        ...
        <div class="features">
            ${report.middleware.features.map((feature: string) =>
...
`;
  ...
  private validatePerformanceRequirements(): void {
    ...
    const performanceTests = this.testResults.filter(result =>
    ...
    performanceTests.forEach(test => {
      if (test.performanceMetrics) {
        ...
        if (avgResponseTime > 500) {
          console.log(`❌ ${test.testSuite}: Average response time ${avgResponseTime}ms exceeds 500ms threshold`);
          allRequirementsMet = false;
        } else {
          console.log(`✅ ${test.testSuite}: Average response time ${avgResponseTime}ms meets requirements`);
        }

        if (maxResponseTime > 1000) {
          console.log(`❌ ${test.testSuite}: Max response time ${maxResponseTime}ms exceeds 1000ms threshold`);
          allRequirementsMet = false;
        } else {
          console.log(`✅ ${test.testSuite}: Max response time ${maxResponseTime}ms meets requirements`);
        }
      }
    });
