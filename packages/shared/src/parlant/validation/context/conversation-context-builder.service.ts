...
export class ConversationContextBuilder {
  ...
  private initializeConfiguration(): void {
    this.config = {
      enableDetailedAnalysis: this.configService.get<boolean>('PARLANT_DETAILED_ANALYSIS') !== false,
      maxParameterDepth: this.configService.get<number>('PARLANT_MAX_PARAM_DEPTH') || 5,
      includeSensitiveData: this.configService.get<boolean>('PARLANT_INCLUDE_SENSITIVE') || false,
      riskSensitivity: (this.configService.get<string>('PARLANT_RISK_SENSITIVITY') as RiskSensitivityLevel) || RiskSensitivityLevel.MEDIUM,
      optimizeContextLength: this.configService.get<boolean>('PARLANT_OPTIMIZE_CONTEXT') !== false,
      maxContextLength: this.configService.get<number>('PARLANT_MAX_CONTEXT_LENGTH') || 4000,
    };
    ...
    const patterns: OperationPattern[] = [
      {
        ...
        mitigations: ['backup_required', 'confirmation_required', 'audit_trail'],
      },
      {
   ...
   */
  async buildContext(request: ValidationRequest, conversationHistory?: ConversationHistoryEntry[]): Promise<ConversationContext> {
    try {
      const startTime = performance.now();

      // Analyze function parameters
      const parameterAnalysis = await this.analyzeParameters(request.parameters);
   ...
   */
  private async analyzeParameters(parameters: Record<string, unknown>): Promise<ParameterAnalysis[]> {
   ...
   */
  private async analyzeParameter(name: string, value: unknown, depth: number): Promise<ParameterAnalysis> {
    ...
    const businessRelevance = this.calculateBusinessRelevance(name, value, valueType);

    // Identify risk factors
    const riskFactors = this.identifyRiskFactors(name, value, sensitivityLevel);

    // Generate description
    const description = this.generateParameterDescription(name, sanitizedValue, valueType);
  ...
  private sanitizeValue(value: unknown): unknown {
    ...
    if (typeof value === 'object') {
      if (Array.isArray(value)) {
        return value.length > 10 ? `[Array with ${value.length} items]` : value.map(item => this.sanitizeValue(item));
      }
      ...
      for (const [key, val] of Object.entries(value)) {
        if (keyCount >= 10) {
          sanitizedObj['...'] = `${Object.keys(value).length - keyCount} more properties`;
          break;
        }
   ...
   */
  private assessDataSensitivity(name: string, value: unknown): DataSensitivityLevel {
  ...
  private isClassifiedData(name: string, value: unknown): boolean {
    const classifiedKeywords = ['ssn', 'social_security', 'financial_account', 'credit_card', 'bank_account'];
  ...
  private isRestrictedData(name: string, value: unknown): boolean {
    const restrictedKeywords = ['password', 'secret', 'private_key', 'token', 'api_key', 'auth'];
  ...
  private isConfidentialData(name: string, value: unknown): boolean {
    const confidentialKeywords = ['email', 'phone', 'address', 'personal', 'private', 'user_data'];
  ...
  private isInternalData(name: string, value: unknown): boolean {
    const internalKeywords = ['id', 'internal', 'config', 'setting', 'preference'];
   ...
   */
  private calculateBusinessRelevance(name: string, value: unknown, type: string): number {
    let relevance = 0.5; // Base relevance

    // Higher relevance for certain parameter names
    const highRelevanceKeywords = ['amount', 'quantity', 'user', 'customer', 'order', 'transaction'];
    if (highRelevanceKeywords.some(keyword => name.toLowerCase().includes(keyword))) {
      relevance += 0.3;
    }

    // Medium relevance for IDs and references
    if (name.toLowerCase().includes('id') || name.toLowerCase().includes('ref')) {
      relevance += 0.1;
    }
   ...
   */
  private identifyRiskFactors(name: string, value: unknown, sensitivityLevel: DataSensitivityLevel): string[] {
    ...
    if (name.toLowerCase().includes('bulk') || name.toLowerCase().includes('batch')) {
    ...
    // SQL-like patterns
    if (typeof value === 'string' && /\b(DROP|DELETE|UPDATE|INSERT|ALTER)\b/i.test(value)) {
   ...
   */
  private generateParameterDescription(name: string, value: unknown, type: string): string {
    if (value === null || value === undefined) {
      return `${name}: ${value}`;
    }
  ...
  private getContextualInfo(parameterAnalysis: ParameterAnalysis[]): string {
    ...
    const sensitiveParams = parameterAnalysis.filter(p =>
    ...
    const bulkParams = parameterAnalysis.filter(p =>
      p.riskFactors.includes('bulk_operation') || p.riskFactors.includes('large_dataset')
    );
   ...
   * Create parameter summary
   */
  private createParameterSummary(parameterAnalysis: ParameterAnalysis[]): string {
    ...
    if (uniqueRiskFactors.length > 0) {
      summaryParts.push(`Risk factors: ${uniqueRiskFactors.slice(0, 3).join(', ')}`);
    }
   ...
   */
  private assessOperationTypeRisk(operationType: DatabaseOperationType): { factors: string[]; score: number } {
    const riskMap: Record<DatabaseOperationType, { factors: string[]; score: number }> = {
      [DatabaseOperationType.READ]: { factors: [], score: 10 },
      [DatabaseOperationType.WRITE]: { factors: ['data_creation'], score: 20 },
      [DatabaseOperationType.UPDATE]: { factors: ['data_modification'], score: 30 },
      [DatabaseOperationType.DELETE]: { factors: ['data_deletion', 'destructive_operation'], score: 50 },
      [DatabaseOperationType.BULK_OPERATION]: { factors: ['bulk_operation', 'high_impact'], score: 60 },
      [DatabaseOperationType.SCHEMA_CHANGE]: { factors: ['schema_modification', 'structural_change'], score: 80 },
      [DatabaseOperationType.TRANSACTION]: { factors: ['transaction_operation'], score: 25 },
      [DatabaseOperationType.ADMIN_OPERATION]: { factors: ['admin_privilege', 'elevated_access'], score: 70 },
    };

    return riskMap[operationType] || { factors: ['unknown_operation'], score: 40 };
  }

  /**
   * Assess risk based on data sensitivity
   */
  private assessSensitivityRisk(parameterAnalysis: ParameterAnalysis[]): { factors: string[]; score: number } {
   ...
   */
  private assessUserContextRisk(userContext: UserValidationContext): { factors: string[]; score: number } {
    const factors: string[] = [];
    let score = 0;

    // Check for admin roles
    const adminRoles = ['admin', 'administrator', 'root', 'superuser'];
    if (userContext.roles.some(role => adminRoles.includes(role.toLowerCase()))) {
      factors.push('admin_user');
      score += 20;
    }

    // Check for service accounts
    if (userContext.userId.includes('service') || userContext.userId.includes('system')) {
   ...
   */
  private assessHistoricalRisk(history: ConversationHistoryEntry[]): { factors: string[]; score: number } {
    const factors: string[] = [];
    let score = 0;

    // Check for recent failures
    const recentFailures = history.filter(h =>
      h.decision.toString().includes('DENY') || h.decision.toString().includes('ESCALATE')
    );
    ...
    const riskyOperations = history.filter(h =>
      h.relatedFunction.includes('delete') || h.relatedFunction.includes('admin')
    );
   ...
   * Generate mitigation strategies
   */
  private generateMitigationStrategies(riskFactors: string[], riskLevel: RiskLevel): string[] {
  ...
  private interpretUserIntent(
    ...
    } else if (functionName.includes('get') || functionName.includes('find') || functionName.includes('search')) {
    ...
    const businessParams = parameterAnalysis.filter(p => p.businessRelevance > 0.6);
  ...
  private assessBusinessImpact(
    ...
    } else if (operationType === DatabaseOperationType.DELETE || operationType === DatabaseOperationType.BULK_OPERATION) {
      severity = riskContext.riskLevel === RiskLevel.CRITICAL ? ImpactSeverity.MAJOR : ImpactSeverity.MODERATE;
    ...
    const hasSensitiveData = parameterAnalysis.some(p =>
      p.sensitivityLevel === DataSensitivityLevel.CONFIDENTIAL ||
      p.sensitivityLevel === DataSensitivityLevel.RESTRICTED ||
      p.sensitivityLevel === DataSensitivityLevel.CLASSIFIED
    );
    ...
    if (hasSensitiveData) {
      ...
      else if (severity === ImpactSeverity.MODERATE) severity = ImpactSeverity.MAJOR;
   ...
   */
  private optimizeContextLength(context: ConversationContext): ConversationContext {
    ...
    if (optimized.parameterSummary.length > 500) {
      optimized.parameterSummary = optimized.parameterSummary.substring(0, 497) + '...';
    }

    // Limit risk factors
    if (optimized.riskContext.riskFactors.length > 5) {
      optimized.riskContext.riskFactors = optimized.riskContext.riskFactors.slice(0, 5);
    }

    // Limit mitigation strategies
    if (optimized.riskContext.mitigationStrategies.length > 3) {
      optimized.riskContext.mitigationStrategies = optimized.riskContext.mitigationStrategies.slice(0, 3);
    }

    // Limit affected areas
    if (optimized.businessImpact.affectedAreas.length > 3) {
      optimized.businessImpact.affectedAreas = optimized.businessImpact.affectedAreas.slice(0, 3);
    }
