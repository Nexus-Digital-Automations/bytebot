...
export class ParlantPerformanceOptimizerService implements OnModuleInit, OnModuleDestroy {
  ...
  async setCached<T>(key: string, data: T, ttl?: number): Promise<void> {
    ...
    if (this.config.caching.compressionEnabled && dataSize > 1024) {
      try {
        ...
        if (compressedSize < dataSize * 0.8) { // Only compress if > 20% reduction
  ...
  async batchProcess<T, R>(
    ...
    return new Promise<R>((resolve, reject) => {
      ...
      if (this.config.batching.priorityBatching) {
        ...
        const insertIndex = queue.findIndex(existing => existing.priority < priority);
        if (insertIndex === -1) {
   ...
   */
  async returnConnection(connectionType: string, connection: any): Promise<void> {
    if (!this.config.connectionPool.enabled) {
  ...
  async executeWithCircuitBreaker<T>(
    ...
    if (this.circuitBreakerState === CircuitBreakerState.OPEN) {
      if (Date.now() - this.circuitBreakerLastFailureTime > this.config.circuitBreaker.recoveryTimeout) {
    ...
    try {
      ...
      if (this.circuitBreakerState === CircuitBreakerState.HALF_OPEN) {
        ...
        this.logger.log('Circuit breaker moving to CLOSED state - recovery successful');
      }
      ...
      if (this.circuitBreakerFailureCount >= this.config.circuitBreaker.failureThreshold) {
        ...
        this.logger.warn('Circuit breaker moving to OPEN state due to failures', {
  ...
  async adaptivePerformanceOptimization(): Promise<void> {
    ...
    try {
      this.logger.debug(`[${operationId}] Running adaptive performance optimization`);
  ...
  async updatePerformanceMetrics(): Promise<void> {
    ...
    if (timeSinceLastUpdate > 0) {
      this.performanceMetrics.requestsPerSecond = (this.requestCount * 1000) / timeSinceLastUpdate;
      this.requestCount = 0;
    }
    ...
    this.performanceMetrics.memoryUsage = process.memoryUsage().rss / 1024 / 1024; // MB
    
    this.lastMetricsUpdate = now;
    
    // Emit performance metrics event
    this.eventEmitter.emit('performance.metrics.updated', this.performanceMetrics);
  }
  ...
  private async processBatch<T, R>(batchType: string, processor: (items: T[]) => Promise<R[]>): Promise<void> {
  ...
  private async initializeConnectionPool(poolKey: string, connectionType: string): Promise<void> {
  ...
  private async applyAdaptiveOptimizations(systemMetrics: any, performanceAnalysis: any): Promise<void> {
