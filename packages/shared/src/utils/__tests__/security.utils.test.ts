...
describe("Password Security Functions", () => {
  describe("hashPassword", () => {
    test("should hash passwords securely", async () => {
      ...
      console.log(`✓ Password successfully hashed: ${hashedPassword.substring(0, 20)}...`);
    });
    ...
    test("should generate different hashes for same password", async () => {
      ...
      console.log("✓ Salt randomness verified - different hashes for same password");
    });
  ...
  describe("validatePassword", () => {
    test("should validate strong passwords", () => {
      ...
      console.log(`✓ Strong password validated successfully (score: ${result.score})`);
    });

    test("should reject weak passwords", () => {
      console.log("Testing weak password validation...");
      
      const result = validatePassword(TEST_WEAK_PASSWORD, DEFAULT_PASSWORD_POLICY);
      
      expect(result.isValid).toBe(false);
      expect(result.errors.length).toBeGreaterThan(0);
      expect(result.score).toBeLessThan(50);
      console.log(`✓ Weak password properly rejected (score: ${result.score}, errors: ${result.errors.length})`);
    });
    ...
    test("should validate password with custom policy", () => {
      ...
      console.log(`✓ Custom policy validation completed (score: ${result.score})`);
    });
    ...
    test("should detect forbidden patterns", () => {
      ...
      expect(result.errors.some(error => error.includes("forbidden"))).toBe(true);
    ...
    test("should validate various password strength levels", () => {
      ...
      passwords.forEach(({ password, expectedWeak, expectedMedium, expectedStrong }) => {
        ...
        console.log(`  - Password "${password.substring(0, 10)}..." scored ${result.score}`);
      });
  ...
  describe("generateSecurePassword", () => {
    test("should generate secure passwords with default options", () => {
      ...
      console.log(`✓ Secure password generated: ${password.substring(0, 8)}... (score: ${validation.score})`);
    });
    ...
    test("should generate different passwords each time", () => {
      ...
      console.log(`✓ ${count} unique passwords generated - randomness verified`);
    });
  ...
  describe("verifyToken", () => {
    ...
    test("should handle various token formats", () => {
      ...
      const testCases = [
        ...
        { payload: { userId: TEST_USER_ID, role: UserRole.ADMIN, permissions: ["read", "write"] }, description: "complex payload" },
        { payload: { userId: "123", nested: { data: "value" } }, description: "nested payload" },
      ];
...
describe("Input Sanitization Functions", () => {
  describe("sanitizeInput", () => {
    ...
    test("should apply custom sanitization options", () => {
      ...
      const input = "<p class='test'>Safe <b>bold</b> text</p><script>alert('xss')</script>";
    ...
    test("should respect maximum length limits", () => {
      ...
      console.log(`✓ Long input (1000 chars) truncated to ${sanitized.length} chars`);
    });
  });
...
describe("Threat Detection Functions", () => {
  describe("detectXSS", () => {
    ...
    test("should detect advanced XSS techniques", () => {
      ...
      advancedPayloads.forEach(payload => {
        const isXSS = detectXSS(payload);
        expect(isXSS).toBe(true);
        console.log(`  ✓ Advanced XSS detected: ${payload.substring(0, 40)}...`);
      });
    });
  });
  ...
  describe("detectSQLInjection", () => {
    test("should detect basic SQL injection attacks", () => {
      ...
      MALICIOUS_SQL_PAYLOADS.forEach(payload => {
        ...
        console.log(`  ✓ SQL injection detected (risk: ${result.riskScore}): ${payload.substring(0, 40)}...`);
      });
    });
    ...
    test("should provide detailed threat analysis", () => {
      ...
      console.log(`✓ Detailed analysis complete - Risk: ${result.riskScore}, Threats: ${result.threats.length}`);
    });
    ...
    test("should detect various database-specific attacks", () => {
      ...
      dbSpecificPayloads.forEach(payload => {
        const result = detectSQLInjection(payload);
        expect(result.hasInjection).toBe(true);
        console.log(`  ✓ DB-specific injection detected: ${payload.substring(0, 40)}...`);
      });
    });
  });
  ...
  describe("detectAdvancedXSS", () => {
    test("should provide detailed XSS analysis", () => {
      ...
      console.log(`✓ Advanced XSS analysis complete - Risk: ${result.riskScore}, Confidence: ${result.confidence}`);
    });
    ...
    test("should detect obfuscated XSS attacks", () => {
      ...
      obfuscatedPayloads.forEach(payload => {
        const result = detectAdvancedXSS(payload);
        expect(result.hasXSS).toBe(true);
        console.log(`  ✓ Obfuscated XSS detected: ${payload.substring(0, 40)}...`);
      });
    });
  });
  ...
  describe("detectComprehensiveMaliciousPatterns", () => {
    test("should detect multiple threat types in single input", () => {
      ...
      const multiThreatPayload = "'; DROP TABLE users; -- <script>alert('xss')</script> && rm -rf /";
      ...
      console.log(`✓ Multiple threats detected - Total threats: ${result.threats.length}, Risk: ${result.riskScore}`);
    });

    test("should provide comprehensive threat analysis", () => {
      console.log("Testing comprehensive threat analysis...");
      
      const complexPayload = "<img src=x onerror=fetch('/api/admin?token='+document.cookie)>";
      ...
      console.log(`✓ Comprehensive analysis complete - Confidence: ${result.confidence}%`);
    });
  });
  ...
  describe("Additional Threat Detection Functions", () => {
    ...
    test("detectXMLInjection should detect XML injection attacks", () => {
      ...
      xmlPayloads.forEach(payload => {
        ...
        expect(result.hasInjection).toBe(true);
        console.log(`  ✓ XML injection detected: ${payload.substring(0, 40)}...`);
      });
    });
    ...
    test("detectNoSQLInjection should detect NoSQL injection attacks", () => {
      ...
      nosqlPayloads.forEach(payload => {
        ...
        expect(result.hasInjection).toBe(true);
        console.log(`  ✓ NoSQL injection detected: ${payload.substring(0, 40)}...`);
      });
    });
  });
});
...
describe("Authorization and Role Management Functions", () => {
  describe("hasPermission", () => {
    test("should correctly check admin permissions", () => {
      ...
      expect(hasPermission(UserRole.ADMIN, Permission.VIEW_ADMIN_PANEL)).toBe(true);
      
      console.log(`✓ Admin permissions verified (${adminPermissions.length} permissions)`);
    });
    ...
    test("should correctly check user permissions", () => {
      ...
      expect(hasPermission(UserRole.USER, Permission.VIEW_ADMIN_PANEL)).toBe(false);
    ...
    test("should correctly check guest permissions", () => {
      ...
      expect(hasPermission(UserRole.GUEST, Permission.VIEW_PUBLIC_CONTENT)).toBe(true);
      expect(hasPermission(UserRole.GUEST, Permission.CREATE_TASK)).toBe(false);
      expect(hasPermission(UserRole.GUEST, Permission.VIEW_OWN_PROFILE)).toBe(false);
    ...
    test("should handle invalid role gracefully", () => {
      ...
      expect(hasPermission(invalidRole, Permission.VIEW_PUBLIC_CONTENT)).toBe(false);
  ...
  describe("ROLE_PERMISSIONS mapping", () => {
    ...
    test("should have appropriate permission distribution", () => {
      ...
      console.log(`✓ Permission distribution: Admin(${adminPerms.length}), User(${userPerms.length}), Guest(${guestPerms.length})`);
    });
  });
});
  ...
  describe("calculateRiskScore", () => {
    test("should calculate risk scores for different security events", () => {
      ...
      testEvents.forEach(({ type, context, expectedRange }) => {
        ...
        console.log(`  ✓ ${type}: Risk score ${score} (expected: ${expectedRange[0]}-${expectedRange[1]})`);
      });
    });
    ...
    test("should handle edge cases in risk calculation", () => {
      ...
      const edgeCases = [
        { type: SecurityEventType.LOGIN_SUCCESS, context: {}, description: "successful login" },
        { type: SecurityEventType.AUTHENTICATION_FAILED, context: { attempts: 10 }, description: "multiple failed attempts" },
        { type: "UNKNOWN_EVENT" as SecurityEventType, context: {}, description: "unknown event type" },
      ];
...
describe("Cryptographic and Utility Functions", () => {
  describe("generateRandomString", () => {
    test("should generate random strings of specified length", () => {
      ...
      lengths.forEach(length => {
        ...
        console.log(`  ✓ Generated ${length}-char string: ${randomString.substring(0, 8)}...`);
      });
    });
  ...
  describe("hashData", () => {
    test("should hash data with different algorithms", () => {
      ...
      console.log(`✓ Data hashed - SHA-256: ${sha256Hash.substring(0, 16)}..., SHA-512: ${sha512Hash.substring(0, 16)}...`);
    });
  ...
  describe("generateRateLimitKey", () => {
    ...
    test("should generate different keys for different users", () => {
      ...
      const user1Key = generateRateLimitKey(RateLimitServiceType.BYTEBOTD, "user1");
      const user2Key = generateRateLimitKey(RateLimitServiceType.BYTEBOTD, "user2");
    ...
    test("should generate different keys for different services", () => {
      ...
      const serviceAKey = generateRateLimitKey(RateLimitServiceType.BYTEBOTD, TEST_USER_ID);
      const serviceBKey = generateRateLimitKey(RateLimitServiceType.BYTEBOT_AGENT, TEST_USER_ID);
  ...
  describe("DEFAULT_RATE_LIMITS", () => {
    test("should have rate limits defined for all presets", () => {
      ...
      presets.forEach(preset => {
        ...
        console.log(`  ✓ ${preset}: ${config.max} requests per ${config.windowMs}ms`);
      });
    });
  ...
  describe("detectMaliciousFileContent", () => {
    test("should detect malicious file content", () => {
      ...
      maliciousContents.forEach(content => {
        ...
        console.log(`  ✓ Malicious content detected (risk: ${result.riskScore}): ${content.substring(0, 30)}...`);
      });
    });
    ...
    test("should provide detailed threat analysis", () => {
      ...
      const phpBackdoor = "<?php if(isset($_GET['cmd'])) { eval($_GET['cmd']); } ?>";
      ...
      console.log(`✓ Detailed analysis complete - Threats: ${result.threats.length}, Risk: ${result.riskScore}`);
    });
  });
  ...
  describe("scanFileContent", () => {
    test("should scan various file types for threats", () => {
      ...
      fileContents.forEach(({ content, type, expectedThreats }) => {
        ...
        console.log(`  ✓ ${type.toUpperCase()} file scanned - Threats: ${result.threats.length}, Risk: ${result.riskScore}`);
      });
    });
    ...
    test("should provide comprehensive scan results", () => {
      ...
      console.log(`✓ Comprehensive scan complete - Confidence: ${result.confidence}%, Context items: ${result.detectionContext.length}`);
    });
  });
});
...
describe("Content Security and CSP Functions", () => {
  describe("generateCSPHeader", () => {
    test("should generate CSP headers for different contexts", () => {
      ...
      contexts.forEach(context => {
        ...
        console.log(`  ✓ ${context.toUpperCase()} CSP: ${cspHeader.substring(0, 60)}...`);
      });
    });
  ...
  describe("sanitizeContentByContext", () => {
    test("should sanitize content based on context", () => {
      ...
      const maliciousContent = "<script>alert('xss')</script><p>Safe content</p><img src=x onerror=alert('xss')>";
      
      const contexts = ["html", "text", "json", "xml"];
      
      contexts.forEach(context => {
        const sanitized = sanitizeContentByContext(maliciousContent, context as any);
        ...
        console.log(`  ✓ ${context.toUpperCase()} context: ${sanitized.substring(0, 50)}...`);
      });
    });
...
describe("Integration and Edge Cases", () => {
  ...
  describe("Performance and Scalability", () => {
    test("should handle batch processing efficiently", () => {
      ...
      const batchSize = 100;
      const inputs = Array.from({ length: batchSize }, (_, i) => `test input ${i}`);
      ...
      console.log(`✓ Processed ${batchSize} inputs in ${duration}ms (avg: ${avgTime.toFixed(2)}ms per input)`);
    });
  ...
  describe("Configuration and Defaults", () => {
    test("should have sensible default configurations", () => {
      ...
      expect(Array.isArray(DEFAULT_SANITIZATION_OPTIONS.allowedTags)).toBe(true);
  ...
  describe("Integration Scenarios", () => {
    ...
    test("should work correctly in file security flow", () => {
      ...
      expect(fileSecurityEvent.type).toBe(SecurityEventType.SUSPICIOUS_ACTIVITY);
