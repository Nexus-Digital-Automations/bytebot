...
export class JwtParlantSecurityModule {
          ...
          useFactory: (
            ...
            async check() {
              ...
              return {
                ...
                services: {
                  bridge: checks[0].status === 'fulfilled' ? 'healthy' : 'unhealthy',
                  rbac: checks[1].status === 'fulfilled' ? 'healthy' : 'unhealthy',
                  emergency: checks[2].status === 'fulfilled' ? 'healthy' : 'unhealthy',
                  audit: checks[3].status === 'fulfilled' ? 'healthy' : 'unhealthy',
                },
                timestamp: new Date(),
              };
            },
   ...
   */
  static forTesting(mockOverrides?: Partial<Record<string, any>>): DynamicModule {
    ...
    const mockProviders = Object.entries(mockOverrides || {}).map(([provide, useValue]) => ({
      provide,
      useValue,
    }));
...
export const InjectSecurityConfig = () => {
  return (target: any, propertyKey: string | symbol | undefined, parameterIndex: number) => {
    return (global as any).__decorate?.[
      'Inject'
    ]?.('JWT_PARLANT_CONFIG')(target, propertyKey, parameterIndex);
  };
};