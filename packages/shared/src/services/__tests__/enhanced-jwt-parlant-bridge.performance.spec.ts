...
describe("EnhancedJwtParlantBridgeService Performance Tests", () => {
  ...
  beforeEach(async () => {
    ...
    jest
      ...
      .mockResolvedValue("fast-translated-token");
    jest
  ...
  describe("Single Request Performance", () => {
    ...
    it("should complete bridge session creation in under 1000ms", async () => {
      ...
      jest
        .spyOn(service as any, "storeSessionInRedis")
        .mockResolvedValue(undefined);
      jest
        .spyOn(service as any, "createParlantSession")
        .mockResolvedValue("parlant-session-fast");
  ...
  describe("Load Testing", () => {
    ...
    it("should handle concurrent requests efficiently", async () => {
      ...
      console.log(
        `Concurrent Test Results (${CONCURRENT_USERS} concurrent users):`,
      );
      console.log(`  Total time: ${totalTime}ms`);
      console.log(
        `  Success rate: ${((successCount / CONCURRENT_USERS) * 100).toFixed(2)}%`,
      );
      console.log(`  Average response time: ${avgResponseTime.toFixed(2)}ms`);
      console.log(`  Max response time: ${maxResponseTime}ms`);
  ...
  describe("Stress Testing", () => {
    it(
      ...
      async () => {
        ...
        console.log(
          `Starting stress test for ${STRESS_TEST_DURATION / 1000} seconds...`,
        );
    ...
    ); // Add buffer time for test completion
  });
  ...
  describe("Performance Optimization", () => {
    it("should achieve performance optimization targets", async () => {
      ...
      console.log(
        `  Auth P95: ${result.currentMetrics.authenticationMetrics.p95ResponseTime}ms`,
      );
      console.log(
        `  Exchange avg: ${result.currentMetrics.exchangeMetrics.averageExchangeTime}ms`,
      );

      // Performance targets should be achievable
      if (result.targetAchieved) {
        expect(
          result.currentMetrics.authenticationMetrics.p95ResponseTime,
        ).toBeLessThan(PERFORMANCE_TARGET_MS);
        expect(
          result.currentMetrics.exchangeMetrics.averageExchangeTime,
        ).toBeLessThan(PERFORMANCE_TARGET_MS);
      }
    });
    ...
    it("should provide real-time performance metrics", async () => {
      ...
      console.log(
        `  Auth success rate: ${metrics.authenticationMetrics.successRate}%`,
      );
      console.log(
        `  Exchange success rate: ${metrics.exchangeMetrics.exchangeSuccessRate}%`,
      );
      console.log(
        `  System uptime: ${metrics.systemMetrics.primarySystemUptime}%`,
      );
    });
  });
});
