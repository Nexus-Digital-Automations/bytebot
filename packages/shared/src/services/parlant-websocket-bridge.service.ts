 ...
 */
@Injectable()
export class ParlantWebSocketBridgeService extends EventEmitter implements OnModuleInit, OnModuleDestroy {
  ...
  private async loadConfiguration(): Promise<void> {
    this.config = {
      ...
      reconnectAttempts: parseInt(process.env.PARLANT_WS_RECONNECT_ATTEMPTS || '5'),
      heartbeatInterval: parseInt(process.env.PARLANT_WS_HEARTBEAT_INTERVAL || '30000'),
      connectionTimeout: parseInt(process.env.PARLANT_WS_CONNECTION_TIMEOUT || '10000')
    };
  ...
  private async initializeClientConnection(): Promise<void> {
    const parlantWsUrl = process.env.PARLANT_WS_URL || 'ws://localhost:8000/ws/aigent';
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        ws.terminate();
        reject(new ParlantConnectionError('Client WebSocket connection timeout'));
      ...
      ws.on('error', (error: Error) => {
        ...
        this.stats.errors++;
        reject(new ParlantConnectionError('Client WebSocket connection failed', error));
      });
   ...
   */
  private handleClientMessage(ws: WebSocket, clientId: string, data: WebSocket.Data): void {
    try {
      const message: ParlantWebSocketMessage = JSON.parse(data.toString());
      this.logger.debug(`📨 Received message from client ${clientId}: ${message.type}`);
      ...
      switch (message.type) {
        ...
        default:
          this.logger.warn(`🚫 Unknown message type from client: ${message.type}`);
      }
      
    } catch (error) {
      this.logger.error(`❌ Failed to parse client message from ${clientId}`, error);
      this.stats.errors++;
    }
  }
  ...
  private handleIncomingMessage(data: WebSocket.Data): void {
    try {
      ...
      switch (message.type) {
        ...
        default:
          this.logger.warn(`🚫 Unknown message type from Parlant: ${message.type}`);
      }
   ...
   */
  private async forwardValidationRequest(message: ParlantWebSocketMessage, clientWs: WebSocket): Promise<void> {
  ...
  private handleValidationResponse(message: ParlantWebSocketMessage): void {
    ...
    if (!pending) {
      this.logger.warn(`⚠️ No pending validation found for message: ${originalMessageId}`);
      return;
    }
    ...
    this.logger.debug(`✅ Validation response forwarded (${latency}ms): ${originalMessageId}`);
  }
  ...
  private async attemptReconnection(): Promise<void> {
    ...
    while (attempts < this.config.reconnectAttempts && !this.isConnected) {
      ...
      this.logger.log(`🔄 Attempting Parlant reconnection (${attempts}/${this.config.reconnectAttempts})`);
    ...
    if (!this.isConnected) {
      this.logger.error('❌ Failed to reconnect to Parlant after maximum attempts');
   ...
   */
  private handleClientHeartbeat(ws: WebSocket, message: ParlantWebSocketMessage): void {
    this.sendToClient(ws, {
      type: ParlantMessageType.HEARTBEAT,
      payload: {
        timestamp: Date.now(),
  ...
  private handleParlantStatusUpdate(message: ParlantWebSocketMessage): void {
    this.logger.debug('📊 Received status update from Parlant', message.payload);
    ...
    return {
      ...
      metrics: {
        ...
        errorRate: this.stats.totalMessages > 0 ? 
          (this.stats.errors / this.stats.totalMessages) * 100 : 0,
   ...
   */
  async sendValidationRequest(request: ParlantValidationRequest): Promise<ParlantValidationResponse> {
