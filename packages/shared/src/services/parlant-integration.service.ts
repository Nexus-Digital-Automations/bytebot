 ...
 */
@Injectable()
export class ParlantIntegrationService extends EventEmitter implements OnModuleInit, OnModuleDestroy {
  ...
  private redisClient: any = null; // Redis client will be injected if available
  ...
  private async initializeWebSocketConnection(): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      ws.on('error', (error) => {
        ...
        reject(new ParlantConnectionError('WebSocket connection failed', error));
      });
    });
  }
  ...
  private async initializeCacheSystem(): Promise<void> {
    ...
    if (this.config.cache.type === 'redis' || this.config.cache.type === 'hybrid') {
      try {
  ...
  async validateFunction(request: ParlantValidationRequest): Promise<ParlantValidationResponse> {
    ...
    this.logger.debug(`🔍 Validating function: ${request.functionName}`, { operationId });
   ...
   */
  private async performValidation(request: ParlantValidationRequest): Promise<ParlantValidationResponse> {
    ...
    try {
      const response = await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new ParlantTimeoutError('Validation timeout')), timeout)
        )
      ]);
   ...
   */
  private transformParlantResponse(data: any, request: ParlantValidationRequest): ParlantValidationResponse {
    return {
      ...
      executionContext: data.execution_context ? {
        ...
        resourceLimits: {
          maxExecutionTime: data.execution_context.max_execution_time || 30000,
        ...
        securityRestrictions: data.execution_context.security_restrictions || [],
        monitoring: {
          realTimeMonitoring: true,
          logAllOperations: true,
          alertOnViolations: true,
   ...
   */
  private async cacheValidation(request: ParlantValidationRequest, response: ParlantValidationResponse): Promise<void> {
    if (!this.config.cache.enabled) {
      return;
    }
   ...
   */
  private async recordAuditEntry(request: ParlantValidationRequest, response: ParlantValidationResponse): Promise<void> {
  ...
  private updateMetrics(update: {
    ...
    if (update.responseTime !== undefined) {
      ...
      this.metrics.averageResponseTime = 
        this.metrics.averageResponseTime * (1 - alpha) + update.responseTime * alpha;
    }
