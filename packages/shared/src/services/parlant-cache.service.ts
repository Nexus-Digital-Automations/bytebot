 ...
 */
@Injectable()
export class ParlantCacheService extends EventEmitter implements OnModuleInit, OnModuleDestroy {
   ...
   */
  async get(request: ParlantValidationRequest): Promise<CacheResult<ParlantValidationResponse>> {
    ...
    try {
      ...
      if (memoryResult.found) {
        ...
        this.logger.debug(`💾 Memory cache hit: ${this.getFunctionName(request)} (${Date.now() - startTime}ms)`);
        return memoryResult;
      }
      ...
      if (this.redisClient) {
        ...
        if (redisResult.found) {
          ...
          if (redisResult.data) {
            await this.setInMemory(cacheKey, redisResult.data, this.config.defaultTtl);
          }
          
          this.logger.debug(`🔄 Redis cache hit: ${this.getFunctionName(request)} (${Date.now() - startTime}ms)`);
          return redisResult;
        }
      }
      ...
      if (this.config.type === 'hybrid' || this.config.type === 'persistent') {
        ...
        if (persistentResult.found) {
          ...
          if (persistentResult.data) {
            await this.setInMemory(cacheKey, persistentResult.data, this.config.defaultTtl);
            if (this.redisClient) {
              await this.setInRedis(cacheKey, persistentResult.data, this.config.defaultTtl);
            }
          }
          
          this.logger.debug(`💿 Persistent cache hit: ${this.getFunctionName(request)} (${Date.now() - startTime}ms)`);
      ...
      this.logger.debug(`❌ Cache miss: ${this.getFunctionName(request)} (${Date.now() - startTime}ms)`);
  ...
  async set(
    ...
    try {
      ...
      if ((this.config.type === 'hybrid' || this.config.type === 'persistent') && priority >= 8) {
      ...
      this.logger.debug(`💾 Cached response: ${this.getFunctionName(request)} (priority: ${priority})`);
  ...
  async preload(): Promise<void> {
    ...
    try {
      ...
      for (const [pattern, confidence] of predictions.entries()) {
        if (confidence > 0.7 && preloadCount < 50) { // Limit preloading
      ...
      this.logger.debug(`🔮 Preloaded ${preloadCount} cache entries (${Date.now() - startTime}ms)`);
  ...
  async invalidate(pattern: string): Promise<number> {
    ...
    try {
      ...
      this.logger.log(`🗑️ Invalidated ${invalidatedCount} cache entries matching pattern: ${pattern}`);
  ...
  private async loadConfiguration(): Promise<void> {
    ...
    this.persistentCachePath = process.env.PARLANT_CACHE_PATH || 
  ...
  private async initializeRedisCache(): Promise<void> {
    ...
    try {
      ...
      if (redisAvailable) {
        ...
        this.logger.log('📡 Redis not available, using memory + persistent cache');
      }
    } catch (error) {
      this.logger.warn('⚠️ Redis initialization failed, using memory cache only', error);
  ...
  private async initializePersistentCache(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`💿 Persistent cache initialized: ${this.persistentCachePath}`);
    } catch (error) {
  ...
  private async getFromMemory(key: string): Promise<CacheResult<ParlantCacheEntry>> {
    const entry = this.memoryCache.get(key);
    
    if (!entry) {
      return { found: false, data: null, layer: CacheLayer.MEMORY, responseTime: 0 };
    }
    ...
    entry.hitCount++;
    return { found: true, data: entry, layer: CacheLayer.MEMORY, responseTime: 1 };
  }

  private async getFromRedis(key: string): Promise<CacheResult<ParlantCacheEntry>> {
    if (!this.redisClient) {
      return { found: false, data: null, layer: CacheLayer.REDIS, responseTime: 0 };
    }
    ...
    try {
      ...
      if (!data) {
        return { found: false, data: null, layer: CacheLayer.REDIS, responseTime };
      }
      ...
      return { found: true, data: entry, layer: CacheLayer.REDIS, responseTime };
  ...
  private async getFromPersistent(key: string): Promise<CacheResult<ParlantCacheEntry>> {
    try {
      const startTime = Date.now();
      ...
      if (this.isExpired(entry)) {
        ...
        return { found: false, data: null, layer: CacheLayer.PERSISTENT, responseTime };
      }
      
      entry.hitCount++;
      
      return { found: true, data: entry, layer: CacheLayer.PERSISTENT, responseTime };
      
    } catch (error) {
      if (error.code !== 'ENOENT') {
        this.logger.error('❌ Persistent cache get operation failed', error);
      }
      return { found: false, data: null, layer: CacheLayer.PERSISTENT, responseTime: 0 };
    }
  }

  private async setInMemory(key: string, entry: ParlantCacheEntry, ttl: number): Promise<void> {
  ...
  private async setInRedis(key: string, entry: ParlantCacheEntry, ttl: number): Promise<void> {
  ...
  private async setInPersistent(key: string, entry: ParlantCacheEntry): Promise<void> {
    try {
      const filePath = this.getPersistentFilePath(key);
  ...
  private async invalidatePersistent(pattern: string): Promise<void> {
    try {
      const files = await fs.readdir(this.persistentCachePath, { recursive: true });
  ...
  private async evictFromMemory(): Promise<void> {
    ...
    entries.sort((a, b) => {
      ...
      switch (this.config.evictionPolicy) {
        case 'lru':
          return metadataA.lastAccess.getTime() - metadataB.lastAccess.getTime();
        case 'fifo':
  ...
  private calculateCachePriority(request: ParlantValidationRequest, response: ParlantValidationResponse): number {
    let priority = 5; // Base priority
    
    // Increase priority based on security level
    switch (request.securityLevel) {
      case SecurityLevel.CRITICAL: priority += 3; break;
      case SecurityLevel.HIGH: priority += 2; break;
      case SecurityLevel.MEDIUM: priority += 1; break;
    }
  ...
  private updateStats(startTime: number, hit: boolean, layer?: CacheLayer): void {
    const responseTime = Date.now() - startTime;
  ...
  private calculateHitRate(): number {
    const totalHits = this.stats.memoryHits + this.stats.redisHits + this.stats.persistentHits;
    return this.stats.totalRequests > 0 ? 
      Math.round((totalHits / this.stats.totalRequests) * 100) : 0;
  }
  ...
  private updatePerformanceStats(): void {
    ...
    if (hitRate < this.PERFORMANCE_TARGETS.hitRate) {
      this.logger.warn(`⚠️ Cache hit rate below target: ${hitRate}% < ${this.PERFORMANCE_TARGETS.hitRate}%`);
    }
    
    if (responseTime > this.PERFORMANCE_TARGETS.maxResponseTime) {
      this.logger.warn(`⚠️ Response time above target: ${responseTime}ms > ${this.PERFORMANCE_TARGETS.maxResponseTime}ms`);
    }
  ...
  private async loadCacheMetadata(): Promise<void> {
    try {
      ...
      this.logger.log(`📋 Loaded cache metadata for ${this.keyMetadata.size} entries`);
    } catch (error) {
  ...
  private async saveCacheMetadata(): Promise<void> {
    try {
      ...
      this.logger.log(`💾 Saved cache metadata for ${this.keyMetadata.size} entries`);
    } catch (error) {
