/**
 * Authentication Performance Security Tests - Fixed Version
 *
 ...
 *
 * Fixes applied:
 * - Proper ExecutionContext mocking with getHandler and getClass methods
 * - Enhanced authentication test helpers with realistic mock implementations
 * - Improved error handling and test isolation
 * - Better mock service setup with proper type safety
 *
...
import {
  ...
  AuthTestUtils,
} from '../../test-utils/helpers/auth-test-helper';
import { createTestBuilder } from '../../test-utils/helpers/nestjs-test-builder';
...
describe('Authentication Performance Security Tests (Fixed)', () => {
  ...
  beforeAll(async () => {
    // Create comprehensive test module with proper mocking
    ...
    testBuilder.addProviders([JwtAuthGuard, RolesGuard]);

    const module: TestingModule = await testBuilder.build();
    ...
    (jest.spyOn(jwtService, 'verify') as jest.Mock).mockImplementation(() => {
    ...
    (
      jest.spyOn(reflector, 'getAllAndOverride') as jest.Mock
    ...
    // Create auth helper for consistent context creation
  ...
  describe('Authentication Performance Baseline', () => {
    it('should measure JWT validation performance under normal load', async () => {
      ...
      // Use enhanced auth test helper to create proper context
      const testUser = AuthTestUtils.DataFactory.createTestUser();
      const mockContext: ExecutionContext =
        authHelper.createAuthenticatedContext(testUser);
    ...
    it('should measure role validation performance under concurrent load', async () => {
      ...
      // Use enhanced auth test helper with admin user
      const adminUser = AuthTestUtils.DataFactory.createAdminUser();
      const mockContext: ExecutionContext =
        authHelper.createAuthenticatedContext(adminUser);
  ...
  describe('System Performance Under Attack', () => {
    it('should maintain performance during brute force attack simulation', async () => {
      ...
      const attackPromises = Array.from({ length: 50 }, (_, i) => {
        const mockContext: ExecutionContext =
          authHelper.createInvalidTokenContext();

        return jwtAuthGuard.canActivate(mockContext).catch(() => false);
      });

      const legitimateUser = AuthTestUtils.DataFactory.createTestUser();

      const legitimatePromises = Array.from({ length: 20 }, () => {
        const mockContext: ExecutionContext =
          authHelper.createAuthenticatedContext(legitimateUser);
