...
describe('BytebotConfigService', () => {
  ...
  beforeEach(async () => {
    ...
    mockedCrypto.scryptSync.mockReturnValue(
      Buffer.from('test-key-32-chars-long-for-aes256'),
    );
  ...
  describe('Service Initialization', () => {
    ...
    it('should create secrets directory if it does not exist', () => {
      ...
      expect(mockedFs.mkdirSync).toHaveBeenCalledWith(testSecretsDir, {
        recursive: true,
      });
  ...
  describe('Module Initialization', () => {
    ...
    it('should initialize critical secrets from environment', async () => {
      ...
      expect(service.storeSecret).toHaveBeenCalledWith(
        'api-keys',
        'ANTHROPIC_API_KEY',
        'test-anthropic-key',
      );
