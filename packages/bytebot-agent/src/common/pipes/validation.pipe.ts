/**
 * Global Validation Pipe - Enterprise Security Input Validation
 *
 * This pipe provides comprehensive input validation and sanitization for all API endpoints.
 * Features include XSS protection, SQL injection prevention, request size limits,
 * and structured error reporting for security audit trails.
 *
 * @fileoverview Enterprise-grade validation pipe for API security
 * @version 1.0.0
...
export class GlobalValidationPipe implements PipeTransform<any> {
  ...
  constructor(options?: Partial<GlobalValidationPipeOptions>) {
    ...
    this.logger.log('Global validation pipe initialized', {
  ...
  async transform(value: any, metadata: ArgumentMetadata): Promise<any> {
    ...
    this.logger.debug(`[${operationId}] Starting validation`, {
      operationId,
      type: metadata.type,
    ...
    try {
      ...
      this.logger.debug(`[${operationId}] Validation completed successfully`, {
        operationId,
        type: metadata.type,
        metatype: metadata.metatype?.name,
  ...
  private async validateValue(
    ...
    if (errors.length > 0) {
      ...
      this.logger.warn(`[${operationId}] Class validation failed`, {
        operationId,
    ...
    this.logger.debug(`[${operationId}] Class validation passed`, {
      operationId,
      metatype: metatype.name,
      validationTimeMs: validationTime,
      errorCount: 0,
...
export const ValidationPipes = {
  /**
   * Maximum security validation with strict sanitization
   */
  MAXIMUM_SECURITY: createValidationPipe({
    transform: true,
    whitelist: true,
    forbidNonWhitelisted: true,
    enableSanitization: true,
  ...
  /**
   * High security validation with moderate sanitization
   */
  HIGH_SECURITY: createValidationPipe({
    transform: true,
    whitelist: true,
    forbidNonWhitelisted: true,
    enableSanitization: true,
  ...
  /**
   * Standard security validation for most endpoints
   */
  STANDARD: createValidationPipe({
    transform: true,
    whitelist: true,
   ...
   */
  DEVELOPMENT: createValidationPipe({
    transform: true,
    whitelist: false,
    forbidNonWhitelisted: false,
