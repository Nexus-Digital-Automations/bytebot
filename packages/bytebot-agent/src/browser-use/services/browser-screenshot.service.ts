...
// Error handling interfaces for type safety
...
export class BrowserScreenshotService {
  ...
  constructor(
    ...
    void this.initializeScreenshotDirectory();
  }
  ...
  async captureScreenshot(
    ...
    try {
      ...
      if (captureDto.annotations && captureDto.annotations.length > 0) {
        this.applyAnnotations(screenshotId, captureDto.annotations);
      }
      ...
      if (captureDto.compareWith) {
        ...
        metadata.comparison = {
          baseScreenshotId: captureDto.compareWith,
          differenceScore: comparisonResult.differenceScore,
          differenceAreas: comparisonResult.differenceAreas,
        };
      }
  ...
  async getScreenshot(screenshotId: string): Promise<ScreenshotResponseDto> {
    try {
      ...
      try {
        ...
        const fsError = fileError as FileSystemError;
        this.logger.error(
      ...
      const screenshotError = error as ScreenshotError;
      this.logger.error(
        `Error retrieving screenshot: ${screenshotError.message}`,
      );
      return {
        id: screenshotId,
        data: '',
  ...
  async compareScreenshots(
    ...
    try {
      ...
      const comparisonError = error as ScreenshotError;
      this.logger.error(
  ...
  async deleteScreenshot(screenshotId: string): Promise<boolean> {
    try {
      ...
      try {
        ...
        const fsError = deleteError as FileSystemError;
        this.logger.warn(
      ...
      const deleteError = error as ScreenshotError;
      this.logger.error(`Failed to delete screenshot: ${deleteError.message}`);
      return false;
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          const fsError = statError as FileSystemError;
          this.logger.debug(`Could not stat file: ${file}`, fsError.message);
        }
      }
    } catch (error) {
      const dirError = error as FileSystemError;
      this.logger.warn(
        `Could not read screenshot directory: ${dirError.message}`,
      );
    }
