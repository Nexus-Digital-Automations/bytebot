 ...
 */
export const BROWSER_AUTOMATION_ERROR_REGISTRY: Record<string, BrowserAutomationErrorCode> = {
  // Network and Connectivity Errors
...
export class BrowserAutomationErrorClassifier {
  ...
  static classifyError(
    ...
    const patterns = [
      { pattern: /connection refused|ECONNREFUSED/i, code: 'NET_CONNECTION_REFUSED' },
      { pattern: /timeout|ETIMEDOUT/i, code: 'NET_TIMEOUT' },
      { pattern: /dns.*resolution|ENOTFOUND|getaddrinfo/i, code: 'NET_DNS_RESOLUTION_FAILED' },
      { pattern: /browser.*crash|process.*crash/i, code: 'BROWSER_PROCESS_CRASHED' },
      { pattern: /session.*expired|session.*invalid/i, code: 'BROWSER_SESSION_EXPIRED' },
      { pattern: /failed.*start.*browser|browser.*startup/i, code: 'BROWSER_PROCESS_STARTUP_FAILED' },
      { pattern: /page.*load.*timeout|navigation.*timeout/i, code: 'PAGE_LOAD_TIMEOUT' },
      { pattern: /navigation.*blocked|blocked.*navigation/i, code: 'NAVIGATION_BLOCKED' },
      { pattern: /element.*not.*found|selector.*not.*found/i, code: 'ELEMENT_NOT_FOUND' },
      { pattern: /element.*not.*interactive|not.*clickable/i, code: 'ELEMENT_NOT_INTERACTIVE' },
      { pattern: /auth.*invalid|authentication.*failed/i, code: 'AUTH_SESSION_INVALID' },
      { pattern: /memory.*limit|out.*of.*memory|OOM/i, code: 'MEMORY_LIMIT_EXCEEDED' },
      { pattern: /task.*execution.*failed|automation.*failed/i, code: 'TASK_EXECUTION_FAILED' },
      { pattern: /config.*invalid|configuration.*error/i, code: 'CONFIG_INVALID' }
    ];
  ...
  static getRecoveryStrategies(
    ...
    const errorCodes = Object.values(BROWSER_AUTOMATION_ERROR_REGISTRY)
  ...
  static isRecoverable(errorCode: string): boolean {
    ...
    return classification?.recoverability === BrowserAutomationErrorRecoverability.RECOVERABLE ||
           classification?.recoverability === BrowserAutomationErrorRecoverability.RETRY_POSSIBLE;
  }
