...
export interface DegradationStrategy {
  ...
  mitigations: Array<{
    type: 'disable_feature' | 'reduce_quality' | 'increase_timeout' | 'use_fallback' | 'limit_concurrency' | 'cache_aggressively';
    target: string;
...
export class BrowserAutomationDegradationManager {
  private readonly logger = new Logger(BrowserAutomationDegradationManager.name);
  private readonly degradationStrategies = new Map<string, DegradationStrategy>();
  private readonly fallbackOperations = new Map<BrowserAutomationOperationType, FallbackOperation[]>();
  private currentDegradationState: DegradationState | null = null;
  private readonly degradationHistory: Array<DegradationState & { deactivatedAt?: Date; duration?: number }> = [];
  ...
  async evaluateDegradationNeed(): Promise<{
    ...
    // Check error rate
    const totalOperations = Array.from(performanceMetrics.values())
      .reduce((sum, m) => sum + m.totalOperations, 0);
    const totalErrors = Array.from(performanceMetrics.values())
    ...
    if (errorRate > 0.5) {
      ...
      trigger = DegradationTrigger.ERROR_RATE_THRESHOLD;
      reasoning.push(`Slightly elevated error rate: ${(errorRate * 100).toFixed(1)}%`);
    ...
    if (systemHealth.resources.memoryUsagePercent > 95) {
      ...
      reasoning.push(`Critical memory usage: ${systemHealth.resources.memoryUsagePercent.toFixed(1)}%`);
    } else if (systemHealth.resources.memoryUsagePercent > 85) {
      if (recommendedLevel === DegradationLevel.NONE || recommendedLevel === DegradationLevel.MINIMAL) {
        recommendedLevel = DegradationLevel.MODERATE;
        trigger = DegradationTrigger.RESOURCE_EXHAUSTION;
        suggestedStrategy = 'resource_optimization';
      }
      reasoning.push(`High memory usage: ${systemHealth.resources.memoryUsagePercent.toFixed(1)}%`);
    }
    ...
    if (systemHealth.resources.cpuUsagePercent > 90) {
      ...
      reasoning.push(`High CPU usage: ${systemHealth.resources.cpuUsagePercent.toFixed(1)}%`);
    }
    ...
    if (systemHealth.browserProcesses.crashed > 0) {
      ...
      reasoning.push(`Browser processes crashed: ${systemHealth.browserProcesses.crashed}`);
    }

    // Check performance degradation
    const avgResponseTime = Array.from(performanceMetrics.values())
      .reduce((sum, m) => sum + m.averageDurationMs, 0) / performanceMetrics.size;
    ...
    try {
      ...
      for (const mitigation of strategy.mitigations) {
        try {
          ...
          this.logger.debug(`Applied mitigation: ${mitigation.type}`, mitigation);
        } catch (error) {
          this.logger.error(`Failed to apply mitigation: ${mitigation.type}`, error);
        }
      }
      ...
      this.logger.error(`Failed to activate degradation mode: ${errorMessage}`, error);
  ...
  async deactivateDegradation(): Promise<{
    ...
    try {
      ...
      this.logger.log(`Deactivating degradation mode: ${this.currentDegradationState.level}`);

      const previousState = { ...this.currentDegradationState };

      // Remove applied mitigations
      for (const mitigationType of this.currentDegradationState.mitigationsApplied) {
        try {
          await this.removeMitigation(mitigationType);
          this.logger.debug(`Removed mitigation: ${mitigationType}`);
        } catch (error) {
          this.logger.error(`Failed to remove mitigation: ${mitigationType}`, error);
        }
      }
      ...
      if (historyEntry && !historyEntry.deactivatedAt) {
        ...
        historyEntry.duration = historyEntry.deactivatedAt.getTime() - historyEntry.activatedAt.getTime();
      }
      ...
      await this.notifyUsers({
        message: 'System has recovered from degraded mode. Full functionality restored.',
        severity: 'INFO',
        showToUser: true,
        alternativeActions: []
      });
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`Failed to deactivate degradation mode: ${errorMessage}`, error);
  ...
  async executeWithDegradation<T>(
    ...
    try {
      ...
      if (!this.currentDegradationState.affectedOperations.includes(operationType)) {
        const result = await originalOperation();
        return {
          result,
          success: true,
      ...
      const fallback = this.getFallbackOperation(operationType, this.currentDegradationState.level);

      if (!fallback) {
        // No fallback available, try original operation with warnings
        this.logger.warn(`No fallback available for operation: ${operationType} in degraded mode`);
      ...
      this.logger.debug(`Executing fallback operation: ${fallback.fallbackMethod} for ${operationType}`);

      const fallbackResult = await this.executeFallbackOperation(fallback, context);
   ...
   */
  getDegradationHistory(): Array<DegradationState & { deactivatedAt?: Date; duration?: number }> {
   ...
   */
  getAvailableFallbacks(): Map<BrowserAutomationOperationType, FallbackOperation[]> {
  ...
  private initializeDegradationStrategies(): void {
    ...
    this.degradationStrategies.set('minimal_degradation', {
      ...
      userNotification: {
        message: 'System is operating with minor optimizations to improve stability.',
        severity: 'INFO',
        showToUser: false
      },
      autoRecovery: {
    ...
    this.degradationStrategies.set('error_rate_management', {
      ...
      userNotification: {
        message: 'System is managing high error rates. Some operations may be slower but more reliable.',
        severity: 'WARNING',
        showToUser: true,
        alternativeActions: ['Use cached data where available', 'Retry failed operations']
      },
      autoRecovery: {
        enabled: true,
    ...
    this.degradationStrategies.set('high_error_rate_mitigation', {
      ...
      userNotification: {
        message: 'System is experiencing high error rates. Operating in limited mode for stability.',
        severity: 'ERROR',
        showToUser: true,
        includeETA: true,
        alternativeActions: ['Wait for system recovery', 'Use manual alternative methods']
      },
      autoRecovery: {
        enabled: true,
    ...
    this.degradationStrategies.set('emergency_mode', {
      ...
      userNotification: {
        message: 'System is in emergency mode. Only critical functions are available.',
        severity: 'ERROR',
        showToUser: true,
        includeETA: true,
        alternativeActions: ['Contact system administrator', 'Use alternative systems']
      },
      autoRecovery: {
        enabled: true,
  ...
  private initializeFallbackOperations(): void {
    ...
    this.fallbackOperations.set(BrowserAutomationOperationType.ELEMENT_INTERACTION, [
      {
      ...
      },
      {
        ...
        limitations: ['May bypass browser security', 'Limited event simulation']
      }
    ]);

    // Data extraction fallbacks
    this.fallbackOperations.set(BrowserAutomationOperationType.DATA_EXTRACTION, [
      {
  ...
  private async evaluateRecoveryConditions(strategy: DegradationStrategy): Promise<boolean> {
    ...
    if (conditions.errorRateBelow) {
      const totalOps = Array.from(performanceMetrics.values())
        .reduce((sum, m) => sum + m.totalOperations, 0);
      const totalErrors = Array.from(performanceMetrics.values())
        .reduce((sum, m) => sum + m.failedOperations, 0);
    ...
    if (conditions.resourceUsageBelow) {
      if (systemHealth.resources.memoryUsagePercent >= conditions.resourceUsageBelow ||
  ...
  private async applyMitigation(mitigation: DegradationStrategy['mitigations'][0]): Promise<void> {
  ...
  private async notifyUsers(notification: DegradationStrategy['userNotification']): Promise<void> {
  ...
  private async executeReducedQualityScreenshot(context?: Record<string, unknown>): Promise<unknown> {
    // Implement reduced quality screenshot logic
    return { screenshot: 'base64_data_reduced_quality', quality: 70 };
  }

  private async executeViewportOnlyScreenshot(context?: Record<string, unknown>): Promise<unknown> {
    // Implement viewport-only screenshot logic
    return { screenshot: 'base64_data_viewport_only', fullPage: false };
  }

  private async executeSimpleNavigation(context?: Record<string, unknown>): Promise<unknown> {
    // Implement simple navigation logic
    return { url: context?.url, navigated: true, method: 'simple' };
  }

  private async executeCachedNavigation(context?: Record<string, unknown>): Promise<unknown> {
    // Implement cached navigation logic
    return { url: context?.url, navigated: true, cached: true };
  }

  private async executeSimpleClick(context?: Record<string, unknown>): Promise<unknown> {
    // Implement simple click logic
    return { clicked: true, method: 'simple' };
  }

  private async executeJavaScriptInteraction(context?: Record<string, unknown>): Promise<unknown> {
    // Implement JavaScript interaction logic
    return { interacted: true, method: 'javascript' };
  }

  private async executeTextOnlyExtraction(context?: Record<string, unknown>): Promise<unknown> {
    // Implement text-only extraction logic
    return { text: 'extracted_text_content', type: 'text_only' };
  }

  private async executeCachedDataExtraction(context?: Record<string, unknown>): Promise<unknown> {
