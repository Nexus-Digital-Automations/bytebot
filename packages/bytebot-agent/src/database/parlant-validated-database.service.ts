...
export interface ParlantDatabaseValidationRequest
  ...
  readonly estimatedImpact: 'MINIMAL' | 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';
}
...
export class ParlantValidatedDatabaseService {
  ...
  async validateAndExecute<T>(
    ...
    try {
      ...
      // 3. Execute database operation with monitoring
   ...
   */
  async getHealthStatus(context: ParlantConversationContext) {
    return this.validateAndExecute(
      'getHealthStatus',
   ...
   */
  async getReliabilityMetrics(context: ParlantConversationContext) {
    return this.validateAndExecute(
