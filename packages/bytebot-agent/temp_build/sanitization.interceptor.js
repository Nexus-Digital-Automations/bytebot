 ...
 */
var CONTROL_CHARS_PATTERN = new RegExp(
  '['
    .concat(String.fromCharCode(0), '-')
    .concat(String.fromCharCode(8))
    .concat(String.fromCharCode(11))
    .concat(String.fromCharCode(12))
    .concat(String.fromCharCode(14), '-')
    .concat(String.fromCharCode(31))
  ...
  var SanitizationInterceptor = (_classThis = /** @class */ (function () {
    function SanitizationInterceptor_1(configService) {
      ...
      var sanitizationConfig =
        (_a = this.configService.get('sanitization')) !== null && _a !== void 0
          ? _a
          : {};
      ...
      try {
        ...
        domPurifyInstance.addHook('beforeSanitizeElements', function (node) {
          ...
          if (
            ...
            _this.logger.warn(
              'Blocked dangerous element: '.concat(node.nodeName),
            );
          }
        });
      } catch (initError) {
        ...
        throw new Error(
          'DOMPurify initialization failed: '.concat(errorMessage),
        );
      }
    ...
    SanitizationInterceptor_1.prototype.intercept = function (context, next) {
      ...
      this.logger.debug(
        '['.concat(operationId, '] Starting input sanitization'),
        {
          ...
          hasBody:
            !!request.body &&
            Object.keys((_a = request.body) !== null && _a !== void 0 ? _a : {})
              .length > 0,
        },
      );
      try {
        ...
        if (request.body && Object.keys(request.body).length > 0) {
          var sanitizedBody = this.sanitizeObject(
            request.body,
            request.path,
            operationId,
          );
        ...
        if (request.query && Object.keys(request.query).length > 0) {
          var sanitizedQuery = this.sanitizeObject(
            request.query,
            request.path,
            operationId,
          );
        ...
        if (request.params && Object.keys(request.params).length > 0) {
          var sanitizedParams = this.sanitizeObject(
            request.params,
            request.path,
            operationId,
          );
        ...
        this.logger.debug(
          '['.concat(operationId, '] Input sanitization completed'),
          {
            operationId: operationId,
        ...
        return next.handle().pipe(
          (0, operators_1.tap)(function () {
            _this.logger.debug(
              '['.concat(operationId, '] Request processing completed'),
            );
          }),
        ...
        var processingTime = Date.now() - startTime;
        var errorMessage =
          (_b = structuredError.message) !== null && _b !== void 0
            ? _b
            : 'Unknown error';
        var errorStack =
          (_c = structuredError.stack) !== null && _c !== void 0 ? _c : '';
     ...
     */
    SanitizationInterceptor_1.prototype.shouldSkipSanitization = function (
      path,
    ) {
     ...
     */
    SanitizationInterceptor_1.prototype.sanitizeObject = function (
      obj,
      endpoint,
      operationId,
      depth,
      ...
      if (depth > this.config.maxObjectDepth) {
        throw new Error(
          'Object depth limit exceeded: '.concat(this.config.maxObjectDepth),
        );
      }
      ...
      if (Array.isArray(obj)) {
        if (obj.length > this.config.maxArrayLength) {
          throw new Error(
            'Array length limit exceeded: '.concat(this.config.maxArrayLength),
          );
        }
        return obj.map(function (item, index) {
          try {
            ...
            _this.logger.warn(
              '['
                .concat(operationId, '] Failed to sanitize array item ')
                .concat(index),
              {
                operationId: operationId,
                index: index,
      ...
      if (typeof obj === 'object') {
        ...
        for (var _i = 0, _a = Object.entries(objRecord); _i < _a.length; _i++) {
          ...
          try {
            ...
            this.logger.warn(
              '['
                .concat(operationId, '] Failed to sanitize object property ')
                .concat(key),
              {
                operationId: operationId,
                key: key,
     ...
     */
    SanitizationInterceptor_1.prototype.sanitizeString = function (
      input,
      endpoint,
      operationId,
    ) {
      ...
      if (input.length > this.config.maxStringLength) {
        this.logger.warn(
          '['.concat(operationId, '] String length limit exceeded'),
          {
            operationId: operationId,
            length: input.length,
        ...
        throw new Error(
          'String length limit exceeded: '.concat(this.config.maxStringLength),
        );
      }
      var sanitized = input;
      var threats = [];
      ...
      if (this.config.detectSqlInjection) {
        for (
          var _b = 0, _c = SECURITY_PATTERNS.sqlInjection;
          _b < _c.length;
          _b++
        ) {
          var pattern = _c[_b];
      ...
      for (
        var _d = 0, _e = SECURITY_PATTERNS.pathTraversal;
        _d < _e.length;
        _d++
      ) {
        var pattern = _e[_d];
      ...
      for (
        var _f = 0, _g = SECURITY_PATTERNS.commandInjection;
        _f < _g.length;
        _f++
      ) {
        var pattern = _g[_f];
      ...
      if (threats.length > 0) {
        this.logger.error(
          '['.concat(operationId, '] Security threats detected in input'),
          {
            operationId: operationId,
            threats: threats,
      ...
      if (this.config.sanitizeHtml || endpointRules.allowHtml) {
        if (endpointRules.stripHtml || this.config.stripHtml) {
          ...
          try {
            ...
            var sanitizedResult = domPurifyInstance.sanitize(
              sanitized,
              purifyConfig,
            );
            ...
            this.logger.warn(
              'DOMPurify sanitization failed, keeping original',
              {
                error: errorMessage,
                operationId: operationId,
              },
    ...
    SanitizationInterceptor_1.prototype.getEndpointRules = function (endpoint) {
      for (
        var _i = 0, _a = Object.entries(this.config.endpointRules);
        _i < _a.length;
        _i++
      ) {
        var _b = _a[_i],
        ...
        if (endpoint.includes(pattern)) {
          return __assign(
            __assign({}, shared_1.DEFAULT_SANITIZATION_OPTIONS),
            rules,
          );
        }
      }
     ...
     */
    SanitizationInterceptor_1.prototype.sanitizeResponseData = function (
      data,
      operationId,
    ) {
      var _a;
      try {
        ...
        var structuredError = error;
        this.logger.warn(
          '['.concat(operationId, '] Failed to sanitize response data'),
          {
            operationId: operationId,
            error:
              (_a = structuredError.message) !== null && _a !== void 0
                ? _a
                : 'Unknown error',
          },
        );
     ...
     */
    SanitizationInterceptor_1.prototype.logSecurityEvent = function (
      request,
      eventType,
      message,
      operationId,
      ...
      try {
        ...
        switch (eventType) {
          ...
          case 'SQL_INJECTION':
            securityEventType =
              shared_1.SecurityEventType.INJECTION_ATTEMPT_BLOCKED;
            break;
          case 'SANITIZATION_FAILED':
            securityEventType = shared_1.SecurityEventType.VALIDATION_FAILED;
            break;
        }
        var securityEvent = (0, shared_1.createSecurityEvent)(
          securityEventType,
          request.path,
          request.method,
          false,
          ...
          ((_a = request.user) === null || _a === void 0 ? void 0 : _a.id)
        ...
        this.logger.warn(
          'Sanitization security event: '.concat(securityEvent.eventId),
          {
